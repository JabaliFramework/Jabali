...
?>
				...
				<div class="order-addresses<?php if( !wcdn_has_shipping_address( $order ) ) : ?> no-shipping-address<?php endif; ?>">
					<div class="billing-address">
						<h3><?php _e( 'Billing Address', 'otas-print' ); ?></h3>
						<address>
							
							<?php if( !$order->get_formatted_billing_address() ) _e( 'N/A', 'otas-print' ); else echo apply_filters( 'wcdn_address_billing', $order->get_formatted_billing_address(), $order ); ?>
							
						</address>
					</div>
					
					<div class="shipping-address">						
						<h3><?php _e( 'Shipping Address', 'otas-print' ); ?></h3>
						<address>
	
							<?php if( !$order->get_formatted_shipping_address() ) _e( 'N/A', 'otas-print' ); else echo apply_filters( 'wcdn_address_shipping', $order->get_formatted_shipping_address(), $order ); ?>
				...
				<div class="order-items">
					<table>
						<thead>
							<tr>
								<th class="head-name"><span><?php _e('Product', 'otas-print'); ?></span></th>
								<th class="head-item-price"><span><?php _e('Price', 'otas-print'); ?></span></th>
								<th class="head-quantity"><span><?php _e('Quantity', 'otas-print'); ?></span></th>
								<th class="head-price"><span><?php _e('Total', 'otas-print'); ?></span></th>
							</tr>
						</thead>
										...
										<td class="product-name">
											...
											<dl class="extras">
												<?php if( $product && $product->exists() && $product->is_downloadable() && $order->is_download_permitted() ) : ?>
													...
													<dt><?php _e( 'Download:', 'otas-print' ); ?></dt>
													<dd><?php printf( __( '%s Files', 'otas-print' ), count( $order->get_item_downloads( $item ) ) ); ?></dd>
				...
				<div class="order-notes">
					<?php if( wcdn_has_customer_notes( $order ) ) : ?>
						<h4><?php _e( 'Customer Note', 'otas-print' ); ?></h4>
