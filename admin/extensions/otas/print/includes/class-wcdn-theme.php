 ...
 */
if ( !class_exists( 'Otas_Print_Theme' ) ) {

	class Otas_Print_Theme {
		...
		public function add_scripts() {
			if ( is_account_page() || is_order_received_page() || $this->is_woocommerce_tracking_page() ) {
				...
				wp_enqueue_script( 'otas-print-theme', Otas_Print::$plugin_url . 'js/theme.js', array( 'jquery', 'otas-print-print-link' ) );
			}
		}		
		...
		public function create_print_button_order_page( $order_id ) {
			...
			if( get_option( 'wcdn_print_button_on_view_order_page' ) == 'yes' ) {				
				...
				?>
				<p class="order-print">
					<a href="<?php echo $print_url; ?>" class="button print"><?php _e( 'Print', 'otas-print' ); ?></a>
				</p>
				<?php
			}
		}
		...
		public function add_email_print_url( $order, $sent_to_admin = true, $plain_text = false ) {
			if( get_option( 'wcdn_email_print_link' ) == 'yes' ) {				
				if( $order->billing_email && !$sent_to_admin ) {
					...
					if( $plain_text ) :
echo __( 'Print your order', 'otas-print' ) . "\n\n";

echo $url . "\n";
 
echo "\n****************************************************\n\n";
					else : ?>
						<p><strong><?php _e( 'Print:', 'otas-print' ); ?></strong> <a href="<?php echo $url; ?>"><?php _e( 'Open print view in browser', 'otas-print' ); ?></a></p>
					<?php endif; 
				}
			}
		}
