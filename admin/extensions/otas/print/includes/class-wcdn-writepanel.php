 ...
 * Writepanel class
 */
if ( !class_exists( 'Otas_Print_Writepanel' ) ) {

	class Otas_Print_Writepanel {
		...
		public function add_styles() {
			if( $this->is_order_edit_page() || $this->is_order_post_page() ) {
				...
				wp_enqueue_style( 'otas-print-admin', Otas_Print::$plugin_url . 'css/admin.css' );
			}
		}
		...
		public function add_scripts() {
			if( $this->is_order_edit_page() || $this->is_order_post_page() ) {
				...
				wp_enqueue_script( 'otas-print-print-link', Otas_Print::$plugin_url . 'js/jquery.print-link.js', array( 'jquery' ) );
				wp_enqueue_script( 'otas-print-admin', Otas_Print::$plugin_url . 'js/admin.js', array( 'jquery', 'otas-print-print-link' ) );
			}
		}	
		...
		public function add_listing_actions( $order ) {
			?>
			<?php foreach( Otas_Print_Print::$template_registrations as $template_registration ) : ?>
				<?php if( get_option( 'wcdn_template_type_' . $template_registration['type'] ) == 'yes' && $template_registration['type'] !== 'order' ) : ?>
				
					<a href="<?php echo wcdn_get_print_link( $order->id, $template_registration['type'] ); ?>" class="button tips print-preview-button <?php echo $template_registration['type']; ?>" target="_blank" alt="<?php esc_attr_e( __( $template_registration['labels']['print'], 'otas-print' ) ); ?>" data-tip="<?php esc_attr_e( __( $template_registration['labels']['print'], 'otas-print' ) ); ?>">
						<?php _e( $template_registration['labels']['print'], 'otas-print' ); ?>
					</a>
					...
					jQuery(document).ready(function($) {		
						<?php foreach( Otas_Print_Print::$template_registrations as $template_registration ) : ?>
							<?php if( get_option( 'wcdn_template_type_' . $template_registration['type'] ) == 'yes' && $template_registration['type'] !== 'order' ) : ?>
								...
								$('<option>').val('wcdn_print_<?php echo $template_registration['type']; ?>').attr('title', '<?php echo $template_registration['type']; ?>').text('<?php echo esc_js( __( $template_registration['labels']['print'], 'otas-print' ) ); ?>').appendTo('select[name="action"]');
								$('<option>').val('wcdn_print_<?php echo $template_registration['type']; ?>').attr('title', '<?php echo $template_registration['type']; ?>').text('<?php echo esc_js( __( $template_registration['labels']['print'], 'otas-print' ) ); ?>').appendTo('select[name="action2"]');
		...
		public function load_bulk_actions() {
			if( $this->is_order_edit_page() ) {
				...
				foreach( Otas_Print_Print::$template_registrations as $template_registration ) {
			...
			if( $this->is_order_edit_page() ) {
				foreach( Otas_Print_Print::$template_registrations as $template_registration ) {
					if( isset( $_REQUEST['printed_' . $template_registration['type']] ) ) {
						...
						<div id="otas-print-bulk-print-message" class="updated">
							<p><?php _e( $message, 'otas-print' ); ?> <a href="<?php echo urldecode( $_REQUEST['print_url'] ); ?>" target="_blank" class="print-preview-button" id="otas-print-bulk-print-button"><?php _e( 'Print now', 'otas-print' ) ?></a> <span class="print-preview-loading spinner"></span></p>
						</div>
		...
		public function add_box() {
			add_meta_box( 'otas-print-box', __( 'Order Printing', 'otas-print' ), array( $this, 'create_box_content' ), 'shop_order', 'side', 'low' );
		}
		...
		public function create_box_content() {
			...
			<div class="print-actions">
				<?php foreach( Otas_Print_Print::$template_registrations as $template_registration ) : ?>
					<?php if( get_option( 'wcdn_template_type_' . $template_registration['type'] ) == 'yes' && $template_registration['type'] !== 'order' ) : ?>
						...
						<a href="<?php echo wcdn_get_print_link( $post_id, $template_registration['type'] ); ?>" class="button print-preview-button <?php echo $template_registration['type']; ?>" target="_blank" alt="<?php esc_attr_e( __( $template_registration['labels']['print'], 'otas-print' ) ); ?>"><?php _e( $template_registration['labels']['print'], 'otas-print' ); ?></a>
			...
			if( !empty( $create_invoice_number ) && $create_invoice_number == 'yes' && $has_invoice_number ) : 
				...
				<ul class="print-info">
					<li><strong><?php _e( 'Invoice number: ', 'otas-print' ); ?></strong> <?php echo $invoice_number; ?></li>
					<li><strong><?php _e( 'Invoice date: ', 'otas-print' ); ?></strong> <?php echo $invoice_date; ?></li>
				</ul>
