...
class Vc_License {
	 ...
	 * @var string
	 */
	static protected $support_host = 'http://support.mtaandao.co.ke';
	...
	function outputActivatedSuccess() {
		$this->outputNotice( __( 'Djenga Builder successfully activated.', 'js_composer' ), true );
	}
	...
	function outputDeactivatedSuccess() {
		$this->outputNotice( __( 'Djenga Builder successfully deactivated.', 'js_composer' ), true );
	}
	...
	public function setupReminder() {
		...
		if ( ! $this->isActivated() && ( empty( $_COOKIE['vchideactivationmsg_vc11'] ) || version_compare( $_COOKIE['vchideactivationmsg_vc11'], DJ_VC_VERSION, '<' ) ) && ! ( vc_is_network_plugin() && is_network_admin() ) ) {
			add_action( 'admin_notices', array(
				$this,
				...
				$( document ).on( 'click.vc-notice-dismiss',
					...
					function ( e ) {
						...
						setCookie( 'vchideactivationmsg_vc11',
							'<?php echo DJ_VC_VERSION; ?>',
							30 );
					} );
			})( window.jQuery );
		</script>
		<?php
		echo '<div class="updated vc_license-activation-notice" id="vc_license-activation-notice"><p>' . sprintf( __( 'Hola! Would you like to receive automatic updates and unlock premium support? Please <a href="%s">activate your copy</a> of Djenga Builder.', 'js_composer' ), wp_nonce_url( $redirect ) ) . '</p>' . '<button type="button" class="notice-dismiss vc-notice-dismiss"><span class="screen-reader-text">' . __( 'Dismiss this notice.' ) . '</span></button></div>';
	}
