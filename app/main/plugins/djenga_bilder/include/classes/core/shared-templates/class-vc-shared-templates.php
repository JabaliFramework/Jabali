...
require_once dirname( __FILE__ ) . '/importer/jabali-importer.php';
...
class Vc_Shared_Templates {
	 ...
	 * @var string
	 */
	protected $download_link_url = 'http://support.mtaandao.co.ke/templates/download-link';
	...
	/**
	 * Post type from templates registration in jabali
	 */
	...
	private function getTemplateDownloadLink( $id ) {
		...
		$url = $this->download_link_url . '?product=vc&url=' . $url . '&key=' . $key . '&version=' . DJ_VC_VERSION . '&id=' . esc_attr( $id );
