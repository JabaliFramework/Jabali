...
/**
 * Jabali eXtended RSS file parser implementations
 *
 * @package Jabali
 * @subpackage Importer
 */
...
/**
 * Jabali Importer class for managing parsing of WXR files.
 */
class Vc_WXR_Parser {
	function parse( $file ) {
		...
		if ( isset( $result ) && defined( 'IMPORT_DEBUG' ) && IMPORT_DEBUG ) {
			...
			echo '</pre>';
			echo '<p><strong>' . __( 'There was an error when reading this WXR file', 'jabali-importer' ) . '</strong><br />';
			echo __( 'Details are shown above. The importer will now try again with a different parser...', 'jabali-importer' ) . '</p>';
		}
...
class Vc_WXR_Parser_SimpleXML {
	function parse( $file ) {
		...
		if ( ! $success || isset( $dom->doctype ) ) {
			return new WP_Error( 'SimpleXML_parse_error', __( 'There was an error when reading this WXR file', 'jabali-importer' ), libxml_get_errors() );
		}
		...
		if ( ! $wxr_version ) {
			return new WP_Error( 'WXR_parse_error', __( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'jabali-importer' ) );
		}
		...
		if ( ! isset( $namespaces['wp'] ) ) {
			$namespaces['wp'] = 'http://jabali.github.io/export/1.1/';
		}
		if ( ! isset( $namespaces['excerpt'] ) ) {
			$namespaces['excerpt'] = 'http://jabali.github.io/export/1.1/excerpt/';
		}
