<?php
/*
Plugin Name: Djenga Builder
Plugin URI: http://vc.mtaandao.co.ke
Description: Drag and drop page builder for Jabali. Take full control over your Jabali site, build any layout you can imagine – no programming knowledge required.
Version: 5.0
Author: Michael M - DJenga.com
...
class Vc_Manager {
	...
	private $mode = 'none';
	/**
	 * Enables Djenga Builder to act as the theme plugin.
	 *
	 * @since 4.2
	 * @var bool
	 */
	...
	protected function setVersion() {
		...
		if ( ! is_string( $version ) || version_compare( $version, DJ_VC_VERSION ) !== 0 ) {
			add_action( 'vc_after_init', array(
				vc_settings(),
				'rebuild',
			) );
			update_option( 'vc_version', DJ_VC_VERSION );
		}
	}
	...
	public function updater() {
		...
		if ( ! isset( $this->factory['updater'] ) ) {
			...
			$updater->setUpdateManager( new Vc_Updating_Manager( DJ_VC_VERSION, $updater->versionUrl(), $this->pluginName() ) );
