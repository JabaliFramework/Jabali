 ...
 * Settings class
 */
if ( ! class_exists( 'Banda_PDF_Invoices_Settings' ) ) {

	class Banda_PDF_Invoices_Settings {
		...
		public function __construct() {
			...
			// Add links to Jabali plugins page
			add_filter( 'plugin_action_links_'.Banda_PDF_Invoices::$plugin_basename, array( &$this, 'wpo_wcpdf_add_settings_link' ) );
			...
			// Banda Order Status & Actions Manager emails compatibility
		...
		public function load_scripts_styles ( $hook ) {
			...
			wp_enqueue_style(
				'wpo-wcpdf',
				Banda_PDF_Invoices::$plugin_url . 'css/style.css',
				array(),
				Banda_PDF_Invoices::$version
			);
			wp_enqueue_media();
		...
		public function wpo_wcpdf_add_settings_link( $links ) {
			$settings_link = '<a href="admin.php?page=wpo_wcpdf_options_page">'. __( 'Settings', 'banda' ) . '</a>';
		...
		public function add_support_links( $links, $file ) {
			...
			if ( $file == Banda_PDF_Invoices::$plugin_basename ) {
		...
		public function settings_page() {
			...
			?>
				<script type="text/javascript">
					jQuery( function( $ ) {
						$("#footer-thankyou").html("If you like <strong>Banda PDF Invoices & Packing Slips</strong> please leave us a <a href='https://jabali.github.io/support/view/plugin-reviews/pdf-invoices-packing-slips?rate=5#postform'>★★★★★</a> rating. A huge thank you in advance!");
					});
				</script>
				<div class="wrap">
					<div class="icon32" id="icon-options-general"><br /></div>
					<h2><?php _e( 'Banda PDF Invoices', 'wpo_wcpdf' ); ?></h2>
					...
					if ( !$hide_ad && !( class_exists('Banda_PDF_IPS_Pro') && class_exists('Banda_PDF_IPS_Dropbox') && class_exists('Banda_PDF_IPS_Templates') && class_exists('Banda_Ext_PrintOrders') ) ) {
		...
		public function init_settings() {
			global $banda, $wpo_wcpdf;
	
			// Whether a third-party plugin claims responsibility for generating invoice numbers
			$invoicenumber_thirdparty = apply_filters('banda_invoice_number_by_plugin', false);
			...
			if ($invoicenumber_thirdparty) {
				$invoice_number_desc = __( 'Invoice numbers are created by a third-party extension.', 'yith-banda-pdf-invoice');
				$config_link = esc_attr(apply_filters('banda_invoice_number_configuration_link', null));
				if ($config_link) {
					$invoice_number_desc .= ' '.sprintf(__( 'Configure it <a href="%s">here</a>.', 'yith-banda-pdf-invoice'), $config_link);
				}
			...
			if (!$invoicenumber_thirdparty) {
				...
				if ( !isset( $next_invoice_number ) ) {
					// Based on code from Banda Sequential Order Numbers
					global $wpdb;
		...
		/**
		 * get all emails registered in Banda
		 * @param  boolean $remove_defaults switch to remove default banda emails
		 * @return array   $emails       list of all email ids/slugs and names
		 */
		public function get_wc_emails ( $remove_defaults = true ) {
			// get emails from Banda
			global $banda;
			$mailer = $banda->mailer();
		...
		public function wc_order_status_actions_emails ( $emails ) {
			...
			if (class_exists('WC_Custom_Status')) {
				// get list of custom statuses from Banda Custom Order Status & Actions
	...
	} // end class Banda_PDF_Invoices_Settings
