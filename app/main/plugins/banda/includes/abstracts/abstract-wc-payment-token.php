 ...
 */
 abstract class WC_Payment_Token extends WC_Data {
	...
	public function read( $token_id ) {
		global $wpdb;
		if ( $token = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}banda_payment_tokens WHERE token_id = %d LIMIT 1;", $token_id ) ) ) {
	...
	public function update() {
		...
		$wpdb->update(
			$wpdb->prefix . 'banda_payment_tokens',
			$payment_token_data,
		...
		do_action( 'banda_payment_token_updated', $this->get_id() );
		return true;
	}
	...
	public function create() {
		...
		$wpdb->insert( $wpdb->prefix . 'banda_payment_tokens', $payment_token_data );
		...
		do_action( 'banda_payment_token_created', $token_id );
		return true;
	}
	...
	public function delete() {
		...
		$wpdb->delete( $wpdb->prefix . 'banda_payment_tokens', array( 'token_id' => $this->get_id() ), array( '%d' ) );
		$wpdb->delete( $wpdb->prefix . 'banda_payment_tokenmeta', array( 'payment_token_id' => $this->get_id() ), array( '%d' ) );
		do_action( 'banda_payment_token_deleted', $this->get_id(), $this );
	}
