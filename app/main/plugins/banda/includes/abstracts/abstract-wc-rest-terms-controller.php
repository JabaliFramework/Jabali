...
abstract class WC_REST_Terms_Controller extends WC_REST_Controller {
	...
	protected function check_permissions( $request, $context = 'read' ) {
		...
		if ( ! $taxonomy ) {
			return new WP_Error( 'banda_rest_taxonomy_invalid', __( "Taxonomy doesn't exist.", 'banda' ), array( 'status' => 404 ) );
		}
		...
		if ( $id = intval( $request['id'] ) ) {
			...
			if ( ! $term || $term->taxonomy !== $taxonomy ) {
				return new WP_Error( 'banda_rest_term_invalid', __( "Resource doesn't exist.", 'banda' ), array( 'status' => 404 ) );
			}
	...
	public function get_items( $request ) {
		 ...
		 * collection request.
		 *
		 * @see https://developer.jabali.github.io/reference/functions/get_terms/
		 *
		 * @param array           $prepared_args Array of arguments to be
		 *                                       passed to get_terms.
		 * @param WP_REST_Request $request       The current request.
		 */
		$prepared_args = apply_filters( "banda_rest_{$taxonomy}_query", $prepared_args, $request );
		...
		if ( ! empty( $prepared_args['product'] )  ) {
			...
			// See https://core.trac.jabali.github.io/ticket/35935
	...
	public function create_item( $request ) {
		...
		if ( isset( $request['parent'] ) ) {
			if ( ! is_taxonomy_hierarchical( $taxonomy ) ) {
				return new WP_Error( 'banda_rest_taxonomy_not_hierarchical', __( 'Can not set resource parent, taxonomy is not hierarchical.', 'banda' ), array( 'status' => 400 ) );
			}

			$parent = get_term( (int) $request['parent'], $taxonomy );

			if ( ! $parent ) {
				return new WP_Error( 'banda_rest_term_invalid', __( "Parent resource doesn't exist.", 'banda' ), array( 'status' => 404 ) );
			}
		 ...
		 */
		do_action( "banda_rest_insert_{$taxonomy}", $term, $request, true );
	...
	public function update_item( $request ) {
		...
		if ( isset( $request['parent'] ) ) {
			...
			if ( ! $parent ) {
				return new WP_Error( 'banda_rest_term_invalid', __( "Parent resource doesn't exist.", 'banda' ), array( 'status' => 400 ) );
			}
		 ...
		 * @param boolean         $creating  True when creating term, false when updating.
		 */
		do_action( "banda_rest_insert_{$taxonomy}", $term, $request, false );
	...
	public function delete_item( $request ) {
		 ...
		 */
		do_action( "banda_rest_delete_{$taxonomy}", $term, $response, $request );
	...
	public function get_collection_params() {
		...
		$params['orderby']    = array(
			'description'           => __( 'Sort collection by resource attribute.', 'banda' ),
			'type'                  => 'string',
			'sanitize_callback'     => 'sanitize_key',
			'default'               => 'name',
			'enum'                  => array(
		...
		$params['hide_empty'] = array(
			'description'           => __( 'Whether to hide resources not assigned to any products.', 'banda' ),
			'type'                  => 'boolean',
			'default'               => false,
			'validate_callback'     => 'rest_validate_request_arg',
		);
		if ( $taxonomy->hierarchical ) {
			$params['parent'] = array(
				'description'        => __( 'Limit result set to resources assigned to a specific parent.', 'banda' ),
				'type'               => 'integer',
				'sanitize_callback'  => 'absint',
				'validate_callback'  => 'rest_validate_request_arg',
			);
		...
		$params['product'] = array(
			'description'           => __( 'Limit result set to resources assigned to a specific product.', 'banda' ),
			'type'                  => 'integer',
			'default'               => null,
			'validate_callback'     => 'rest_validate_request_arg',
		);
		$params['slug']    = array(
			'description'        => __( 'Limit result set to resources with a specific slug.', 'banda' ),
			'type'               => 'string',
			'validate_callback'  => 'rest_validate_request_arg',
		);
