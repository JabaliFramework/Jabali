...
abstract class WC_REST_Posts_Controller extends WC_REST_Controller {
	...
	public function get_item( $request ) {
		...
		if ( empty( $id ) || empty( $post->ID ) || ! in_array( $post->post_type, $this->get_post_types() ) ) {
			return new WP_Error( "banda_rest_invalid_{$this->post_type}_id", __( 'Invalid id.', 'banda' ), array( 'status' => 404 ) );
		}
	...
	public function get_items( $request ) {
		 ...
		 */
		$args = apply_filters( "banda_rest_{$this->post_type}_query", $args, $request );
	...
	protected function prepare_items_query( $prepared_args = array(), $request = null ) {
		...
		foreach ( $valid_vars as $var => $index ) {
			if ( isset( $prepared_args[ $var ] ) ) {
				 ...
				 *
				 */
				$query_args[ $var ] = apply_filters( "banda_rest_query_var-{$var}", $prepared_args[ $var ] );
			}
		}
	...
	protected function get_allowed_query_vars() {
		...
		if ( current_user_can( $post_type_obj->cap->edit_posts ) ) {
			 ...
			 * `add_filter( 'banda_rest_private_query_vars', '__return_empty_array' );`
			 *
			 * @param array $private_query_vars Array of allowed query vars for authorized users.
			 * }
			 */
			$private = apply_filters( 'banda_rest_private_query_vars', $wp->private_query_vars );
		 ...
		 * }
		 */
		$valid_vars = apply_filters( 'banda_rest_query_vars', $valid_vars );
	...
	public function get_collection_params() {
		...
		$params['after'] = array(
			'description'        => __( 'Limit response to resources published after a given ISO8601 compliant date.', 'banda' ),
			'type'               => 'string',
			'format'             => 'date-time',
			'validate_callback'  => 'rest_validate_request_arg',
		);
		$params['before'] = array(
			'description'        => __( 'Limit response to resources published before a given ISO8601 compliant date.', 'banda' ),
			'type'               => 'string',
			'format'             => 'date-time',
			'validate_callback'  => 'rest_validate_request_arg',
		);
		...
		if ( isset( $post_type_obj->hierarchical ) && $post_type_obj->hierarchical ) {
			$params['parent'] = array(
				'description'       => __( 'Limit result set to those of particular parent ids.', 'banda' ),
				'type'              => 'array',
				'sanitize_callback' => 'wp_parse_id_list',
				'default'           => array(),
			);
			$params['parent_exclude'] = array(
				'description'       => __( 'Limit result set to all items except those of a particular parent id.', 'banda' ),
				'type'              => 'array',
				'sanitize_callback' => 'wp_parse_id_list',
				'default'           => array(),
			);
		...
		$params['filter'] = array(
			'description' => __( 'Use WP Query arguments to modify the response; private query vars require appropriate authorization.', 'banda' ),
		);
