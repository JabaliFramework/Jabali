 ...
 *
 * The Banda product class handles individual product data.
 *
 * @class       WC_Product
 * @var         WP_Post
 * @version     2.1.0
 * @package     Banda/Abstracts
...
class WC_Product {
	...
	public function supports( $feature ) {
		return apply_filters( 'banda_product_supports', in_array( $feature, $this->supports ) ? true : false, $feature, $this );
	}
	...
	public function get_gallery_attachment_ids() {
		return apply_filters( 'banda_product_gallery_attachment_ids', array_filter( array_filter( (array) explode( ',', $this->product_image_gallery ) ), 'wp_attachment_is_image' ), $this );
	}
	...
	public function get_sku() {
		return apply_filters( 'banda_get_sku', $this->sku, $this );
	}
	...
	public function get_stock_quantity() {
		return apply_filters( 'banda_get_stock_quantity', $this->managing_stock() ? wc_stock_amount( $this->stock ) : null, $this );
	}
	...
	public function check_stock_status() {
		if ( ! $this->backorders_allowed() && $this->get_total_stock() <= get_option( 'banda_notify_no_stock_amount' ) ) {
			if ( $this->stock_status !== 'outofstock' ) {
				$this->set_stock_status( 'outofstock' );
			}
		} elseif ( $this->backorders_allowed() || $this->get_total_stock() > get_option( 'banda_notify_no_stock_amount' ) ) {
	...
	public function set_stock( $amount = null, $mode = 'set' ) {
		...
		if ( ! is_null( $amount ) && $this->managing_stock() ) {
			...
			// Trigger action
			do_action( 'banda_product_set_stock', $this );
		}
	...
	public function set_stock_status( $status ) {
		...
		if ( $this->managing_stock() ) {
			if ( ! $this->backorders_allowed() && $this->get_stock_quantity() <= get_option( 'banda_notify_no_stock_amount' ) ) {
				$status = 'outofstock';
			}
		}
		...
		if ( update_post_meta( $this->id, '_stock_status', $status ) ) {
			...
			do_action( 'banda_product_set_stock_status', $this->id, $status );
		}
	}
	...
	public function get_files() {
		...
		if ( ! empty( $downloadable_files ) ) {
			...
			foreach ( $downloadable_files as $key => $file ) {
				...
				// Filter URL
				$downloadable_files[ $key ]['file'] = apply_filters( 'banda_file_download_path', $downloadable_files[ $key ]['file'], $this, $key );
			}
		}

		return apply_filters( 'banda_product_files', $downloadable_files, $this );
	}
	...
	public function get_file( $download_id = '' ) {
		...
		return apply_filters( 'banda_product_file', $file, $this, $download_id );
	}
	...
	public function get_file_download_path( $download_id ) {
		...
		// allow overriding based on the particular file being requested
		return apply_filters( 'banda_product_file_download_path', $file_path, $this, $download_id );
	}
	...
	public function is_virtual() {
		return apply_filters( 'banda_is_virtual', $this->virtual == 'yes' ? true : false, $this );
	}
	...
	public function needs_shipping() {
		return apply_filters( 'banda_product_needs_shipping', $this->is_virtual() ? false : true, $this );
	}
	...
	public function is_sold_individually() {
		...
		return apply_filters( 'banda_is_sold_individually', $return, $this );
	}
	...
	public function is_taxable() {
		...
		return apply_filters( 'banda_product_is_taxable', $taxable, $this );
	}
	...
	public function get_title() {
		return apply_filters( 'banda_product_title', $this->post ? $this->post->post_title : '', $this );
	}
	...
	public function get_parent() {
		return apply_filters( 'banda_product_parent', absint( $this->post->post_parent ), $this );
	}
	...
	public function add_to_cart_url() {
		return apply_filters( 'banda_product_add_to_cart_url', get_permalink( $this->id ), $this );
	}
	...
	public function add_to_cart_text() {
		return apply_filters( 'banda_product_add_to_cart_text', __( 'Read more', 'banda' ), $this );
	}
	...
	public function managing_stock() {
		return ( ! isset( $this->manage_stock ) || $this->manage_stock == 'no' || get_option( 'banda_manage_stock' ) !== 'yes' ) ? false : true;
	}
	...
	public function is_in_stock() {
		 ...
		 */
		if ( $this->managing_stock() && ! $this->backorders_allowed() && $this->get_total_stock() <= get_option( 'banda_notify_no_stock_amount' ) ) {
			$status = false;
		}

		return apply_filters( 'banda_product_is_in_stock', $status );
	}
	...
	public function backorders_allowed() {
		return apply_filters( 'banda_product_backorders_allowed', $this->backorders === 'yes' || $this->backorders === 'notify' ? true : false, $this->id, $this );
	}
	...
	public function backorders_require_notification() {
		return apply_filters( 'banda_product_backorders_require_notification', $this->managing_stock() && $this->backorders === 'notify' ? true : false, $this );
	}
	 ...
	 *
	 * This can all be changed through use of the banda_get_availability filter.
	 *
	 * @return string
	 */
	public function get_availability() {
		return apply_filters( 'banda_get_availability', array(
	...
	protected function get_availability_text() {
		if ( ! $this->is_in_stock() ) {
			$availability = __( 'Out of stock', 'banda' );
		} elseif ( $this->managing_stock() && $this->is_on_backorder( 1 ) ) {
			$availability = $this->backorders_require_notification() ? __( 'Available on backorder', 'banda' ) : __( 'In stock', 'banda' );
		} elseif ( $this->managing_stock() ) {
			switch ( get_option( 'banda_stock_format' ) ) {
				case 'no_amount' :
					$availability = __( 'In stock', 'banda' );
				break;
				case 'low_amount' :
					if ( $this->get_total_stock() <= get_option( 'banda_notify_low_stock_amount' ) ) {
						$availability = sprintf( __( 'Only %s left in stock', 'banda' ), $this->get_total_stock() );

						if ( $this->backorders_allowed() && $this->backorders_require_notification() ) {
							$availability .= ' ' . __( '(also available on backorder)', 'banda' );
						}
					} else {
						$availability = __( 'In stock', 'banda' );
					}
				break;
				default :
					$availability = sprintf( __( '%s in stock', 'banda' ), $this->get_total_stock() );
		...
		return apply_filters( 'banda_get_availability_text', $availability, $this );
	}
	...
	protected function get_availability_class() {
		...
		return apply_filters( 'banda_get_availability_class', $class, $this );
	}
	...
	public function is_visible() {
		...
		} elseif ( 'yes' === get_option( 'banda_hide_out_of_stock_items' ) && ! $this->is_in_stock() ) {
			$visible = false;
		...
		return apply_filters( 'banda_product_is_visible', $visible, $this->id );
	}
	...
	public function is_on_sale() {
		return apply_filters( 'banda_product_is_on_sale', ( $this->get_sale_price() !== $this->get_regular_price() && $this->get_sale_price() === $this->get_price() ), $this );
	}
	...
	public function is_purchasable() {
		...
		return apply_filters( 'banda_is_purchasable', $purchasable, $this );
	}
	...
	public function get_sale_price() {
		return apply_filters( 'banda_get_sale_price', $this->sale_price, $this );
	}
	...
	public function get_regular_price() {
		return apply_filters( 'banda_get_regular_price', $this->regular_price, $this );
	}
	...
	public function get_price() {
		return apply_filters( 'banda_get_price', $this->price, $this );
	}
	...
	public function get_price_including_tax( $qty = 1, $price = '' ) {
		...
		if ( $this->is_taxable() ) {
			...
			if ( get_option( 'banda_prices_include_tax' ) === 'no' ) {
				 ...
				 */
				} elseif ( $tax_rates !== $base_tax_rates && apply_filters( 'banda_adjust_non_base_location_prices', true ) ) {
		...
		return apply_filters( 'banda_get_price_including_tax', $price, $qty, $this );
	}
	...
	public function get_price_excluding_tax( $qty = 1, $price = '' ) {
		...
		if ( $this->is_taxable() && 'yes' === get_option( 'banda_prices_include_tax' ) ) {
		...
		return apply_filters( 'banda_get_price_excluding_tax', $price, $qty, $this );
	}

	/**
	 * Returns the price including or excluding tax, based on the 'banda_tax_display_shop' setting.
	 *
	 * @param  string  $price to calculate, left blank to just use get_price()
	...
	public function get_price_suffix( $price = '', $qty = 1 ) {
		...
		$price_display_suffix  = get_option( 'banda_price_display_suffix' );
		$banda_calc_taxes = get_option( 'banda_calc_taxes', 'no' );

		if ( $price_display_suffix && 'yes' === $banda_calc_taxes ) {

			$price_display_suffix = ' <small class="banda-price-suffix">' . $price_display_suffix . '</small>';
		...
		return apply_filters( 'banda_get_price_suffix', $price_display_suffix, $this );
	}
	...
	public function get_price_html( $price = '' ) {
		...
		if ( $this->get_price() > 0 ) {
			...
			if ( $this->is_on_sale() && $this->get_regular_price() ) {
				...
				$price = apply_filters( 'banda_sale_price_html', $price, $this );

			} else {

				$price .= wc_price( $display_price ) . $this->get_price_suffix();

				$price = apply_filters( 'banda_price_html', $price, $this );

			}

		} elseif ( $this->get_price() === '' ) {

			$price = apply_filters( 'banda_empty_price_html', '', $this );

		} elseif ( $this->get_price() == 0 ) {

			if ( $this->is_on_sale() && $this->get_regular_price() ) {

				$price .= $this->get_price_html_from_to( $display_regular_price, __( 'Free!', 'banda' ) );

				$price = apply_filters( 'banda_free_sale_price_html', $price, $this );

			} else {

				$price = '<span class="amount">' . __( 'Free!', 'banda' ) . '</span>';

				$price = apply_filters( 'banda_free_price_html', $price, $this );
	...
	public function get_price_html_from_text() {
		$from = '<span class="from">' . _x( 'From:', 'min_price', 'banda' ) . ' </span>';

		return apply_filters( 'banda_get_price_html_from_text', $from, $this );
	}
	...
	public function get_price_html_from_to( $from, $to ) {
		...
		return apply_filters( 'banda_get_price_html_from_to', $price, $from, $to, $this );
	}
	...
	public function get_tax_class() {
		return apply_filters( 'banda_product_tax_class', $this->tax_class, $this );
	}
	...
	public function get_rating_html( $rating = null ) {
		...
		if ( $rating > 0 ) {
			...
			$rating_html  = '<div class="star-rating" title="' . sprintf( __( 'Rated %s out of 5', 'banda' ), $rating ) . '">';

			$rating_html .= '<span style="width:' . ( ( $rating / 5 ) * 100 ) . '%"><strong class="rating">' . $rating . '</strong> ' . __( 'out of 5', 'banda' ) . '</span>';

			$rating_html .= '</div>';
		}

		return apply_filters( 'banda_product_get_rating_html', $rating_html, $rating );
	}
	...
	public function get_review_count() {
		...
		return apply_filters( 'banda_product_review_count', $count, $this );
	}
	...
	public function get_upsells() {
		return apply_filters( 'banda_product_upsell_ids', (array) maybe_unserialize( $this->upsell_ids ), $this );
	}
	...
	public function get_cross_sells() {
		return apply_filters( 'banda_product_crosssell_ids', (array) maybe_unserialize( $this->crosssell_ids ), $this );
	}
	...
	public function get_attributes() {
		...
		return apply_filters( 'banda_get_product_attributes', $attributes );
	}
	...
	public function get_length() {
		return apply_filters( 'banda_product_length', $this->length ? $this->length : '', $this );
	}
	...
	public function get_width() {
		return apply_filters( 'banda_product_width', $this->width ? $this->width : '', $this );
	}
	...
	public function get_height() {
		return apply_filters( 'banda_product_height', $this->height ? $this->height : '', $this );
	}

	/**
	 * Returns the product's weight.
	 * @todo   refactor filters in this class to naming banda_product_METHOD
	 * @return string
	 */
	public function get_weight() {
		return apply_filters( 'banda_product_weight', apply_filters( 'banda_product_get_weight', $this->weight ? $this->weight : '' ), $this );
	}
	...
	public function get_dimensions() {
		...
		if ( ! empty( $dimensions ) ) {
			$dimensions .= ' ' . get_option( 'banda_dimension_unit' );
		}

		return  apply_filters( 'banda_product_dimensions', $dimensions, $this );
	}
	...
	protected function get_related_terms( $term ) {
		...
		$terms = apply_filters( 'banda_get_related_' . $term . '_terms', wp_get_post_terms( $this->id, $term ), $this->id );
	...
	protected function build_related_query( $cats_array, $tags_array, $exclude_ids, $limit ) {
		...
		if ( get_option( 'banda_hide_out_of_stock_items' ) === 'yes' ) {
		...
		$relate_by_category = apply_filters( 'banda_product_related_posts_relate_by_category', true, $this->id );
		$relate_by_tag      = apply_filters( 'banda_product_related_posts_relate_by_tag', true, $this->id );
		...
		$query           = apply_filters( 'banda_product_related_posts_query', $query, $this->id );
