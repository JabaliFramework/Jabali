 ...
 * Abstract Order
 *
 * The Banda order class handles order data.
 *
 * @class       WC_Order
 * @version     2.2.0
 * @package     Banda/Classes
 * @category    Class
 * @author      Jabali
 *
...
abstract class WC_Abstract_Order {
	...
	public function __construct( $order = 0 ) {
		$this->prices_include_tax    = get_option('banda_prices_include_tax') == 'yes' ? true : false;
		$this->tax_display_cart      = get_option( 'banda_tax_display_cart' );
	...
	public function remove_order_items( $type = null ) {
		...
		if ( ! empty( $type ) ) {
			$wpdb->query( $wpdb->prepare( "DELETE FROM itemmeta USING {$wpdb->prefix}banda_order_itemmeta itemmeta INNER JOIN {$wpdb->prefix}banda_order_items items WHERE itemmeta.order_item_id = items.order_item_id AND items.order_id = %d AND items.order_item_type = %s", $this->id, $type ) );
			$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}banda_order_items WHERE order_id = %d AND order_item_type = %s", $this->id, $type ) );
		} else {
			$wpdb->query( $wpdb->prepare( "DELETE FROM itemmeta USING {$wpdb->prefix}banda_order_itemmeta itemmeta INNER JOIN {$wpdb->prefix}banda_order_items items WHERE itemmeta.order_item_id = items.order_item_id and items.order_id = %d", $this->id ) );
			$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}banda_order_items WHERE order_id = %d", $this->id ) );
		}
	}
	...
	public function add_payment_token( $token ) {
		...
		do_action( 'banda_payment_token_added_to_order', $this->id, $token->get_id(), $token, $token_ids );
		return true;
	}
	...
	public function get_address( $type = 'billing' ) {
		...
		return apply_filters( 'banda_get_order_address', $address, $type, $this );
	}
	...
	public function add_product( $product, $qty = 1, $args = array() ) {
		...
		if ( $product->backorders_require_notification() && $product->is_on_backorder( $qty ) ) {
			wc_add_order_item_meta( $item_id, apply_filters( 'banda_backordered_item_meta_name', __( 'Backordered', 'banda' ) ), $qty - max( 0, $product->get_total_stock() ) );
		}

		do_action( 'banda_order_add_product', $this->id, $item_id, $product, $qty, $args );
	...
	public function update_product( $item_id, $product, $args ) {
		...
		if ( isset( $args['qty'] ) && $product->backorders_require_notification() && $product->is_on_backorder( $args['qty'] ) ) {
			wc_update_order_item_meta( $item_id, apply_filters( 'banda_backordered_item_meta_name', __( 'Backordered', 'banda' ) ), $args['qty'] - max( 0, $product->get_total_stock() ) );
		}

		do_action( 'banda_order_edit_product', $this->id, $item_id, $args, $product );
	...
	public function add_coupon( $code, $discount_amount = 0, $discount_amount_tax = 0 ) {
		...
		do_action( 'banda_order_add_coupon', $this->id, $item_id, $code, $discount_amount, $discount_amount_tax );
	...
	public function update_coupon( $item_id, $args ) {
		...
		do_action( 'banda_order_update_coupon', $this->id, $item_id, $args );
	...
	public function add_tax( $tax_rate_id, $tax_amount = 0, $shipping_tax_amount = 0 ) {
		...
		do_action( 'banda_order_add_tax', $this->id, $item_id, $tax_rate_id, $tax_amount, $shipping_tax_amount );
	...
	public function add_shipping( $shipping_rate ) {
		...
		do_action( 'banda_order_add_shipping', $this->id, $item_id, $shipping_rate );
	...
	public function update_shipping( $item_id, $args ) {
		...
		do_action( 'banda_order_update_shipping', $this->id, $item_id, $args );
	...
	public function add_fee( $fee ) {
		...
		do_action( 'banda_order_add_fee', $this->id, $item_id, $fee );
	...
	public function update_fee( $item_id, $args ) {
		...
		do_action( 'banda_order_update_fee', $this->id, $item_id, $args );
	...
	public function calculate_taxes() {
		...
		$tax_based_on       = get_option( 'banda_tax_based_on' );
		...
		foreach ( $this->get_shipping_methods() as $item_id => $item ) {
			$shipping_tax_class = get_option( 'banda_shipping_tax_class' );
	...
	public function get_status() {
		...
		return apply_filters( 'banda_order_get_status', 'wc-' === substr( $this->post_status, 0, 3 ) ? substr( $this->post_status, 3 ) : $this->post_status, $this );
	}
	...
	public function has_status( $status ) {
		return apply_filters( 'banda_order_has_status', ( is_array( $status ) && in_array( $this->get_status(), $status ) ) || $this->get_status() === $status ? true : false, $this, $status );
	}
	...
	public function get_order_number() {
		return apply_filters( 'banda_order_number', $this->id, $this );
	}
	...
	public function get_formatted_billing_address() {
		if ( ! $this->formatted_billing_address ) {
			...
			// Formatted Addresses.
			$address = apply_filters( 'banda_order_formatted_billing_address', array(
				'first_name'    => $this->billing_first_name,
				'last_name'     => $this->billing_last_name,
				'company'       => $this->billing_company,
				'address_1'     => $this->billing_address_1,
	...
	public function get_formatted_shipping_address() {
		if ( ! $this->formatted_shipping_address ) {
			...
			if ( $this->shipping_address_1 || $this->shipping_address_2 ) {
				...
				// Formatted Addresses
				$address = apply_filters( 'banda_order_formatted_shipping_address', array(
					'first_name'    => $this->shipping_first_name,
					'last_name'     => $this->shipping_last_name,
					'company'       => $this->shipping_company,
					'address_1'     => $this->shipping_address_1,
	...
	public function get_shipping_address_map_url() {
		$address = apply_filters( 'banda_shipping_address_map_url_parts', array(
			'address_1'     => $this->shipping_address_1,
			'address_2'     => $this->shipping_address_2,
			'city'          => $this->shipping_city,
			'state'         => $this->shipping_state,
		...
		return apply_filters( 'banda_shipping_address_map_url', 'https://maps.google.com/maps?&q=' . urlencode( implode( ', ', $address ) ) . '&z=16', $this );
	}
	...
	public function get_formatted_billing_full_name() {
		return sprintf( _x( '%1$s %2$s', 'full name', 'banda' ),  $this->billing_first_name, $this->billing_last_name );
	}
	...
	public function get_formatted_shipping_full_name() {
		return sprintf( _x( '%1$s %2$s', 'full name', 'banda' ),  $this->shipping_first_name, $this->shipping_last_name );
	}
	...
	public function get_items( $type = '' ) {
		...
		$items          = array();
		$get_items_sql  = $wpdb->prepare( "SELECT order_item_id, order_item_name, order_item_type FROM {$wpdb->prefix}banda_order_items WHERE order_id = %d ", $this->id );
		...
		return apply_filters( 'banda_order_get_items', $items, $this );
	}
	...
	public function get_item_count( $item_type = '' ) {
		...
		return apply_filters( 'banda_get_item_count', $count, $item_type, $this );
	}
	...
	public function get_tax_totals() {
		...
		if ( apply_filters( 'banda_order_hide_zero_taxes', true ) ) {
			$amounts    = array_filter( wp_list_pluck( $tax_totals, 'amount' ) );
			$tax_totals = array_intersect_key( $tax_totals, $amounts );
		}

		return apply_filters( 'banda_order_tax_totals', $tax_totals, $this );
	}
	...
	public function has_meta( $order_item_id ) {
		...
		return $wpdb->get_results( $wpdb->prepare( "SELECT meta_key, meta_value, meta_id, order_item_id
			FROM {$wpdb->prefix}banda_order_itemmeta WHERE order_item_id = %d
	...
	public function get_item_meta_array( $order_item_id ) {
		...
		if ( false === $item_meta_array ) {
			...
			$metadata        = $wpdb->get_results( $wpdb->prepare( "SELECT meta_key, meta_value, meta_id FROM {$wpdb->prefix}banda_order_itemmeta WHERE order_item_id = %d ORDER BY meta_id", absint( $order_item_id ) ) );
	...
	public function get_total_discount( $ex_tax = true ) {
		...
		return apply_filters( 'banda_order_amount_total_discount', round( $total_discount, wc_get_rounding_precision() ), $this );
	}
	...
	public function get_cart_discount() {
		...
		return apply_filters( 'banda_order_amount_cart_discount', $this->get_total_discount(), $this );
	}
	...
	public function get_order_discount() {
		...
		return apply_filters( 'banda_order_amount_order_discount', (double) $this->order_discount, $this );
	}
	...
	public function get_cart_tax() {
		return apply_filters( 'banda_order_amount_cart_tax', (double) $this->order_tax, $this );
	}
	...
	public function get_shipping_tax() {
		return apply_filters( 'banda_order_amount_shipping_tax', (double) $this->order_shipping_tax, $this );
	}
	...
	public function get_total_tax() {
		return apply_filters( 'banda_order_amount_total_tax', wc_round_tax_total( $this->get_cart_tax() + $this->get_shipping_tax() ), $this );
	}
	...
	public function get_total_shipping() {
		return apply_filters( 'banda_order_amount_total_shipping', (double) $this->order_shipping, $this );
	}
	...
	public function get_total() {
		return apply_filters( 'banda_order_amount_total', (double) $this->order_total, $this );
	}
	...
	public function get_subtotal() {
		...
		return apply_filters( 'banda_order_amount_subtotal', (double) $subtotal, $this );
	}
	...
	public function get_item_subtotal( $item, $inc_tax = false, $round = true ) {
		...
		return apply_filters( 'banda_order_amount_item_subtotal', $price, $this, $item, $inc_tax, $round );
	}
	...
	public function get_line_subtotal( $item, $inc_tax = false, $round = true ) {
		...
		return apply_filters( 'banda_order_amount_line_subtotal', $price, $this, $item, $inc_tax, $round );
	}
	...
	public function get_item_total( $item, $inc_tax = false, $round = true ) {
		...
		return apply_filters( 'banda_order_amount_item_total', $price, $this, $item, $inc_tax, $round );
	}
	...
	public function get_line_total( $item, $inc_tax = false, $round = true ) {
		...
		return apply_filters( 'banda_order_amount_line_total', $line_total, $this, $item, $inc_tax, $round );
	}
	...
	public function get_item_tax( $item, $round = true ) {
		...
		return apply_filters( 'banda_order_amount_item_tax', $price, $item, $round, $this );
	}
	...
	public function get_line_tax( $item ) {
		return apply_filters( 'banda_order_amount_line_tax', wc_round_tax_total( $item['line_tax'] ), $item, $this );
	}
	...
	public function get_shipping_method() {
		...
		if ( $this->shipping_method_title ) {
			...
			foreach ( $shipping_methods as $shipping ) {
				$labels[] = $shipping['name'] ? $shipping['name'] : __( 'Shipping', 'banda' );
			}
		}

		return apply_filters( 'banda_order_shipping_method', implode( ', ', $labels ), $this );
	}
	...
	public function get_formatted_line_subtotal( $item, $tax_display = '' ) {
		...
		return apply_filters( 'banda_order_formatted_line_subtotal', $subtotal, $item, $this );
	}
	...
	public function get_order_currency() {
		return apply_filters( 'banda_get_order_currency', $this->order_currency, $this );
	}
	...
	public function get_formatted_order_total() {
		...
		return apply_filters( 'banda_get_formatted_order_total', $formatted_total, $this );
	}
	...
	public function get_subtotal_to_display( $compound = false, $tax_display = '' ) {
		...
		return apply_filters( 'banda_order_subtotal_to_display', $subtotal, $compound, $this );
	}
		...
		if ( $this->order_shipping != 0 ) {
			...
			if ( $tax_display == 'excl' ) {
				...
				if ( $this->order_shipping_tax != 0 && $this->prices_include_tax ) {
					$shipping .= apply_filters( 'banda_order_shipping_to_display_tax_label', '&nbsp;<small class="tax_label">' . WC()->countries->ex_tax_or_vat() . '</small>', $this, $tax_display );
				}
				...
				if ( $this->order_shipping_tax != 0 && ! $this->prices_include_tax ) {
					$shipping .= apply_filters( 'banda_order_shipping_to_display_tax_label', '&nbsp;<small class="tax_label">' . WC()->countries->inc_tax_or_vat() . '</small>', $this, $tax_display );
				}

			}

			$shipping .= apply_filters( 'banda_order_shipping_to_display_shipped_via', '&nbsp;<small class="shipped_via">' . sprintf( __( 'via %s', 'banda' ), $this->get_shipping_method() ) . '</small>', $this );

		} elseif ( $this->get_shipping_method() ) {
			$shipping = $this->get_shipping_method();
		} else {
			$shipping = __( 'Free!', 'banda' );
		}

		return apply_filters( 'banda_order_shipping_to_display', $shipping, $this );
	}
	...
	public function get_discount_to_display( $tax_display = '' ) {
		...
		return apply_filters( 'banda_order_discount_to_display', wc_price( $this->get_total_discount( $tax_display === 'excl' && $this->display_totals_ex_tax ), array( 'currency' => $this->get_order_currency() ) ), $this );
	}
	...
	public function get_cart_discount_to_display( $tax_display = '' ) {
		...
		return apply_filters( 'banda_order_cart_discount_to_display', $this->get_discount_to_display( $tax_display ), $this );
	}
	...
	public function get_product_from_item( $item ) {
		...
		return apply_filters( 'banda_get_product_from_item', $_product, $item, $this );
	}
	...
	public function get_order_item_totals( $tax_display = '' ) {
		...
		if ( $fees = $this->get_fees() ) {
			foreach ( $fees as $id => $fee ) {
				...
				if ( apply_filters( 'banda_get_order_item_totals_excl_free_fees', $fee['line_total'] + $fee['line_tax'] == 0, $id ) ) {
					continue;
				}
				...
				if ( 'excl' == $tax_display ) {
					...
					$total_rows[ 'fee_' . $id ] = array(
						'label' => ( $fee['name'] ? $fee['name'] : __( 'Fee', 'banda' ) ) . ':',
		...
		if ( 'excl' === $tax_display ) {
			...
			if ( get_option( 'banda_tax_total_display' ) == 'itemized' ) {
		...
		if ( $refunds = $this->get_refunds() ) {
			foreach ( $refunds as $id => $refund ) {
				$total_rows[ 'refund_' . $id ] = array(
					'label' => $refund->get_refund_reason() ? $refund->get_refund_reason() : __( 'Refund', 'banda' ) . ':',
		...
		$total_rows['order_total'] = array(
			'label' => __( 'Total:', 'banda' ),
			'value'	=> $this->get_formatted_order_total( $tax_display )
		);

		return apply_filters( 'banda_get_order_item_totals', $total_rows, $this );
	}
	...
	public function email_order_items_table( $args = array(), $deprecated1 = null, $deprecated2 = null, $deprecated3 = null, $deprecated4 = null, $deprecated5 = null ) {
		...
		wc_get_template( $template, apply_filters( 'banda_email_order_items_args', array(
			'order'               => $this,
		...
		return apply_filters( 'banda_email_order_items_table', ob_get_clean(), $this );
	}
	...
	public function is_paid() {
		return apply_filters( 'banda_order_is_paid', $this->has_status( apply_filters( 'banda_order_is_paid_statuses', array( 'processing', 'completed' ) ) ), $this );
	}
	...
	public function is_download_permitted() {
		return apply_filters( 'banda_order_is_download_permitted', $this->has_status( 'completed' ) || ( get_option( 'banda_downloads_grant_access_after_payment' ) == 'yes' && $this->has_status( 'processing' ) ), $this );
	}
	...
	public function get_checkout_payment_url( $on_checkout = false ) {
		...
		if ( 'yes' == get_option( 'banda_force_ssl_checkout' ) || is_ssl() ) {
		...
		return apply_filters( 'banda_get_checkout_payment_url', $pay_url, $this );
	}
	...
	public function get_checkout_order_received_url() {
		...
		return apply_filters( 'banda_get_checkout_order_received_url', $order_received_url, $this );
	}
	...
	public function get_cancel_order_url( $redirect = '' ) {
		...
		return apply_filters( 'banda_get_cancel_order_url', esc_url( add_query_arg( array(
			'cancel_order' => 'true',
	...
	public function get_cancel_order_url_raw( $redirect = '' ) {
		...
		return apply_filters( 'banda_get_cancel_order_url_raw', add_query_arg( array(
			'cancel_order' => 'true',
			'order'        => $this->order_key,
			'order_id'     => $this->id,
			'redirect'     => $redirect,
	...
	public function get_view_order_url() {
		...
		return apply_filters( 'banda_get_view_order_url', $view_order_url, $this );
	}
	...
	public function get_item_downloads( $item ) {
		...
		$download_ids = $wpdb->get_col( $wpdb->prepare("
			SELECT download_id
			FROM {$wpdb->prefix}banda_downloadable_product_permissions
			WHERE user_email = %s
			AND order_key = %s
			AND product_id = %s
		...
		return apply_filters( 'banda_get_item_downloads', $files, $item, $this );
	}
	...
	public function display_item_downloads( $item ) {
		...
		if ( $product && $product->exists() && $product->is_downloadable() && $this->is_download_permitted() ) {
			...
			foreach ( $download_files as $download_id => $file ) {
				$i++;
				$prefix  = count( $download_files ) > 1 ? sprintf( __( 'Download %d', 'banda' ), $i ) : __( 'Download', 'banda' );
	...
	public function add_order_note( $note, $is_customer_note = 0, $added_by_user = false ) {
		if ( is_user_logged_in() && current_user_can( 'edit_shop_order', $this->id ) && $added_by_user ) {
			...
			$comment_author       = __( 'Banda', 'banda' );
			$comment_author_email = strtolower( __( 'Banda', 'banda' ) ) . '@';
		...
		$comment_parent         = 0;
		$comment_approved       = 1;
		$commentdata            = apply_filters( 'banda_new_order_note_data', compact( 'comment_post_ID', 'comment_author', 'comment_author_email', 'comment_author_url', 'comment_content', 'comment_agent', 'comment_type', 'comment_parent', 'comment_approved' ), array( 'order_id' => $this->id, 'is_customer_note' => $is_customer_note ) );
		...
		if ( $is_customer_note ) {
			...
			do_action( 'banda_new_customer_note', array( 'order_id' => $this->id, 'customer_note' => $commentdata['comment_content'] ) );
		}
	...
	public function update_status( $new_status, $note = '', $manual = false ) {
		...
		if ( ! wp_update_post( $update_post_data ) ) {
			$this->add_order_note( sprintf( __( 'Unable to update order from %1$s to %2$s.', 'banda' ), wc_get_order_status_name( $old_status ), wc_get_order_status_name( $new_status ) ), 0, $manual );
			return false;
		}

		// Status was set.
		do_action( 'banda_order_status_' . $new_status, $this->id );

		// Status was changed.
		if ( $new_status !== $old_status ) {
			$this->add_order_note( trim( $note . ' ' . sprintf( __( 'Order status changed from %1$s to %2$s.', 'banda' ), wc_get_order_status_name( $old_status ), wc_get_order_status_name( $new_status ) ) ), 0, $manual );
			do_action( 'banda_order_status_' . $old_status . '_to_' . $new_status, $this->id );
			do_action( 'banda_order_status_changed', $this->id, $old_status, $new_status );
		} else {
			$this->add_order_note( trim( $note . ' ' . sprintf( __( 'Order status changed to %s.', 'banda' ), wc_get_order_status_name( $new_status ) ) ), 0, $manual );
		}
	...
	public function payment_complete( $transaction_id = '' ) {
		do_action( 'banda_pre_payment_complete', $this->id );

		if ( null !== WC()->session ) {
			WC()->session->set( 'order_awaiting_payment', false );
		}

		$valid_order_statuses = apply_filters( 'banda_valid_order_statuses_for_payment_complete', array( 'on-hold', 'pending', 'failed', 'cancelled' ), $this );
		...
		if ( $this->id && $this->has_status( $valid_order_statuses ) ) {
			...
			if ( sizeof( $this->get_items() ) > 0 ) {
				foreach ( $this->get_items() as $item ) {
					if ( $_product = $this->get_product_from_item( $item ) ) {
						...
						if ( apply_filters( 'banda_order_item_needs_processing', ! $virtual_downloadable_item, $_product, $this->id ) ) {
			...
			$this->update_status( apply_filters( 'banda_payment_complete_order_status', $order_needs_processing ? 'processing' : 'completed', $this->id ) );
			...
			if ( apply_filters( 'banda_payment_complete_reduce_order_stock', ! get_post_meta( $this->id, '_order_stock_reduced', true ), $this->id ) ) {
				$this->reduce_order_stock();
			}

			do_action( 'banda_payment_complete', $this->id );
		} else {
			do_action( 'banda_payment_complete_order_status_' . $this->get_status(), $this->id );
		}
	}
	...
	public function record_product_sales() {
		 ...
		 */
		do_action( 'banda_recorded_sales', $this->id );
	}
	 ...
	 * Runs if stock management is enabled, but can be disabled on per-order basis by extensions @since 2.4.0 via banda_can_reduce_order_stock hook.
	 */
	public function reduce_order_stock() {
		if ( 'yes' === get_option( 'banda_manage_stock' ) && apply_filters( 'banda_can_reduce_order_stock', true, $this ) && sizeof( $this->get_items() ) > 0 ) {
			foreach ( $this->get_items() as $item ) {
				if ( $item['product_id'] > 0 ) {
					$_product = $this->get_product_from_item( $item );

					if ( $_product && $_product->exists() && $_product->managing_stock() ) {
						$qty       = apply_filters( 'banda_order_item_quantity', $item['qty'], $this, $item );
						$new_stock = $_product->reduce_stock( $qty );
						$item_name = $_product->get_sku() ? $_product->get_sku(): $item['product_id'];

						if ( isset( $item['variation_id'] ) && $item['variation_id'] ) {
							$this->add_order_note( sprintf( __( 'Item %1$s variation #%2$s stock reduced from %3$s to %4$s.', 'banda' ), $item_name, $item['variation_id'], $new_stock + $qty, $new_stock) );
						} else {
							$this->add_order_note( sprintf( __( 'Item %1$s stock reduced from %2$s to %3$s.', 'banda' ), $item_name, $new_stock + $qty, $new_stock) );
						}
			...
			do_action( 'banda_reduce_order_stock', $this );
		}
	}
	...
	public function send_stock_notifications( $product, $new_stock, $qty_ordered ) {
		...
		// Backorders
		if ( $new_stock < 0 ) {
			do_action( 'banda_product_on_backorder', array( 'product' => $product, 'order_id' => $this->id, 'quantity' => $qty_ordered ) );
		}

		// stock status notifications
		$notification_sent = false;

		if ( 'yes' == get_option( 'banda_notify_no_stock' ) && get_option( 'banda_notify_no_stock_amount' ) >= $new_stock ) {
			do_action( 'banda_no_stock', $product );
			$notification_sent = true;
		}

		if ( ! $notification_sent && 'yes' == get_option( 'banda_notify_low_stock' ) && get_option( 'banda_notify_low_stock_amount' ) >= $new_stock ) {
			do_action( 'banda_low_stock', $product );
		}

		do_action( 'banda_after_send_stock_notifications', $product, $new_stock, $qty_ordered );
	}
	...
	public function needs_payment() {
		...
		$valid_order_statuses = apply_filters( 'banda_valid_order_statuses_for_payment', array( 'pending', 'failed' ), $this );
		...
		return apply_filters( 'banda_order_needs_payment', $needs_payment, $this, $valid_order_statuses );
	}
	...
	public function needs_shipping_address() {
		...
		$hide          = apply_filters( 'banda_order_hide_shipping_address', array( 'local_pickup' ), $this );
		$needs_address = false;
		...
		return apply_filters( 'banda_order_needs_shipping_address', $needs_address, $hide, $this );
	}
