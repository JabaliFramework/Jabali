...
function wc_get_attribute_taxonomies() {
	if ( false === ( $attribute_taxonomies = get_transient( 'wc_attribute_taxonomies' ) ) ) {
		...
		$attribute_taxonomies = $wpdb->get_results( "SELECT * FROM " . $wpdb->prefix . "banda_attribute_taxonomies order by attribute_name ASC;" );

		set_transient( 'wc_attribute_taxonomies', $attribute_taxonomies );
	}

	return (array) array_filter( apply_filters( 'banda_attribute_taxonomies', $attribute_taxonomies ) );
}
...
function wc_attribute_label( $name, $product = '' ) {
	...
	if ( taxonomy_is_product_attribute( $name ) ) {
		...
		$label = $wpdb->get_var( $wpdb->prepare( "SELECT attribute_label FROM {$wpdb->prefix}banda_attribute_taxonomies WHERE attribute_name = %s;", $name ) );
	...
	return apply_filters( 'banda_attribute_label', $label, $name, $product );
}
...
function wc_attribute_orderby( $name ) {
	...
	if ( isset( $wc_product_attributes[ 'pa_' . $name ] ) ) {
		...
		$orderby = $wpdb->get_var( $wpdb->prepare( "SELECT attribute_orderby FROM " . $wpdb->prefix . "banda_attribute_taxonomies WHERE attribute_name = %s;", $name ) );
	}

	return apply_filters( 'banda_attribute_orderby', $orderby, $name );
}
