...
class WC_Shipping_Zones {
	...
	public static function get_zones() {
		...
		$raw_zones = $wpdb->get_results( "SELECT zone_id, zone_name, zone_order FROM {$wpdb->prefix}banda_shipping_zones order by zone_order ASC;" );
		$zones     = array();
		...
		switch ( $by ) {
			case 'zone_id' :
				if ( 0 === $id ) {
					...
					$raw_zone = $wpdb->get_row( $wpdb->prepare( "SELECT zone_id, zone_name, zone_order FROM {$wpdb->prefix}banda_shipping_zones WHERE zone_id = %d LIMIT 1;", $id ) );
				}
			break;
			case 'instance_id' :
				$zone_id = $wpdb->get_var( $wpdb->prepare( "SELECT zone_id FROM {$wpdb->prefix}banda_shipping_zone_methods as methods WHERE methods.instance_id = %d LIMIT 1;", $id ) );
	...
	public static function get_shipping_method( $instance_id ) {
		global $wpdb;
		$raw_shipping_method = $wpdb->get_row( $wpdb->prepare( "SELECT instance_id, method_id FROM {$wpdb->prefix}banda_shipping_zone_methods WHERE instance_id = %d LIMIT 1;", $instance_id ) );
	...
	public static function get_zone_matching_package( $package ) {
		...
		if ( false === $matching_zone_id ) {
			...
			$postcode_locations = $wpdb->get_results( "SELECT zone_id, location_code FROM {$wpdb->prefix}banda_shipping_zone_locations WHERE location_type = 'postcode';" );
			...
			$matching_zone_id = $wpdb->get_var( "
				SELECT zones.zone_id FROM {$wpdb->prefix}banda_shipping_zones as zones
				LEFT OUTER JOIN {$wpdb->prefix}banda_shipping_zone_locations as locations ON zones.zone_id = locations.zone_id AND location_type != 'postcode'
