...
function wc_get_order_statuses() {
	$order_statuses = array(
		'wc-pending'    => _x( 'Pending Payment', 'Order status', 'banda' ),
		'wc-processing' => _x( 'Processing', 'Order status', 'banda' ),
		'wc-on-hold'    => _x( 'On Hold', 'Order status', 'banda' ),
		'wc-completed'  => _x( 'Completed', 'Order status', 'banda' ),
		'wc-cancelled'  => _x( 'Cancelled', 'Order status', 'banda' ),
		'wc-refunded'   => _x( 'Refunded', 'Order status', 'banda' ),
		'wc-failed'     => _x( 'Failed', 'Order status', 'banda' ),
	);
...
function wc_get_order( $the_order = false ) {
	if ( ! did_action( 'banda_init' ) ) {
		_doing_it_wrong( __FUNCTION__, __( 'wc_get_order should not be called before the banda_init action.', 'banda' ), '2.5' );
		return false;
	}
...
function wc_downloadable_file_permission( $download_id, $product_id, $order, $qty = 1 ) {
	...
	$data = apply_filters( 'banda_downloadable_file_permission_data', array(
	...
	$format = apply_filters( 'banda_downloadable_file_permission_format', array(
		'%s',
		'%s',
		'%s',
		'%s',
	...
	$result = $wpdb->insert( $wpdb->prefix . 'banda_downloadable_product_permissions',
		$data,
		$format
	);

	do_action( 'banda_grant_product_download_access', $data );
...
function wc_downloadable_product_permissions( $order_id ) {
	...
	if ( $order && $order->has_status( 'processing' ) && get_option( 'banda_downloads_grant_access_after_payment' ) == 'no' ) {
		return;
	}
	...
	do_action( 'banda_grant_product_download_permissions', $order_id );
}
add_action( 'banda_order_status_completed', 'wc_downloadable_product_permissions' );
add_action( 'banda_order_status_processing', 'wc_downloadable_product_permissions' );
...
function wc_add_order_item( $order_id, $item ) {
	...
	do_action( 'banda_new_order_item', $item_id, $item, $order_id );
...
function wc_update_order_item( $item_id, $args ) {
	...
	$update = $wpdb->update( $wpdb->prefix . 'banda_order_items', $args, array( 'order_item_id' => $item_id ) );

	if ( false === $update ) {
		return false;
	}

	do_action( 'banda_update_order_item', $item_id, $args );
...
function wc_delete_order_item( $item_id ) {
	...
	do_action( 'banda_before_delete_order_item', $item_id );

	$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}banda_order_items WHERE order_item_id = %d", $item_id ) );
	$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}banda_order_itemmeta WHERE order_item_id = %d", $item_id ) );

	do_action( 'banda_delete_order_item', $item_id );

	return true;
}

/**
 * Banda Order Item Meta API - Update term meta.
 *
 * @access public
 * @param mixed $item_id
...
/**
 * Banda Order Item Meta API - Add term meta.
 *
 * @access public
 * @param mixed $item_id
 * @param mixed $meta_key
...
/**
 * Banda Order Item Meta API - Delete term meta.
 *
 * @access public
 * @param mixed $item_id
 * @param mixed $meta_key
...
/**
 * Banda Order Item Meta API - Get term meta.
 *
 * @access public
 * @param mixed $item_id
 * @param mixed $key
...
function wc_cancel_unpaid_orders() {
	...
	$held_duration = get_option( 'banda_hold_stock_minutes' );

	if ( $held_duration < 1 || get_option( 'banda_manage_stock' ) != 'yes' )
		return;
	...
	if ( $unpaid_orders ) {
		foreach ( $unpaid_orders as $unpaid_order ) {
			...
			if ( apply_filters( 'banda_cancel_unpaid_order', 'checkout' === get_post_meta( $unpaid_order, '_created_via', true ), $order ) ) {
				$order->update_status( 'cancelled', __( 'Unpaid order cancelled - time limit reached.', 'banda' ) );
			}
		}
	}
...
add_action( 'banda_cancel_unpaid_orders', 'wc_cancel_unpaid_orders' );
...
function wc_delete_shop_order_transients( $post_id = 0 ) {
	...
	do_action( 'banda_delete_shop_order_transients', $post_id );
}
...
function wc_ship_to_billing_address_only() {
	return 'billing_only' === get_option( 'banda_ship_to_destination' );
}
...
function wc_create_refund( $args = array() ) {
	...
	if ( $args['refund_id'] > 0 ) {
		...
		$refund_data['post_title']    = sprintf( __( 'Refund &ndash; %s', 'banda' ), strftime( _x( '%b %d, %Y @ %I:%M %p', 'Order date parsed by strftime', 'banda' ) ) );
	...
	if ( $updating ) {
		...
		$refund_id = wp_insert_post( apply_filters( 'banda_new_refund_data', $refund_data ), true );
	}
	...
	if ( ! $updating ) {
		...
		do_action( 'banda_refund_created', $refund_id, $args );
	}
...
function wc_get_tax_class_by_tax_id( $tax_id ) {
	...
	$tax_class = $wpdb->get_var( $wpdb->prepare( "SELECT tax_rate_class FROM {$wpdb->prefix}banda_tax_rates WHERE tax_rate_id = %d", $tax_id ) );
...
add_action( 'banda_order_status_refunded', 'wc_order_fully_refunded' );
...
function wc_order_search( $term ) {
	...
	// Search fields.
	$search_fields = array_map( 'wc_clean', apply_filters( 'banda_shop_order_search_fields', array(
		'_order_key',
		'_billing_company',
		'_billing_address_1',
		'_billing_address_2',
	...
	if ( is_numeric( $term ) ) {
		...
		$post_ids = array_unique( array_merge(
			...
			$wpdb->get_col(
				$wpdb->prepare( "
					SELECT order_id
					FROM {$wpdb->prefix}banda_order_items as order_items
