...
class WC_REST_Product_Reviews_Controller extends WC_REST_Controller {
	...
	public function get_items( $request ) {
		...
		if ( empty( $product->post_type ) || 'product' !== $product->post_type ) {
			return new WP_Error( 'banda_rest_product_invalid_id', __( 'Invalid product id.', 'banda' ), array( 'status' => 404 ) );
		}
	...
	public function prepare_item_for_response( $review, $request ) {
		 ...
		 * @param WP_REST_Request  $request  Request object.
		 */
		return apply_filters( 'banda_rest_prepare_product_review', $response, $review, $request );
	}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'date_created' => array(
					'description' => __( "The date the review was created, in the site's timezone.", 'banda' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'rating' => array(
					'description' => __( 'Review rating (0 to 5).', 'banda' ),
					'type'        => 'integer',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'name' => array(
					'description' => __( 'Reviewer name.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'email' => array(
					'description' => __( 'Reviewer email.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'verified' => array(
					'description' => __( 'Shows if the reviewer bought the product or not.', 'banda' ),
					'type'        => 'boolean',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
