...
class WC_REST_Order_Notes_Controller extends WC_REST_Controller {
	...
	public function get_items( $request ) {
		...
		if ( empty( $order->post_type ) || $this->post_type !== $order->post_type ) {
			return new WP_Error( "banda_rest_{$this->post_type}_invalid_id", __( 'Invalid order ID.', 'banda' ), array( 'status' => 404 ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( empty( $order->post_type ) || $this->post_type !== $order->post_type ) {
			return new WP_Error( 'banda_rest_order_invalid_id', __( 'Invalid order id.', 'banda' ), array( 'status' => 404 ) );
		}
		...
		if ( ! $note_id ) {
			return new WP_Error( 'banda_api_cannot_create_order_note', __( 'Cannot create order note, please try again.', 'banda' ), array( 'status' => 500 ) );
		}
		 ...
		 */
		do_action( 'banda_rest_insert_order_note', $note, $request, true );
	...
	public function delete_item( $request ) {
		...
		if ( ! $result ) {
			return new WP_Error( 'banda_rest_cannot_delete', sprintf( __( 'The %s cannot be deleted.', 'banda' ), 'order_note' ), array( 'status' => 500 ) );
		}
		 ...
		 */
		do_action( 'banda_rest_delete_order_note', $note, $response, $request );
	...
	public function prepare_item_for_response( $note, $request ) {
		 ...
		 * @param WP_REST_Request  $request  Request object.
		 */
		return apply_filters( 'banda_rest_prepare_order_note', $response, $note, $request );
	}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'date_created' => array(
					'description' => __( "The date the order note was created, in the site's timezone.", 'banda' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'note' => array(
					'description' => __( 'Order note.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'customer_note' => array(
					'description' => __( 'Shows/define if the note is only for reference or for the customer (the user will be notified).', 'banda' ),
					'type'        => 'boolean',
					'default'     => false,
					'context'     => array( 'view', 'edit' ),
				),
