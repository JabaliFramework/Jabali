...
class WC_REST_Orders_Controller extends WC_REST_Posts_Controller {
	...
	protected function create_order( $request ) {
		...
		try {
			...
			if ( 0 !== $request['customer_id'] && false === get_user_by( 'id', $request['customer_id'] ) ) {
				throw new WC_REST_Exception( 'banda_rest_invalid_customer_id',__( 'Customer ID is invalid.', 'banda' ), 400 );
			}
			...
			if ( is_wp_error( $order ) ) {
				throw new WC_REST_Exception( 'banda_rest_cannot_create_order', sprintf( __( 'Cannot create order: %s.', 'banda' ), implode( ', ', $order->get_error_messages() ) ), 400 );
			}
	...
	protected function set_line_item( $order, $item, $action = 'create' ) {
		...
		if ( empty( $item['product_id'] ) && empty( $item['sku'] ) && empty( $item['variation_id'] ) ) {
			throw new WC_REST_Exception( 'banda_rest_required_product_reference', __( 'Product ID or SKU is required.', 'banda' ), 400 );
		}
		...
		if ( 'update' === $action && ! empty( $item['id'] ) ) {
			...
			if ( $product_id !== $item_product_id && $product_id !== $item_variation_id ) {
				throw new WC_REST_Exception( 'banda_rest_required_product_reference', __( 'Product ID or variation ID provided does not match this line item.', 'banda' ), 400 );
			}
		}
		...
		if ( ! is_object( $product ) ) {
			throw new WC_REST_Exception( 'banda_rest_invalid_product', __( 'Product is invalid.', 'banda' ), 400 );
		}

		// Quantity must be positive float.
		if ( isset( $item['quantity'] ) && 0 >= floatval( $item['quantity'] ) ) {
			throw new WC_REST_Exception( 'banda_rest_invalid_product_quantity', __( 'Product quantity must be a positive float.', 'banda' ), 400 );
		}

		// Quantity is required when creating.
		if ( $creating && ! isset( $item['quantity'] ) ) {
			throw new WC_REST_Exception( 'banda_rest_invalid_product_quantity', __( 'Product quantity is required.', 'banda' ), 400 );
		}
		...
		if ( $creating ) {
			...
			if ( ! $item_id ) {
				throw new WC_REST_Exception( 'banda_rest_cannot_create_line_item', __( 'Cannot create line item, try again.', 'banda' ), 500 );
			}
		} else {
			$item_id = $order->update_product( $item['id'], $product, $item_args );
			if ( ! $item_id ) {
				throw new WC_REST_Exception( 'banda_rest_cannot_update_line_item', __( 'Cannot update line item, try again.', 'banda' ), 500 );
			}
		}
	}
	...
	protected function set_shipping( $order, $shipping, $action ) {
		...
		if ( ! empty( $shipping['total'] ) && 0 > floatval( $shipping['total'] ) ) {
			throw new WC_REST_Exception( 'banda_rest_invalid_shipping_total', __( 'Shipping total must be a positive amount.', 'banda' ), 400 );
		}

		if ( 'create' === $action ) {
			// Method ID is required.
			if ( empty( $shipping['method_id'] ) ) {
				throw new WC_REST_Exception( 'banda_rest_invalid_shipping_item', __( 'Shipping method ID is required.', 'banda' ), 400 );
			}
			...
			if ( ! $shipping_id ) {
				throw new WC_REST_Exception( 'banda_rest_cannot_create_shipping', __( 'Cannot create shipping method, try again.', 'banda' ), 500 );
			}
			...
			if ( ! $shipping_id ) {
				throw new WC_REST_Exception( 'banda_rest_cannot_update_shipping', __( 'Cannot update shipping method, try again.', 'banda' ), 500 );
			}
		}
	}
	...
	protected function set_fee( $order, $fee, $action ) {
		if ( 'create' === $action ) {
			...
			if ( empty( $fee['name'] ) ) {
				throw new WC_REST_Exception( 'banda_rest_invalid_fee_item', __( 'Fee name is required.', 'banda' ), 400 );
			}
			...
			if ( isset( $fee['tax_status'] ) && 'taxable' === $fee['tax_status'] ) {
				...
				if ( ! isset( $fee['tax_class'] ) ) {
					throw new WC_REST_Exception( 'banda_rest_invalid_fee_item', __( 'Fee tax class is required when fee is taxable.', 'banda' ), 400 );
				}
			...
			if ( ! $fee_id ) {
				throw new WC_REST_Exception( 'banda_rest_cannot_create_fee', __( 'Cannot create fee, try again.', 'banda' ), 500 );
			}
			...
			if ( ! $fee_id ) {
				throw new WC_REST_Exception( 'banda_rest_cannot_update_fee', __( 'Cannot update fee, try again.', 'banda' ), 500 );
			}
		}
	}
	...
	protected function set_coupon( $order, $coupon, $action ) {
		...
		if ( isset( $coupon['discount'] ) && 0 > floatval( $coupon['discount'] ) ) {
			throw new WC_REST_Exception( 'banda_rest_invalid_coupon_total', __( 'Coupon discount must be a positive amount.', 'banda' ), 400 );
		}

		if ( 'create' === $action ) {
			// Coupon code is required.
			if ( empty( $coupon['code'] ) ) {
				throw new WC_REST_Exception( 'banda_rest_invalid_coupon_coupon', __( 'Coupon code is required.', 'banda' ), 400 );
			}

			$coupon_id = $order->add_coupon( $coupon['code'], floatval( $coupon['discount'] ) );

			if ( ! $coupon_id ) {
				throw new WC_REST_Exception( 'banda_rest_cannot_create_order_coupon', __( 'Cannot create coupon, try again.', 'banda' ), 500 );
			}
			...
			if ( ! $coupon_id ) {
				throw new WC_REST_Exception( 'banda_rest_cannot_update_order_coupon', __( 'Cannot update coupon, try again.', 'banda' ), 500 );
			}
		}
	}
	...
	public function create_item( $request ) {
		if ( ! empty( $request['id'] ) ) {
			return new WP_Error( "banda_rest_{$this->post_type}_exists", sprintf( __( 'Cannot create existing %s.', 'banda' ), $this->post_type ), array( 'status' => 400 ) );
		}
		 ...
		 */
		do_action( "banda_rest_insert_{$this->post_type}", $post, $request, true );
	...
	protected function set_item( $order, $item_type, $item, $action ) {
		...
		if ( 'update' === $action ) {
			...
			if ( is_null( $result ) ) {
				throw new WC_REST_Exception( 'banda_rest_invalid_item_id', __( 'Order item ID provided is not associated with order.', 'banda' ), 400 );
			}
		}
	...
	public function update_item( $request ) {
		...
		if ( empty( $id ) || empty( $post->ID ) || $this->post_type !== $post->post_type ) {
			return new WP_Error( "banda_rest_{$this->post_type}_invalid_id", __( 'ID is invalid.', 'banda' ), array( 'status' => 400 ) );
		}
		 ...
		 * @param boolean         $creating  True when creating item, false when updating.
		 */
		do_action( "banda_rest_insert_{$this->post_type}", $post, $request, false );
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'parent_id' => array(
					'description' => __( 'Parent order ID.', 'banda' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
				),
				'status' => array(
					'description' => __( 'Order status.', 'banda' ),
					'type'        => 'string',
					'default'     => 'pending',
				...
				'order_key' => array(
					'description' => __( 'Order key.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'number' => array(
					'description' => __( 'Order number.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'currency' => array(
					'description' => __( 'Currency the order was created with, in ISO format.', 'banda' ),
					'type'        => 'string',
					'default'     => get_banda_currency(),
					'enum'        => array_keys( get_banda_currencies() ),
					'context'     => array( 'view', 'edit' ),
				),
				'version' => array(
					'description' => __( 'Version of Banda when the order was made.', 'banda' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'prices_include_tax' => array(
					'description' => __( 'Shows if the prices included tax during checkout.', 'banda' ),
					'type'        => 'boolean',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'date_created' => array(
					'description' => __( "The date the order was created, in the site's timezone.", 'banda' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'date_modified' => array(
					'description' => __( "The date the order was last modified, in the site's timezone.", 'banda' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'customer_id' => array(
					'description' => __( 'User ID who owns the order. 0 for guests.', 'banda' ),
					'type'        => 'integer',
					'default'     => 0,
					'context'     => array( 'view', 'edit' ),
				),
				'discount_total' => array(
					'description' => __( 'Total discount amount for the order.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'discount_tax' => array(
					'description' => __( 'Total discount tax amount for the order.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'shipping_total' => array(
					'description' => __( 'Total shipping amount for the order.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'shipping_tax' => array(
					'description' => __( 'Total shipping tax amount for the order.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'cart_tax' => array(
					'description' => __( 'Sum of line item taxes only.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'total' => array(
					'description' => __( 'Grand total.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'total_tax' => array(
					'description' => __( 'Sum of all taxes.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'billing' => array(
					'description' => __( 'Billing address.', 'banda' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'properties'  => array(
						'first_name' => array(
						...
						'country' => array(
							'description' => __( 'Country code in ISO 3166-1 alpha-2 format.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'email' => array(
				...
				'shipping' => array(
					'description' => __( 'Shipping address.', 'banda' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'properties'  => array(
						'first_name' => array(
				...
				'payment_method' => array(
					'description' => __( 'Payment method ID.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'payment_method_title' => array(
					'description' => __( 'Payment method title.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'set_paid' => array(
					'description' => __( 'Define if the order is paid. It will set the status to processing and reduce stock items.', 'banda' ),
					'type'        => 'boolean',
					'default'     => false,
					'context'     => array( 'edit' ),
				),
				'transaction_id' => array(
					'description' => __( 'Unique transaction ID.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'customer_ip_address' => array(
					'description' => __( "Customer's IP address.", 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'customer_user_agent' => array(
					'description' => __( 'User agent of the customer.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'created_via' => array(
					'description' => __( 'Shows where the order was created.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'customer_note' => array(
					'description' => __( 'Note left by customer during checkout.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'date_completed' => array(
					'description' => __( "The date the order was completed, in the site's timezone.", 'banda' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'date_paid' => array(
					'description' => __( "The date the order has been paid, in the site's timezone.", 'banda' ),
					'type'        => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'cart_hash' => array(
					'description' => __( 'MD5 hash of cart items to ensure orders are not modified.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'line_items' => array(
					'description' => __( 'Line items data.', 'banda' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'properties'  => array(
						'id' => array(
							'description' => __( 'Item ID.', 'banda' ),
							'type'        => 'integer',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						...
						'sku' => array(
							'description' => __( 'Product SKU.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						'product_id' => array(
							'description' => __( 'Product ID.', 'banda' ),
							'type'        => 'integer',
							'context'     => array( 'view', 'edit' ),
						),
						'variation_id' => array(
							'description' => __( 'Variation ID, if applicable.', 'banda' ),
							'type'        => 'integer',
							'context'     => array( 'view', 'edit' ),
						),
						'quantity' => array(
							'description' => __( 'Quantity ordered.', 'banda' ),
							'type'        => 'integer',
							'context'     => array( 'view', 'edit' ),
						),
						'tax_class' => array(
							'description' => __( 'Tax class of product.', 'banda' ),
							'type'        => 'integer',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						'price' => array(
							'description' => __( 'Product price.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						'subtotal' => array(
							'description' => __( 'Line subtotal (before discounts).', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'subtotal_tax' => array(
							'description' => __( 'Line subtotal tax (before discounts).', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'total' => array(
							'description' => __( 'Line total (after discounts).', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'total_tax' => array(
							'description' => __( 'Line total tax (after discounts).', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'taxes' => array(
							'description' => __( 'Line taxes.', 'banda' ),
							'type'        => 'array',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
							'properties'  => array(
								'id' => array(
									'description' => __( 'Tax rate ID.', 'banda' ),
									'type'        => 'integer',
									'context'     => array( 'view', 'edit' ),
									'readonly'    => true,
								),
								'total' => array(
									'description' => __( 'Tax total.', 'banda' ),
									'type'        => 'string',
									'context'     => array( 'view', 'edit' ),
									'readonly'    => true,
								),
								'subtotal' => array(
									'description' => __( 'Tax subtotal.', 'banda' ),
									'type'        => 'string',
									'context'     => array( 'view', 'edit' ),
									'readonly'    => true,
								),
						...
						'meta' => array(
							'description' => __( 'Line item meta data.', 'banda' ),
							'type'        => 'array',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
							'properties'  => array(
								'key' => array(
									'description' => __( 'Meta key.', 'banda' ),
									'type'        => 'string',
									'context'     => array( 'view', 'edit' ),
									'readonly'    => true,
								),
								'label' => array(
									'description' => __( 'Meta label.', 'banda' ),
									'type'        => 'string',
									'context'     => array( 'view', 'edit' ),
									'readonly'    => true,
								),
								'value' => array(
									'description' => __( 'Meta value.', 'banda' ),
									'type'        => 'string',
									'context'     => array( 'view', 'edit' ),
									'readonly'    => true,
								),
				...
				'tax_lines' => array(
					'description' => __( 'Tax lines data.', 'banda' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
					'properties'  => array(
						...
						'rate_code' => array(
							'description' => __( 'Tax rate code.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						...
						'label' => array(
							'description' => __( 'Tax rate label.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						'compound' => array(
							'description' => __( 'Show if is a compound tax rate.', 'banda' ),
							'type'        => 'boolean',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						'tax_total' => array(
							'description' => __( 'Tax total (not including shipping taxes).', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						'shipping_tax_total' => array(
							'description' => __( 'Shipping tax total.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
				...
				'shipping_lines' => array(
					'description' => __( 'Shipping lines data.', 'banda' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'properties'  => array(
						'id' => array(
						...
						'method_title' => array(
							'description' => __( 'Shipping method name.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'method_id' => array(
							'description' => __( 'Shipping method ID.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'total' => array(
				...
				'fee_lines' => array(
					'description' => __( 'Fee lines data.', 'banda' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'properties'  => array(
						'id' => array(
						...
						'name' => array(
							'description' => __( 'Fee name.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'tax_class' => array(
							'description' => __( 'Tax class of fee.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'tax_status' => array(
							'description' => __( 'Tax status of fee.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'total' => array(
				...
				'coupon_lines' => array(
					'description' => __( 'Coupons line data.', 'banda' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'properties'  => array(
						'id' => array(
						...
						'code' => array(
							'description' => __( 'Coupon code.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'discount' => array(
							'description' => __( 'Discount total.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
						),
						'discount_tax' => array(
							'description' => __( 'Discount total tax.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
				...
				'refunds' => array(
					'description' => __( 'List of refunds.', 'banda' ),
					'type'        => 'array',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
					'properties'  => array(
						'id' => array(
							'description' => __( 'Refund ID.', 'banda' ),
							'type'        => 'integer',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						'reason' => array(
							'description' => __( 'Refund reason.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
						'total' => array(
							'description' => __( 'Refund total.', 'banda' ),
							'type'        => 'string',
							'context'     => array( 'view', 'edit' ),
							'readonly'    => true,
						),
	...
	public function get_collection_params() {
		...
		$params['status'] = array(
			'default'           => 'any',
			'description'       => __( 'Limit result set to orders assigned a specific status.', 'banda' ),
			'type'              => 'string',
		...
		$params['customer'] = array(
			'description'       => __( 'Limit result set to orders assigned a specific customer.', 'banda' ),
			'type'              => 'integer',
			'sanitize_callback' => 'absint',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['product'] = array(
			'description'       => __( 'Limit result set to orders assigned a specific product.', 'banda' ),
			'type'              => 'integer',
			'sanitize_callback' => 'absint',
			'validate_callback' => 'rest_validate_request_arg',
		);
		$params['dp'] = array(
			'default'           => 2,
			'description'       => __( 'Number of decimal points to use in each resource.', 'banda' ),
			'type'              => 'integer',
			'sanitize_callback' => 'absint',
			'validate_callback' => 'rest_validate_request_arg',
		);
