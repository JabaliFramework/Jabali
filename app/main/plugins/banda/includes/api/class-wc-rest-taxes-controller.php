...
class WC_REST_Taxes_Controller extends WC_REST_Controller {
	...
	public function get_items( $request ) {
		 ...
		 */
		$prepared_args = apply_filters( 'banda_rest_tax_query', $prepared_args, $request );
	...
	public function create_item( $request ) {
		if ( ! empty( $request['id'] ) ) {
			return new WP_Error( 'banda_rest_tax_exists', __( 'Cannot create existing resource.', 'banda' ), array( 'status' => 400 ) );
		}
		 ...
		 */
		do_action( 'banda_rest_insert_tax', $tax, $request, true );
	...
	public function update_item( $request ) {
		 ...
		 * @param boolean         $creating  True when creating tax, false when updating tax.
		 */
		do_action( 'banda_rest_insert_tax', $tax, $request, false );
	...
	public function delete_item( $request ) {
		...
		if ( ! $force ) {
			return new WP_Error( 'banda_rest_trash_not_supported', __( 'Taxes do not support trashing.', 'banda' ), array( 'status' => 501 ) );
		}
		 ...
		 */
		do_action( 'banda_rest_delete_tax', $tax, $response, $request );
	...
	public function prepare_item_for_response( $tax, $request ) {
		 ...
		 * @param WP_REST_Request  $request  Request object.
		 */
		return apply_filters( 'banda_rest_prepare_tax', $response, $tax, $request );
	}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'country' => array(
					'description' => __( 'Country ISO 3166 code.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'state' => array(
					'description' => __( 'State code.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'postcode' => array(
					'description' => __( 'Postcode/ZIP.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'city' => array(
				...
				'rate' => array(
					'description' => __( 'Tax rate.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'name' => array(
					'description' => __( 'Tax rate name.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
				),
				'priority' => array(
					'description' => __( 'Tax priority.', 'banda' ),
					'type'        => 'integer',
					'default'     => 1,
					'context'     => array( 'view', 'edit' ),
				),
				'compound' => array(
					'description' => __( 'Whether or not this is a compound rate.', 'banda' ),
					'type'        => 'boolean',
					'default'     => false,
					'context'     => array( 'view', 'edit' ),
				),
				'shipping' => array(
					'description' => __( 'Whether or not this tax rate also gets applied to shipping.', 'banda' ),
					'type'        => 'boolean',
					'default'     => true,
					'context'     => array( 'view', 'edit' ),
				),
				'order' => array(
					'description' => __( 'Indicates the order that will appear in queries.', 'banda' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
				),
				'class' => array(
	...
	public function get_collection_params() {
		...
		$params['class'] = array(
			'description'        => __( 'Sort by tax class.', 'banda' ),
			'enum'               => array_merge( array( 'standard' ), array_map( 'sanitize_title', WC_Tax::get_tax_classes() ) ),
