...
class WC_REST_Product_Attributes_Controller extends WC_REST_Controller {
	...
	public function create_item_permissions_check( $request ) {
		if ( ! wc_rest_check_manager_permissions( 'attributes', 'create' ) ) {
			return new WP_Error( 'banda_rest_cannot_create', __( 'Sorry, you cannot create new resource.', 'banda' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function get_item_permissions_check( $request ) {
		if ( ! $this->get_taxonomy( $request ) ) {
			return new WP_Error( "banda_rest_taxonomy_invalid", __( "Resource doesn't exist.", 'banda' ), array( 'status' => 404 ) );
		}
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! wc_rest_check_manager_permissions( 'attributes', 'edit' ) ) {
			return new WP_Error( 'banda_rest_cannot_update', __( 'Sorry, you cannot update resource.', 'banda' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! wc_rest_check_manager_permissions( 'attributes', 'delete' ) ) {
			return new WP_Error( 'banda_rest_cannot_delete', __( 'Sorry, you cannot delete resource.', 'banda' ), array( 'status' => rest_authorization_required_code() ) );
		}
	...
	public function create_item( $request ) {
		...
		if ( is_wp_error( $insert ) ) {
			return new WP_Error( 'banda_rest_cannot_create', $insert->get_error_message(), array( 'status' => 400 ) );
		}
		 ...
		 */
		do_action( 'banda_rest_insert_product_attribute', $attribute, $request, true );
	...
	public function update_item( $request ) {
		...
		// Checks for errors.
		if ( false === $update ) {
			return new WP_Error( 'banda_rest_cannot_edit', __( 'Could not edit the attribute', 'banda' ), array( 'status' => 400 ) );
		}
		 ...
		 * @param boolean         $creating  True when creating attribute, false when updating.
		 */
		do_action( 'banda_rest_insert_product_attribute', $attribute, $request, false );
	...
	public function delete_item( $request ) {
		...
		if ( ! $force ) {
			return new WP_Error( 'banda_rest_trash_not_supported', __( 'Resource does not support trashing.', 'banda' ), array( 'status' => 501 ) );
		}
		 ...
		 */
		do_action( 'banda_rest_delete_product_attribute', $attribute, $response, $request );

		// Fires banda_attribute_deleted hook.
		do_action( 'banda_attribute_deleted', $attribute->attribute_id, $attribute->attribute_name, $taxonomy );
	...
	public function prepare_item_for_response( $item, $request ) {
		 ...
		 */
		return apply_filters( 'banda_rest_prepare_product_attribute', $response, $item, $request );
	}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties'           => array(
				...
				'slug' => array(
					'description' => __( 'An alphanumeric identifier for the resource unique to its type.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'arg_options' => array(
				...
				'type' => array(
					'description' => __( 'Type of attribute.', 'banda' ),
					'type'        => 'string',
					'default'     => 'select',
				...
				'order_by' => array(
					'description' => __( 'Default sort order.', 'banda' ),
					'type'        => 'string',
					'default'     => 'menu_order',
				...
				'has_archives' => array(
					'description' => __( 'Enable/Disable attribute archives.', 'banda' ),
					'type'        => 'boolean',
					'default'     => false,
					'context'     => array( 'view', 'edit' ),
				),
	...
	protected function get_attribute( $id ) {
		...
		if ( is_wp_error( $attribute ) || is_null( $attribute ) ) {
			return new WP_Error( 'banda_rest_attribute_invalid', __( "Resource doesn't exist.", 'banda' ), array( 'status' => 404 ) );
		}
	...
	protected function validate_attribute_slug( $slug, $new_data = true ) {
		if ( strlen( $slug ) >= 28 ) {
			return new WP_Error( 'banda_rest_invalid_product_attribute_slug_too_long', sprintf( __( 'Slug "%s" is too long (28 characters max).', 'banda' ), $slug ), array( 'status' => 400 ) );
		} elseif ( wc_check_if_attribute_name_is_reserved( $slug ) ) {
			return new WP_Error( 'banda_rest_invalid_product_attribute_slug_reserved_name', sprintf( __( 'Slug "%s" is not allowed because it is a reserved term.', 'banda' ), $slug ), array( 'status' => 400 ) );
		} elseif ( $new_data && taxonomy_exists( wc_attribute_taxonomy_name( $slug ) ) ) {
			return new WP_Error( 'banda_rest_invalid_product_attribute_slug_already_exists', sprintf( __( 'Slug "%s" is already in use.', 'banda' ), $slug ), array( 'status' => 400 ) );
		}
