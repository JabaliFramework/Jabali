...
class WC_REST_Tax_Classes_Controller extends WC_REST_Controller {
	...
	public function create_item( $request ) {
		...
		if ( $exists ) {
			return new WP_Error( 'banda_rest_tax_class_exists', __( 'Cannot create existing resource.', 'banda' ), array( 'status' => 400 ) );
		}
		 ...
		 */
		do_action( 'banda_rest_insert_tax_class', (object) $tax_class, $request, true );
	...
	public function delete_item( $request ) {
		...
		$wpdb->delete( $wpdb->prefix . 'banda_tax_rates', array( 'tax_rate_class' => $tax_class['slug'] ), array( '%s' ) );
		 ...
		 */
		do_action( 'banda_rest_delete_tax', (object) $tax_class, $response, $request );
	...
	public function prepare_item_for_response( $tax_class, $request ) {
		 ...
		 * @param WP_REST_Request  $request   Request object.
		 */
		return apply_filters( 'banda_rest_prepare_tax', $response, (object) $tax_class, $request );
	}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'name' => array(
					'description' => __( 'Tax class name.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view', 'edit' ),
					'required'    => true,
					'arg_options' => array(
