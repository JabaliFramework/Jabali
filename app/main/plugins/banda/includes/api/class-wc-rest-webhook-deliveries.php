...
class WC_REST_Webhook_Deliveries_Controller extends WC_REST_Controller {
	...
	public function get_items( $request ) {
		...
		if ( empty( $webhook->post_data->post_type ) || 'shop_webhook' !== $webhook->post_data->post_type ) {
			return new WP_Error( 'banda_rest_webhook_invalid_id', __( 'Invalid webhook id.', 'banda' ), array( 'status' => 404 ) );
		}
	...
	public function prepare_item_for_response( $log, $request ) {
		 ...
		 * @param WP_REST_Request  $request  Request object.
		 */
		return apply_filters( 'banda_rest_prepare_webhook_delivery', $response, $log, $request );
	}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				...
				'duration' => array(
					'description' => __( 'The delivery duration, in seconds.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'summary' => array(
					'description' => __( 'A friendly summary of the response including the HTTP response code, message, and body.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'request_url' => array(
					'description' => __( 'The URL where the webhook was delivered.', 'banda' ),
					'type'        => 'string',
					'format'      => 'uri',
					'context'     => array( 'view' ),
					'readonly'    => true,
				...
				'request_headers' => array(
					'description' => __( 'Request headers.', 'banda' ),
					'type'        => 'array',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'request_body' => array(
					'description' => __( 'Request body.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'response_code' => array(
					'description' => __( 'The HTTP response code from the receiving server.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'response_message' => array(
					'description' => __( 'The HTTP response message from the receiving server.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'response_headers' => array(
					'description' => __( 'Array of the response headers from the receiving server.', 'banda' ),
					'type'        => 'array',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'response_body' => array(
					'description' => __( 'The response body from the receiving server.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'date_created' => array(
					'description' => __( "The date the webhook delivery was logged, in the site's timezone.", 'banda' ),
					'type'        => 'date-time',
