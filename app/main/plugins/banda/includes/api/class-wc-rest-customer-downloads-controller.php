...
class WC_REST_Customer_Downloads_Controller extends WC_REST_Controller {
	...
	public function get_items_permissions_check( $request ) {
		...
		if ( ! $customer ) {
			return new WP_Error( "banda_rest_customer_invalid", __( "Resource doesn't exist.", 'banda' ), array( 'status' => 404 ) );
		}
	...
	public function prepare_item_for_response( $download, $request ) {
		 ...
		 * @param WP_REST_Request  $request   Request object.
		 */
		return apply_filters( 'banda_rest_prepare_customer_download', $response, $download, $request );
	}
	...
	public function get_item_schema() {
		$schema = array(
			...
			'properties' => array(
				'download_url' => array(
					'description' => __( 'Download file URL.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'download_id' => array(
					'description' => __( 'Download ID (MD5).', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'product_id' => array(
					'description' => __( 'Downloadable product ID.', 'banda' ),
					'type'        => 'integer',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'download_name' => array(
					'description' => __( 'Downloadable file name.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'order_id' => array(
					'description' => __( 'Order ID.', 'banda' ),
					'type'        => 'integer',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				...
				'downloads_remaining' => array(
					'description' => __( 'Amount of downloads remaining.', 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'access_expires' => array(
					'description' => __( "The date when the download access expires, in the site's timezone.", 'banda' ),
					'type'        => 'string',
					'context'     => array( 'view' ),
					'readonly'    => true,
				),
				'file' => array(
					'description' => __( 'File details.', 'banda' ),
					'type'        => 'array',
					'context'     => array( 'view' ),
					'readonly'    => true,
					'properties' => array(
