...
class WC_API_Server {
	...
	public function __construct( $path ) {
		...
		$handler_class = apply_filters( 'banda_api_default_response_handler', 'WC_API_JSON_Handler', $this->path, $this );
	...
	public function check_authentication() {
		...
		$user = apply_filters( 'banda_api_check_authentication', null, $this );
		...
		elseif ( ! is_wp_error( $user ) ) {
			$user = new WP_Error( 'banda_api_authentication_error', __( 'Invalid authentication method', 'banda' ), array( 'code' => 500 ) );
		}
	...
	public function serve_request() {
		...
		do_action( 'banda_api_server_before_serve', $this );

		$this->header( 'Content-Type', $this->handler->get_content_type(), true );

		// the API is enabled by default
		if ( ! apply_filters( 'banda_api_enabled', true, $this ) || ( 'no' === get_option( 'banda_api_enabled' ) ) ) {

			$this->send_status( 404 );

			echo $this->handler->generate_response( array( 'errors' => array( 'code' => 'banda_api_disabled', 'message' => 'The Banda API is disabled on this site' ) ) );
		...
		$served = apply_filters( 'banda_api_serve_request', false, $result, $this );
	...
	public function get_routes() {
		...
		$endpoints = apply_filters( 'banda_api_endpoints', $endpoints );
	...
	public function dispatch() {
		...
		switch ( $this->method ) {
			...
			default :
				return new WP_Error( 'banda_api_unsupported_method', __( 'Unsupported request method', 'banda' ), array( 'status' => 400 ) );
		}
		...
		foreach ( $this->get_routes() as $route => $handlers ) {
			foreach ( $handlers as $handler ) {
				...
				if ( ! is_callable( $callback ) ) {
					return new WP_Error( 'banda_api_invalid_handler', __( 'The handler for the route is invalid', 'banda' ), array( 'status' => 500 ) );
				}
				...
				$args = apply_filters( 'banda_api_dispatch_args', $args, $callback );
		...
		return new WP_Error( 'banda_api_no_route', __( 'No route was found matching the URL and request method', 'banda' ), array( 'status' => 404 ) );
	}
	...
	protected function sort_callback_params( $callback, $provided ) {
		...
		foreach ( $wanted as $param ) {
			if ( isset( $provided[ $param->getName() ] ) ) {
				...
				return new WP_Error( 'banda_api_missing_callback_param', sprintf( __( 'Missing parameter %s', 'banda' ), $param->getName() ), array( 'status' => 400 ) );
			}
		}
	...
	public function get_index() {
		...
		$available = array( 'store' => array(
			...
			'meta'        => array(
				...
				'currency_format'    => get_banda_currency_symbol(),
				'currency_position'  => get_option( 'banda_currency_pos' ),
				'thousand_separator' => get_option( 'banda_price_thousand_sep' ),
				'decimal_separator'  => get_option( 'banda_price_decimal_sep' ),
				...
				'ssl_enabled'        => ( 'yes' === get_option( 'banda_force_ssl_checkout' ) || wc_site_is_https() ),
				'permalinks_enabled' => ( '' !== get_option( 'permalink_structure' ) ),
				'generate_password'  => ( 'yes' === get_option( 'banda_registration_generate_password' ) ),
				'links'              => array(
					'help' => 'https://mtaandao.github.io/banda-rest-api-docs/',
				),
			),
		) );
			...
			foreach ( self::$method_map as $name => $bitmask ) {
				foreach ( $callbacks as $callback ) {
					...
					if ( strpos( $route, '<' ) === false ) {
						$data['meta'] = array(
							'self' => get_banda_api_url( $route ),
						);
					}
				}
			}

			$available['store']['routes'][ $route ] = apply_filters( 'banda_api_endpoints_description', $data );
		}

		return apply_filters( 'banda_api_index', $available );
	}
	...
	public function add_pagination_headers( $query ) {
		...
		do_action( 'banda_api_pagination_headers', $this, $query );
	}
