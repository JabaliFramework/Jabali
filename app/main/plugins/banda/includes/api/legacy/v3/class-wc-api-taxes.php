...
class WC_API_Taxes extends WC_API_Resource {
	...
	public function get_tax( $id, $fields = null ) {
		...
		try {
			...
			if ( ! current_user_can( 'manage_banda' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_read_tax', __( 'You do not have permission to read tax rate', 'banda' ), 401 );
			}
			...
			if ( is_wp_error( $tax ) || empty( $tax ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_tax_id', __( 'A tax rate with the provided ID could not be found', 'banda' ), 404 );
			}
			...
			$locales = $wpdb->get_results( $wpdb->prepare( "
				...
				FROM {$wpdb->prefix}banda_tax_rate_locations
				WHERE tax_rate_id = %d
			", $id ) );
			...
			return array( 'tax' => apply_filters( 'banda_api_tax_response', $tax_data, $tax, $fields, $this ) );
	...
	public function create_tax( $data ) {
		try {
			if ( ! isset( $data['tax'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_tax_data', sprintf( __( 'No %1$s data specified to create %1$s', 'banda' ), 'tax' ), 400 );
			}

			// Check permissions
			if ( ! current_user_can( 'manage_banda' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_create_tax', __( 'You do not have permission to create tax rates', 'banda' ), 401 );
			}

			$data = apply_filters( 'banda_api_create_tax_data', $data['tax'], $this );
			...
			do_action( 'banda_api_create_tax', $id, $data );
	...
	public function edit_tax( $id, $data ) {
		try {
			if ( ! isset( $data['tax'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_tax_data', sprintf( __( 'No %1$s data specified to edit %1$s', 'banda' ), 'tax' ), 400 );
			}

			// Check permissions
			if ( ! current_user_can( 'manage_banda' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_edit_tax', __( 'You do not have permission to edit tax rates', 'banda' ), 401 );
			}
			...
			$data           = apply_filters( 'banda_api_edit_tax_data', $data, $this );
			$tax_data       = array();
			...
			do_action( 'banda_api_edit_tax_rate', $id, $data );
	...
	public function delete_tax( $id ) {
		...
		try {
			// Check permissions
			if ( ! current_user_can( 'manage_banda' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_delete_tax', __( 'You do not have permission to delete tax rates', 'banda' ), 401 );
			}
			...
			if ( 0 === $wpdb->rows_affected ) {
				throw new WC_API_Exception( 'banda_api_cannot_delete_tax', __( 'Could not delete the tax rate', 'banda' ), 401 );
			}

			return array( 'message' => sprintf( __( 'Deleted %s', 'banda' ), 'tax' ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function get_taxes_count( $class = null, $filter = array() ) {
		try {
			if ( ! current_user_can( 'manage_banda' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_read_taxes_count', __( 'You do not have permission to read the taxes count', 'banda' ), 401 );
			}
	...
	protected function query_tax_rates( $args, $count_only = false ) {
		...
		$query = "
			SELECT tax_rate_id
			FROM {$wpdb->prefix}banda_tax_rates
			WHERE 1 = 1
		";
	...
	public function bulk( $data ) {
		try {
			if ( ! isset( $data['taxes'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_taxes_data', sprintf( __( 'No %1$s data specified to create/edit %1$s', 'banda' ), 'taxes' ), 400 );
			}

			$data  = $data['taxes'];
			$limit = apply_filters( 'banda_api_bulk_limit', 100, 'taxes' );

			// Limit bulk operation
			if ( count( $data ) > $limit ) {
				throw new WC_API_Exception( 'banda_api_taxes_request_entity_too_large', sprintf( __( 'Unable to accept more than %s items for this request', 'banda' ), $limit ), 413 );
			}
			...
			return array( 'taxes' => apply_filters( 'banda_api_taxes_bulk_response', $taxes, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function get_tax_classes( $fields = null ) {
		try {
			// Permissions check
			if ( ! current_user_can( 'manage_banda' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_read_tax_classes', __( 'You do not have permission to read tax classes', 'banda' ), 401 );
			}
			...
			$tax_classes[] = array(
				'slug' => 'standard',
				'name' => __( 'Standard Rate', 'banda' )
			);

			$classes = WC_Tax::get_tax_classes();

			foreach ( $classes as $class ) {
				$tax_classes[] = apply_filters( 'banda_api_tax_class_response', array(
					'slug' => sanitize_title( $class ),
					'name' => $class
				), $class, $fields, $this );
			}

			return array( 'tax_classes' => apply_filters( 'banda_api_tax_classes_response', $tax_classes, $classes, $fields, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function create_tax_class( $data ) {
		try {
			if ( ! isset( $data['tax_class'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_tax_class_data', sprintf( __( 'No %1$s data specified to create %1$s', 'banda' ), 'tax_class' ), 400 );
			}

			// Check permissions
			if ( ! current_user_can( 'manage_banda' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_create_tax_class', __( 'You do not have permission to create tax classes', 'banda' ), 401 );
			}

			$data = $data['tax_class'];

			if ( empty( $data['name'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_tax_class_name', sprintf( __( 'Missing parameter %s', 'banda' ), 'name' ), 400 );
			}
			...
			if ( $exists ) {
				throw new WC_API_Exception( 'banda_api_cannot_create_tax_class', __( 'Tax class already exists', 'banda' ), 401 );
			}

			// Add the new class.
			$classes[] = $name;

			update_option( 'banda_tax_classes', implode( "\n", $classes ) );

			do_action( 'banda_api_create_tax_class', $slug, $data );
	...
	public function delete_tax_class( $slug ) {
		...
		try {
			// Check permissions
			if ( ! current_user_can( 'manage_banda' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_delete_tax_class', __( 'You do not have permission to delete tax classes', 'banda' ), 401 );
			}
			...
			if ( ! $deleted ) {
				throw new WC_API_Exception( 'banda_api_cannot_delete_tax_class', __( 'Could not delete the tax class', 'banda' ), 401 );
			}
			...
			$wpdb->query( $wpdb->prepare( "
				DELETE locations.*
				FROM {$wpdb->prefix}banda_tax_rate_locations AS locations
				INNER JOIN
					{$wpdb->prefix}banda_tax_rates AS rates
					ON rates.tax_rate_id = locations.tax_rate_id
				WHERE rates.tax_rate_class = '%s'
			", $slug ) );

			// Delete tax rates in the selected class.
			$wpdb->delete( $wpdb->prefix . 'banda_tax_rates', array( 'tax_rate_class' => $slug ), array( '%s' ) );

			return array( 'message' => sprintf( __( 'Deleted %s', 'banda' ), 'tax_class' ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function get_tax_classes_count() {
		try {
			if ( ! current_user_can( 'manage_banda' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_read_tax_classes_count', __( 'You do not have permission to read the tax classes count', 'banda' ), 401 );
			}
