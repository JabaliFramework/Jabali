...
class WC_API_Orders extends WC_API_Resource {
	...
	public function get_order( $id, $fields = null, $filter = array() ) {
		...
		return array( 'order' => apply_filters( 'banda_api_order_response', $order_data, $order, $fields, $this->server ) );
	}
	...
	public function get_orders_count( $status = null, $filter = array() ) {
		...
		try {
			if ( ! current_user_can( 'read_private_shop_orders' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_read_orders_count', __( 'You do not have permission to read the orders count', 'banda' ), 401 );
			}
	...
	public function get_order_statuses() {
		...
		return array( 'order_statuses' => apply_filters( 'banda_api_order_statuses_response', $order_statuses, $this ) );
	}
	...
	public function create_order( $data ) {
		...
		try {
			if ( ! isset( $data['order'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_order_data', sprintf( __( 'No %1$s data specified to create %1$s', 'banda' ), 'order' ), 400 );
			}
			...
			if ( ! current_user_can( 'publish_shop_orders' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_create_order', __( 'You do not have permission to create orders', 'banda' ), 401 );
			}

			$data = apply_filters( 'banda_api_create_order_data', $data, $this );
			...
			if ( ! empty( $data['customer_id'] ) ) {
				...
				if ( false === get_user_by( 'id', $data['customer_id'] ) ) {
					throw new WC_API_Exception( 'banda_api_invalid_customer_id', __( 'Customer ID is invalid', 'banda' ), 400 );
				}
			...
			if ( is_wp_error( $order ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_create_order', sprintf( __( 'Cannot create order: %s', 'banda' ), implode( ', ', $order->get_error_messages() ) ), 400 );
			}
			...
			if ( isset( $data['payment_details'] ) && is_array( $data['payment_details'] ) ) {
				...
				if ( empty( $data['payment_details']['method_id'] ) || empty( $data['payment_details']['method_title'] ) ) {
					throw new WC_API_Exception( 'banda_invalid_payment_details', __( 'Payment method ID and title are required', 'banda' ), 400 );
				}
			...
			if ( isset( $data['currency'] ) ) {
				...
				if ( ! array_key_exists( $data['currency'], get_banda_currencies() ) ) {
					throw new WC_API_Exception( 'banda_invalid_order_currency', __( 'Provided order currency is invalid', 'banda'), 400 );
				}
			...
			do_action( 'banda_api_create_order', $order->id, $data, $this );
	...
	public function edit_order( $id, $data ) {
		try {
			if ( ! isset( $data['order'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_order_data', sprintf( __( 'No %1$s data specified to edit %1$s', 'banda' ), 'order' ), 400 );
			}
			...
			$data  = apply_filters( 'banda_api_edit_order_data', $data, $id, $this );
			$order = wc_get_order( $id );

			if ( empty( $order ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_order_id', __( 'Order ID is invalid', 'banda' ), 400 );
			}
			...
			do_action( 'banda_api_edit_order', $order->id, $data, $this );
	...
	public function delete_order( $id, $force = false ) {
		...
		do_action( 'banda_api_delete_order', $id, $this );
	...
	protected function set_item( $order, $item_type, $item, $action ) {
		...
		if ( 'update' === $action ) {
			...
			$result = $wpdb->get_row(
				$wpdb->prepare( "SELECT * FROM {$wpdb->prefix}banda_order_items WHERE order_item_id = %d AND order_id = %d",
				absint( $item['id'] ),
				absint( $order->id )
			) );

			if ( is_null( $result ) ) {
				throw new WC_API_Exception( 'banda_invalid_item_id', __( 'Order item ID provided is not associated with order', 'banda' ), 400 );
			}
		}
	...
	protected function set_line_item( $order, $item, $action ) {
		...
		if ( ! isset( $item['product_id'] ) && ! isset( $item['sku'] ) ) {
			throw new WC_API_Exception( 'banda_api_invalid_product_id', __( 'Product ID or SKU is required', 'banda' ), 400 );
		}
		...
		if ( 'update' === $action ) {
			...
			if ( $item['product_id'] != $item_product_id && $item['product_id'] != $item_variation_id ) {
				throw new WC_API_Exception( 'banda_api_invalid_product_id', __( 'Product ID provided does not match this line item', 'banda' ), 400 );
			}
		}
		...
		if ( isset( $item['variations'] ) && is_array( $item['variations'] ) ) {
			foreach ( $item['variations'] as $key => $value ) {
				if ( ! $key || ! $value ) {
					throw new WC_API_Exception( 'banda_api_invalid_product_variation', __( 'The product variation is invalid', 'banda' ), 400 );
				}
			}
		...
		if ( ! is_object( $product ) ) {
			throw new WC_API_Exception( 'banda_api_invalid_product', __( 'Product is invalid', 'banda' ), 400 );
		}

		// quantity must be positive float
		if ( isset( $item['quantity'] ) && floatval( $item['quantity'] ) <= 0 ) {
			throw new WC_API_Exception( 'banda_api_invalid_product_quantity', __( 'Product quantity must be a positive float', 'banda' ), 400 );
		}

		// quantity is required when creating
		if ( $creating && ! isset( $item['quantity'] ) ) {
			throw new WC_API_Exception( 'banda_api_invalid_product_quantity', __( 'Product quantity is required', 'banda' ), 400 );
		}
		...
		$item_args = apply_filters( 'banda_api_order_line_item_args', $item_args, $item, $order, $action );
		...
		if ( $creating ) {
			...
			if ( ! $item_id ) {
				throw new WC_API_Exception( 'banda_cannot_create_line_item', __( 'Cannot create line item, try again', 'banda' ), 500 );
			}
			...
			if ( ! $item_id ) {
				throw new WC_API_Exception( 'banda_cannot_update_line_item', __( 'Cannot update line item, try again', 'banda' ), 500 );
			}
		}
	}
	...
	protected function set_shipping( $order, $shipping, $action ) {
		...
		if ( isset( $shipping['total'] ) && floatval( $shipping['total'] ) < 0 ) {
			throw new WC_API_Exception( 'banda_invalid_shipping_total', __( 'Shipping total must be a positive amount', 'banda' ), 400 );
		}
		...
		if ( 'create' === $action ) {
			...
			if ( ! isset( $shipping['method_id'] ) ) {
				throw new WC_API_Exception( 'banda_invalid_shipping_item', __( 'Shipping method ID is required', 'banda' ), 400 );
			}
			...
			if ( ! $shipping_id ) {
				throw new WC_API_Exception( 'banda_cannot_create_shipping', __( 'Cannot create shipping method, try again', 'banda' ), 500 );
			}
			...
			if ( ! $shipping_id ) {
				throw new WC_API_Exception( 'banda_cannot_update_shipping', __( 'Cannot update shipping method, try again', 'banda' ), 500 );
			}
		}
	}
	...
	protected function set_fee( $order, $fee, $action ) {
		...
		if ( 'create' === $action ) {
			...
			if ( ! isset( $fee['title'] ) ) {
				throw new WC_API_Exception( 'banda_invalid_fee_item', __( 'Fee title is required', 'banda' ), 400 );
			}
			...
			if ( isset( $fee['taxable'] ) && $fee['taxable'] ) {
				...
				if ( ! isset( $fee['tax_class'] ) ) {
					throw new WC_API_Exception( 'banda_invalid_fee_item', __( 'Fee tax class is required when fee is taxable', 'banda' ), 400 );
				}
			...
			if ( ! $fee_id ) {
				throw new WC_API_Exception( 'banda_cannot_create_fee', __( 'Cannot create fee, try again', 'banda' ), 500 );
			}
			...
			if ( ! $fee_id ) {
				throw new WC_API_Exception( 'banda_cannot_update_fee', __( 'Cannot update fee, try again', 'banda' ), 500 );
			}
		}
	}
	...
	protected function set_coupon( $order, $coupon, $action ) {
		...
		if ( isset( $coupon['amount'] ) && floatval( $coupon['amount'] ) < 0 ) {
			throw new WC_API_Exception( 'banda_invalid_coupon_total', __( 'Coupon discount total must be a positive amount', 'banda' ), 400 );
		}
		...
		if ( 'create' === $action ) {
			...
			if ( empty( $coupon['code'] ) ) {
				throw new WC_API_Exception( 'banda_invalid_coupon_coupon', __( 'Coupon code is required', 'banda' ), 400 );
			}

			$coupon_id = $order->add_coupon( $coupon['code'], isset( $coupon['amount'] ) ? floatval( $coupon['amount'] ) : 0 );

			if ( ! $coupon_id ) {
				throw new WC_API_Exception( 'banda_cannot_create_order_coupon', __( 'Cannot create coupon, try again', 'banda' ), 500 );
			}
			...
			if ( ! $coupon_id ) {
				throw new WC_API_Exception( 'banda_cannot_update_order_coupon', __( 'Cannot update coupon, try again', 'banda' ), 500 );
			}
		}
	}
	...
	public function get_order_notes( $order_id, $fields = null ) {
		...
		return array( 'order_notes' => apply_filters( 'banda_api_order_notes_response', $order_notes, $order_id, $fields, $notes, $this->server ) );
	}
	...
	public function get_order_note( $order_id, $id, $fields = null ) {
		try {
			...
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_order_note_id', __( 'Invalid order note ID', 'banda' ), 400 );
			}

			$note = get_comment( $id );

			if ( is_null( $note ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_order_note_id', __( 'An order note with the provided ID could not be found', 'banda' ), 404 );
			}
			...
			return array( 'order_note' => apply_filters( 'banda_api_order_note_response', $order_note, $id, $fields, $note, $order_id, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function create_order_note( $order_id, $data ) {
		try {
			if ( ! isset( $data['order_note'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_order_note_data', sprintf( __( 'No %1$s data specified to create %1$s', 'banda' ), 'order_note' ), 400 );
			}
			...
			// permission check
			if ( ! current_user_can( 'publish_shop_orders' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_create_order_note', __( 'You do not have permission to create order notes', 'banda' ), 401 );
			}
			...
			$data = apply_filters( 'banda_api_create_order_note_data', $data, $order_id, $this );

			// note content is required
			if ( ! isset( $data['note'] ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_order_note', __( 'Order note is required', 'banda' ), 400 );
			}
			...
			if ( ! $note_id ) {
				throw new WC_API_Exception( 'banda_api_cannot_create_order_note', __( 'Cannot create order note, please try again', 'banda' ), 500 );
			}

			// HTTP 201 Created
			$this->server->send_status( 201 );

			do_action( 'banda_api_create_order_note', $note_id, $order_id, $this );
	...
	public function edit_order_note( $order_id, $id, $data ) {
		try {
			if ( ! isset( $data['order_note'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_order_note_data', sprintf( __( 'No %1$s data specified to edit %1$s', 'banda' ), 'order_note' ), 400 );
			}
			...
			if ( $note->comment_post_ID != $order->id ) {
				throw new WC_API_Exception( 'banda_api_invalid_order_note_id', __( 'The order note ID provided is not associated with the order', 'banda' ), 400 );
			}

			$data = apply_filters( 'banda_api_edit_order_note_data', $data, $note->comment_ID, $order->id, $this );
			...
			do_action( 'banda_api_edit_order_note', $note->comment_ID, $order->id, $this );
	...
	public function delete_order_note( $order_id, $id ) {
		try {
			...
			if ( ! $result ) {
				throw new WC_API_Exception( 'banda_api_cannot_delete_order_note', __( 'This order note cannot be deleted', 'banda' ), 500 );
			}

			do_action( 'banda_api_delete_order_note', $note->comment_ID, $order_id, $this );

			return array( 'message' => __( 'Permanently deleted order note', 'banda' ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function get_order_refunds( $order_id, $fields = null ) {
		...
		return array( 'order_refunds' => apply_filters( 'banda_api_order_refunds_response', $order_refunds, $order_id, $fields, $refund_items, $this ) );
	}
	...
	public function get_order_refund( $order_id, $id, $fields = null ) {
		try {
			...
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_order_refund_id', __( 'Invalid order refund ID', 'banda' ), 400 );
			}
			...
			if ( ! $refund ) {
				throw new WC_API_Exception( 'banda_api_invalid_order_refund_id', __( 'An order refund with the provided ID could not be found', 'banda' ), 404 );
			}
			...
			return array( 'order_refund' => apply_filters( 'banda_api_order_refund_response', $order_refund, $id, $fields, $refund, $order_id, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function create_order_refund( $order_id, $data, $api_refund = true ) {
		try {
			if ( ! isset( $data['order_refund'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_order_refund_data', sprintf( __( 'No %1$s data specified to create %1$s', 'banda' ), 'order_refund' ), 400 );
			}
			...
			// Permission check
			if ( ! current_user_can( 'publish_shop_orders' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_create_order_refund', __( 'You do not have permission to create order refunds', 'banda' ), 401 );
			}
			...
			$data = apply_filters( 'banda_api_create_order_refund_data', $data, $order_id, $this );

			// Refund amount is required
			if ( ! isset( $data['amount'] ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_order_refund', __( 'Refund amount is required', 'banda' ), 400 );
			} elseif ( 0 > $data['amount'] ) {
				throw new WC_API_Exception( 'banda_api_invalid_order_refund', __( 'Refund amount must be positive', 'banda' ), 400 );
			}
			...
			if ( ! $refund ) {
				throw new WC_API_Exception( 'banda_api_cannot_create_order_refund', __( 'Cannot create order refund, please try again', 'banda' ), 500 );
			}
			...
			if ( $api_refund ) {
				...
				if ( isset( $payment_gateways[ $order->payment_method ] ) && $payment_gateways[ $order->payment_method ]->supports( 'refunds' ) ) {
					...
					if ( is_wp_error( $result ) ) {
						...
						throw new WC_API_Exception( 'banda_api_create_order_refund_api_failed', __( 'An error occurred while attempting to create the refund using the payment gateway API', 'banda' ), 500 );
					}
				}
			}
			...
			do_action( 'banda_api_create_order_refund', $refund->id, $order_id, $this );
	...
	public function edit_order_refund( $order_id, $id, $data ) {
		try {
			if ( ! isset( $data['order_refund'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_order_refund_data', sprintf( __( 'No %1$s data specified to edit %1$s', 'banda' ), 'order_refund' ), 400 );
			}
			...
			if ( $refund->post_parent != $order_id ) {
				throw new WC_API_Exception( 'banda_api_invalid_order_refund_id', __( 'The order refund ID provided is not associated with the order', 'banda' ), 400 );
			}

			$data = apply_filters( 'banda_api_edit_order_refund_data', $data, $refund->ID, $order_id, $this );
			...
			do_action( 'banda_api_edit_order_refund', $refund->ID, $order_id, $this );
	...
	public function delete_order_refund( $order_id, $id ) {
		try {
			...
			do_action( 'banda_api_delete_order_refund', $refund->ID, $order_id, $this );
	...
	public function bulk( $data ) {
		...
		try {
			if ( ! isset( $data['orders'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_orders_data', sprintf( __( 'No %1$s data specified to create/edit %1$s', 'banda' ), 'orders' ), 400 );
			}

			$data  = $data['orders'];
			$limit = apply_filters( 'banda_api_bulk_limit', 100, 'orders' );

			// Limit bulk operation
			if ( count( $data ) > $limit ) {
				throw new WC_API_Exception( 'banda_api_orders_request_entity_too_large', sprintf( __( 'Unable to accept more than %s items for this request', 'banda' ), $limit ), 413 );
			}
			...
			return array( 'orders' => apply_filters( 'banda_api_orders_bulk_response', $orders, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
