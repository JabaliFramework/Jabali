...
class WC_API_Coupons extends WC_API_Resource {
	...
	public function get_coupon( $id, $fields = null ) {
		...
		try {
			...
			if ( is_null( $code ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_coupon_id', __( 'Invalid coupon ID', 'banda' ), 404 );
			}
			...
			return array( 'coupon' => apply_filters( 'banda_api_coupon_response', $coupon_data, $coupon, $fields, $this->server ) );
	...
	public function get_coupons_count( $filter = array() ) {
		try {
			if ( ! current_user_can( 'read_private_shop_coupons' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_read_coupons_count', __( 'You do not have permission to read the coupons count', 'banda' ), 401 );
			}
	...
	public function get_coupon_by_code( $code, $fields = null ) {
		...
		try {
			...
			if ( is_null( $id ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_coupon_code', __( 'Invalid coupon code', 'banda' ), 404 );
			}
	...
	public function create_coupon( $data ) {
		...
		try {
			if ( ! isset( $data['coupon'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_coupon_data', sprintf( __( 'No %1$s data specified to create %1$s', 'banda' ), 'coupon' ), 400 );
			}
			...
			if ( ! current_user_can( 'publish_shop_coupons' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_create_coupon', __( 'You do not have permission to create coupons', 'banda' ), 401 );
			}

			$data = apply_filters( 'banda_api_create_coupon_data', $data, $this );

			// Check if coupon code is specified
			if ( ! isset( $data['code'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_coupon_code', sprintf( __( 'Missing parameter %s', 'banda' ), 'code' ), 400 );
			}

			$coupon_code = apply_filters( 'banda_coupon_code', $data['code'] );
			...
			if ( $coupon_found ) {
				throw new WC_API_Exception( 'banda_api_coupon_code_already_exists', __( 'The coupon code already exists', 'banda' ), 400 );
			}
			...
			if ( ! in_array( wc_clean( $coupon_data['type'] ), array_keys( wc_get_coupon_types() ) ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_coupon_type', sprintf( __( 'Invalid coupon type - the coupon type must be any of these: %s', 'banda' ), implode( ', ', array_keys( wc_get_coupon_types() ) ) ), 400 );
			}
			...
			if ( is_wp_error( $id ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_create_coupon', $id->get_error_message(), 400 );
			}
			...
			do_action( 'banda_api_create_coupon', $id, $data );
	...
	public function edit_coupon( $id, $data ) {
		...
		try {
			if ( ! isset( $data['coupon'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_coupon_data', sprintf( __( 'No %1$s data specified to edit %1$s', 'banda' ), 'coupon' ), 400 );
			}
			...
			$data = apply_filters( 'banda_api_edit_coupon_data', $data, $id, $this );
			...
			if ( isset( $data['code'] ) ) {
				...
				if ( 0 === $updated ) {
					throw new WC_API_Exception( 'banda_api_cannot_update_coupon', __( 'Failed to update coupon', 'banda' ), 400 );
				}
			}
			...
			do_action( 'banda_api_edit_coupon', $id, $data );
	...
	public function delete_coupon( $id, $force = false ) {
		...
		do_action( 'banda_api_delete_coupon', $id, $this );
	...
	public function bulk( $data ) {
		...
		try {
			if ( ! isset( $data['coupons'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_coupons_data', sprintf( __( 'No %1$s data specified to create/edit %1$s', 'banda' ), 'coupons' ), 400 );
			}

			$data  = $data['coupons'];
			$limit = apply_filters( 'banda_api_bulk_limit', 100, 'coupons' );

			// Limit bulk operation
			if ( count( $data ) > $limit ) {
				throw new WC_API_Exception( 'banda_api_coupons_request_entity_too_large', sprintf( __( 'Unable to accept more than %s items for this request', 'banda' ), $limit ), 413 );
			}
			...
			return array( 'coupons' => apply_filters( 'banda_api_coupons_bulk_response', $coupons, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
