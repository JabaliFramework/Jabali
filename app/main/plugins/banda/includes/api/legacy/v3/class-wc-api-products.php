...
class WC_API_Products extends WC_API_Resource {
	...
	public function get_product( $id, $fields = null ) {
		...
		return array( 'product' => apply_filters( 'banda_api_product_response', $product_data, $product, $fields, $this->server ) );
	}
	...
	public function get_products_count( $type = null, $filter = array() ) {
		try {
			if ( ! current_user_can( 'read_private_products' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_read_products_count', __( 'You do not have permission to read the products count', 'banda' ), 401 );
			}
	...
	public function create_product( $data ) {
		...
		try {
			if ( ! isset( $data['product'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_product_data', sprintf( __( 'No %1$s data specified to create %1$s', 'banda' ), 'product' ), 400 );
			}
			...
			if ( ! current_user_can( 'publish_products' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_create_product', __( 'You do not have permission to create products', 'banda' ), 401 );
			}

			$data = apply_filters( 'banda_api_create_product_data', $data, $this );

			// Check if product title is specified.
			if ( ! isset( $data['title'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_product_title', sprintf( __( 'Missing parameter %s', 'banda' ), 'title' ), 400 );
			}
			...
			if ( ! in_array( wc_clean( $data['type'] ), array_keys( wc_get_product_types() ) ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_product_type', sprintf( __( 'Invalid product type - the product type must be any of these: %s', 'banda' ), implode( ', ', array_keys( wc_get_product_types() ) ) ), 400 );
			}
			...
			if ( is_wp_error( $id ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_create_product', $id->get_error_message(), 400 );
			}
			...
			do_action( 'banda_api_create_product', $id, $data );
	...
	public function edit_product( $id, $data ) {
		try {
			if ( ! isset( $data['product'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_product_data', sprintf( __( 'No %1$s data specified to edit %1$s', 'banda' ), 'product' ), 400 );
			}
			...
			$data = apply_filters( 'banda_api_edit_product_data', $data, $this );
			...
			do_action( 'banda_api_edit_product', $id, $data );
	...
	public function delete_product( $id, $force = false ) {
		...
		do_action( 'banda_api_delete_product', $id, $this );
		...
		if ( ! $result ) {
			return new WP_Error( 'banda_api_cannot_delete_product', sprintf( __( 'This %s cannot be deleted', 'banda' ), 'product' ), array( 'status' => 500 ) );
		}
		...
		if ( $force ) {
			return array( 'message' => sprintf( __( 'Permanently deleted %s', 'banda' ), 'product' ) );
		} else {
			$this->server->send_status( '202' );

			return array( 'message' => sprintf( __( 'Deleted %s', 'banda' ), 'product' ) );
		}
	}
	...
	public function get_product_reviews( $id, $fields = null ) {
		...
		return array( 'product_reviews' => apply_filters( 'banda_api_product_reviews_response', $reviews, $id, $fields, $comments, $this->server ) );
	}
	...
	public function get_product_orders( $id, $fields = null, $filter = array(), $status = null, $page = 1 ) {
		...
		$order_ids = $wpdb->get_col( $wpdb->prepare( "
			SELECT order_id
			FROM {$wpdb->prefix}banda_order_items
			WHERE order_item_id IN ( SELECT order_item_id FROM {$wpdb->prefix}banda_order_itemmeta WHERE meta_key = '_product_id' AND meta_value = %d )
		...
		return array( 'orders' => apply_filters( 'banda_api_product_orders_response', $orders['orders'], $id, $filter, $fields, $this->server ) );
	}
	...
	public function get_product_categories( $fields = null ) {
		try {
			...
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_read_product_categories', __( 'You do not have permission to read product categories', 'banda' ), 401 );
			}
			...
			return array( 'product_categories' => apply_filters( 'banda_api_product_categories_response', $product_categories, $terms, $fields, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function get_product_category( $id, $fields = null ) {
		try {
			...
			// Validate ID
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_product_category_id', __( 'Invalid product category ID', 'banda' ), 400 );
			}
			...
			if ( is_wp_error( $term ) || is_null( $term ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_product_category_id', __( 'A product category with the provided ID could not be found', 'banda' ), 404 );
			}

			$term_id = intval( $term->term_id );

			// Get category display type
			$display_type = get_banda_term_meta( $term_id, 'display_type' );

			// Get category image
			$image = '';
			if ( $image_id = get_banda_term_meta( $term_id, 'thumbnail_id' ) ) {
			...
			return array( 'product_category' => apply_filters( 'banda_api_product_category_response', $product_category, $id, $fields, $term, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function create_product_category( $data ) {
		...
		try {
			if ( ! isset( $data['product_category'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_product_category_data', sprintf( __( 'No %1$s data specified to create %1$s', 'banda' ), 'product_category' ), 400 );
			}

			// Check permissions
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_create_product_category', __( 'You do not have permission to create product categories', 'banda' ), 401 );
			}
			...
			$data = apply_filters( 'banda_api_create_product_category_data', $data, $this );
			...
			if ( $data['parent'] ) {
				...
				if ( ! $parent ) {
					throw new WC_API_Exception( 'banda_api_invalid_product_category_parent', __( 'Product category parent is invalid', 'banda' ), 400 );
				}
			}
			...
			if ( is_wp_error( $insert ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_create_product_category', $insert->get_error_message(), 400 );
			}

			$id = $insert['term_id'];

			update_banda_term_meta( $id, 'display_type', 'default' === $data['display'] ? '' : sanitize_text_field( $data['display'] ) );

			// Check if image_id is a valid image attachment before updating the term meta.
			if ( $image_id && wp_attachment_is_image( $image_id ) ) {
				update_banda_term_meta( $id, 'thumbnail_id', $image_id );
			}

			do_action( 'banda_api_create_product_category', $id, $data );
	...
	public function edit_product_category( $id, $data ) {
		...
		try {
			if ( ! isset( $data['product_category'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_product_category', sprintf( __( 'No %1$s data specified to edit %1$s', 'banda' ), 'product_category' ), 400 );
			}
			...
			// Check permissions.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_edit_product_category', __( 'You do not have permission to edit product categories', 'banda' ), 401 );
			}

			$data     = apply_filters( 'banda_api_edit_product_category_data', $data, $this );
			...
			if ( is_wp_error( $update ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_edit_product_catgory', __( 'Could not edit the category', 'banda' ), 400 );
			}
			...
			do_action( 'banda_api_edit_product_category', $id, $data );
	...
	public function delete_product_category( $id ) {
		...
		try {
			// Check permissions
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_delete_product_category', __( 'You do not have permission to delete product category', 'banda' ), 401 );
			}

			$id      = absint( $id );
			$deleted = wp_delete_term( $id, 'product_cat' );
			if ( ! $deleted || is_wp_error( $deleted ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_delete_product_category', __( 'Could not delete the category', 'banda' ), 401 );
			}

			// When a term is deleted, delete its meta.
			if ( get_option( 'db_version' ) < 34370 ) {
				$wpdb->delete( $wpdb->banda_termmeta, array( 'banda_term_id' => $id ), array( '%d' ) );
			}

			do_action( 'banda_api_delete_product_category', $id, $this );

			return array( 'message' => sprintf( __( 'Deleted %s', 'banda' ), 'product_category' ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function get_product_tags( $fields = null ) {
		try {
			// Permissions check
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_read_product_tags', __( 'You do not have permission to read product tags', 'banda' ), 401 );
			}
			...
			return array( 'product_tags' => apply_filters( 'banda_api_product_tags_response', $product_tags, $terms, $fields, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function get_product_tag( $id, $fields = null ) {
		try {
			...
			// Validate ID
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_product_tag_id', __( 'Invalid product tag ID', 'banda' ), 400 );
			}
			...
			if ( is_wp_error( $term ) || is_null( $term ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_product_tag_id', __( 'A product tag with the provided ID could not be found', 'banda' ), 404 );
			}
			...
			return array( 'product_tag' => apply_filters( 'banda_api_product_tag_response', $tag, $id, $fields, $term, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function create_product_tag( $data ) {
		try {
			if ( ! isset( $data['product_tag'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_product_tag_data', sprintf( __( 'No %1$s data specified to create %1$s', 'banda' ), 'product_tag' ), 400 );
			}

			// Check permissions
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_create_product_tag', __( 'You do not have permission to create product tags', 'banda' ), 401 );
			}
			...
			$data = apply_filters( 'banda_api_create_product_tag_data', $data, $this );

			$insert = wp_insert_term( $data['name'], 'product_tag', $data );
			if ( is_wp_error( $insert ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_create_product_tag', $insert->get_error_message(), 400 );
			}
			$id = $insert['term_id'];

			do_action( 'banda_api_create_product_tag', $id, $data );
	...
	public function edit_product_tag( $id, $data ) {
		try {
			if ( ! isset( $data['product_tag'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_product_tag', sprintf( __( 'No %1$s data specified to edit %1$s', 'banda' ), 'product_tag' ), 400 );
			}
			...
			// Check permissions.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_edit_product_tag', __( 'You do not have permission to edit product tags', 'banda' ), 401 );
			}

			$data = apply_filters( 'banda_api_edit_product_tag_data', $data, $this );
			...
			if ( is_wp_error( $update ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_edit_product_tag', __( 'Could not edit the tag', 'banda' ), 400 );
			}

			do_action( 'banda_api_edit_product_tag', $id, $data );
	...
	public function delete_product_tag( $id ) {
		try {
			// Check permissions
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_delete_product_tag', __( 'You do not have permission to delete product tag', 'banda' ), 401 );
			}

			$id      = absint( $id );
			$deleted = wp_delete_term( $id, 'product_tag' );
			if ( ! $deleted || is_wp_error( $deleted ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_delete_product_tag', __( 'Could not delete the tag', 'banda' ), 401 );
			}

			do_action( 'banda_api_delete_product_tag', $id, $this );

			return array( 'message' => sprintf( __( 'Deleted %s', 'banda' ), 'product_tag' ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	private function get_product_data( $product ) {
		return array(
			...
			'dimensions'         => array(
				...
				'unit'   => get_option( 'banda_dimension_unit' ),
			),
			...
			'short_description'  => apply_filters( 'banda_short_description', $product->get_post_data()->post_excerpt ),
	...
	private function get_product_menu_order( $product ) {
		...
		return apply_filters( 'banda_api_product_menu_order', $menu_order, $product );
	}
	...
	protected function save_product_meta( $product_id, $data ) {
		...
		if ( isset( $data['sku'] ) ) {
			...
			if ( '' == $new_sku ) {
				...
				if ( ! empty( $new_sku ) ) {
					...
					if ( ! $unique_sku ) {
						throw new WC_API_Exception( 'banda_api_product_sku_already_exists', __( 'The SKU already exists on another product', 'banda' ), 400 );
					} else {
		...
		// Stock Data.
		if ( 'yes' == get_option( 'banda_manage_stock' ) ) {
			// Manage stock.
		...
		do_action( 'banda_api_process_product_meta_' . $product_type, $product_id, $data );
	...
	protected function save_variations( $id, $data ) {
		...
		foreach ( $variations as $menu_order => $variation ) {
			...
			$variation_post_title = sprintf( __( 'Variation #%s of %s', 'banda' ), $variation_id, esc_html( get_the_title( $id ) ) );
			...
			if ( ! $variation_id ) {
				...
				do_action( 'banda_create_product_variation', $variation_id );
			} else {
				...
				do_action( 'banda_update_product_variation', $variation_id );
			}

			// Stop with we don't have a variation ID
			if ( is_wp_error( $variation_id ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_save_product_variation', $variation_id->get_error_message(), 400 );
			}
					...
					if ( isset( $image['position'] ) && $image['position'] == 0 ) {
						if ( isset( $image['src'] ) ) {
							...
							if ( is_wp_error( $upload ) ) {
								throw new WC_API_Exception( 'banda_api_cannot_upload_product_image', $upload->get_error_message(), 400 );
							}
			...
			do_action( 'banda_api_save_product_variation', $variation_id, $menu_order, $variation );
		}
	...
	private function save_downloadable_files( $product_id, $downloads, $variation_id = 0 ) {
		...
		do_action( 'banda_process_product_file_download_paths', $product_id, $variation_id, $files );
	...
	private function get_images( $product ) {
		...
		if ( empty( $images ) ) {
			...
			$images[] = array(
				...
				'title'      => __( 'Placeholder', 'banda' ),
				'alt'        => __( 'Placeholder', 'banda' ),
				'position'   => 0,
			);
		}
	...
	protected function upload_image_from_url( $image_url, $upload_for = 'product_image' ) {
		...
		if ( ! $parsed_url || ! is_array( $parsed_url ) ) {
			throw new WC_API_Exception( 'banda_api_invalid_' . $upload_for, sprintf( __( 'Invalid URL %s', 'banda' ), $image_url ), 400 );
		}
		...
		if ( is_wp_error( $response ) ) {
			throw new WC_API_Exception( 'banda_api_invalid_remote_' . $upload_for, sprintf( __( 'Error getting remote image %s.', 'banda' ), $image_url ) . ' ' . sprintf( __( 'Error: %s.', 'banda' ), $response->get_error_message() ), 400 );
		} elseif ( 200 !== wp_remote_retrieve_response_code( $response ) ) {
			throw new WC_API_Exception( 'banda_api_invalid_remote_' . $upload_for, sprintf( __( 'Error getting remote image %s.', 'banda' ), $image_url ), 400 );
		}
		...
		if ( ! $wp_filetype['type'] ) {
			...
			if ( ! $wp_filetype['type'] ) {
				throw new WC_API_Exception( 'banda_api_invalid_' . $upload_for, __( 'Invalid image type.', 'banda' ), 400 );
			}
		}
		...
		if ( $upload['error'] ) {
			throw new WC_API_Exception( 'banda_api_' . $upload_for . '_upload_error', $upload['error'], 400 );
		}
		...
		if ( 0 == $filesize ) {
			@unlink( $upload['file'] );
			unset( $upload );
			throw new WC_API_Exception( 'banda_api_' . $upload_for . '_upload_file_error', __( 'Zero size file downloaded', 'banda' ), 400 );
		}

		unset( $response );

		do_action( 'banda_api_uploaded_image_from_url', $upload, $image_url, $upload_for  );
	...
	public function get_product_attributes( $fields = null ) {
		try {
			// Permissions check.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_read_product_attributes', __( 'You do not have permission to read product attributes', 'banda' ), 401 );
			}
			...
			return array( 'product_attributes' => apply_filters( 'banda_api_product_attributes_response', $product_attributes, $attribute_taxonomies, $fields, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function get_product_attribute( $id, $fields = null ) {
		...
		try {
			...
			// Validate ID
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_product_attribute_id', __( 'Invalid product attribute ID', 'banda' ), 400 );
			}
			...
			$attribute = $wpdb->get_row( $wpdb->prepare( "
				SELECT *
				FROM {$wpdb->prefix}banda_attribute_taxonomies
				WHERE attribute_id = %d
			 ", $id ) );

			if ( is_wp_error( $attribute ) || is_null( $attribute ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_product_attribute_id', __( 'A product attribute with the provided ID could not be found', 'banda' ), 404 );
			}
			...
			return array( 'product_attribute' => apply_filters( 'banda_api_product_attribute_response', $product_attribute, $id, $fields, $attribute, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	protected function validate_attribute_data( $name, $slug, $type, $order_by, $new_data = true ) {
		if ( empty( $name ) ) {
			throw new WC_API_Exception( 'banda_api_missing_product_attribute_name', sprintf( __( 'Missing parameter %s', 'banda' ), 'name' ), 400 );
		}

		if ( strlen( $slug ) >= 28 ) {
			throw new WC_API_Exception( 'banda_api_invalid_product_attribute_slug_too_long', sprintf( __( 'Slug "%s" is too long (28 characters max). Shorten it, please.', 'banda' ), $slug ), 400 );
		} else if ( wc_check_if_attribute_name_is_reserved( $slug ) ) {
			throw new WC_API_Exception( 'banda_api_invalid_product_attribute_slug_reserved_name', sprintf( __( 'Slug "%s" is not allowed because it is a reserved term. Change it, please.', 'banda' ), $slug ), 400 );
		} else if ( $new_data && taxonomy_exists( wc_attribute_taxonomy_name( $slug ) ) ) {
			throw new WC_API_Exception( 'banda_api_invalid_product_attribute_slug_already_exists', sprintf( __( 'Slug "%s" is already in use. Change it, please.', 'banda' ), $slug ), 400 );
		}

		// Validate the attribute type
		if ( ! in_array( wc_clean( $type ), array_keys( wc_get_attribute_types() ) ) ) {
			throw new WC_API_Exception( 'banda_api_invalid_product_attribute_type', sprintf( __( 'Invalid product attribute type - the product attribute type must be any of these: %s', 'banda' ), implode( ', ', array_keys( wc_get_attribute_types() ) ) ), 400 );
		}

		// Validate the attribute order by
		if ( ! in_array( wc_clean( $order_by ), array( 'menu_order', 'name', 'name_num', 'id' ) ) ) {
			throw new WC_API_Exception( 'banda_api_invalid_product_attribute_order_by', sprintf( __( 'Invalid product attribute order_by type - the product attribute order_by type must be any of these: %s', 'banda' ), implode( ', ', array( 'menu_order', 'name', 'name_num', 'id' ) ) ), 400 );
		}
	...
	public function create_product_attribute( $data ) {
		...
		try {
			if ( ! isset( $data['product_attribute'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_product_attribute_data', sprintf( __( 'No %1$s data specified to create %1$s', 'banda' ), 'product_attribute' ), 400 );
			}
			...
			// Check permissions.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_create_product_attribute', __( 'You do not have permission to create product attributes', 'banda' ), 401 );
			}

			$data = apply_filters( 'banda_api_create_product_attribute_data', $data, $this );
			...
			$insert = $wpdb->insert(
				$wpdb->prefix . 'banda_attribute_taxonomies',
				array(
			...
			// Checks for an error in the product creation.
			if ( is_wp_error( $insert ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_create_product_attribute', $insert->get_error_message(), 400 );
			}

			$id = $wpdb->insert_id;

			do_action( 'banda_api_create_product_attribute', $id, $data );
	...
	public function edit_product_attribute( $id, $data ) {
		...
		try {
			if ( ! isset( $data['product_attribute'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_product_attribute_data', sprintf( __( 'No %1$s data specified to edit %1$s', 'banda' ), 'product_attribute' ), 400 );
			}
			...
			// Check permissions.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_edit_product_attribute', __( 'You do not have permission to edit product attributes', 'banda' ), 401 );
			}

			$data      = apply_filters( 'banda_api_edit_product_attribute_data', $data, $this );
			...
			// Checks for an error in the product creation.
			if ( false === $update ) {
				throw new WC_API_Exception( 'banda_api_cannot_edit_product_attribute', __( 'Could not edit the attribute', 'banda' ), 400 );
			}

			do_action( 'banda_api_edit_product_attribute', $id, $data );
	...
	public function delete_product_attribute( $id ) {
		...
		try {
			// Check permissions.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_delete_product_attribute', __( 'You do not have permission to delete product attributes', 'banda' ), 401 );
			}
			...
			if ( false === $deleted ) {
				throw new WC_API_Exception( 'banda_api_cannot_delete_product_attribute', __( 'Could not delete the attribute', 'banda' ), 401 );
			}
			...
			do_action( 'banda_attribute_deleted', $id, $attribute_name, $taxonomy );
			do_action( 'banda_api_delete_product_attribute', $id, $this );

			// Clear transients.
			flush_rewrite_rules();
			delete_transient( 'wc_attribute_taxonomies' );

			return array( 'message' => sprintf( __( 'Deleted %s', 'banda' ), 'product_attribute' ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function get_product_attribute_terms( $attribute_id, $fields = null ) {
		try {
			// Permissions check.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_read_product_attribute_terms', __( 'You do not have permission to read product attribute terms', 'banda' ), 401 );
			}
			...
			return array( 'product_attribute_terms' => apply_filters( 'banda_api_product_attribute_terms_response', $attribute_terms, $terms, $fields, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function get_product_attribute_term( $attribute_id, $id, $fields = null ) {
		...
		try {
			...
			// Validate ID
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_product_attribute_term_id', __( 'Invalid product attribute ID', 'banda' ), 400 );
			}
			...
			if ( is_wp_error( $term ) || is_null( $term ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_product_attribute_term_id', __( 'A product attribute term with the provided ID could not be found', 'banda' ), 404 );
			}
			...
			return array( 'product_attribute_term' => apply_filters( 'banda_api_product_attribute_response', $attribute_term, $id, $fields, $term, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function create_product_attribute_term( $attribute_id, $data ) {
		...
		try {
			if ( ! isset( $data['product_attribute_term'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_product_attribute_term_data', sprintf( __( 'No %1$s data specified to create %1$s', 'banda' ), 'product_attribute_term' ), 400 );
			}
			...
			$data = apply_filters( 'banda_api_create_product_attribute_term_data', $data, $this );

			// Check if attribute term name is specified.
			if ( ! isset( $data['name'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_product_attribute_term_name', sprintf( __( 'Missing parameter %s', 'banda' ), 'name' ), 400 );
			}
			...
			if ( is_wp_error( $term ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_create_product_attribute', $term->get_error_message(), 400 );
			}

			$id = $term['term_id'];

			do_action( 'banda_api_create_product_attribute_term', $id, $data );
	...
	public function edit_product_attribute_term( $attribute_id, $id, $data ) {
		...
		try {
			if ( ! isset( $data['product_attribute_term'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_product_attribute_term_data', sprintf( __( 'No %1$s data specified to edit %1$s', 'banda' ), 'product_attribute_term' ), 400 );
			}
			...
			$data = apply_filters( 'banda_api_edit_product_attribute_term_data', $data, $this );
			...
			if ( is_wp_error( $term ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_edit_product_attribute_term', $term->get_error_message(), 400 );
			}

			do_action( 'banda_api_edit_product_attribute_term', $id, $data );
	...
	public function delete_product_attribute_term( $attribute_id, $id ) {
		...
		try {
			// Check permissions.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_delete_product_attribute_term', __( 'You do not have permission to delete product attribute terms', 'banda' ), 401 );
			}
			...
			if ( ! $term ) {
				throw new WC_API_Exception( 'banda_api_cannot_delete_product_attribute_term', sprintf( __( 'This %s cannot be deleted', 'banda' ), 'product_attribute_term' ), 500 );
			} else if ( is_wp_error( $term ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_delete_product_attribute_term', $term->get_error_message(), 400 );
			}

			do_action( 'banda_api_delete_product_attribute_term', $id, $this );
	...
	public function bulk( $data ) {
		...
		try {
			if ( ! isset( $data['products'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_products_data', sprintf( __( 'No %1$s data specified to create/edit %1$s', 'banda' ), 'products' ), 400 );
			}

			$data  = $data['products'];
			$limit = apply_filters( 'banda_api_bulk_limit', 100, 'products' );

			// Limit bulk operation
			if ( count( $data ) > $limit ) {
				throw new WC_API_Exception( 'banda_api_products_request_entity_too_large', sprintf( __( 'Unable to accept more than %s items for this request', 'banda' ), $limit ), 413 );
			}
			...
			return array( 'products' => apply_filters( 'banda_api_products_bulk_response', $products, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function get_product_shipping_classes( $fields = null ) {
		try {
			// Permissions check
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_read_product_shipping_classes', __( 'You do not have permission to read product shipping classes', 'banda' ), 401 );
			}
			...
			return array( 'product_shipping_classes' => apply_filters( 'banda_api_product_shipping_classes_response', $product_shipping_classes, $terms, $fields, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function get_product_shipping_class( $id, $fields = null ) {
		try {
			$id = absint( $id );
			if ( ! $id ) {
				throw new WC_API_Exception( 'banda_api_invalid_product_shipping_class_id', __( 'Invalid product shipping class ID', 'banda' ), 400 );
			}
			...
			if ( is_wp_error( $term ) || is_null( $term ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_product_shipping_class_id', __( 'A product shipping class with the provided ID could not be found', 'banda' ), 404 );
			}
			...
			return array( 'product_shipping_class' => apply_filters( 'banda_api_product_shipping_class_response', $product_shipping_class, $id, $fields, $term, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
	...
	public function create_product_shipping_class( $data ) {
		...
		try {
			if ( ! isset( $data['product_shipping_class'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_product_shipping_class_data', sprintf( __( 'No %1$s data specified to create %1$s', 'banda' ), 'product_shipping_class' ), 400 );
			}

			// Check permissions
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_create_product_shipping_class', __( 'You do not have permission to create product shipping classes', 'banda' ), 401 );
			}
			...
			$data = apply_filters( 'banda_api_create_product_shipping_class_data', $data, $this );
			...
			if ( $data['parent'] ) {
				...
				if ( ! $parent ) {
					throw new WC_API_Exception( 'banda_api_invalid_product_shipping_class_parent', __( 'Product shipping class parent is invalid', 'banda' ), 400 );
				}
			}

			$insert = wp_insert_term( $data['name'], 'product_shipping_class', $data );
			if ( is_wp_error( $insert ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_create_product_shipping_class', $insert->get_error_message(), 400 );
			}

			$id = $insert['term_id'];

			do_action( 'banda_api_create_product_shipping_class', $id, $data );
	...
	public function edit_product_shipping_class( $id, $data ) {
		...
		try {
			if ( ! isset( $data['product_shipping_class'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_product_shipping_class', sprintf( __( 'No %1$s data specified to edit %1$s', 'banda' ), 'product_shipping_class' ), 400 );
			}
			...
			// Check permissions
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_edit_product_shipping_class', __( 'You do not have permission to edit product shipping classes', 'banda' ), 401 );
			}

			$data           = apply_filters( 'banda_api_edit_product_shipping_class_data', $data, $this );
			...
			if ( is_wp_error( $update ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_edit_product_shipping_class', __( 'Could not edit the shipping class', 'banda' ), 400 );
			}

			do_action( 'banda_api_edit_product_shipping_class', $id, $data );
	...
	public function delete_product_shipping_class( $id ) {
		...
		try {
			// Check permissions
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_delete_product_shipping_class', __( 'You do not have permission to delete product shipping classes', 'banda' ), 401 );
			}

			$id      = absint( $id );
			$deleted = wp_delete_term( $id, 'product_shipping_class' );
			if ( ! $deleted || is_wp_error( $deleted ) ) {
				throw new WC_API_Exception( 'banda_api_cannot_delete_product_shipping_class', __( 'Could not delete the shipping class', 'banda' ), 401 );
			}

			do_action( 'banda_api_delete_product_shipping_class', $id, $this );

			return array( 'message' => sprintf( __( 'Deleted %s', 'banda' ), 'product_shipping_class' ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
