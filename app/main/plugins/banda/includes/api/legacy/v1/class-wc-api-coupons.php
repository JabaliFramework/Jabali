...
class WC_API_Coupons extends WC_API_Resource {
	...
	public function get_coupon( $id, $fields = null ) {
		...
		if ( is_null( $code ) )
			return new WP_Error( 'banda_api_invalid_coupon_id', __( 'Invalid coupon ID', 'banda' ), array( 'status' => 404 ) );
	...
	public function get_coupons_count( $filter = array() ) {
		...
		if ( ! current_user_can( 'read_private_shop_coupons' ) )
			return new WP_Error( 'banda_api_user_cannot_read_coupons_count', __( 'You do not have permission to read the coupons count', 'banda' ), array( 'status' => 401 ) );
	...
	public function get_coupon_by_code( $code, $fields = null ) {
		...
		if ( is_null( $id ) )
			return new WP_Error( 'banda_api_invalid_coupon_code', __( 'Invalid coupon code', 'banda' ), array( 'status' => 404 ) );
