...
class WC_API_Customers extends WC_API_Resource {
	...
	public function __construct( WC_API_Server $server ) {
		...
		add_filter( 'banda_api_order_response', array( $this, 'add_customer_data' ), 10, 2 );
	...
	public function get_customer( $id, $fields = null ) {
		...
		return array( 'customer' => apply_filters( 'banda_api_customer_response', $customer_data, $customer, $fields, $this->server ) );
	}
		...
		try {
			if ( is_email( $email ) ) {
				...
				if ( ! is_object( $customer ) ) {
					throw new WC_API_Exception( 'banda_api_invalid_customer_email', __( 'Invalid customer Email', 'banda' ), 404 );
				}
			} else {
				throw new WC_API_Exception( 'banda_api_invalid_customer_email', __( 'Invalid customer Email', 'banda' ), 404 );
			}
	...
	public function get_customers_count( $filter = array() ) {
		try {
			if ( ! current_user_can( 'list_users' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_read_customers_count', __( 'You do not have permission to read the customers count', 'banda' ), 401 );
			}
	...
	protected function get_customer_billing_address() {
		$billing_address = apply_filters( 'banda_api_customer_billing_address', array(
			'first_name',
			'last_name',
			'company',
			'address_1',
	...
	protected function get_customer_shipping_address() {
		$shipping_address = apply_filters( 'banda_api_customer_shipping_address', array(
			'first_name',
			'last_name',
			'company',
			'address_1',
	...
	protected function update_customer_data( $id, $data ) {
		...
		do_action( 'banda_api_update_customer_data', $id, $data );
	}
	...
	public function create_customer( $data ) {
		try {
			if ( ! isset( $data['customer'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_customer_data', sprintf( __( 'No %1$s data specified to create %1$s', 'banda' ), 'customer' ), 400 );
			}
			...
			if ( ! current_user_can( 'create_users' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_create_customer', __( 'You do not have permission to create this customer', 'banda' ), 401 );
			}

			$data = apply_filters( 'banda_api_create_customer_data', $data, $this );

			// Checks with the email is missing.
			if ( ! isset( $data['email'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_customer_email', sprintf( __( 'Missing parameter %s', 'banda' ), 'email' ), 400 );
			}
			...
			do_action( 'banda_api_create_customer', $id, $data );
	...
	public function edit_customer( $id, $data ) {
		try {
			if ( ! isset( $data['customer'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_customer_data', sprintf( __( 'No %1$s data specified to edit %1$s', 'banda' ), 'customer' ), 400 );
			}
			...
			$data = apply_filters( 'banda_api_edit_customer_data', $data, $this );
			...
			do_action( 'banda_api_edit_customer', $id, $data );
	...
	public function delete_customer( $id ) {
		...
		do_action( 'banda_api_delete_customer', $id, $this );
	...
	public function get_customer_orders( $id, $fields = null ) {
		...
		return array( 'orders' => apply_filters( 'banda_api_customer_orders_response', $orders, $id, $fields, $order_ids, $this->server ) );
	}
	...
	public function get_customer_downloads( $id, $fields = null ) {
		...
		return array( 'downloads' => apply_filters( 'banda_api_customer_downloads_response', $downloads, $id, $fields, $this->server ) );
	}
	...
	protected function validate_request( $id, $type, $context ) {
		...
		try {
			...
			// validate ID
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_customer_id', __( 'Invalid customer ID', 'banda' ), 404 );
			}
			...
			if ( 0 === $customer->ID ) {
				throw new WC_API_Exception( 'banda_api_invalid_customer', __( 'Invalid customer', 'banda' ), 404 );
			}
			...
			switch ( $context ) {
				...
				case 'read':
					if ( ! current_user_can( 'list_users' ) ) {
						throw new WC_API_Exception( 'banda_api_user_cannot_read_customer', __( 'You do not have permission to read this customer', 'banda' ), 401 );
					}
					break;

				case 'edit':
					if ( ! current_user_can( 'edit_users' ) ) {
						throw new WC_API_Exception( 'banda_api_user_cannot_edit_customer', __( 'You do not have permission to edit this customer', 'banda' ), 401 );
					}
					break;

				case 'delete':
					if ( ! current_user_can( 'delete_users' ) ) {
						throw new WC_API_Exception( 'banda_api_user_cannot_delete_customer', __( 'You do not have permission to delete this customer', 'banda' ), 401 );
					}
					break;
			}
	...
	public function bulk( $data ) {
		...
		try {
			if ( ! isset( $data['customers'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_customers_data', sprintf( __( 'No %1$s data specified to create/edit %1$s', 'banda' ), 'customers' ), 400 );
			}

			$data  = $data['customers'];
			$limit = apply_filters( 'banda_api_bulk_limit', 100, 'customers' );

			// Limit bulk operation
			if ( count( $data ) > $limit ) {
				throw new WC_API_Exception( 'banda_api_customers_request_entity_too_large', sprintf( __( 'Unable to accept more than %s items for this request', 'banda' ), $limit ), 413 );
			}
			...
			return array( 'customers' => apply_filters( 'banda_api_customers_bulk_response', $customers, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
