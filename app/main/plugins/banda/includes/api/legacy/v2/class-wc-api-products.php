...
class WC_API_Products extends WC_API_Resource {
	...
	public function upload_product_image( $image_url ) {
		...
		if ( ! $parsed_url || ! is_array( $parsed_url ) ) {
			throw new WC_API_Exception( 'banda_api_invalid_product_image', sprintf( __( 'Invalid URL %s', 'banda' ), $image_url ), 400 );
		}
		...
		if ( is_wp_error( $response ) ) {
			throw new WC_API_Exception( 'banda_api_invalid_remote_product_image', sprintf( __( 'Error getting remote image %s.', 'banda' ), $image_url ) . ' ' . sprintf( __( 'Error: %s.', 'banda' ), $response->get_error_message() ), 400 );
		} elseif ( 200 !== wp_remote_retrieve_response_code( $response ) ) {
			throw new WC_API_Exception( 'banda_api_invalid_remote_product_image', sprintf( __( 'Error getting remote image %s.', 'banda' ), $image_url ), 400 );
		}
		...
		if ( ! $wp_filetype['type'] ) {
			...
			if ( ! $wp_filetype['type'] ) {
				throw new WC_API_Exception( 'banda_api_invalid_product_image', __( 'Invalid image type.', 'banda' ), 400 );
			}
		}
		...
		if ( $upload['error'] ) {
			throw new WC_API_Exception( 'banda_api_product_image_upload_error', $upload['error'], 400 );
		}
		...
		if ( 0 == $filesize ) {
			@unlink( $upload['file'] );
			unset( $upload );
			throw new WC_API_Exception( 'banda_api_product_image_upload_file_error', __( 'Zero size file downloaded', 'banda' ), 400 );
		}
	...
	public function get_product_attribute( $id, $fields = null ) {
		...
		try {
			...
			// Permissions check
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_read_product_categories', __( 'You do not have permission to read product attributes', 'banda' ), 401 );
			}
	...
	public function get_product_by_sku( $sku, $fields = null ) {
		try {
			...
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_product_sku', __( 'Invalid product SKU', 'banda' ), 404 );
			}
