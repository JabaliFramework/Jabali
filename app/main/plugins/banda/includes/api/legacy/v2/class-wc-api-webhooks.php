...
class WC_API_Webhooks extends WC_API_Resource {
	...
	public function get_webhook( $id, $fields = null ) {
		...
		return array( 'webhook' => apply_filters( 'banda_api_webhook_response', $webhook_data, $webhook, $fields, $this ) );
	}
	...
	public function get_webhooks_count( $status = null, $filter = array() ) {
		try {
			if ( ! current_user_can( 'read_private_shop_webhooks' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_read_webhooks_count', __( 'You do not have permission to read the webhooks count', 'banda' ), 401 );
			}
	...
	public function create_webhook( $data ) {
		...
		try {
			if ( ! isset( $data['webhook'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_webhook_data', sprintf( __( 'No %1$s data specified to create %1$s', 'banda' ), 'webhook' ), 400 );
			}
			...
			if ( ! current_user_can( 'publish_shop_webhooks' ) ) {
				throw new WC_API_Exception( 'banda_api_user_cannot_create_webhooks', __( 'You do not have permission to create webhooks', 'banda' ), 401 );
			}

			$data = apply_filters( 'banda_api_create_webhook_data', $data, $this );

			// validate topic
			if ( empty( $data['topic'] ) || ! wc_is_webhook_valid_topic( strtolower( $data['topic'] ) ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_webhook_topic', __( 'Webhook topic is required and must be valid', 'banda' ), 400 );
			}

			// validate delivery URL
			if ( empty( $data['delivery_url'] ) || ! wc_is_valid_url( $data['delivery_url'] ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_webhook_delivery_url', __( 'Webhook delivery URL must be a valid URL starting with http:// or https://', 'banda' ), 400 );
			}

			$webhook_data = apply_filters( 'banda_new_webhook_data', array(
				'post_type'     => 'shop_webhook',
				'post_status'   => 'publish',
				'ping_status'   => 'closed',
				'post_author'   => get_current_user_id(),
				'post_password' => strlen( ( $password = uniqid( 'webhook_' ) ) ) > 20 ? substr( $password, 0, 20 ) : $password,
				'post_title'    => ! empty( $data['name'] ) ? $data['name'] : sprintf( __( 'Webhook created on %s', 'banda' ), strftime( _x( '%b %d, %Y @ %I:%M %p', 'Webhook created on date parsed by strftime', 'banda' ) ) ),
			), $data, $this );

			$webhook_id = wp_insert_post( $webhook_data );

			if ( is_wp_error( $webhook_id ) || ! $webhook_id ) {
				throw new WC_API_Exception( 'banda_api_cannot_create_webhook', sprintf( __( 'Cannot create webhook: %s', 'banda' ), is_wp_error( $webhook_id ) ? implode( ', ', $webhook_id->get_error_messages() ) : '0' ), 500 );
			}
			...
			do_action( 'banda_api_create_webhook', $webhook->id, $this );
	...
	public function edit_webhook( $id, $data ) {
		...
		try {
			if ( ! isset( $data['webhook'] ) ) {
				throw new WC_API_Exception( 'banda_api_missing_webhook_data', sprintf( __( 'No %1$s data specified to edit %1$s', 'banda' ), 'webhook' ), 400 );
			}
			...
			$data = apply_filters( 'banda_api_edit_webhook_data', $data, $id, $this );
			...
			if ( ! empty( $data['topic'] ) ) {
				...
				if ( wc_is_webhook_valid_topic( strtolower( $data['topic'] ) ) ) {
					...
					throw new WC_API_Exception( 'banda_api_invalid_webhook_topic', __( 'Webhook topic must be valid', 'banda' ), 400 );
				}
			}
			...
			do_action( 'banda_api_edit_webhook', $webhook->id, $this );
	...
	public function delete_webhook( $id ) {
		...
		do_action( 'banda_api_delete_webhook', $id, $this );
	...
	public function get_webhook_delivery( $webhook_id, $id, $fields = null ) {
		try {
			...
			if ( empty( $id ) ) {
				throw new WC_API_Exception( 'banda_api_invalid_webhook_delivery_id', __( 'Invalid webhook delivery ID', 'banda' ), 404 );
			}
			...
			if ( ! $log ) {
				throw new WC_API_Exception( 'banda_api_invalid_webhook_delivery_id', __( 'Invalid webhook delivery', 'banda' ), 400 );
			}
			...
			return array( 'webhook_delivery' => apply_filters( 'banda_api_webhook_delivery_response', $delivery_log, $id, $fields, $log, $webhook_id, $this ) );
		} catch ( WC_API_Exception $e ) {
			return new WP_Error( $e->getErrorCode(), $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	}
