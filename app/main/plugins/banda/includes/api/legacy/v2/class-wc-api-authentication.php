<?php
/**
 * Banda API Authentication Class
 *
 * @author   Jabali
 * @category API
 * @package  Banda/API
...
class WC_API_Authentication {
	...
	public function __construct() {
		...
		add_filter( 'banda_api_check_authentication', array( $this, 'authenticate' ), 0 );
	}
	...
	public function authenticate( $user ) {
		...
		try {
			...
			$user = new WP_Error( 'banda_api_authentication_error', $e->getMessage(), array( 'status' => $e->getCode() ) );
		}
	...
	private function perform_ssl_authentication() {
		...
		if ( ! empty( $_SERVER['PHP_AUTH_USER'] ) ) {
			...
			throw new Exception( __( 'Consumer Key is missing', 'banda' ), 404 );
		}
		...
		if ( ! empty( $_SERVER['PHP_AUTH_PW'] ) ) {
			...
			throw new Exception( __( 'Consumer Secret is missing', 'banda' ), 404 );
		}

		$keys = $this->get_keys_by_consumer_key( $consumer_key );

		if ( ! $this->is_consumer_secret_valid( $keys['consumer_secret'], $consumer_secret ) ) {
			throw new Exception( __( 'Consumer Secret is invalid', 'banda' ), 401 );
		}
	...
	private function perform_oauth_authentication() {
		...
		foreach ( $param_names as $param_name ) {
			...
			if ( empty( $params[ $param_name ] ) ) {
				throw new Exception( sprintf( __( '%s parameter is missing', 'banda' ), $param_name ), 404 );
			}
		}
	...
	private function get_keys_by_consumer_key( $consumer_key ) {
		...
		if ( empty( $keys ) ) {
			throw new Exception( __( 'Consumer Key is invalid', 'banda' ), 401 );
		}
	...
	private function get_user_by_id( $user_id ) {
		...
		if ( ! $user ) {
			throw new Exception( __( 'API user is invalid', 'banda' ), 401 );
		}
	...
	private function check_oauth_signature( $keys, $params ) {
		...
		$base_request_uri = rawurlencode( untrailingslashit( get_banda_api_url( '' ) ) . WC()->api->server->path );
		...
		if ( ! uksort( $params, 'strcmp' ) ) {
			throw new Exception( __( 'Invalid Signature - failed to sort parameters', 'banda' ), 401 );
		}
		...
		if ( $params['oauth_signature_method'] !== 'HMAC-SHA1' && $params['oauth_signature_method'] !== 'HMAC-SHA256' ) {
			throw new Exception( __( 'Invalid Signature - signature method is invalid', 'banda' ), 401 );
		}
		...
		if ( ! hash_equals( $signature, $consumer_signature ) ) {
			throw new Exception( __( 'Invalid Signature - provided signature does not match', 'banda' ), 401 );
		}
	}
	...
	private function check_oauth_timestamp_and_nonce( $keys, $timestamp, $nonce ) {
		...
		if ( ( $timestamp < time() - $valid_window ) || ( $timestamp > time() + $valid_window ) ) {
			throw new Exception( __( 'Invalid timestamp', 'banda' ), 401 );
		}
		...
		if ( in_array( $nonce, $used_nonces ) ) {
			throw new Exception( __( 'Invalid nonce - nonce has already been used', 'banda' ), 401 );
		}
		...
		switch ( WC()->api->server->method ) {
			...
			case 'GET':
				if ( 'read' !== $key_permissions && 'read_write' !== $key_permissions ) {
					throw new Exception( __( 'The API key provided does not have read permissions', 'banda' ), 401 );
				}
				break;
			...
			case 'DELETE':
				if ( 'write' !== $key_permissions && 'read_write' !== $key_permissions ) {
					throw new Exception( __( 'The API key provided does not have write permissions', 'banda' ), 401 );
				}
				break;
		}
	}
