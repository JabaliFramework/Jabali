...
class WC_REST_Authentication {
	...
	protected function is_request_to_rest_api() {
		...
		$banda = false !== strpos( $_SERVER['REQUEST_URI'], $rest_prefix . 'wc/' );

		// Allow third party plugins use our authentication methods.
		$third_party = false !== strpos( $_SERVER['REQUEST_URI'], $rest_prefix . 'wc-' );

		return apply_filters( 'banda_rest_is_request_to_rest_api', $banda || $third_party );
	}
	...
	private function perform_basic_authentication() {
		...
		if ( ! hash_equals( $user->consumer_secret, $consumer_secret ) ) {
			$wc_rest_authentication_error = new WP_Error( 'banda_rest_authentication_error', __( 'Consumer Secret is invalid.', 'banda' ), array( 'status' => 401 ) );
	...
	private function perform_oauth_authentication() {
		...
		if ( empty( $user ) ) {
			$wc_rest_authentication_error = new WP_Error( 'banda_rest_authentication_error', __( 'Consumer Key is invalid.', 'banda' ), array( 'status' => 401 ) );
	...
	private function check_oauth_signature( $user, $params ) {
		...
		if ( ! uksort( $params, 'strcmp' ) ) {
			return new WP_Error( 'banda_rest_authentication_error', __( 'Invalid Signature - failed to sort parameters.', 'banda' ), array( 'status' => 401 ) );
		}
		...
		if ( $params['oauth_signature_method'] !== 'HMAC-SHA1' && $params['oauth_signature_method'] !== 'HMAC-SHA256' ) {
			return new WP_Error( 'banda_rest_authentication_error', __( 'Invalid Signature - signature method is invalid.', 'banda' ), array( 'status' => 401 ) );
		}
		...
		if ( ! hash_equals( $signature, $consumer_signature ) ) {
			return new WP_Error( 'banda_rest_authentication_error', __( 'Invalid Signature - provided signature does not match.', 'banda' ), array( 'status' => 401 ) );
		}
	...
	private function check_oauth_timestamp_and_nonce( $user, $timestamp, $nonce ) {
		...
		if ( ( $timestamp < time() - $valid_window ) || ( $timestamp > time() + $valid_window ) ) {
			return new WP_Error( 'banda_rest_authentication_error', __( 'Invalid timestamp.', 'banda' ), array( 'status' => 401 ) );
		}
		...
		if ( in_array( $nonce, $used_nonces ) ) {
			return new WP_Error( 'banda_rest_authentication_error', __( 'Invalid nonce - nonce has already been used.', 'banda' ), array( 'status' => 401 ) );
		}
	...
	private function get_user_data_by_consumer_key( $consumer_key ) {
		...
		$user         = $wpdb->get_row( $wpdb->prepare( "
			...
			FROM {$wpdb->prefix}banda_api_keys
		...
		switch ( $_SERVER['REQUEST_METHOD'] ) {
			...
			case 'GET' :
				if ( 'read' !== $permissions && 'read_write' !== $permissions ) {
					$wc_rest_authentication_error = new WP_Error( 'banda_rest_authentication_error', __( 'The API key provided does not have read permissions.', 'banda' ), array( 'status' => 401 ) );
					$valid = false;
				}
				break;
			...
			case 'DELETE' :
				if ( 'write' !== $permissions && 'read_write' !== $permissions ) {
					$wc_rest_authentication_error = new WP_Error( 'banda_rest_authentication_error', __( 'The API key provided does not have write permissions.', 'banda' ), array( 'status' => 401 ) );
					$valid = false;
				}
				break;
		}
	...
	public function send_unauthorized_headers( $response ) {
		...
		if ( is_wp_error( $wc_rest_authentication_error ) && is_ssl() ) {
			$auth_message = __( 'Banda API - Use a consumer key in the username field and a consumer secret in the password field.', 'banda' );
