...
class WC_Frontend_Scripts {
	...
	public static function get_styles() {
		return apply_filters( 'banda_enqueue_styles', array(
			'banda-layout' => array(
				'src'     => str_replace( array( 'http:', 'https:' ), '', WC()->plugin_url() ) . '/assets/css/banda-layout.css',
				'deps'    => '',
				'version' => WC_VERSION,
				'media'   => 'all'
			),
			'banda-smallscreen' => array(
				'src'     => str_replace( array( 'http:', 'https:' ), '', WC()->plugin_url() ) . '/assets/css/banda-smallscreen.css',
				'deps'    => 'banda-layout',
				'version' => WC_VERSION,
				'media'   => 'only screen and (max-width: ' . apply_filters( 'banda_style_smallscreen_breakpoint', $breakpoint = '768px' ) . ')'
			),
			'banda-general' => array(
				'src'     => str_replace( array( 'http:', 'https:' ), '', WC()->plugin_url() ) . '/assets/css/banda.css',
				'deps'    => '',
				'version' => WC_VERSION,
				'media'   => 'all'
			),
	...
	public static function load_scripts() {
		...
		if ( ! did_action( 'before_banda_init' ) ) {
			return;
		}

		$suffix               = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ? '' : '.min';
		$lightbox_en          = 'yes' === get_option( 'banda_enable_lightbox' );
		$ajax_cart_en         = 'yes' === get_option( 'banda_enable_ajax_add_to_cart' );
		...
		if ( is_checkout() || is_account_page() ) {
			...
			if ( ( 'no' === get_option( 'banda_registration_generate_password' ) && ! is_user_logged_in() ) || is_edit_account_page() ) {
				self::enqueue_script( 'wc-password-strength-meter' );
			}
		}
		if ( is_checkout() ) {
			self::enqueue_script( 'wc-checkout', $frontend_script_path . 'checkout' . $suffix . '.js', array( 'jquery', 'banda', 'wc-country-select', 'wc-address-i18n' ) );
		}
		if ( is_add_payment_method_page() ) {
			self::enqueue_script( 'wc-add-payment-method', $frontend_script_path . 'add-payment-method' . $suffix . '.js', array( 'jquery', 'banda' ) );
		}
		if ( is_lost_password_page() ) {
			self::enqueue_script( 'wc-lost-password', $frontend_script_path . 'lost-password' . $suffix . '.js', array( 'jquery', 'banda' ) );
		}
		if ( $lightbox_en && ( is_product() || ( ! empty( $post->post_content ) && strstr( $post->post_content, '[product_page' ) ) ) ) {
			...
			self::enqueue_style( 'banda_prettyPhoto_css', $assets_path . 'css/prettyPhoto.css' );
		}
		if ( is_product() ) {
			self::enqueue_script( 'wc-single-product' );
		}
		if ( 'geolocation_ajax' === get_option( 'banda_default_customer_address' ) ) {
		...
		self::enqueue_script( 'banda', $frontend_script_path . 'banda' . $suffix . '.js', array( 'jquery', 'jquery-blockui' ) );
	 ...
	 * @access private
	 * @since  2.3.0 this needs less wp_script_is() calls due to https://core.trac.jabali.github.io/ticket/28404 being added in WP 4.0.
	 * @param  string $handle
	 */
	...
	private static function get_script_data( $handle ) {
		...
		switch ( $handle ) {
			...
			case 'wc-single-product' :
				return array(
					'i18n_required_rating_text' => esc_attr__( 'Please select a rating', 'banda' ),
					'review_rating_required'    => get_option( 'banda_review_rating_required' ),
				);
			break;
			case 'wc-checkout' :
				return array(
					...
					'option_guest_checkout'     => get_option( 'banda_enable_guest_checkout' ),
					'checkout_url'              => WC_AJAX::get_endpoint( "checkout" ),
					'is_checkout'               => is_page( wc_get_page_id( 'checkout' ) ) && empty( $wp->query_vars['order-pay'] ) && ! isset( $wp->query_vars['order-received'] ) ? 1 : 0,
					'debug_mode'                => defined('WP_DEBUG') && WP_DEBUG,
					'i18n_checkout_error'       => esc_attr__( 'Error processing checkout. Please try again.', 'banda' ),
				);
			break;
			case 'wc-address-i18n' :
				return array(
					...
					'i18n_required_text' => esc_attr__( 'required', 'banda' ),
				);
			break;
			case 'wc-cart' :
				return array(
			...
			case 'wc-cart-fragments' :
				return array(
					'ajax_url'      => WC()->ajax_url(),
					'wc_ajax_url'   => WC_AJAX::get_endpoint( "%%endpoint%%" ),
					'fragment_name' => apply_filters( 'banda_cart_fragment_name', 'wc_fragments' )
				);
			break;
			case 'wc-add-to-cart' :
				return array(
					'ajax_url'                => WC()->ajax_url(),
					'wc_ajax_url'             => WC_AJAX::get_endpoint( "%%endpoint%%" ),
					'i18n_view_cart'          => esc_attr__( 'View Cart', 'banda' ),
					'cart_url'                => apply_filters( 'banda_add_to_cart_redirect', wc_get_cart_url() ),
					'is_cart'                 => is_cart(),
					'cart_redirect_after_add' => get_option( 'banda_cart_redirect_after_add' )
				);
			break;
			case 'wc-add-to-cart-variation' :
				...
				return array(
					'i18n_no_matching_variations_text' => esc_attr__( 'Sorry, no products matched your selection. Please choose a different combination.', 'banda' ),
					'i18n_make_a_selection_text'       => esc_attr__( 'Please select some product options before adding this product to your cart.', 'banda' ),
					'i18n_unavailable_text'            => esc_attr__( 'Sorry, this product is unavailable. Please choose a different combination.', 'banda' )
				);
			break;
			...
			case 'wc-password-strength-meter' :
				return array(
					'min_password_strength' => apply_filters( 'banda_min_password_strength', 3 ),
					'i18n_password_error'   => esc_attr__( 'Please enter a stronger password.', 'banda' ),
					'i18n_password_hint'    => esc_attr__( 'The password should be at least seven characters long. To make it stronger, use upper and lower case letters, numbers and symbols like ! " ? $ % ^ &amp; ).', 'banda' )
				);
			break;
		}
		return false;
