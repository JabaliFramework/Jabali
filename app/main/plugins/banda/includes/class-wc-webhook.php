...
class WC_Webhook {
	...
	public function process( $arg ) {
		...
		if ( apply_filters( 'banda_webhook_deliver_async', true, $this, $arg ) ) {

			// deliver in background
			wp_schedule_single_event( time(), 'banda_deliver_webhook_async', array( $this->id, $arg ) );
	...
	private function should_deliver( $arg ) {
		...
		} elseif ( 0 === strpos( $current_action, 'banda_process_shop' ) || 0 === strpos( $current_action, 'banda_process_product' ) ) {
			// the `banda_process_shop_*` and `banda_process_product_*` hooks
		 ...
		 */
		return apply_filters( 'banda_webhook_should_deliver', $should_deliver, $this, $arg );
	}
	...
	public function deliver( $arg ) {
		...
		$http_args = array(
			...
			'httpversion' => '1.0',
			'blocking'    => true,
			'user-agent'  => sprintf( 'Banda/%s Hookshot (Jabali/%s)', WC_VERSION, $GLOBALS['wp_version'] ),
			'body'        => trim( json_encode( $payload ) ),
			'headers'     => array( 'Content-Type' => 'application/json' ),
			'cookies'     => array(),
		);

		$http_args = apply_filters( 'banda_webhook_http_args', $http_args, $arg, $this->id );
		...
		do_action( 'banda_webhook_delivery', $http_args, $response, $duration, $arg, $this->id );
	}
	...
	private function build_payload( $resource_id ) {
		...
		return apply_filters( 'banda_webhook_payload', $payload, $resource, $resource_id, $this->id );
	}
	...
	public function generate_signature( $payload ) {
		...
		$hash_algo = apply_filters( 'banda_webhook_hash_algorithm', 'sha256', $payload, $this->id );
	...
	public function get_new_delivery_id() {
		...
		$comment_data = apply_filters( 'banda_new_webhook_delivery_data', array(
			'comment_author'       => __( 'Banda', 'banda' ),
			'comment_author_email' => sanitize_email( sprintf( '%s@%s', strtolower( __( 'Banda', 'banda' ) ), isset( $_SERVER['HTTP_HOST'] ) ? str_replace( 'www.', '', $_SERVER['HTTP_HOST'] ) : 'noreply.com' ) ),
			'comment_post_ID'      => $this->id,
			'comment_agent'        => 'Banda Hookshot',
	...
	public function log_delivery( $delivery_id, $request, $response, $duration ) {
		...
		if ( $log->total_comments > apply_filters( 'banda_max_webhook_delivery_logs', 25 ) ) {
			global $wpdb;
	...
	private function failed_delivery() {
		...
		if ( $failures > apply_filters( 'banda_max_webhook_delivery_failures', 5 ) ) {
	...
	public function get_delivery_log( $delivery_id ) {
		...
		return apply_filters( 'banda_webhook_delivery_log', $delivery_log, $delivery_id, $this->id );
	}
	...
	private function get_topic_hooks( $topic ) {
		...
		$topic_hooks = array(
			'coupon.created' => array(
				'banda_process_shop_coupon_meta',
				'banda_api_create_coupon',
			),
			'coupon.updated' => array(
				'banda_process_shop_coupon_meta',
				'banda_api_edit_coupon',
			),
			...
			'customer.created' => array(
				...
				'banda_api_create_customer'
			),
			'customer.updated' => array(
				'profile_update',
				'banda_api_edit_customer',
				'banda_customer_save_address',
			),
			'customer.deleted' => array(
				'delete_user',
			),
			'order.created'    => array(
				'banda_checkout_order_processed',
				'banda_process_shop_order_meta',
				'banda_api_create_order',
			),
			'order.updated' => array(
				'banda_process_shop_order_meta',
				'banda_api_edit_order',
				'banda_order_edit_status',
				'banda_order_status_changed'
			),
			'order.deleted' => array(
				'wp_trash_post',
			),
			'product.created' => array(
				'banda_process_product_meta',
				'banda_api_create_product',
			),
			'product.updated' => array(
				'banda_process_product_meta',
				'banda_api_edit_product',
			),
		...
		$topic_hooks = apply_filters( 'banda_webhook_topic_hooks', $topic_hooks, $this );
	...
	public function deliver_ping() {
		...
		if ( is_wp_error( $test ) ) {
			return new WP_Error( 'error', sprintf( __( 'Error: Delivery URL cannot be reached: %s', 'banda' ), $test->get_error_message() ) );
		}

		if ( 200 !== $response_code ) {
			return new WP_Error( 'error', sprintf( __( 'Error: Delivery URL returned response code: %s', 'banda' ), absint( $response_code ) ) );
		}
	...
	public function get_status() {
		...
		return apply_filters( 'banda_webhook_status', $status, $this->id );
	}
	...
	public function get_delivery_url() {
		...
		return apply_filters( 'banda_webhook_delivery_url', $this->delivery_url, $this->id );
	}
	...
	public function get_secret() {
		return apply_filters( 'banda_webhook_secret', $this->secret, $this->id );
	}
	...
	public function get_name() {
		return apply_filters( 'banda_webhook_name', $this->get_post_data()->post_title, $this->id );
	}
	...
	public function get_topic() {
		return apply_filters( 'banda_webhook_topic', $this->topic, $this->id );
	}
	...
	public function get_hooks() {
		return apply_filters( 'banda_webhook_hooks', $this->hooks, $this->id );
	}
	...
	public function get_resource() {
		return apply_filters( 'banda_webhook_resource', $this->resource, $this->id );
	}
	...
	public function get_event() {
		return apply_filters( 'banda_webhook_event', $this->event, $this->id );
	}
