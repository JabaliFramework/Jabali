...
class WC_Widget_Rating_Filter extends WC_Widget {
	...
	public function __construct() {
		$this->widget_cssclass    = 'banda widget_rating_filter';
		$this->widget_description = __( 'Filter products by rating when viewing product archives and categories.', 'banda' );
		$this->widget_id          = 'banda_rating_filter';
		$this->widget_name        = __( 'Banda Average Rating Filter', 'banda' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'std'   => __( 'Average Rating', 'banda' ),
	...
	public function widget( $args, $instance ) {
		...
		for ( $rating = 4; $rating >= 1; $rating-- ) {
			...
			echo '<a href="' . esc_url( apply_filters( 'banda_rating_filter_link', $link ) ) . '">';

			echo '<span class="star-rating" title="' . esc_attr( sprintf( __( 'Rated %s and above', 'banda' ), $rating ) ). '">
					<span style="width:' . esc_attr( ( $rating / 5 ) * 100 ) . '%">' . sprintf( __( 'Rated %s and above', 'banda'), $rating ) . '</span>
				</span> (' . esc_html( $count ) . ')';
