...
class WC_Tax {
	...
	public static function init() {
		...
		self::$round_at_subtotal = 'yes' === get_option( 'banda_tax_round_at_subtotal' );
	}
	...
	public static function calc_tax( $price, $rates, $price_includes_tax = false, $suppress_rounding = false ) {
		...
		return apply_filters( 'banda_calc_tax', $taxes, $price, $rates, $price_includes_tax, $suppress_rounding );
	}
	 ...
	 * }
	 * add_filter( 'banda_tax_round', 'euro_5cent_rounding' );
	 * @return double
	 */
	public static function round( $in ) {
		return apply_filters( 'banda_tax_round', round( $in, self::$precision ), $in );
	}
	...
	public static function calc_inclusive_tax( $price, $rates ) {
		...
		foreach ( $rates as $key => $rate ) {
			...
			$taxes[ $key ]   += apply_filters( 'banda_price_inc_tax_amount', $tax_amount, $key, $rate, $price );
		}
	...
	public static function calc_exclusive_tax( $price, $rates ) {
		...
		if ( ! empty( $rates ) ) {
			...
			foreach ( $rates as $key => $rate ) {
				...
				$tax_amount = apply_filters( 'banda_price_ex_tax_amount', $tax_amount, $key, $rate, $price );
			...
			foreach ( $rates as $key => $rate ) {
				...
				// ADVANCED: Allow third parties to modify this rate
				$tax_amount = apply_filters( 'banda_price_ex_tax_amount', $tax_amount, $key, $rate, $price, $the_price_inc_tax, $pre_compound_total );
	...
	public static function find_rates( $args = array() ) {
		...
		return apply_filters( 'banda_find_rates', $matched_tax_rates, $args );
	}
	...
	private static function get_matched_tax_rates( $country, $state, $postcode, $city, $tax_class ) {
		...
		$postcode_ranges = $wpdb->get_results( "SELECT tax_rate_id, location_code FROM {$wpdb->prefix}banda_tax_rate_locations WHERE location_type = 'postcode' AND location_code LIKE '%...%';" );
		...
		$locations_criteria[] = "
			...
			AND (
				...
				OR NOT EXISTS (
					SELECT sub.tax_rate_id FROM {$wpdb->prefix}banda_tax_rate_locations as sub
		...
		$found_rates = $wpdb->get_results( "
			SELECT tax_rates.*
			FROM {$wpdb->prefix}banda_tax_rates as tax_rates
			LEFT OUTER JOIN {$wpdb->prefix}banda_tax_rate_locations as locations ON tax_rates.tax_rate_id = locations.tax_rate_id
			LEFT OUTER JOIN {$wpdb->prefix}banda_tax_rate_locations as locations2 ON tax_rates.tax_rate_id = locations2.tax_rate_id
		...
		return apply_filters( 'banda_matched_tax_rates', $matched_tax_rates, $country, $state, $postcode, $city, $tax_class );
	}
	...
	public static function get_tax_location( $tax_class = '' ) {
		...
		} elseif ( wc_prices_include_tax() || 'base' === get_option( 'banda_default_customer_address' ) || 'base' === get_option( 'banda_tax_based_on' ) ) {
			$location = array(
		...
		return apply_filters( 'banda_get_tax_location', $location, $tax_class );
	}
	...
	public static function get_rates( $tax_class = '' ) {
		...
		return apply_filters( 'banda_matched_rates', $matched_tax_rates, $tax_class );
	}
	...
	public static function get_base_tax_rates( $tax_class = '' ) {
		return apply_filters( 'banda_base_tax_rates', self::find_rates( array(
	...
	public static function get_shipping_tax_rates( $tax_class = null ) {
		...
		if ( $shipping_tax_class = get_option( 'banda_shipping_tax_class' ) ) {
	...
	public static function is_compound( $key ) {
		global $wpdb;
		return $wpdb->get_var( $wpdb->prepare( "SELECT tax_rate_compound FROM {$wpdb->prefix}banda_tax_rates WHERE tax_rate_id = %s", $key ) ) ? true : false;
	}
	...
	public static function get_rate_label( $key_or_rate ) {
		...
		if ( is_object( $key_or_rate ) ) {
			...
			$key       = $key_or_rate;
			$rate_name = $wpdb->get_var( $wpdb->prepare( "SELECT tax_rate_name FROM {$wpdb->prefix}banda_tax_rates WHERE tax_rate_id = %s", $key ) );
		}
		...
		return apply_filters( 'banda_rate_label', $rate_name, $key );
	}
	...
	public static function get_rate_percent( $key_or_rate ) {
		...
		if ( is_object( $key_or_rate ) ) {
			...
			$key      = $key_or_rate;
			$tax_rate = $wpdb->get_var( $wpdb->prepare( "SELECT tax_rate FROM {$wpdb->prefix}banda_tax_rates WHERE tax_rate_id = %s", $key ) );
		}

		return apply_filters( 'banda_rate_percent', floatval( $tax_rate ) . '%', $key );
	}
	...
	public static function get_rate_code( $key_or_rate ) {
		...
		if ( is_object( $key_or_rate ) ) {
			...
			$key  = $key_or_rate;
			$rate = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}banda_tax_rates WHERE tax_rate_id = %s", $key ) );
		}
		...
		return apply_filters( 'banda_rate_code', $code_string, $key );
	}
	...
	public static function _insert_tax_rate( $tax_rate ) {
		...
		$wpdb->insert( $wpdb->prefix . 'banda_tax_rates', self::prepare_tax_rate( $tax_rate ) );

		WC_Cache_Helper::incr_cache_prefix( 'taxes' );

		do_action( 'banda_tax_rate_added', $wpdb->insert_id, $tax_rate );
	...
	public static function _update_tax_rate( $tax_rate_id, $tax_rate ) {
		...
		do_action( 'banda_tax_rate_updated', $tax_rate_id, $tax_rate );
	}
	...
	public static function _delete_tax_rate( $tax_rate_id ) {
		...
		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}banda_tax_rate_locations WHERE tax_rate_id = %d;", $tax_rate_id ) );
		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}banda_tax_rates WHERE tax_rate_id = %d;", $tax_rate_id ) );

		WC_Cache_Helper::incr_cache_prefix( 'taxes' );

		do_action( 'banda_tax_rate_deleted', $tax_rate_id );
	}
	...
	private static function _update_tax_rate_locations( $tax_rate_id, $values, $type ) {
		...
		$wpdb->query(
			$wpdb->prepare( "
				DELETE FROM {$wpdb->prefix}banda_tax_rate_locations WHERE tax_rate_id = %d AND location_type = %s;
				", $tax_rate_id, $type
			)
		);
		...
		if ( sizeof( $values ) > 0 ) {
			...
			$wpdb->query( "
				INSERT INTO {$wpdb->prefix}banda_tax_rate_locations ( location_code, tax_rate_id, location_type ) VALUES $sql;
				" );
		}
	...
	public static function get_rates_for_tax_class( $tax_class ) {
		...
		$rates     = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM `{$wpdb->prefix}banda_tax_rates` WHERE `tax_rate_class` = %s ORDER BY `tax_rate_order`;", sanitize_title( $tax_class ) ) );
		$locations = $wpdb->get_results( "SELECT * FROM `{$wpdb->prefix}banda_tax_rate_locations`" );
