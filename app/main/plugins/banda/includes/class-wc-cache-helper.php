...
class WC_Cache_Helper {
	...
	public static function init() {
		...
		add_action( 'before_banda_init', array( __CLASS__, 'prevent_caching' ) );
	...
	public static function geolocation_ajax_redirect() {
		if ( 'geolocation_ajax' === get_option( 'banda_default_customer_address' ) && ! is_checkout() && ! is_cart() && ! is_account_page() && ! is_ajax() && empty( $_POST ) ) {
	 ...
	 *
	 * Raised in issue https://github.com/mtaandao/banda/issues/5777.
	...
	public static function delete_version_transients( $version = '' ) {
		if ( ! wp_using_ext_object_cache() && ! empty( $version ) ) {
			...
			$limit    = apply_filters( 'banda_delete_version_transients_limit', 1000 );
	...
	public static function notices() {
		...
		if ( $enabled && ! in_array( '_wc_session_', $settings ) ) {
			?>
			<div class="error">
				<p><?php printf( __( 'In order for <strong>database caching</strong> to work with Banda you must add <code>_wc_session_</code> to the "Ignored Query Strings" option in W3 Total Cache settings <a href="%s">here</a>.', 'banda' ), admin_url( 'admin.php?page=w3tc_dbcache' ) ); ?></p>
			</div>
			<?php
		}
	}
