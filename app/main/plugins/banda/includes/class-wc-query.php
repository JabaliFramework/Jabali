<?php
/**
 * Contains the query functions for Banda which alter the front-end post queries and loops
 *
 * @class 		WC_Query
 * @version		2.6.0
...
class WC_Query {
	...
	public function init_query_vars() {
		...
		$this->query_vars = array(
			// Checkout actions.
			'order-pay'          => get_option( 'banda_checkout_pay_endpoint', 'order-pay' ),
			'order-received'     => get_option( 'banda_checkout_order_received_endpoint', 'order-received' ),
			// My account actions.
			'orders'                     => get_option( 'banda_myaccount_orders_endpoint', 'orders' ),
			'view-order'                 => get_option( 'banda_myaccount_view_order_endpoint', 'view-order' ),
			...
			'lost-password'              => get_option( 'banda_myaccount_lost_password_endpoint', 'lost-password' ),
			'customer-logout'            => get_option( 'banda_logout_endpoint', 'customer-logout' ),
			'add-payment-method'         => get_option( 'banda_myaccount_add_payment_method_endpoint', 'add-payment-method' ),
			'delete-payment-method'      => get_option( 'banda_myaccount_delete_payment_method_endpoint', 'delete-payment-method' ),
			'set-default-payment-method' => get_option( 'banda_myaccount_set_default_payment_method_endpoint', 'set-default-payment-method' ),
		);
	}
		...
		switch ( $endpoint ) {
			...
			case 'orders' :
				if ( ! empty( $wp->query_vars['orders'] ) ) {
					$title = sprintf( __( 'Orders (page %d)', 'banda' ), intval( $wp->query_vars['orders'] ) );
				} else {
			...
			case 'view-order' :
				...
				$title = ( $order ) ? sprintf( __( 'Order #%s', 'banda' ), $order->get_order_number() ) : '';
			break;
			case 'downloads' :
				$title = __( 'Downloads', 'banda' );
			break;
			case 'edit-account' :
				$title = __( 'Account Details', 'banda' );
			break;
			case 'edit-address' :
			...
			break;
			default :
				$title = apply_filters( 'banda_endpoint_' . $endpoint . '_title', '' );
			break;
		}
	...
	protected function get_endpoints_mask() {
		if ( 'page' === get_option( 'show_on_front' ) ) {
			...
			$myaccount_page_id = get_option( 'banda_myaccount_page_id' );
			$checkout_page_id  = get_option( 'banda_checkout_page_id' );
	...
	public function pre_get_posts( $q ) {
		...
		// When orderby is set, Jabali shows posts. Get around that here.
		...
		if ( $q->is_page() && 'page' === get_option( 'show_on_front' ) && absint( $q->get( 'page_id' ) ) === wc_get_page_id( 'shop' ) ) {
			...
			// This is hacky but works. Awaiting https://core.trac.jabali.github.io/ticket/21096
	...
	/**
	 * Query the products, applying sorting/ordering etc. This applies to the main jabali loop.
	 *
	 * @param mixed $q
	 */
	public function product_query( $q ) {
		...
		do_action( 'banda_product_query', $q, $this );
	}
	...
	public function get_catalog_ordering_args( $orderby = '', $order = '' ) {
		...
		if ( ! $orderby ) {
			$orderby_value = isset( $_GET['orderby'] ) ? wc_clean( $_GET['orderby'] ) : apply_filters( 'banda_default_catalog_orderby', get_option( 'banda_default_catalog_orderby' ) );
		...
		return apply_filters( 'banda_get_catalog_ordering_args', $args );
	}

	/**
	 * WP Core doens't let us change the sort direction for invidual orderby params - https://core.trac.jabali.github.io/ticket/17065.
	 *
	...
	public function get_meta_query( $meta_query = array() ) {
		...
		return array_filter( apply_filters( 'banda_product_query_meta_query', $meta_query, $this ) );
	}
	...
	private function price_filter_meta_query() {
		if ( isset( $_GET['max_price'] ) || isset( $_GET['min_price'] ) ) {
			 ...
			 */
			if ( wc_tax_enabled() && 'incl' === get_option( 'banda_tax_display_shop' ) && ! wc_prices_include_tax() ) {
	...
	public function stock_status_meta_query( $status = 'instock' ) {
		return 'yes' === get_option( 'banda_hide_out_of_stock_items' ) ? array(
			'key' 		=> '_stock_status',
			'value' 	=> $status,
			'compare' 	=> '=',
		) : array();
	...
	public function get_tax_query( $tax_query = array() ) {
		...
		return array_filter( apply_filters( 'banda_product_query_tax_query', $tax_query, $this ) );
	}
		...
		if ( ! is_array( self::$_chosen_attributes ) ) {
			...
			if ( $attribute_taxonomies = wc_get_attribute_taxonomies() ) {
				foreach ( $attribute_taxonomies as $tax ) {
					...
					self::$_chosen_attributes[ $taxonomy ]['query_type'] = $query_type ? $query_type : apply_filters( 'banda_layered_nav_default_query_type', 'and' );
				}
			}
		}
