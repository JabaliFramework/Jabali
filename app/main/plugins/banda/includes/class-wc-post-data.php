...
class WC_Post_Data {
	...
	public static function init() {
		...
		add_action( 'banda_product_set_stock_status', array( __CLASS__, 'delete_product_query_transients' ) );
		add_action( 'banda_product_set_visibility', array( __CLASS__, 'delete_product_query_transients' ) );
		...
		if ( 'product' === get_post_type( $post_id ) ) {
			...
			if ( isset( $_POST['_visibility'] ) ) {
				if ( update_post_meta( $post_id, '_visibility', wc_clean( $_POST['_visibility'] ) ) ) {
					do_action( 'banda_product_set_visibility', $post_id, wc_clean( $_POST['_visibility'] ) );
				}
			}
