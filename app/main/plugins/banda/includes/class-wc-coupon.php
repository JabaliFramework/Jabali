 ...
 * Banda coupons
 *
 * The Banda coupons class gets coupon data from storage and checks coupon validity.
 *
 * @class 		WC_Coupon
 * @version		2.3.0
...
class WC_Coupon {
	...
	private function get_coupon( $code ) {
		$this->code  = apply_filters( 'banda_coupon_code', $code );

		// Coupon data lets developers create coupons through code
		if ( $coupon = apply_filters( 'banda_get_shop_coupon_data', false, $this->code ) ) {
		...
		if ( $coupon_post && $this->code === apply_filters( 'banda_coupon_code', $coupon_post->post_title ) ) {
			$this->populate();
			return true;
		}
	 ...
	 * @since 2.5.0 banda_coupon_code_query was removed in favour of banda_get_coupon_id_from_code filter on the return. wp_cache was also implemented.
	 * @param  string $code
	 * @return int
	 */
	private function get_coupon_id_from_code( $code ) {
		...
		if ( false === $coupon_id ) {
			...
			if ( $coupon_id = apply_filters( 'banda_get_coupon_id_from_code', $wpdb->get_var( $sql ), $this->code ) ) {
	...
	private function populate( $data = array() ) {
		...
		do_action( 'banda_coupon_loaded', $this );
	}
	...
	private function validate_minimum_amount() {
		if ( $this->minimum_amount > 0 && apply_filters( 'banda_coupon_validate_minimum_amount', wc_format_decimal( $this->minimum_amount ) > WC()->cart->get_displayed_subtotal(), $this ) ) {
	...
	private function validate_maximum_amount() {
		if ( $this->maximum_amount > 0 && apply_filters( 'banda_coupon_validate_maximum_amount', wc_format_decimal( $this->maximum_amount ) < WC()->cart->get_displayed_subtotal(), $this ) ) {
	...
	public function is_valid() {
		try {
			...
			if ( ! apply_filters( 'banda_coupon_is_valid', true, $this ) ) {
	...
	public function is_valid_for_cart() {
		return apply_filters( 'banda_coupon_is_valid_for_cart', $this->is_type( wc_get_cart_coupon_types() ), $this );
	}
	...
	public function is_valid_for_product( $product, $values = array() ) {
		if ( ! $this->is_type( wc_get_product_coupon_types() ) ) {
			return apply_filters( 'banda_coupon_is_valid_for_product', false, $product, $this, $values );
		}
		...
		return apply_filters( 'banda_coupon_is_valid_for_product', $valid, $product, $this, $values );
	}
	...
	public function get_discount_amount( $discounting_amount, $cart_item = null, $single = false ) {
		...
		if ( $this->is_type( array( 'percent_product', 'percent' ) ) ) {
			 ...
			 *
			 * Uses price inc tax if prices include tax to work around https://github.com/mtaandao/banda/issues/7669 and https://github.com/mtaandao/banda/issues/8074.
			 */
		...
		return apply_filters( 'banda_coupon_get_discount_amount', $discount, $discounting_amount, $cart_item, $single, $this );
	}
	...
	public function get_coupon_message( $msg_code ) {
		switch ( $msg_code ) {
			case self::WC_COUPON_SUCCESS :
				$msg = __( 'Coupon code applied successfully.', 'banda' );
			break;
			case self::WC_COUPON_REMOVED :
				$msg = __( 'Coupon code removed successfully.', 'banda' );
			break;
			default:
				$msg = '';
			break;
		}
		return apply_filters( 'banda_coupon_message', $msg, $msg_code, $this );
	}
	...
	public function get_coupon_error( $err_code ) {
		switch ( $err_code ) {
			case self::E_WC_COUPON_INVALID_FILTERED:
				$err = __( 'Coupon is not valid.', 'banda' );
			break;
			case self::E_WC_COUPON_NOT_EXIST:
				$err = sprintf( __( 'Coupon "%s" does not exist!', 'banda' ), $this->code );
			break;
			case self::E_WC_COUPON_INVALID_REMOVED:
				$err = sprintf( __( 'Sorry, it seems the coupon "%s" is invalid - it has now been removed from your order.', 'banda' ), $this->code );
			break;
			case self::E_WC_COUPON_NOT_YOURS_REMOVED:
				$err = sprintf( __( 'Sorry, it seems the coupon "%s" is not yours - it has now been removed from your order.', 'banda' ), $this->code );
			break;
			case self::E_WC_COUPON_ALREADY_APPLIED:
				$err = __( 'Coupon code already applied!', 'banda' );
			break;
			case self::E_WC_COUPON_ALREADY_APPLIED_INDIV_USE_ONLY:
				$err = sprintf( __( 'Sorry, coupon "%s" has already been applied and cannot be used in conjunction with other coupons.', 'banda' ), $this->code );
			break;
			case self::E_WC_COUPON_USAGE_LIMIT_REACHED:
				$err = __( 'Coupon usage limit has been reached.', 'banda' );
			break;
			case self::E_WC_COUPON_EXPIRED:
				$err = __( 'This coupon has expired.', 'banda' );
			break;
			case self::E_WC_COUPON_MIN_SPEND_LIMIT_NOT_MET:
				$err = sprintf( __( 'The minimum spend for this coupon is %s.', 'banda' ), wc_price( $this->minimum_amount ) );
			break;
			case self::E_WC_COUPON_MAX_SPEND_LIMIT_MET:
				$err = sprintf( __( 'The maximum spend for this coupon is %s.', 'banda' ), wc_price( $this->maximum_amount ) );
			break;
			case self::E_WC_COUPON_NOT_APPLICABLE:
				$err = __( 'Sorry, this coupon is not applicable to your cart contents.', 'banda' );
			break;
			case self::E_WC_COUPON_EXCLUDED_PRODUCTS:
				...
				$err = sprintf( __( 'Sorry, this coupon is not applicable to the products: %s.', 'banda' ), implode( ', ', $products ) );
				break;
			case self::E_WC_COUPON_EXCLUDED_CATEGORIES:
				...
				$err = sprintf( __( 'Sorry, this coupon is not applicable to the categories: %s.', 'banda' ), implode( ', ', array_unique( $categories ) ) );
				break;
			case self::E_WC_COUPON_NOT_VALID_SALE_ITEMS:
				$err = __( 'Sorry, this coupon is not valid for sale items.', 'banda' );
			break;
			default:
				$err = '';
			break;
		}
		return apply_filters( 'banda_coupon_error', $err, $err_code, $this );
	}
	...
	public static function get_generic_coupon_error( $err_code ) {
		switch ( $err_code ) {
			case self::E_WC_COUPON_NOT_EXIST:
				$err = __( 'Coupon does not exist!', 'banda' );
			break;
			case self::E_WC_COUPON_PLEASE_ENTER:
				$err = __( 'Please enter a coupon code.', 'banda' );
			break;
			default:
				$err = '';
			break;
		...
		return apply_filters( 'banda_coupon_error', $err, $err_code, null );
	}
}
