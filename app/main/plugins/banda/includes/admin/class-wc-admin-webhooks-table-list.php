...
class WC_Admin_Webhooks_Table_List extends WP_List_Table {
	...
	public function __construct() {
		parent::__construct( array(
			'singular' => __( 'webhook', 'banda' ),
			'plural'   => __( 'webhooks', 'banda' ),
			'ajax'     => false
		) );
	}
	...
	public function get_columns() {
		return array(
			...
			'title'        => __( 'Name', 'banda' ),
			'status'       => __( 'Status', 'banda' ),
			'topic'        => __( 'Topic', 'banda' ),
			'delivery_url' => __( 'Delivery URL', 'banda' ),
		);
	}
	...
	public function column_title( $webhook ) {
		...
		// Get actions
		$actions = array(
			'id' => sprintf( __( 'ID: %d', 'banda' ), $the_webhook->id )
		);

		if ( current_user_can( $post_type_object->cap->edit_post, $the_webhook->id ) && 'trash' !== $post_status ) {
			$actions['edit'] = '<a href="' . esc_url( $edit_link ) . '">' . __( 'Edit', 'banda' ) . '</a>';
		}

		if ( current_user_can( $post_type_object->cap->delete_post, $the_webhook->id ) ) {
			if ( 'trash' == $post_status ) {
				$actions['untrash'] = '<a title="' . esc_attr__( 'Restore this item from the Trash', 'banda' ) . '" href="' . wp_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $the_webhook->id ) ), 'untrash-post_' . $the_webhook->id ) . '">' . __( 'Restore', 'banda' ) . '</a>';
			} elseif ( EMPTY_TRASH_DAYS ) {
				$actions['trash'] = '<a class="submitdelete" title="' . esc_attr( __( 'Move this item to the Trash', 'banda' ) ) . '" href="' . get_delete_post_link( $the_webhook->id ) . '">' . __( 'Trash', 'banda' ) . '</a>';
			}
			if ( 'trash' == $post_status || ! EMPTY_TRASH_DAYS ) {
				$actions['delete'] = '<a class="submitdelete" title="' . esc_attr( __( 'Delete this item permanently', 'banda' ) ) . '" href="' . get_delete_post_link( $the_webhook->id, '', true ) . '">' . __( 'Delete Permanently', 'banda' ) . '</a>';
			}
		}
	...
	private function get_status_label( $status_name, $status ) {
		switch ( $status_name ) {
			case 'publish' :
				$label = array(
					'singular' => __( 'Activated <span class="count">(%s)</span>', 'banda' ),
					'plural'   => __( 'Activated <span class="count">(%s)</span>', 'banda' ),
					'context'  => '',
					'domain'   => 'banda',
				);
				break;
			case 'draft' :
				$label = array(
					'singular' => __( 'Paused <span class="count">(%s)</span>', 'banda' ),
					'plural'   => __( 'Paused <span class="count">(%s)</span>', 'banda' ),
					'context'  => '',
					'domain'   => 'banda',
				);
				break;
			case 'pending' :
				$label = array(
					'singular' => __( 'Disabled <span class="count">(%s)</span>', 'banda' ),
					'plural'   => __( 'Disabled <span class="count">(%s)</span>', 'banda' ),
					'context'  => '',
					'domain'   => 'banda',
				);
				break;
	...
	protected function get_views() {
		...
		$status_links['all'] = "<a href='admin.php?page=wc-settings&amp;tab=api&amp;section=webhooks'$class>" . sprintf( _nx( 'All <span class="count">(%s)</span>', 'All <span class="count">(%s)</span>', $total_posts, 'posts', 'banda' ), number_format_i18n( $total_posts ) ) . '</a>';
	...
	protected function get_bulk_actions() {
		if ( isset( $_GET['status'] ) && 'trash' == $_GET['status'] ) {
			return array(
				'untrash' => __( 'Restore', 'banda' ),
				'delete'  => __( 'Delete Permanently', 'banda' )
			);
		}

		return array(
			'trash' => __( 'Move to Trash', 'banda' )
		);
	}
	...
	protected function extra_tablenav( $which ) {
		if ( 'top' == $which && isset( $_GET['status'] ) && 'trash' == $_GET['status'] && current_user_can( 'delete_shop_webhooks' ) ) {
			echo '<div class="alignleft actions"><a class="button apply" href="' . esc_url( wp_nonce_url( admin_url( 'admin.php?page=wc-settings&tab=api&section=webhooks&status=trash&empty_trash=1' ), 'empty_trash' ) ) . '">' . __( 'Empty Trash', 'banda' ) . '</a></div>';
		}
	}
	...
	public function prepare_items() {
		$per_page = apply_filters( 'banda_webhooks_settings_posts_per_page', 10 );
