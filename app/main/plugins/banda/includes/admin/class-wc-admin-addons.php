...
class WC_Admin_Addons {
	...
	public static function get_featured() {
		if ( false === ( $featured = get_transient( 'wc_addons_featured' ) ) ) {
			$raw_featured = wp_safe_remote_get( 'https://d3t0oesq8995hv.cloudfront.net/add-ons/featured.json', array( 'user-agent' => 'Banda Addons Page' ) );
	...
	public static function get_sections() {
		if ( false === ( $sections = get_transient( 'wc_addons_sections' ) ) ) {
			$raw_sections = wp_safe_remote_get( 'https://d3t0oesq8995hv.cloudfront.net/addon-sections.json', array( 'user-agent' => 'Banda Addons Page' ) );
		...
		return apply_filters( 'banda_addons_sections', $addon_sections );
	}
	...
	public static function get_section_data( $section_id ) {
		...
		if ( ! empty( $section->endpoint ) ) {
			if ( false === ( $section_data = get_transient( 'wc_addons_section_' . $section_id ) ) ) {
				$raw_section = wp_safe_remote_get( esc_url_raw( $section->endpoint ), array( 'user-agent' => 'Banda Addons Page' ) );
		...
		return apply_filters( 'banda_addons_section_data', $section_data->products, $section_id );
	}
	...
	public static function output_button( $url, $text, $theme, $plugin = '' ) {
		$theme = __( 'Free', 'banda' ) === $text ? 'addons-button-outline-green' : $theme;
		$theme = is_plugin_active( $plugin ) ? 'addons-button-installed' : $theme;
		$text = is_plugin_active( $plugin ) ? __( 'Installed', 'banda' ) : $text;
		?>
		<a
