...
class WC_Admin_Importers {
	...
	public function register_importers() {
		register_importer( 'banda_tax_rate_csv', __( 'Banda Tax Rates (CSV)', 'banda' ), __( 'Import <strong>tax rates</strong> to your store via a csv file.', 'banda'), array( $this, 'tax_rates_importer' ) );
	}
	 ...
	 *
	 * Jabali import should work - however, it fails to import custom product attribute taxonomies.
							...
							foreach ( $post['terms'] as $term ) {
								if ( strstr( $term['domain'], 'pa_' ) ) {
									if ( ! taxonomy_exists( $term['domain'] ) ) {
										...
										register_taxonomy(
											$term['domain'],
											apply_filters( 'banda_taxonomy_objects_' . $term['domain'], array( 'product' ) ),
											apply_filters( 'banda_taxonomy_args_' . $term['domain'], array(
												'hierarchical' => true,
												'show_ui'      => false,
												'query_var'    => true,
												'rewrite'      => false,
