...
class WC_Admin_Duplicate_Product {
	...
	public function dupe_link( $actions, $post ) {
		if ( ! current_user_can( apply_filters( 'banda_duplicate_product_capability', 'manage_banda' ) ) ) {
			return $actions;
		}
		...
		$actions['duplicate'] = '<a href="' . wp_nonce_url( admin_url( 'edit.php?post_type=product&action=duplicate_product&amp;post=' . $post->ID ), 'banda-duplicate-product_' . $post->ID ) . '" title="' . esc_attr__( 'Make a duplicate from this product', 'banda' )
			. '" rel="permalink">' .  __( 'Duplicate', 'banda' ) . '</a>';
	...
	public function dupe_button() {
		...
		if ( isset( $_GET['post'] ) ) {
			$notify_url = wp_nonce_url( admin_url( "edit.php?post_type=product&action=duplicate_product&post=" . absint( $_GET['post'] ) ), 'banda-duplicate-product_' . $_GET['post'] );
			?>
			<div id="duplicate-action"><a class="submitduplicate duplication" href="<?php echo esc_url( $notify_url ); ?>"><?php _e( 'Copy to a new draft', 'banda' ); ?></a></div>
			<?php
		}
	}
	...
	public function duplicate_product_action() {
		...
		if ( empty( $_REQUEST['post'] ) ) {
			wp_die( __( 'No product to duplicate has been supplied!', 'banda' ) );
		}

		// Get the original page
		$id = isset( $_REQUEST['post'] ) ? absint( $_REQUEST['post'] ) : '';

		check_admin_referer( 'banda-duplicate-product_' . $id );
		...
		if ( ! empty( $post ) ) {
			...
			do_action( 'banda_duplicate_product', $new_id, $post );

			// Redirect to the edit screen for the new draft page
			wp_redirect( admin_url( 'post.php?action=edit&post=' . $new_id ) );
			exit;
		} else {
			wp_die( __( 'Product creation failed, could not find original product:', 'banda' ) . ' ' . $id );
		}
	}
	...
	public function duplicate_product( $post, $parent = 0, $post_status = '' ) {
		...
		if ( 'product_variation' === $post->post_type ) {
			$post_title = sprintf( __( 'Variation #%s of %s', 'banda' ), absint( $new_post_id ), esc_html( get_the_title( $post_parent ) ) );
			$wpdb->update(
				$wpdb->posts,
				array(
					'post_title' => $post_title,
		...
		$exclude = apply_filters( 'banda_duplicate_product_exclude_children', false );
	...
	private function duplicate_post_taxonomies( $id, $new_id, $post_type ) {
		$exclude    = array_filter( apply_filters( 'banda_duplicate_product_exclude_taxonomies', array() ) );
	...
	private function duplicate_post_meta( $id, $new_id ) {
		...
		$exclude = array_map( 'esc_sql', array_filter( apply_filters( 'banda_duplicate_product_exclude_meta', array( 'total_sales', '_wc_average_rating', '_wc_rating_count', '_wc_review_count', '_sku' ) ) ) );
