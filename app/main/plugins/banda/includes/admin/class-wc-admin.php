...
class WC_Admin {
	...
	public function includes() {
		...
		// Help Tabs
		if ( apply_filters( 'banda_enable_admin_help_tab', true ) ) {
	...
	public function admin_redirects() {
		...
		if ( ! empty( $_GET['wc-install-plugin-redirect'] ) ) {
			...
			if ( current_user_can( 'install_plugins' ) && in_array( $plugin_slug, array( 'banda-gateway-stripe' ) ) ) {
		...
		if ( get_transient( '_wc_activation_redirect' ) ) {
			...
			if ( ( ! empty( $_GET['page'] ) && in_array( $_GET['page'], array( 'wc-setup' ) ) ) || is_network_admin() || isset( $_GET['activate-multi'] ) || ! current_user_can( 'manage_banda' ) || apply_filters( 'banda_prevent_automatic_wizard_redirect', false ) ) {
				return;
			}
	...
	public function prevent_admin_access() {
		...
		if ( 'yes' === get_option( 'banda_lock_down_admin', 'yes' ) && ! is_ajax() && basename( $_SERVER["SCRIPT_FILENAME"] ) !== 'admin-post.php' ) {
			$has_cap     = false;
			$access_caps = array( 'edit_posts', 'manage_banda', 'view_admin_dashboard' );
		...
		if ( apply_filters( 'banda_prevent_admin_access', $prevent_access ) ) {
	...
	public function preview_emails() {
		...
		if ( isset( $_GET['preview_banda_mail'] ) ) {
			...
			$email_heading = __( 'HTML Email Template', 'banda' );
			...
			$message       = apply_filters( 'banda_mail_content', $email->style_inline( $mailer->wrap_message( $email_heading, $message ) ) );
	...
	/**
	 * Change the admin footer text on Banda admin pages.
	 *
	 * @since  2.3
	...
	public function admin_footer_text( $footer_text ) {
		...
		// Check to make sure we're on a Banda admin page
