...
$thumbnail     = $_product ? apply_filters( 'banda_admin_order_item_thumbnail', $_product->get_image( 'thumbnail', array( 'title' => '' ), false ), $item_id, $item ) : '';
...
?>
<tr class="item <?php echo apply_filters( 'banda_admin_html_order_item_class', ( ! empty( $class ) ? $class : '' ), $item, $order ); ?>" data-order_item_id="<?php echo $item_id; ?>">
	<td class="thumb">
		<?php
	...
	<td class="name" data-sort-value="<?php echo esc_attr( $item['name'] ); ?>">
		<?php
			...
			if ( $_product && $_product->get_sku() ) {
				echo '<div class="wc-order-item-sku"><strong>' . __( 'SKU:', 'banda' ) . '</strong> ' . esc_html( $_product->get_sku() ) . '</div>';
			}

			if ( ! empty( $item['variation_id'] ) ) {
				echo '<div class="wc-order-item-variation"><strong>' . __( 'Variation ID:', 'banda' ) . '</strong> ';
				if ( ! empty( $item['variation_id'] ) && 'product_variation' === get_post_type( $item['variation_id'] ) ) {
					echo esc_html( $item['variation_id'] );
				} elseif ( ! empty( $item['variation_id'] ) ) {
					echo esc_html( $item['variation_id'] ) . ' (' . __( 'No longer exists', 'banda' ) . ')';
				}
				echo '</div>';
			}
		?>
		...
		<?php do_action( 'banda_before_order_itemmeta', $item_id, $item, $_product ) ?>
		<?php include( 'html-order-item-meta.php' ); ?>
		<?php do_action( 'banda_after_order_itemmeta', $item_id, $item, $_product ) ?>
	</td>

	<?php do_action( 'banda_admin_order_item_values', $_product, $item, absint( $item_id ) ); ?>
	...
	<td class="quantity" width="1%">
		...
		<div class="edit" style="display: none;">
			...
			<input type="number" step="<?php echo apply_filters( 'banda_quantity_input_step', '1', $_product ); ?>" min="0" autocomplete="off" name="order_item_qty[<?php echo absint( $item_id ); ?>]" placeholder="0" value="<?php echo $item_qty; ?>" data-qty="<?php echo $item_qty; ?>" size="4" class="quantity" />
		</div>
		<div class="refund" style="display: none;">
			<input type="number" step="<?php echo apply_filters( 'banda_quantity_input_step', '1', $_product ); ?>" min="0" max="<?php echo $item['qty']; ?>" autocomplete="off" name="refund_order_item_qty[<?php echo absint( $item_id ); ?>]" placeholder="0" size="4" class="refund_order_item_qty" />
		</div>
	</td>
	<td class="line_cost" width="1%" data-sort-value="<?php echo esc_attr( isset( $item['line_total'] ) ? $item['line_total'] : '' ); ?>">
		...
		<div class="edit" style="display: none;">
			<div class="split-input">
				<div class="input">
					<label><?php esc_attr_e( 'Pre-discount:', 'banda' ); ?></label>
				...
				<div class="input">
					<label><?php esc_attr_e( 'Total:', 'banda' ); ?></label>
					<input type="text" name="line_total[<?php echo absint( $item_id ); ?>]" placeholder="<?php echo wc_format_localized_price( 0 ); ?>" value="<?php echo $item_total; ?>" class="line_total wc_input_price" data-tip="<?php esc_attr_e( 'After pre-tax discounts.', 'banda' ); ?>" data-total="<?php echo $item_total; ?>" />
				</div>
			</div>
		</div>
		<div class="refund" style="display: none;">
	...
	<td class="wc-order-edit-line-item" width="1%">
		<div class="wc-order-edit-line-item-actions">
			<?php if ( $order->is_editable() ) : ?>
				<a class="edit-order-item tips" href="#" data-tip="<?php esc_attr_e( 'Edit item', 'banda' ); ?>"></a><a class="delete-order-item tips" href="#" data-tip="<?php esc_attr_e( 'Delete item', 'banda' ); ?>"></a>
			<?php endif; ?>
		</div>
	</td>
</tr>
