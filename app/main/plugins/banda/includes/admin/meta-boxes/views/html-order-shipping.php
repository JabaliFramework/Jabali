...
<tr class="shipping <?php echo ( ! empty( $class ) ) ? $class : ''; ?>" data-order_item_id="<?php echo $item_id; ?>">
	...
	<td class="name">
		<div class="view">
			<?php echo ! empty( $item['name'] ) ? wc_clean( $item['name'] ) : __( 'Shipping', 'banda' ); ?>
		</div>
		<div class="edit" style="display: none;">
			<input type="hidden" name="shipping_method_id[]" value="<?php echo esc_attr( $item_id ); ?>" />
			<input type="text" class="shipping_method_name" placeholder="<?php esc_attr_e( 'Shipping Name', 'banda' ); ?>" name="shipping_method_title[<?php echo $item_id; ?>]" value="<?php echo ( isset( $item['name'] ) ) ? wc_clean( $item['name'] ) : ''; ?>" />
			<select class="shipping_method" name="shipping_method[<?php echo esc_attr( $item_id ); ?>]">
				<optgroup label="<?php esc_attr_e( 'Shipping Method', 'banda' ); ?>">
					...
					<?php
						...
						if ( ! $found_method && ! empty( $method_id ) ) {
							echo '<option value="' . esc_attr( $method_id ) . '" selected="selected">' . __( 'Other', 'banda' ) . '</option>';
						} else {
		...
		<?php do_action( 'banda_before_order_itemmeta', $item_id, $item, null ) ?>
		<?php include( 'html-order-item-meta.php' ); ?>
		<?php do_action( 'banda_after_order_itemmeta', $item_id, $item, null ) ?>
	</td>

	<?php do_action( 'banda_admin_order_item_values', null, $item, absint( $item_id ) ); ?>
