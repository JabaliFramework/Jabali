...
<tr class="refund <?php echo ( ! empty( $class ) ) ? $class : ''; ?>" data-order_refund_id="<?php echo $refund->id; ?>">
	...
	<td class="name">
		<?php
			echo esc_attr__( 'Refund', 'banda' ) . ' #' . absint( $refund->id ) . ' - ' . esc_attr( date_i18n( get_option( 'date_format' ) . ', ' . get_option( 'time_format' ), strtotime( $refund->post->post_date ) ) );

			if ( $who_refunded->exists() ) {
				echo ' ' . esc_attr_x( 'by', 'Ex: Refund - $date >by< $username', 'banda' ) . ' ' . '<abbr class="refund_by" title="' . esc_attr__( 'ID: ', 'banda' ) . absint( $who_refunded->ID ) . '">' . esc_attr( $who_refunded->display_name ) . '</abbr>' ;
			}
		?>
	...
	<?php do_action( 'banda_admin_order_item_values', null, $refund, absint( $refund->id ) ); ?>
