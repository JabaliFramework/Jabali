...
class WC_Meta_Box_Order_Downloads {
	...
	public static function output( $post ) {
		...
		<div class="order_download_permissions wc-metaboxes-wrapper">
			...
			<div class="wc-metaboxes">
				<?php
					$download_permissions = $wpdb->get_results( $wpdb->prepare( "
						SELECT * FROM {$wpdb->prefix}banda_downloadable_product_permissions
			...
			<div class="toolbar">
				<p class="buttons">
					<input type="hidden" id="grant_access_id" name="grant_access_id" data-multiple="true" class="wc-product-search" style="width: 400px;" data-placeholder="<?php esc_attr_e( 'Search for a downloadable product&hellip;', 'banda' ); ?>" data-action="banda_json_search_downloadable_products_and_variations" />
					<button type="button" class="button grant_access"><?php _e( 'Grant Access', 'banda' ); ?></button>
				</p>
				<div class="clear"></div>
			</div>
