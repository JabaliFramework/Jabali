...
class WC_Admin_Report {
	...
	public function get_order_report_data( $args = array() ) {
		...
		$args = apply_filters( 'banda_reports_get_order_report_data_args', $args );
		...
		$order_status = apply_filters( 'banda_reports_order_statuses', $order_status );
		...
		foreach ( ( $data + $where ) as $key => $value ) {
			...
			switch ( $type ) {
				...
				case 'parent_meta' :
					...
					break;
				case 'order_item_meta' :
					$joins["order_items"] = "{$join_type} JOIN {$wpdb->prefix}banda_order_items AS order_items ON (posts.ID = order_items.order_id)";

					if ( ! empty( $value['order_item_type'] ) ) {
						$joins["order_items"] .= " AND (order_items.order_item_type = '{$value['order_item_type']}')";
					}

					$joins["order_item_meta_{$key}"]  = "{$join_type} JOIN {$wpdb->prefix}banda_order_itemmeta AS order_item_meta_{$key} ON " .
		...
		if ( ! empty( $where_meta ) ) {
			foreach ( $where_meta as $value ) {
				...
				if ( 'order_item_meta' === $type ) {
					...
					$joins["order_items"] = "{$join_type} JOIN {$wpdb->prefix}banda_order_items AS order_items ON posts.ID = order_items.order_id";
					$joins["order_item_meta_{$key}"] = "{$join_type} JOIN {$wpdb->prefix}banda_order_itemmeta AS order_item_meta_{$key} ON order_items.order_item_id = order_item_meta_{$key}.order_item_id";
		...
		$query          = apply_filters( 'banda_reports_get_order_report_query', $query );
		$query          = implode( ' ', $query );
		...
		if ( $debug || $nocache || false === $cached_results || ! isset( $cached_results[ $query_hash ] ) ) {
			...
			$cached_results[ $query_hash ] = apply_filters( 'banda_reports_get_order_report_data', $wpdb->$query_type( $query ), $data );
	...
	public function sales_sparkline( $id = '', $days = 7, $type = 'sales' ) {
		...
		if ( $type == 'sales' ) {
			$tooltip = sprintf( __( 'Sold %s worth in the last %d days', 'banda' ), strip_tags( wc_price( $total ) ), $days );
		} else {
			$tooltip = sprintf( _n( 'Sold 1 item in the last %d days', 'Sold %d items in the last %d days', $total, 'banda' ), $total, $days );
		}
	...
	/**
	 * Return currency tooltip JS based on Banda currency position settings.
	 *
	 * @return string
	 */
	public function get_currency_tooltip() {
		switch( get_option( 'banda_currency_pos' ) ) {
			case 'right':
				$currency_tooltip = 'append_tooltip: "' . get_banda_currency_symbol() . '"'; break;
			case 'right_space':
				$currency_tooltip = 'append_tooltip: "&nbsp;' . get_banda_currency_symbol() . '"'; break;
			case 'left':
				$currency_tooltip = 'prepend_tooltip: "' . get_banda_currency_symbol() . '"'; break;
			case 'left_space':
			default:
				$currency_tooltip = 'prepend_tooltip: "' . get_banda_currency_symbol() . '&nbsp;"'; break;
		}
