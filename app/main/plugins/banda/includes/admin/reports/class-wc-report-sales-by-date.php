...
class WC_Report_Sales_By_Date extends WC_Admin_Report {
	...
	private function query_report_data() {
		...
		$this->report_data = apply_filters( 'banda_admin_report_data', $this->report_data );
	}
	...
	public function get_chart_legend() {
		...
		switch ( $this->chart_groupby ) {
			case 'day' :
				$average_total_sales_title = sprintf( __( '%s average gross daily sales', 'banda' ), '<strong>' . wc_price( $data->average_total_sales ) . '</strong>' );
				$average_sales_title       = sprintf( __( '%s average net daily sales', 'banda' ), '<strong>' . wc_price( $data->average_sales ) . '</strong>' );
			break;
			case 'month' :
			default :
				$average_total_sales_title = sprintf( __( '%s average gross monthly sales', 'banda' ), '<strong>' . wc_price( $data->average_total_sales ) . '</strong>' );
				$average_sales_title       = sprintf( __( '%s average net monthly sales', 'banda' ), '<strong>' . wc_price( $data->average_sales ) . '</strong>' );
			break;
		}

		$legend[] = array(
			'title'            => sprintf( __( '%s gross sales in this period', 'banda' ), '<strong>' . wc_price( $data->total_sales ) . '</strong>' ),
			'placeholder'      => __( 'This is the sum of the order totals after any refunds and including shipping and taxes.', 'banda' ),
		...
		$legend[] = array(
			'title'            => sprintf( __( '%s net sales in this period', 'banda' ), '<strong>' . wc_price( $data->net_sales ) . '</strong>' ),
			'placeholder'      => __( 'This is the sum of the order totals after any refunds and excluding shipping and taxes.', 'banda' ),
		...
		$legend[] = array(
			'title' => sprintf( __( '%s orders placed', 'banda' ), '<strong>' . $data->total_orders . '</strong>' ),
			'color' => $this->chart_colours['order_count'],
			'highlight_series' => 1
		);

		$legend[] = array(
			'title' => sprintf( __( '%s items purchased', 'banda' ), '<strong>' . $data->total_items . '</strong>' ),
			'color' => $this->chart_colours['item_count'],
			'highlight_series' => 0
		);
		$legend[] = array(
			'title' => sprintf( _nx( '%s refunded %d order', '%s refunded %d orders', $this->report_data->total_refunded_orders, '%s = amount of the refunds, %d = number of refunded orders.', 'banda' ), '<strong>' . wc_price( $data->total_refunds ) . '</strong>', $this->report_data->total_refunded_orders ) . ' (' . sprintf( _n( '%d item', '%d items', $this->report_data->refunded_order_items, 'banda' ), $this->report_data->refunded_order_items ) . ')',
			'color' => $this->chart_colours['refund_amount'],
			'highlight_series' => 8
		);
		$legend[] = array(
			'title' => sprintf( __( '%s charged for shipping', 'banda' ), '<strong>' . wc_price( $data->total_shipping ) . '</strong>' ),
			'color' => $this->chart_colours['shipping_amount'],
			'highlight_series' => 5
		);
		$legend[] = array(
			'title' => sprintf( __( '%s worth of coupons used', 'banda' ), '<strong>' . wc_price( $data->total_coupons ) . '</strong>' ),
	...
	public function output_report() {
		$ranges = array(
			'year'         => __( 'Year', 'banda' ),
			'last_month'   => __( 'Last Month', 'banda' ),
			'month'        => __( 'This Month', 'banda' ),
			'7day'         => __( 'Last 7 Days', 'banda' )
		);
	...
	public function get_export_button() {
		...
		<a
			...
			class="export_csv"
			data-export="chart"
			data-xaxes="<?php esc_attr_e( 'Date', 'banda' ); ?>"
			data-exclude_series="2"
			data-groupby="<?php echo $this->chart_groupby; ?>"
		>
			<?php _e( 'Export CSV', 'banda' ); ?>
		</a>
		<?php
	}
	...
	public function get_main_chart() {
		...
		// 3rd party filtering of report data
		$data = apply_filters( 'banda_admin_report_chart_data', $data );
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				...
				var drawGraph = function( highlight ) {
					var series = [
						{
							label: "<?php echo esc_js( __( 'Number of items sold', 'banda' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Number of orders', 'banda' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Average gross sales amount', 'banda' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Average net sales amount', 'banda' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Coupon amount', 'banda' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Shipping amount', 'banda' ) ) ?>",
							...
							lines: { show: true, lineWidth: 2, fill: false },
							shadowSize: 0,
							prepend_tooltip: "<?php echo get_banda_currency_symbol(); ?>"
						},
						{
							label: "<?php echo esc_js( __( 'Gross Sales amount', 'banda' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Net Sales amount', 'banda' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Refund amount', 'banda' ) ) ?>",
