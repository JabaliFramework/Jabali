...
class WC_Report_Sales_By_Category extends WC_Admin_Report {
	...
	public function get_products_in_category( $category_id ) {
		...
		return array_unique( apply_filters( 'banda_report_sales_by_category_get_products_in_category', $product_ids, $category_id ) );
	}
	...
	public function get_chart_legend() {
		...
		foreach ( $this->show_categories as $category ) {
			...
			$legend[] = array(
				'title'            => sprintf( __( '%s sales in %s', 'banda' ), '<strong>' . wc_price( $total ) . '</strong>', $category->name ),
	...
	public function category_widget() {
		...
		<form method="GET">
			<div>
				<select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select categories&hellip;', 'banda' ); ?>" class="wc-enhanced-select" id="show_categories" name="show_categories[]" style="width: 205px;">
					<?php
						$r = array();
						$r['pad_counts'] 	= 1;
						$r['hierarchical'] 	= 1;
				...
				<a href="#" class="select_none"><?php _e( 'None', 'banda' ); ?></a>
				<a href="#" class="select_all"><?php _e( 'All', 'banda' ); ?></a>
	...
	public function get_main_chart() {
		...
		if ( empty( $this->show_categories ) ) {
			...
			<div class="chart-container">
				<p class="chart-prompt"><?php _e( '&larr; Choose a category to view stats', 'banda' ); ?></p>
			</div>
			<?php
		} else {
			$chart_data = array();
