...
class WC_Report_Coupon_Usage extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			'title' => sprintf( __( '%s discounts in total', 'banda' ), '<strong>' . wc_price( $total_discount ) . '</strong>' ),
			'color' => $this->chart_colours['discount_amount'],
			'highlight_series' => 1
		);

		$legend[] = array(
			'title' => sprintf( __( '%s coupons used in total', 'banda' ), '<strong>' . $total_coupons . '</strong>' ),
	...
	public function coupons_widget() {
		?>
		<h4 class="section_title"><span><?php _e( 'Filter by coupon', 'banda' ); ?></span></h4>
		<div class="section">
			<form method="GET">
				<div>
					<?php
					...
					?>
						<select id="coupon_codes" name="coupon_codes" class="wc-enhanced-select" data-placeholder="<?php esc_attr_e( 'Choose coupons&hellip;', 'banda' ); ?>" style="width:100%;">
							<option value=""><?php _e( 'All coupons', 'banda' ); ?></option>
							<?php
					...
					<?php else : ?>
						<span><?php _e( 'No used coupons found', 'banda' ); ?></span>
					<?php endif; ?>
				</div>
			</form>
		</div>
		<h4 class="section_title"><span><?php _e( 'Most Popular', 'banda' ); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
				...
				if ( ! empty( $most_popular ) && is_array( $most_popular ) ) {
					...
					echo '<tr><td colspan="2">' . __( 'No coupons found in range', 'banda' ) . '</td></tr>';
				}
				?>
			</table>
		</div>
		<h4 class="section_title"><span><?php _e( 'Most Discount', 'banda' ); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
				...
				if ( ! empty( $most_discount ) && is_array( $most_discount ) ) {
					...
					echo '<tr><td colspan="3">' . __( 'No coupons found in range', 'banda' ) . '</td></tr>';
				}
				?>
			</table>
		</div>
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				...
				var drawGraph = function( highlight ) {
					var series = [
						{
							label: "<?php echo esc_js( __( 'Number of coupons used', 'banda' ) ) ?>",
						...
						{
							label: "<?php echo esc_js( __( 'Discount amount', 'banda' ) ) ?>",
