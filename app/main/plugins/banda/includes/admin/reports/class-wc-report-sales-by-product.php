...
class WC_Report_Sales_By_Product extends WC_Admin_Report {
	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			'title' => sprintf( __( '%s sales for the selected items', 'banda' ), '<strong>' . wc_price( $total_sales ) . '</strong>' ),
			'color' => $this->chart_colours['sales_amount'],
			'highlight_series' => 1
		);

		$legend[] = array(
			'title' => sprintf( __( '%s purchases for the selected items', 'banda' ), '<strong>' . ( $total_items ) . '</strong>' ),
	...
	public function get_chart_widgets() {
		...
		if ( ! empty( $this->product_ids ) ) {
			$widgets[] = array(
				'title'    => __( 'Showing reports for:', 'banda' ),
	...
	public function current_filters() {
		...
		echo '<p><a class="button" href="' . esc_url( remove_query_arg( 'product_ids' ) ) . '">' . __( 'Reset', 'banda' ) . '</a></p>';
	}
	...
	public function products_widget() {
		?>
		<h4 class="section_title"><span><?php _e( 'Product Search', 'banda' ); ?></span></h4>
		<div class="section">
			<form method="GET">
				<div>
					<input type="hidden" class="wc-product-search" style="width:203px;" name="product_ids[]" data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'banda' ); ?>" data-action="banda_json_search_products_and_variations" />
					<input type="submit" class="submit button" value="<?php esc_attr_e( 'Show', 'banda' ); ?>" />
		...
		<h4 class="section_title"><span><?php _e( 'Top Sellers', 'banda' ); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
				...
				if ( $top_sellers ) {
					...
					echo '<tr><td colspan="3">' . __( 'No products found in range', 'banda' ) . '</td></tr>';
				}
				?>
			</table>
		</div>
		<h4 class="section_title"><span><?php _e( 'Top Freebies', 'banda' ); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
		...
		<h4 class="section_title"><span><?php _e( 'Top Earners', 'banda' ); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
	...
	public function get_main_chart() {
		...
		if ( empty( $this->product_ids ) ) {
			...
			<div class="chart-container">
				<p class="chart-prompt"><?php _e( '&larr; Choose a product to view stats', 'banda' ); ?></p>
			</div>
			<?php
		} else {
				...
				jQuery(function(){
					...
					var drawGraph = function( highlight ) {
						...
						var series = [
							...
							{
								label: "<?php echo esc_js( __( 'Sales amount', 'banda' ) ) ?>",
