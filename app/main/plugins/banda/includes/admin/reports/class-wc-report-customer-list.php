...
class WC_Report_Customer_List extends WP_List_Table {
	...
	public function __construct() {
		...
		parent::__construct( array(
			'singular'  => __( 'Customer', 'banda' ),
			'plural'    => __( 'Customers', 'banda' ),
			'ajax'      => false
		) );
	}
	...
	public function output_report() {
		...
		if ( ! empty( $_GET['link_orders'] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], 'link_orders' ) ) {
			...
			echo '<div class="updated"><p>' . sprintf( _n( '%s previous order linked', '%s previous orders linked', $linked, 'banda' ), $linked ) . '</p></div>';
		}
		...
		if ( ! empty( $_GET['refresh'] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], 'refresh' ) ) {
			...
			echo '<div class="updated"><p>' . sprintf( __( 'Refreshed stats for %s', 'banda' ), $user->display_name ) . '</p></div>';
		}

		echo '<form method="post" id="banda_customers">';

		$this->search_box( __( 'Search customers', 'banda' ), 'customer_search' );
		$this->display();
	...
	public function column_default( $user, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'last_order' :
				...
				if ( ! empty( $orders ) ) {
					$order = $orders[0];
					return '<a href="' . admin_url( 'post.php?post=' . $order->id . '&action=edit' ) . '">' . _x( '#', 'hash before order number', 'banda' ) . $order->get_order_number() . '</a> &ndash; ' . date_i18n( get_option( 'date_format' ), strtotime( $order->order_date ) );
				} else {
					return '-';
				}
			...
			case 'user_actions' :
				...
				?><p>
					<?php
						do_action( 'banda_admin_user_actions_start', $user );

						$actions = array();

						$actions['refresh'] = array(
							'url'       => wp_nonce_url( add_query_arg( 'refresh', $user->ID ), 'refresh' ),
							'name'      => __( 'Refresh stats', 'banda' ),
							'action'    => "refresh"
						);
						...
						if ( $orders ) {
							$actions['link'] = array(
								...
								'name'      => __( 'Link previous orders', 'banda' ),
								'action'    => "link"
							);
						}

						$actions = apply_filters( 'banda_admin_user_actions', $actions, $user );

						foreach ( $actions as $action ) {
							printf( '<a class="button tips %s" href="%s" data-tip="%s">%s</a>', esc_attr( $action['action'] ), esc_url( $action['url'] ), esc_attr( $action['name'] ), esc_attr( $action['name'] ) );
						}

						do_action( 'banda_admin_user_actions_end', $user );
					?>
				</p><?php
	...
	public function get_columns() {
		$columns = array(
			'customer_name'   => __( 'Name (Last, First)', 'banda' ),
			'username'        => __( 'Username', 'banda' ),
			'email'           => __( 'Email', 'banda' ),
			'location'        => __( 'Location', 'banda' ),
			'orders'          => __( 'Orders', 'banda' ),
			'spent'           => __( 'Money Spent', 'banda' ),
			'last_order'      => __( 'Last order', 'banda' ),
			'user_actions'    => __( 'Actions', 'banda' )
		);
