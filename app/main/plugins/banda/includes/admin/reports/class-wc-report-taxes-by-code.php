...
class WC_Report_Taxes_By_Code extends WC_Admin_Report {
	...
	public function get_main_chart() {
		...
		<table class="widefat">
			<thead>
				<tr>
					<th><?php _e( 'Tax', 'banda' ); ?></th>
					<th><?php _e( 'Rate', 'banda' ); ?></th>
					<th class="total_row"><?php _e( 'Number of Orders', 'banda' ); ?></th>
					<th class="total_row"><?php _e( 'Tax Amount', 'banda' ); ?> <?php echo wc_help_tip( __( 'This is the sum of the "Tax Rows" tax amount within your orders.', 'banda' ) ); ?></th>
					<th class="total_row"><?php _e( 'Shipping Tax Amount', 'banda' ); ?> <?php echo wc_help_tip( __( 'This is the sum of the "Tax Rows" shipping tax amount within your orders.', 'banda' ) ); ?></th>
					<th class="total_row"><?php _e( 'Total Tax', 'banda' ); ?> <?php echo wc_help_tip( __( 'This is the total tax for the rate (shipping tax + product tax).', 'banda' ) ); ?></th>
				</tr>
			</thead>
			<?php if ( ! empty( $tax_rows ) ) : ?>
				<tbody>
					...
					foreach ( $tax_rows as $rate_id => $tax_row ) {
						...
						?>
						<tr>
							<th scope="row"><?php echo apply_filters( 'banda_reports_taxes_tax_rate', $tax_row->tax_rate, $rate_id, $tax_row ); ?></th>
							<td><?php echo apply_filters( 'banda_reports_taxes_rate', $rate, $rate_id, $tax_row ); ?>%</td>
				...
				<tfoot>
					<tr>
						<th scope="row" colspan="3"><?php _e( 'Total', 'banda' ); ?></th>
			...
			<?php else : ?>
				<tbody>
					<tr>
						<td><?php _e( 'No taxes found in this period', 'banda' ); ?></td>
					</tr>
				</tbody>
			<?php endif; ?>
		</table>
