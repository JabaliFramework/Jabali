...
class WC_Settings_Tax extends WC_Settings_Page {
	...
	public function get_sections() {
		$sections = array(
			''         => __( 'Tax Options', 'banda' ),
			'standard' => __( 'Standard Rates', 'banda' )
		);
		...
		foreach ( $tax_classes as $class ) {
			$sections[ sanitize_title( $class ) ] = sprintf( __( '%s Rates', 'banda' ), $class );
		}
	...
	public function get_settings() {
		...
		return apply_filters( 'banda_get_settings_' . $this->id, include( 'views/settings-tax.php' ) );
	}
	...
	public function save_tax_rates() {
		...
		$tax_rate_order = absint( $wpdb->get_var( $wpdb->prepare( "SELECT tax_rate_order FROM {$wpdb->prefix}banda_tax_rates WHERE tax_rate_id = %s", $first_tax_rate_id ) ) );
