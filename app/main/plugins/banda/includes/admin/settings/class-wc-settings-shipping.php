...
class WC_Settings_Shipping extends WC_Settings_Page {
	...
	public function get_sections() {
		$sections = array(
			''        => __( 'Shipping Zones', 'banda' ),
			'options' => __( 'Shipping Options', 'banda' ),
			'classes' => __( 'Shipping Classes', 'banda' )
		);
	...
	public function get_settings() {
		$settings = apply_filters( 'banda_shipping_settings', array(

			array( 'title' => __( 'Shipping Options', 'banda' ), 'type' => 'title', 'id' => 'shipping_options' ),

			array(
				'title'         => __( 'Calculations', 'banda' ),
				'desc'          => __( 'Enable the shipping calculator on the cart page', 'banda' ),
				'id'            => 'banda_enable_shipping_calc',
				'default'       => 'yes',
				'type'          => 'checkbox',
				'checkboxgroup' => 'start',
				'autoload'      => false
			...
			array(
				'desc'          => __( 'Hide shipping costs until an address is entered', 'banda' ),
				'id'            => 'banda_shipping_cost_requires_address',
				'default'       => 'no',
				'type'          => 'checkbox',
				'checkboxgroup' => 'end',
				'autoload'      => false
			...
			array(
				'title'   => __( 'Shipping Destination', 'banda' ),
				'desc'    => __( 'This controls which shipping address is used by default.', 'banda' ),
				'id'      => 'banda_ship_to_destination',
				'default' => 'billing',
				'type'    => 'radio',
				'options' => array(
					'shipping'     => __( 'Default to customer shipping address', 'banda' ),
					'billing'      => __( 'Default to customer billing address', 'banda' ),
					'billing_only' => __( 'Force shipping to the customer billing address', 'banda' ),
				),
				'autoload'        => false,
				'desc_tip'        =>  true,
			...
			default :
				...
				foreach ( $wc_shipping->get_shipping_methods() as $method_id => $method ) {
					if ( in_array( $current_section, array( $method->id, sanitize_title( get_class( $method ) ) ) ) ) {
						do_action( 'banda_update_options_' . $this->id . '_' . $method->id );
					}
				}
			break;
		}
	...
	protected function zone_methods_screen( $zone_id ) {
		...
		if ( ! $zone ) {
			wp_die( __( 'Zone does not exist!', 'banda' ) );
		}
		...
		wp_localize_script( 'wc-shipping-zone-methods', 'shippingZoneMethodsLocalizeScript', array(
			...
			'strings'                 => array(
				...
				'save_changes_prompt'     => __( 'Do you wish to save your changes first? Your changed data will be discarded if you choose to cancel.', 'banda' ),
				'save_failed'             => __( 'Your changes were not saved. Please retry.', 'banda' ),
				'add_method_failed'       => __( 'Shipping method could not be added. Please retry.', 'banda' ),
				'yes'                     => __( 'Yes', 'banda' ),
				'no'                      => __( 'No', 'banda' ),
			),
		) );
	...
	protected function instance_settings_screen( $instance_id ) {
		...
		if ( ! $shipping_method ) {
			wp_die( __( 'Invalid shipping method!', 'banda' ) );
		}
		if ( ! $zone ) {
			wp_die( __( 'Zone does not exist!', 'banda' ) );
		}
		if ( ! $shipping_method->has_settings() ) {
			wp_die( __( 'This shipping method does not have any settings to configure.', 'banda' ) );
		}

		if ( ! empty( $_POST['save'] ) ) {

			if ( empty( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( $_REQUEST['_wpnonce'], 'banda-settings' ) ) {
				echo '<div class="updated error"><p>' . __( 'Edit failed. Please try again.', 'banda' ) . '</p></div>';
			}
	...
	protected function output_shipping_class_screen() {
		...
		$shipping_class_columns = apply_filters( 'banda_shipping_classes_columns', array(
			'wc-shipping-class-name'        => __( 'Shipping Class', 'banda' ),
			'wc-shipping-class-slug'        => __( 'Slug', 'banda' ),
			'wc-shipping-class-description' => __( 'Description', 'banda' ),
			'wc-shipping-class-count'       => __( 'Product Count', 'banda' ),
		) );
