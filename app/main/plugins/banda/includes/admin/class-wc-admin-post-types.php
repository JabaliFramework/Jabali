...
class WC_Admin_Post_Types {
	...
	public function __construct() {
		...
		add_action( 'banda_process_product_file_download_paths', array( $this, 'process_product_file_download_paths' ), 10, 3 );
	...
	public function post_updated_messages( $messages ) {
		...
		$messages['product'] = array(
			...
			1 => sprintf( __( 'Product updated. <a href="%s">View Product</a>', 'banda' ), esc_url( get_permalink( $post_ID ) ) ),
			2 => __( 'Custom field updated.', 'banda' ),
			3 => __( 'Custom field deleted.', 'banda' ),
			4 => __( 'Product updated.', 'banda' ),
			5 => isset( $_GET['revision'] ) ? sprintf( __( 'Product restored to revision from %s', 'banda' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6 => sprintf( __( 'Product published. <a href="%s">View Product</a>', 'banda' ), esc_url( get_permalink( $post_ID ) ) ),
			7 => __( 'Product saved.', 'banda' ),
			8 => sprintf( __( 'Product submitted. <a target="_blank" href="%s">Preview Product</a>', 'banda' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) ),
			9 => sprintf( __( 'Product scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview Product</a>', 'banda' ),
			  date_i18n( __( 'M j, Y @ G:i', 'banda' ), strtotime( $post->post_date ) ), esc_url( get_permalink( $post_ID ) ) ),
			10 => sprintf( __( 'Product draft updated. <a target="_blank" href="%s">Preview Product</a>', 'banda' ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post_ID ) ) ) ),
		);

		$messages['shop_order'] = array(
			0 => '', // Unused. Messages start at index 1.
			1 => __( 'Order updated.', 'banda' ),
			2 => __( 'Custom field updated.', 'banda' ),
			3 => __( 'Custom field deleted.', 'banda' ),
			4 => __( 'Order updated.', 'banda' ),
			5 => isset( $_GET['revision'] ) ? sprintf( __( 'Order restored to revision from %s', 'banda' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6 => __( 'Order updated.', 'banda' ),
			7 => __( 'Order saved.', 'banda' ),
			8 => __( 'Order submitted.', 'banda' ),
			9 => sprintf( __( 'Order scheduled for: <strong>%1$s</strong>.', 'banda' ),
			  date_i18n( __( 'M j, Y @ G:i', 'banda' ), strtotime( $post->post_date ) ) ),
			10 => __( 'Order draft updated.', 'banda' ),
			11 => __( 'Order updated and email sent.', 'banda' )
		);

		$messages['shop_coupon'] = array(
			0 => '', // Unused. Messages start at index 1.
			1 => __( 'Coupon updated.', 'banda' ),
			2 => __( 'Custom field updated.', 'banda' ),
			3 => __( 'Custom field deleted.', 'banda' ),
			4 => __( 'Coupon updated.', 'banda' ),
			5 => isset( $_GET['revision'] ) ? sprintf( __( 'Coupon restored to revision from %s', 'banda' ), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
			6 => __( 'Coupon updated.', 'banda' ),
			7 => __( 'Coupon saved.', 'banda' ),
			8 => __( 'Coupon submitted.', 'banda' ),
			9 => sprintf( __( 'Coupon scheduled for: <strong>%1$s</strong>.', 'banda' ),
			  date_i18n( __( 'M j, Y @ G:i', 'banda' ), strtotime( $post->post_date ) ) ),
			10 => __( 'Coupon draft updated.', 'banda' )
		);
	...
	public function bulk_post_updated_messages( $bulk_messages, $bulk_counts ) {
		...
		$bulk_messages['product'] = array(
			'updated'   => _n( '%s product updated.', '%s products updated.', $bulk_counts['updated'], 'banda' ),
			'locked'    => _n( '%s product not updated, somebody is editing it.', '%s products not updated, somebody is editing them.', $bulk_counts['locked'], 'banda' ),
			'deleted'   => _n( '%s product permanently deleted.', '%s products permanently deleted.', $bulk_counts['deleted'], 'banda' ),
			'trashed'   => _n( '%s product moved to the Trash.', '%s products moved to the Trash.', $bulk_counts['trashed'], 'banda' ),
			'untrashed' => _n( '%s product restored from the Trash.', '%s products restored from the Trash.', $bulk_counts['untrashed'], 'banda' ),
		);

		$bulk_messages['shop_order'] = array(
			'updated'   => _n( '%s order updated.', '%s orders updated.', $bulk_counts['updated'], 'banda' ),
			'locked'    => _n( '%s order not updated, somebody is editing it.', '%s orders not updated, somebody is editing them.', $bulk_counts['locked'], 'banda' ),
			'deleted'   => _n( '%s order permanently deleted.', '%s orders permanently deleted.', $bulk_counts['deleted'], 'banda' ),
			'trashed'   => _n( '%s order moved to the Trash.', '%s orders moved to the Trash.', $bulk_counts['trashed'], 'banda' ),
			'untrashed' => _n( '%s order restored from the Trash.', '%s orders restored from the Trash.', $bulk_counts['untrashed'], 'banda' ),
		);

		$bulk_messages['shop_coupon'] = array(
			'updated'   => _n( '%s coupon updated.', '%s coupons updated.', $bulk_counts['updated'], 'banda' ),
			'locked'    => _n( '%s coupon not updated, somebody is editing it.', '%s coupons not updated, somebody is editing them.', $bulk_counts['locked'], 'banda' ),
			'deleted'   => _n( '%s coupon permanently deleted.', '%s coupons permanently deleted.', $bulk_counts['deleted'], 'banda' ),
			'trashed'   => _n( '%s coupon moved to the Trash.', '%s coupons moved to the Trash.', $bulk_counts['trashed'], 'banda' ),
			'untrashed' => _n( '%s coupon restored from the Trash.', '%s coupons restored from the Trash.', $bulk_counts['untrashed'], 'banda' ),
		);
	...
	public function product_columns( $existing_columns ) {
		...
		$columns['thumb'] = '<span class="wc-image tips" data-tip="' . esc_attr__( 'Image', 'banda' ) . '">' . __( 'Image', 'banda' ) . '</span>';
		$columns['name']  = __( 'Name', 'banda' );

		if ( wc_product_sku_enabled() ) {
			$columns['sku'] = __( 'SKU', 'banda' );
		}

		if ( 'yes' === get_option( 'banda_manage_stock' ) ) {
			$columns['is_in_stock'] = __( 'Stock', 'banda' );
		}

		$columns['price']        = __( 'Price', 'banda' );
		$columns['product_cat']  = __( 'Categories', 'banda' );
		$columns['product_tag']  = __( 'Tags', 'banda' );
		$columns['featured']     = '<span class="wc-featured parent-tips" data-tip="' . esc_attr__( 'Featured', 'banda' ) . '">' . __( 'Featured', 'banda' ) . '</span>';
		$columns['product_type'] = '<span class="wc-type parent-tips" data-tip="' . esc_attr__( 'Type', 'banda' ) . '">' . __( 'Type', 'banda' ) . '</span>';
		$columns['date']         = __( 'Date', 'banda' );
	...
	public function shop_coupon_columns( $existing_columns ) {
		...
		$columns['coupon_code'] = __( 'Code', 'banda' );
		$columns['type']        = __( 'Coupon type', 'banda' );
		$columns['amount']      = __( 'Coupon amount', 'banda' );
		$columns['description'] = __( 'Description', 'banda' );
		$columns['products']    = __( 'Product IDs', 'banda' );
		$columns['usage']       = __( 'Usage / Limit', 'banda' );
		$columns['expiry_date'] = __( 'Expiry date', 'banda' );
	...
	public function shop_order_columns( $existing_columns ) {
		$columns                     = array();
		$columns['cb']               = $existing_columns['cb'];
		$columns['order_status']     = '<span class="status_head tips" data-tip="' . esc_attr__( 'Status', 'banda' ) . '">' . esc_attr__( 'Status', 'banda' ) . '</span>';
		$columns['order_title']      = __( 'Order', 'banda' );
		$columns['order_items']      = __( 'Purchased', 'banda' );
		$columns['billing_address']  = __( 'Billing', 'banda' );
		$columns['shipping_address'] = __( 'Deliver to', 'banda' );
		$columns['customer_message'] = '<span class="notes_head tips" data-tip="' . esc_attr__( 'Customer Message', 'banda' ) . '">' . esc_attr__( 'Customer Message', 'banda' ) . '</span>';
		$columns['order_notes']      = '<span class="order-notes_head tips" data-tip="' . esc_attr__( 'Order Notes', 'banda' ) . '">' . esc_attr__( 'Order Notes', 'banda' ) . '</span>';
		$columns['order_date']       = __( 'Date', 'banda' );
		$columns['order_total']      = __( 'Total', 'banda' );
		$columns['order_actions']    = __( 'Actions', 'banda' );
	...
	public function render_product_columns( $column ) {
		...
		switch ( $column ) {
			...
			case 'name' :
				...
				/* Custom inline data for banda. */
				echo '
					<div class="hidden" id="banda_inline_' . $post->ID . '">
			...
			case 'product_type' :
				if ( 'grouped' == $the_product->product_type ) {
					echo '<span class="product-type tips grouped" data-tip="' . esc_attr__( 'Grouped', 'banda' ) . '"></span>';
				} elseif ( 'external' == $the_product->product_type ) {
					echo '<span class="product-type tips external" data-tip="' . esc_attr__( 'External/Affiliate', 'banda' ) . '"></span>';
				} elseif ( 'simple' == $the_product->product_type ) {

					if ( $the_product->is_virtual() ) {
						echo '<span class="product-type tips virtual" data-tip="' . esc_attr__( 'Virtual', 'banda' ) . '"></span>';
					} elseif ( $the_product->is_downloadable() ) {
						echo '<span class="product-type tips downloadable" data-tip="' . esc_attr__( 'Downloadable', 'banda' ) . '"></span>';
					} else {
						echo '<span class="product-type tips simple" data-tip="' . esc_attr__( 'Simple', 'banda' ) . '"></span>';
					}

				} elseif ( 'variable' == $the_product->product_type ) {
					echo '<span class="product-type tips variable" data-tip="' . esc_attr__( 'Variable', 'banda' ) . '"></span>';
				} else {
			...
			case 'product_tag' :
				...
				break;
			case 'featured' :
				$url = wp_nonce_url( admin_url( 'admin-ajax.php?action=banda_feature_product&product_id=' . $post->ID ), 'banda-feature-product' );
				echo '<a href="' . esc_url( $url ) . '" title="'. __( 'Toggle featured', 'banda' ) . '">';
				if ( $the_product->is_featured() ) {
					echo '<span class="wc-featured tips" data-tip="' . esc_attr__( 'Yes', 'banda' ) . '">' . __( 'Yes', 'banda' ) . '</span>';
				} else {
					echo '<span class="wc-featured not-featured tips" data-tip="' . esc_attr__( 'No', 'banda' ) . '">' . __( 'No', 'banda' ) . '</span>';
				}
				echo '</a>';
				break;
			case 'is_in_stock' :
				...
				echo apply_filters( 'banda_admin_stock_html', $stock_html, $the_product );
	...
	/**
	 * Render product row actions for old version of Jabali.
	 * Since Jabali 4.3 we don't have to build the row actions.
	 *
	 * @param WP_Post $post
	 * @param string  $title
	 */
	private function _render_product_row_actions( $post, $title ) {
		...
		if ( $can_edit_post && 'trash' != $post->post_status ) {
			$actions['edit'] = '<a href="' . get_edit_post_link( $post->ID, true ) . '" title="' . esc_attr( __( 'Edit this item', 'banda' ) ) . '">' . __( 'Edit', 'banda' ) . '</a>';
			$actions['inline hide-if-no-js'] = '<a href="#" class="editinline" title="' . esc_attr( __( 'Edit this item inline', 'banda' ) ) . '">' . __( 'Quick&nbsp;Edit', 'banda' ) . '</a>';
		}
		if ( current_user_can( $post_type_object->cap->delete_post, $post->ID ) ) {
			if ( 'trash' == $post->post_status ) {
				$actions['untrash'] = '<a title="' . esc_attr( __( 'Restore this item from the Trash', 'banda' ) ) . '" href="' . wp_nonce_url( admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=untrash', $post->ID ) ), 'untrash-post_' . $post->ID ) . '">' . __( 'Restore', 'banda' ) . '</a>';
			} elseif ( EMPTY_TRASH_DAYS ) {
				$actions['trash'] = '<a class="submitdelete" title="' . esc_attr( __( 'Move this item to the Trash', 'banda' ) ) . '" href="' . get_delete_post_link( $post->ID ) . '">' . __( 'Trash', 'banda' ) . '</a>';
			}

			if ( 'trash' == $post->post_status || ! EMPTY_TRASH_DAYS ) {
				$actions['delete'] = '<a class="submitdelete" title="' . esc_attr( __( 'Delete this item permanently', 'banda' ) ) . '" href="' . get_delete_post_link( $post->ID, '', true ) . '">' . __( 'Delete Permanently', 'banda' ) . '</a>';
			}
		}
		if ( $post_type_object->public ) {
			if ( in_array( $post->post_status, array( 'pending', 'draft', 'future' ) ) ) {
				if ( $can_edit_post )
					$actions['view'] = '<a href="' . esc_url( add_query_arg( 'preview', 'true', get_permalink( $post->ID ) ) ) . '" title="' . esc_attr( sprintf( __( 'Preview &#8220;%s&#8221;', 'banda' ), $title ) ) . '" rel="permalink">' . __( 'Preview', 'banda' ) . '</a>';
			} elseif ( 'trash' != $post->post_status ) {
				$actions['view'] = '<a href="' . get_permalink( $post->ID ) . '" title="' . esc_attr( sprintf( __( 'View &#8220;%s&#8221;', 'banda' ), $title ) ) . '" rel="permalink">' . __( 'View', 'banda' ) . '</a>';
			}
		}
	...
	public function render_shop_coupon_columns( $column ) {
		...
		switch ( $column ) {
			...
			case 'usage' :
				...
				printf( _x( '%1$s / %2$s', 'Count / Limit', 'banda' ), $usage_url, $usage_limit ? $usage_limit : '&infin;' );
			break;
			case 'expiry_date' :
	...
	/**
	 * Render shop_coupon row actions for old version of Jabali.
	 * Since Jabali 4.3 we don't have to build the row actions.
	 *
	 * @param WP_Post $post
	 * @param string  $title
	...
	private function _render_shop_coupon_row_actions( $post, $title ) {
		...
		if ( current_user_can( $post_type_object->cap->edit_post, $post->ID ) ) {
			$actions['edit'] = '<a href="' . admin_url( sprintf( $post_type_object->_edit_link . '&amp;action=edit', $post->ID ) ) . '">' . __( 'Edit', 'banda' ) . '</a>';
		}
		...
		switch ( $column ) {
			...
			case 'order_date' :
				...
				if ( '0000-00-00 00:00:00' == $post->post_date ) {
					$t_time = $h_time = __( 'Unpublished', 'banda' );
				} else {
					$t_time = get_the_time( __( 'Y/m/d g:i:s A', 'banda' ), $post );
					$h_time = get_the_time( __( 'Y/m/d', 'banda' ), $post );
				}
			...
			case 'customer_message' :
				if ( $the_order->customer_message ) {
					echo '<span class="note-on tips" data-tip="' . wc_sanitize_tooltip( $the_order->customer_message ) . '">' . __( 'Yes', 'banda' ) . '</span>';
				} else {
					echo '<span class="na">&ndash;</span>';
				}
			...
			case 'order_items' :
				...
				echo '<a href="#" class="show_order_items">' . apply_filters( 'banda_admin_order_item_count', sprintf( _n( '%d item', '%d items', $the_order->get_item_count(), 'banda' ), $the_order->get_item_count() ), $the_order ) . '</a>';
				...
				if ( sizeof( $the_order->get_items() ) > 0 ) {
					...
					foreach ( $the_order->get_items() as $item ) {
						$product        = apply_filters( 'banda_order_item_product', $the_order->get_product_from_item( $item ), $item );
						$item_meta      = new WC_Order_Item_Meta( $item, $product );
						$item_meta_html = $item_meta->display( true, true );
						?>
						<tr class="<?php echo apply_filters( 'banda_admin_order_item_class', '', $item, $the_order ); ?>">
							<td class="qty"><?php echo absint( $item['qty'] ); ?></td>
							<td class="name">
								<?php  if ( $product ) : ?>
									<?php echo ( wc_product_sku_enabled() && $product->get_sku() ) ? $product->get_sku() . ' - ' : ''; ?><a href="<?php echo get_edit_post_link( $product->id ); ?>" title="<?php echo apply_filters( 'banda_order_item_name', $item['name'], $item, false ); ?>"><?php echo apply_filters( 'banda_order_item_name', $item['name'], $item, false ); ?></a>
								<?php else : ?>
									<?php echo apply_filters( 'banda_order_item_name', $item['name'], $item, false ); ?>
								<?php endif; ?>
			...
			case 'billing_address' :
				...
				if ( $the_order->billing_phone ) {
					echo '<small class="meta">' . __( 'Tel:', 'banda' ) . ' ' . esc_html( $the_order->billing_phone ) . '</small>';
				}
			...
			case 'shipping_address' :
				...
				if ( $the_order->get_shipping_method() ) {
					echo '<small class="meta">' . __( 'Via', 'banda' ) . ' ' . esc_html( $the_order->get_shipping_method() ) . '</small>';
				}
			...
			case 'order_notes' :
				...
				if ( $post->comment_count ) {
					...
					if ( isset( $latest_note->comment_content ) && $post->comment_count == 1 ) {
						echo '<span class="note-on tips" data-tip="' . wc_sanitize_tooltip( $latest_note->comment_content ) . '">' . __( 'Yes', 'banda' ) . '</span>';
					} elseif ( isset( $latest_note->comment_content ) ) {
						echo '<span class="note-on tips" data-tip="' . wc_sanitize_tooltip( $latest_note->comment_content . '<br/><small style="display:block">' . sprintf( _n( 'plus %d other note', 'plus %d other notes', ( $post->comment_count - 1 ), 'banda' ), $post->comment_count - 1 ) . '</small>' ) . '">' . __( 'Yes', 'banda' ) . '</span>';
					} else {
						echo '<span class="note-on tips" data-tip="' . wc_sanitize_tooltip( sprintf( _n( '%d note', '%d notes', $post->comment_count, 'banda' ), $post->comment_count ) ) . '">' . __( 'Yes', 'banda' ) . '</span>';
					}
			...
			case 'order_total' :
				...
				if ( $the_order->payment_method_title ) {
					echo '<small class="meta">' . __( 'Via', 'banda' ) . ' ' . esc_html( $the_order->payment_method_title ) . '</small>';
				}
			break;
			case 'order_title' :
				...
				if ( ! empty( $user_info ) ) {
					...
					if ( $user_info->first_name || $user_info->last_name ) {
						$username .= esc_html( sprintf( _x( '%1$s %2$s', 'full name', 'banda' ), ucfirst( $user_info->first_name ), ucfirst( $user_info->last_name ) ) );
					} else {
					...
					if ( $the_order->billing_first_name || $the_order->billing_last_name ) {
						$username = trim( sprintf( _x( '%1$s %2$s', 'full name', 'banda' ), $the_order->billing_first_name, $the_order->billing_last_name ) );
					} else if ( $the_order->billing_company ) {
						$username = trim( $the_order->billing_company );
					} else {
						$username = __( 'Guest', 'banda' );
					}
				}

				printf( _x( '%s by %s', 'Order number by X', 'banda' ), '<a href="' . admin_url( 'post.php?post=' . absint( $post->ID ) . '&action=edit' ) . '" class="row-title"><strong>#' . esc_attr( $the_order->get_order_number() ) . '</strong></a>', $username );
				...
				echo '<button type="button" class="toggle-row"><span class="screen-reader-text">' . __( 'Show more details', 'banda' ) . '</span></button>';
			...
			case 'order_actions' :
				...
				?><p>
					<?php
						do_action( 'banda_admin_order_actions_start', $the_order );

						$actions = array();

						if ( $the_order->has_status( array( 'pending', 'on-hold' ) ) ) {
							$actions['processing'] = array(
								'url'       => wp_nonce_url( admin_url( 'admin-ajax.php?action=banda_mark_order_status&status=processing&order_id=' . $post->ID ), 'banda-mark-order-status' ),
								'name'      => __( 'Processing', 'banda' ),
								'action'    => "processing"
							);
						}
						...
						if ( $the_order->has_status( array( 'pending', 'on-hold', 'processing' ) ) ) {
							$actions['complete'] = array(
								'url'       => wp_nonce_url( admin_url( 'admin-ajax.php?action=banda_mark_order_status&status=completed&order_id=' . $post->ID ), 'banda-mark-order-status' ),
								'name'      => __( 'Complete', 'banda' ),
								'action'    => "complete"
							);
						}
						...
						$actions = apply_filters( 'banda_admin_order_actions', $actions, $the_order );

						foreach ( $actions as $action ) {
							printf( '<a class="button tips %s" href="%s" data-tip="%s">%s</a>', esc_attr( $action['action'] ), esc_url( $action['url'] ), esc_attr( $action['name'] ), esc_attr( $action['name'] ) );
						}

						do_action( 'banda_admin_order_actions_end', $the_order );
					?>
				</p><?php
	 ...
	 *
	 * Based on Simple Page Ordering by 10up (https://jabali.github.io/extend/plugins/simple-page-ordering/).
	 *
	 * @param  array $views
	 * @return array
	 */
	public function product_sorting_link( $views ) {
		...
		$views['byorder'] = '<a href="' . esc_url( $query_string ) . '" class="' . esc_attr( $class ) . '">' . __( 'Sort Products', 'banda' ) . '</a>';
	...
	public function bulk_and_quick_edit_save_post( $post_id, $post ) {
		...
		// Check nonces
		if ( ! isset( $_REQUEST['banda_quick_edit_nonce'] ) && ! isset( $_REQUEST['banda_bulk_edit_nonce'] ) ) {
			return $post_id;
		}
		if ( isset( $_REQUEST['banda_quick_edit_nonce'] ) && ! wp_verify_nonce( $_REQUEST['banda_quick_edit_nonce'], 'banda_quick_edit_nonce' ) ) {
			return $post_id;
		}
		if ( isset( $_REQUEST['banda_bulk_edit_nonce'] ) && ! wp_verify_nonce( $_REQUEST['banda_bulk_edit_nonce'], 'banda_bulk_edit_nonce' ) ) {
			return $post_id;
		}
		...
		if ( ! empty( $_REQUEST['banda_quick_edit'] ) ) {
	...
	private function quick_edit_save( $post_id, $product ) {
		...
		if ( isset( $_REQUEST['_visibility'] ) ) {
			if ( update_post_meta( $post_id, '_visibility', wc_clean( $_REQUEST['_visibility'] ) ) ) {
				do_action( 'banda_product_set_visibility', $post_id, wc_clean( $_REQUEST['_visibility'] ) );
			}
		}
		...
		do_action( 'banda_product_quick_edit_save', $product );
	}
	...
	public function bulk_edit_save( $post_id, $product ) {
		...
		// Handle price - remove dates and set to lowest
		$change_price_product_types = apply_filters( 'banda_bulk_edit_save_price_product_types', array( 'simple', 'external' ) );
		...
		do_action( 'banda_product_bulk_edit_save', $product );
	}

	/**
	 * Add extra bulk action options to mark orders as complete or processing.
	 *
	 * Using Javascript until Jabali core fixes: https://core.trac.jabali.github.io/ticket/16031.
	 */
	public function bulk_admin_footer() {
		...
		if ( 'shop_order' == $post_type ) {
			...
			jQuery(function() {
				jQuery('<option>').val('mark_processing').text('<?php _e( 'Mark processing', 'banda' )?>').appendTo('select[name="action"]');
				jQuery('<option>').val('mark_processing').text('<?php _e( 'Mark processing', 'banda' )?>').appendTo('select[name="action2"]');

				jQuery('<option>').val('mark_on-hold').text('<?php _e( 'Mark on-hold', 'banda' )?>').appendTo('select[name="action"]');
				jQuery('<option>').val('mark_on-hold').text('<?php _e( 'Mark on-hold', 'banda' )?>').appendTo('select[name="action2"]');

				jQuery('<option>').val('mark_completed').text('<?php _e( 'Mark complete', 'banda' )?>').appendTo('select[name="action"]');
				jQuery('<option>').val('mark_completed').text('<?php _e( 'Mark complete', 'banda' )?>').appendTo('select[name="action2"]');
			});
			</script>
			<?php
		}
	...
	public function bulk_action() {
		...
		foreach ( $post_ids as $post_id ) {
			...
			$order->update_status( $new_status, __( 'Order status changed by bulk edit:', 'banda' ), true );
			do_action( 'banda_order_edit_status', $post_id, $new_status );
			$changed++;
		}
	...
	public function bulk_admin_notices() {
		...
		foreach ( $order_statuses as $slug => $name ) {
			...
			if ( isset( $_REQUEST[ 'marked_' . str_replace( 'wc-', '', $slug ) ] ) ) {
				...
				$message = sprintf( _n( 'Order status changed.', '%s order statuses changed.', $number, 'banda' ), number_format_i18n( $number ) );
	...
	public function product_filters() {
		...
		$output .= '<option value="">' . __( 'Show all product types', 'banda' ) . '</option>';
		...
		foreach ( $terms as $term ) {
			...
			switch ( $term->name ) {
				case 'grouped' :
					$output .= __( 'Grouped product', 'banda' );
					break;
				case 'external' :
					$output .= __( 'External/Affiliate product', 'banda' );
					break;
				case 'variable' :
					$output .= __( 'Variable product', 'banda' );
					break;
				case 'simple' :
					$output .= __( 'Simple product', 'banda' );
					break;
				default :
					// Assuming that we have other types in future
			...
			if ( 'simple' == $term->name ) {
				...
				$output .= '> &rarr; ' . __( 'Downloadable', 'banda' ) . '</option>';
				...
				$output .= '> &rarr;  ' . __( 'Virtual', 'banda' ) . '</option>';
			}
		}

		$output .= '</select>';

		echo apply_filters( 'banda_product_filters', $output );
	}
	...
	public function shop_coupon_filters() {
		...
		<select name="coupon_type" id="dropdown_shop_coupon_type">
			<option value=""><?php _e( 'Show all types', 'banda' ); ?></option>
			<?php
				...
				foreach ( $types as $name => $type ) {
					...
					echo '>' . esc_html__( $type, 'banda' ) . '</option>';
				}
			?>
		</select>
		<?php
	...
	public function shop_order_filters() {
		...
		?>
		<input type="hidden" class="wc-customer-search" name="_customer_user" data-placeholder="<?php esc_attr_e( 'Search for a customer&hellip;', 'banda' ); ?>" data-selected="<?php echo htmlspecialchars( $user_string ); ?>" value="<?php echo $user_id; ?>" data-allow_clear="true" />
		<?php
	}
	...
	public function trash_post( $id ) {
		...
		if ( $id > 0 ) {
			...
			if ( in_array( $post_type, wc_get_order_types( 'order-count' ) ) ) {
				...
				delete_transient( 'banda_processing_order_count' );
	...
	public function delete_order_items( $postid ) {
		...
		if ( in_array( get_post_type( $postid ), wc_get_order_types() ) ) {
			do_action( 'banda_delete_order_items', $postid );

			$wpdb->query( "
				DELETE {$wpdb->prefix}banda_order_items, {$wpdb->prefix}banda_order_itemmeta
				FROM {$wpdb->prefix}banda_order_items
				JOIN {$wpdb->prefix}banda_order_itemmeta ON {$wpdb->prefix}banda_order_items.order_item_id = {$wpdb->prefix}banda_order_itemmeta.order_item_id
				WHERE {$wpdb->prefix}banda_order_items.order_id = '{$postid}';
				" );

			do_action( 'banda_deleted_order_items', $postid );
		}
	}
	...
	public function delete_order_downloadable_permissions( $postid ) {
		...
		if ( in_array( get_post_type( $postid ), wc_get_order_types() ) ) {
			do_action( 'banda_delete_order_downloadable_permissions', $postid );

			$wpdb->query( $wpdb->prepare( "
				DELETE FROM {$wpdb->prefix}banda_downloadable_product_permissions
				WHERE order_id = %d
			", $postid ) );

			do_action( 'banda_deleted_order_downloadable_permissions', $postid );
		}
	}
	...
	public function enter_title_here( $text, $post ) {
		switch ( $post->post_type ) {
			case 'product' :
				$text = __( 'Product name', 'banda' );
			break;
			case 'shop_coupon' :
				$text = __( 'Coupon code', 'banda' );
			break;
		}
	...
	public function edit_form_after_title( $post ) {
		if ( 'shop_coupon' === $post->post_type ) {
			?>
			<textarea id="banda-coupon-description" name="excerpt" cols="5" rows="2" placeholder="<?php esc_attr_e( 'Description (optional)', 'banda' ); ?>"><?php echo $post->post_excerpt; // This is already escaped in core ?></textarea>
			<?php
		}
	}
	...
	public function product_data_visibility() {
		...
		$current_visibility = ( $current_visibility = get_post_meta( $post->ID, '_visibility', true ) ) ? $current_visibility : apply_filters( 'banda_product_visibility_default' , 'visible' );
		$current_featured   = ( $current_featured = get_post_meta( $post->ID, '_featured', true ) ) ? $current_featured : 'no';

		$visibility_options = apply_filters( 'banda_product_visibility_options', array(
		...
		<div class="misc-pub-section" id="catalog-visibility">
			<?php _e( 'Catalog visibility:', 'banda' ); ?> <strong id="catalog-visibility-display"><?php
				echo isset( $visibility_options[ $current_visibility ]  ) ? esc_html( $visibility_options[ $current_visibility ] ) : esc_html( $current_visibility );

				if ( 'yes' == $current_featured ) {
					echo ', ' . __( 'Featured', 'banda' );
				}
			?></strong>

			<a href="#catalog-visibility" class="edit-catalog-visibility hide-if-no-js"><?php _e( 'Edit', 'banda' ); ?></a>
			...
			<div id="catalog-visibility-select" class="hide-if-js">
				...
				<?php
					echo '<p>' . __( 'Choose where this product should be displayed in your catalog. The product will always be accessible directly.', 'banda' ) . '</p>';

					foreach ( $visibility_options as $name => $label ) {
						echo '<input type="radio" name="_visibility" id="_visibility_' . esc_attr( $name ) . '" value="' . esc_attr( $name ) . '" ' . checked( $current_visibility, $name, false ) . ' data-label="' . esc_attr( $label ) . '" /> <label for="_visibility_' . esc_attr( $name ) . '" class="selectit">' . esc_html( $label ) . '</label><br />';
					}

					echo '<p>' . __( 'Enable this option to feature this product.', 'banda' ) . '</p>';

					echo '<input type="checkbox" name="_featured" id="_featured" ' . checked( $current_featured, 'yes', false ) . ' /> <label for="_featured">' . __( 'Featured Product', 'banda' ) . '</label><br />';
				?>
				<p>
					<a href="#catalog-visibility" class="save-post-visibility hide-if-no-js button"><?php _e( 'OK', 'banda' ); ?></a>
					<a href="#catalog-visibility" class="cancel-post-visibility hide-if-no-js"><?php _e( 'Cancel', 'banda' ); ?></a>
				</p>
			</div>
		</div>
		<?php
	...
	public function upload_dir( $pathdata ) {
		...
		if ( isset( $_POST['type'] ) && 'downloadable_product' == $_POST['type'] ) {
			...
			if ( empty( $pathdata['subdir'] ) ) {
				$pathdata['path']   = $pathdata['path'] . '/banda_uploads';
				$pathdata['url']    = $pathdata['url']. '/banda_uploads';
				$pathdata['subdir'] = '/banda_uploads';
			} else {
				$new_subdir = '/banda_uploads' . $pathdata['subdir'];
	 ...
	 */
	public function banda_media_upload_downloadable_product() {
	...
	public function process_product_file_download_paths( $product_id, $variation_id, $downloadable_files ) {
		...
		if ( ! empty( $new_download_ids ) || ! empty( $removed_download_ids ) ) {
			...
			$existing_permissions = $wpdb->get_results( $wpdb->prepare( "SELECT * from {$wpdb->prefix}banda_downloadable_product_permissions WHERE product_id = %d GROUP BY order_id", $product_id ) );
				...
				if ( ! empty( $order->id ) ) {
					...
					if ( ! empty( $removed_download_ids ) ) {
						foreach ( $removed_download_ids as $download_id ) {
							if ( apply_filters( 'banda_process_product_file_download_paths_remove_access_to_old_file', true, $download_id, $product_id, $order ) ) {
								$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}banda_downloadable_product_permissions WHERE order_id = %d AND product_id = %d AND download_id = %s", $order->id, $product_id, $download_id ) );
							}
						}
					}
					// Add permissions
					if ( ! empty( $new_download_ids ) ) {
						...
						foreach ( $new_download_ids as $download_id ) {
							...
							if ( apply_filters( 'banda_process_product_file_download_paths_grant_access_to_new_file', true, $download_id, $product_id, $order ) ) {
								// grant permission if it doesn't already exist
								if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT 1=1 FROM {$wpdb->prefix}banda_downloadable_product_permissions WHERE order_id = %d AND product_id = %d AND download_id = %s", $order->id, $product_id, $download_id ) ) ) {
	...
	public function maybe_render_blank_state( $which ) {
		...
		if ( in_array( $post_type, array( 'shop_order', 'product', 'shop_coupon' ) ) && 'bottom' === $which ) {
			...
			echo '<div class="banda-BlankState">';
			...
			switch ( $post_type ) {
				...
				case 'product' :
					?>
					<h2 class="banda-BlankState-message"><?php _e( 'Ready to start selling something awesome?', 'banda' ); ?></h2>
					<a class="banda-BlankState-cta button-primary button" href="<?php echo esc_url( admin_url( 'post-new.php?post_type=product&tutorial=true' ) ); ?>"><?php _e( 'Create your first product!', 'banda' ); ?></a>
					<?php
				break;
			}
