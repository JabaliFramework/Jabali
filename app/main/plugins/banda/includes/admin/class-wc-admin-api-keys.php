...
class WC_Admin_API_Keys {
	...
	private static function table_list_output() {
		echo '<h2>' . __( 'Keys/Apps', 'banda' ) . ' <a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=api&section=keys&create-key=1' ) ) . '" class="add-new-h2">' . __( 'Add Key', 'banda' ) . '</a></h2>';
		...
		$keys_table_list->search_box( __( 'Search Key', 'banda' ), 'key' );
	...
	public static function notices() {
		if ( isset( $_GET['revoked'] ) && 1 == $_GET['revoked'] ) {
			WC_Admin_Settings::add_message( __( 'API Key revoked successfully.', 'banda' ) );
		}
	}
	...
	private function remove_key( $key_id ) {
		...
		$delete = $wpdb->delete( $wpdb->prefix . 'banda_api_keys', array( 'key_id' => $key_id ), array( '%d' ) );
