...
<div id="poststuff" class="banda-reports-wide">
	<div class="postbox">

	<?php if ( 'custom' === $current_range && isset( $_GET['start_date'], $_GET['end_date'] ) ) : ?>
		<h3 class="screen-reader-text"><?php echo esc_html( sprintf( _x( 'From %s to %s', 'start date and end date', 'banda' ), wc_clean( $_GET['start_date'] ), wc_clean( $_GET['end_date'] ) ) ); ?></h3>
	<?php else : ?>
				...
				<li class="custom <?php echo $current_range == 'custom' ? 'active' : ''; ?>">
					...
					<form method="GET">
						<div>
							...
							<input type="submit" class="button" value="<?php esc_attr_e( 'Go', 'banda' ); ?>" />
						</div>
					</form>
				</li>
			</ul>
