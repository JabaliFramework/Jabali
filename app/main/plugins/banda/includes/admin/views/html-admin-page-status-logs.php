...
<?php if ( $logs ) : ?>
	<div id="log-viewer-select">
		<div class="alignleft">
			<h2><?php printf( __( 'Log file: %s (%s)', 'banda' ), esc_html( $viewed_log ), date_i18n( get_option( 'date_format') . ' ' . get_option( 'time_format'), filemtime( WC_LOG_DIR . $viewed_log ) ) ); ?></h2>
		</div>
		<div class="alignright">
			<form action="<?php echo admin_url( 'admin.php?page=wc-status&tab=logs' ); ?>" method="post">
				...
				<input type="submit" class="button" value="<?php esc_attr_e( 'View', 'banda' ); ?>" />
			</form>
		</div>
		<div class="clear"></div>
	</div>
...
<?php else : ?>
	<div class="updated banda-message inline"><p><?php _e( 'There are currently no logs to view.', 'banda' ); ?></p></div>
<?php endif; ?>
