...
class WC_Admin_API_Keys_Table_List extends WP_List_Table {
	...
	public function __construct() {
		parent::__construct( array(
			'singular' => __( 'key', 'banda' ),
			'plural'   => __( 'keys', 'banda' ),
			'ajax'     => false
		) );
	}
	...
	public function get_columns() {
		return array(
			...
			'description'   => __( 'Description', 'banda' ),
			'truncated_key' => __( 'Consumer Key Ending In', 'banda' ),
			'user'          => __( 'User', 'banda' ),
			'permissions'   => __( 'Permissions', 'banda' ),
			'last_access'   => __( 'Last Access', 'banda' )
		);
	}
	...
	public function column_description( $key ) {
		...
		if ( empty( $key['description'] ) ) {
			$output .= esc_html__( 'API Key', 'banda' );
		} else {
		...
		// Get actions
		$actions = array(
			'id'    => sprintf( __( 'ID: %d', 'banda' ), $key['key_id'] ),
			'edit'  => '<a href="' . esc_url( $url ) . '">' . __( 'View/Edit', 'banda' ) . '</a>',
			'trash' => '<a class="submitdelete" title="' . esc_attr__( 'Revoke API Key', 'banda' ) . '" href="' . esc_url( wp_nonce_url( add_query_arg( array( 'revoke-key' => $key['key_id'] ), admin_url( 'admin.php?page=wc-settings&tab=api&section=keys' ) ), 'revoke' ) ) . '">' . __( 'Revoke', 'banda' ) . '</a>'
		);
	...
	public function column_last_access( $key ) {
		if ( ! empty( $key['last_access'] ) ) {
			$date = sprintf( _x( '%1$s at %2$s', 'date and time', 'banda' ), date_i18n( wc_date_format(), strtotime( $key['last_access'] ) ), date_i18n( wc_time_format(), strtotime( $key['last_access'] ) ) );

			return apply_filters( 'banda_api_key_last_access_datetime', $date, $key['last_access'] );
		}

		return __( 'Unknown', 'banda' );
	}
	...
	public function prepare_items() {
		...
		$per_page = apply_filters( 'banda_api_keys_settings_items_per_page', 10 );
		...
		$keys = $wpdb->get_results(
			"SELECT key_id, user_id, description, permissions, truncated_key, last_access FROM {$wpdb->prefix}banda_api_keys WHERE 1 = 1 {$search}" .
			$wpdb->prepare( "ORDER BY key_id DESC LIMIT %d OFFSET %d;", $per_page, $offset ), ARRAY_A
		);

		$count = $wpdb->get_var( "SELECT COUNT(key_id) FROM {$wpdb->prefix}banda_api_keys WHERE 1 = 1 {$search};" );
