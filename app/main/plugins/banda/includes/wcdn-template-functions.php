...
function wcdn_get_template_title() {
	global $wcdn;
	return apply_filters( 'wcdn_template_type', __( $wcdn->print->template['labels']['name'], 'banda-delivery-notes' ) );
}
...
function wcdn_navigation() {
	?>
	<div id="navigation">
		<a href="#" class="button" onclick="window.print();return false;"><?php _e( 'Print', 'banda-delivery-notes' ); ?></a>
	</div><!-- #navigation -->
	<?php
}
...
function wcdn_content( $order, $template_type ) {
	...
	// Add Banda hooks here to not 
	// make global changes to the totals 
	add_filter( 'banda_get_order_item_totals', 'wcdn_remove_semicolon_from_totals', 10, 2 );
	add_filter( 'banda_get_order_item_totals', 'wcdn_remove_payment_method_from_totals', 20, 2 );
	add_filter( 'banda_get_order_item_totals', 'wcdn_add_refunded_order_totals', 30, 2 );
...
function wcdn_get_company_logo_id() {
	global $wcdn;
	return apply_filters( 'wcdn_company_logo_id', get_option( Banda_Delivery_Notes::$plugin_prefix . 'company_logo_image_id' ) );
}
...
function wcdn_company_logo() {
	...
	$company = get_option( Banda_Delivery_Notes::$plugin_prefix . 'custom_company_name' );
	if( $attachment_id ) {
...
function wcdn_company_name() {
	global $wcdn;
	$name = trim( get_option( Banda_Delivery_Notes::$plugin_prefix . 'custom_company_name' ) );
	if( !empty( $name ) ) {
...
function wcdn_company_info() {
	global $wcdn;
	echo stripslashes( wpautop( wptexturize( get_option( Banda_Delivery_Notes::$plugin_prefix . 'company_address' ) ) ) );
}
...
function wcdn_get_order_info( $order ) {
	...
	if( wcdn_get_template_type() == 'invoice' && !empty( $create_invoice_number ) ) {
		$fields['invoice_number'] = array( 
			'label' => __( 'Invoice Number', 'banda-delivery-notes' ),
			'value' => wcdn_get_order_invoice_number( $order->id )
		);
	}
	
	$fields['order_number'] = array( 
		'label' => __( 'Order Number', 'banda-delivery-notes' ),
		'value' => $order->get_order_number() 
	);
	
	$fields['order_date'] = array( 
		'label' => __( 'Order Date', 'banda-delivery-notes' ),
		'value' => date_i18n( get_option( 'date_format' ), strtotime( $order->order_date ) )
	);
	
	$fields['payment_method'] = array( 
		'label' => __( 'Payment Method', 'banda-delivery-notes' ),
		'value' => __( $order->payment_method_title, 'banda' )
	);
	
	if( $order->billing_email ) {
		$fields['billing_email'] = array(
			'label' => __( 'Email', 'banda-delivery-notes' ),
	...
	if( $order->billing_phone ) {
		$fields['billing_phone'] = array(
			'label' => __( 'Telephone', 'banda-delivery-notes' ),
...
function wcdn_additional_product_fields( $fields = null, $product = null, $order ) {
	...
	if( $product && $product->exists() && $product->get_sku() ) {
		$fields[] = array(
			'label' => __( 'SKU:', 'banda-delivery-notes' ),
...
function wcdn_has_shipping_address( $order ) {
	// Works only with Banda 2.2 and higher
	if( version_compare( WC_VERSION, '2.2.0', '>=' ) ) {
		if( ! wc_ship_to_billing_address_only() && $order->needs_shipping_address() && get_option( 'banda_calc_shipping' ) !== 'no' ) {
			return true;
		} else {
			return false;
		}
	...
	if( wcdn_has_refund( $order ) ) {
		if( version_compare( WC_VERSION, '2.3.0', '>=' ) ) {
			...
			if ( wc_tax_enabled() && 'incl' == $order->tax_display_cart ) {
				...
				if ( 'itemized' == get_option( 'banda_tax_total_display' ) ) {
				...
				if ( ! empty( $tax_del_array ) ) {
					$refunded_tax_del .= ' ' . sprintf( __( '(Includes %s)', 'banda' ), implode( ', ', $tax_del_array ) );
				}
	
				if ( ! empty( $tax_ins_array ) ) {
					$refunded_tax_ins .= ' ' . sprintf( __( '(Includes %s)', 'banda' ), implode( ', ', $tax_ins_array ) );
				}
			}
		...
		$total_rows['wcdn_refunded_total'] = array(
			'label' => __( 'Refund', 'banda-delivery-notes' ), 
		...
		$total_rows['order_total'] = array(
			'label' => __( 'Order Subtotal', 'banda-delivery-notes' ), 
...
function wcdn_get_personal_notes() {
	global $wcdn;
	return stripslashes( wpautop( wptexturize( get_option( Banda_Delivery_Notes::$plugin_prefix . 'personal_notes' ) ) ) );
}
...
function wcdn_get_policies_conditions() {
	global $wcdn;
	return stripslashes( wpautop( wptexturize( get_option( Banda_Delivery_Notes::$plugin_prefix . 'policies_conditions' ) ) ) );
}
...
function wcdn_get_imprint() {
	global $wcdn;
	return stripslashes( wpautop( wptexturize( get_option( Banda_Delivery_Notes::$plugin_prefix . 'footer_imprint' ) ) ) );
}
