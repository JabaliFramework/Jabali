...
class WC_CLI_Customer extends WC_CLI_Command {
	...
	public function create( $args, $assoc_args ) {
		...
		try {
			...
			$data                = apply_filters( 'banda_cli_create_customer_data', $this->unflatten_array( $assoc_args ) );
			...
			do_action( 'banda_cli_create_customer', $id, $data );
	...
	public function delete( $args, $assoc_args ) {
		...
		foreach ( $args as $arg ) {
			try {
				...
				do_action( 'banda_cli_delete_customer', $customer['id'] );
	...
	public function downloads( $args, $assoc_args ) {
		try {
			...
			$downloads = apply_filters( 'banda_cli_customer_downloads', $downloads, $user, $assoc_args );
	...
	public function update( $args, $assoc_args ) {
		try {
			...
			$data = apply_filters( 'banda_cli_update_customer_data', $data );
			...
			do_action( 'banda_cli_update_customer', $user['id'], $data );
	...
	protected function get_user( $id_email_or_login ) {
		...
		if ( ! $user ) {
			throw new WC_CLI_Exception( 'banda_cli_invalid_customer', sprintf( __( 'Invalid customer "%s"', 'banda' ), $id_email_or_login ) );
		}
	...
	protected function update_customer_data( $id, $data ) {
		...
		do_action( 'banda_cli_update_customer_data', $id, $data );
	}
	...
	protected function get_customer_billing_address_fields() {
		return apply_filters( 'banda_cli_customer_billing_address_fields', array(
			'first_name',
			'last_name',
			'company',
			'address_1',
	...
	protected function get_customer_shipping_address_fields() {
		return apply_filters( 'banda_cli_customer_shipping_address_fields', array(
			'first_name',
			'last_name',
			'company',
			'address_1',
	...
	protected function get_customer_download_fields() {
		return apply_filters( 'banda_cli_customer_download_fields', array(
			'download_id',
			'download_name',
			'access_expires',
		) );
