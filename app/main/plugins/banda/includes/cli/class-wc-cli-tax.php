...
class WC_CLI_Tax extends WC_CLI_Command {
	...
	public function create( $__, $assoc_args ) {
		...
		$assoc_args = apply_filters( 'banda_cli_create_tax_rate_data', $assoc_args );
		...
		do_action( 'banda_cli_create_tax_rate', $id, $tax_data );
	...
	public function create_class( $__, $assoc_args ) {
		try {
			...
			$assoc_args = apply_filters( 'banda_cli_create_tax_class_data', $assoc_args );

			// Check if name is specified.
			if ( ! isset( $assoc_args['name'] ) ) {
				throw new WC_CLI_Exception( 'banda_cli_missing_name', sprintf( __( 'Missing parameter %s', 'banda' ), 'name' ) );
			}
			...
			if ( $exists ) {
				throw new WC_CLI_Exception( 'banda_cli_cannot_create_tax_class', __( 'Tax class already exists', 'banda' ) );
			}
			...
			do_action( 'banda_cli_create_tax_class', $slug, array( 'name' => $name ) );
	...
	public function delete( $args, $assoc_args ) {
		...
		foreach ( $args as $tax_id ) {
			...
			do_action( 'banda_cli_delete_tax_rate', $tax_id );
	...
	public function get( $args, $assoc_args ) {
		...
		try {
			...
			if ( empty( $tax_data ) ) {
				throw new WC_CLI_Exception( 'banda_cli_invalid_tax_rate', sprintf( __( 'Invalid tax rate ID: %s', 'banda' ), $args[0] ) );
			}

			$tax_data = apply_filters( 'banda_cli_get_tax_rate', $tax_data[0] );
	...
	public function list_class( $__, $assoc_args ) {
		...
		foreach ( $classes as $class ) {
			$items[] = apply_filters( 'banda_cli_tax_class_response', array(
	...
	public function update( $args, $assoc_args ) {
		try {
			...
			$id             = $current_data['id'];
			$data           = apply_filters( 'banda_cli_update_tax_rate_data', $assoc_args, $id );
			$new_data       = array();
			...
			do_action( 'banda_cli_update_tax_rate', $id, $data );
	...
	protected function merge_tax_query_args( $base_args, $assoc_args ) {
		...
		$args = apply_filters( 'banda_cli_tax_query_args', $args, $assoc_args );
