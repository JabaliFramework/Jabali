...
class WC_CLI_Order extends WC_CLI_Command {
	...
	public function create( $__, $assoc_args ) {
		...
		try {
			...
			$data = apply_filters( 'banda_cli_create_order_data', $this->unflatten_array( $assoc_args ) );
			...
			if ( empty( $data['customer_id'] ) ) {
				throw new WC_CLI_Exception( 'banda_cli_missing_customer_id', __( 'Missing customer_id field', 'banda' ) );
			}

			// make sure customer exists
			if ( false === get_user_by( 'id', $data['customer_id'] ) ) {
				throw new WC_CLI_Exception( 'banda_cli_invalid_customer_id', __( 'Customer ID is invalid', 'banda' ) );
			}
			...
			if ( is_wp_error( $order ) ) {
				throw new WC_CLI_Exception( 'banda_cli_cannot_create_order', sprintf( __( 'Cannot create order: %s', 'banda' ), implode( ', ', $order->get_error_messages() ) ) );
			}
			...
			if ( isset( $data['payment_details'] ) && is_array( $data['payment_details'] ) ) {
				...
				if ( empty( $data['payment_details']['method_id'] ) || empty( $data['payment_details']['method_title'] ) ) {
					throw new WC_CLI_Exception( 'banda_invalid_payment_details', __( 'Payment method ID and title are required', 'banda' ) );
				}
			...
			if ( isset( $data['currency'] ) ) {
				if ( ! array_key_exists( $data['currency'], get_banda_currencies() ) ) {
					throw new WC_CLI_Exception( 'banda_invalid_order_currency', __( 'Provided order currency is invalid', 'banda') );
				}
			...
			do_action( 'banda_cli_create_order', $order->id, $data );
	...
	public function delete( $args, $assoc_args ) {
		...
		foreach ( $args as $id ) {
			...
			do_action( 'banda_cli_delete_order', $id );
	...
	public function get( $args, $assoc_args ) {
		try {
			...
			if ( ! $order ) {
				throw new WC_CLI_Exception( 'banda_cli_invalid_order', sprintf( __( 'Invalid order "%s"', 'banda' ), $args[0] ) );
			}
	...
	public function update( $args, $assoc_args ) {
		try {
			$id            = $args[0];
			$data          = apply_filters( 'banda_cli_update_order_data', $this->unflatten_array( $assoc_args ) );
			$update_totals = false;
			$order         = wc_get_order( $id );

			if ( empty( $order ) ) {
				throw new WC_CLI_Exception( 'banda_cli_invalid_order_id', __( 'Order ID is invalid', 'banda' ) );
			}
			...
			do_action( 'banda_cli_update_order', $order->id, $data );
	...
	protected function get_order_data( $order ) {
		...
		$order_data = apply_filters( 'banda_cli_order_data', $order_data );
	...
	protected function set_item( $order, $item_type, $item, $action ) {
		...
		if ( 'update' === $action ) {
			...
			if ( is_null( $result ) ) {
				throw new WC_CLI_Exception( 'banda_invalid_item_id', __( 'Order item ID provided is not associated with order', 'banda' ) );
			}
		}
	...
	protected function set_line_item( $order, $item, $action ) {
		...
		if ( ! isset( $item['product_id'] ) && ! isset( $item['sku'] ) ) {
			throw new WC_CLI_Exception( 'banda_cli_invalid_product_id', __( 'Product ID or SKU is required', 'banda' ) );
		}
		...
		if ( 'update' === $action ) {
			...
			if ( $item['product_id'] != $item_product_id && $item['product_id'] != $item_variation_id ) {
				throw new WC_CLI_Exception( 'banda_cli_invalid_product_id', __( 'Product ID provided does not match this line item', 'banda' ) );
			}
		}
		...
		if ( isset( $item['variations'] ) && is_array( $item['variations'] ) ) {
			foreach ( $item['variations'] as $key => $value ) {
				if ( ! $key || ! $value ) {
					throw new WC_CLI_Exception( 'banda_cli_invalid_product_variation', __( 'The product variation is invalid', 'banda' ) );
				}
			}
		...
		if ( ! is_object( $product ) ) {
			throw new WC_CLI_Exception( 'banda_cli_invalid_product', __( 'Product is invalid', 'banda' ) );
		}

		// quantity must be positive float
		if ( isset( $item['quantity'] ) && floatval( $item['quantity'] ) <= 0 ) {
			throw new WC_CLI_Exception( 'banda_cli_invalid_product_quantity', __( 'Product quantity must be a positive float', 'banda' ) );
		}

		// quantity is required when creating
		if ( $creating && ! isset( $item['quantity'] ) ) {
			throw new WC_CLI_Exception( 'banda_cli_invalid_product_quantity', __( 'Product quantity is required', 'banda' ) );
		}
		...
		$item_args = apply_filters( 'banda_cli_order_line_item_args', $item_args, $item, $order, $action );
		...
		if ( $creating ) {
			...
			if ( ! $item_id ) {
				throw new WC_CLI_Exception( 'banda_cannot_create_line_item', __( 'Cannot create line item, try again', 'banda' ) );
			}
			...
			if ( ! $item_id ) {
				throw new WC_CLI_Exception( 'banda_cannot_update_line_item', __( 'Cannot update line item, try again', 'banda' ) );
			}
		}
	}
	...
	protected function set_shipping( $order, $shipping, $action ) {
		...
		if ( isset( $shipping['total'] ) && floatval( $shipping['total'] ) < 0 ) {
			throw new WC_CLI_Exception( 'banda_invalid_shipping_total', __( 'Shipping total must be a positive amount', 'banda' ) );
		}
		...
		if ( 'create' === $action ) {
			...
			if ( ! isset( $shipping['method_id'] ) ) {
				throw new WC_CLI_Exception( 'banda_invalid_shipping_item', __( 'Shipping method ID is required', 'banda' ) );
			}
			...
			if ( ! $shipping_id ) {
				throw new WC_CLI_Exception( 'banda_cannot_create_shipping', __( 'Cannot create shipping method, try again', 'banda' ) );
			}
			...
			if ( ! $shipping_id ) {
				throw new WC_CLI_Exception( 'banda_cannot_update_shipping', __( 'Cannot update shipping method, try again', 'banda' ) );
			}
		}
	}
	...
	protected function set_fee( $order, $fee, $action ) {
		...
		if ( 'create' === $action ) {
			...
			if ( ! isset( $fee['title'] ) ) {
				throw new WC_CLI_Exception( 'banda_invalid_fee_item', __( 'Fee title is required', 'banda' ) );
			}
			...
			if ( isset( $fee['taxable'] ) && $fee['taxable'] ) {
				...
				if ( ! isset( $fee['tax_class'] ) ) {
					throw new WC_CLI_Exception( 'banda_invalid_fee_item', __( 'Fee tax class is required when fee is taxable', 'banda' ) );
				}
			...
			if ( ! $fee_id ) {
				throw new WC_CLI_Exception( 'banda_cannot_create_fee', __( 'Cannot create fee, try again', 'banda' ) );
			}
			...
			if ( ! $fee_id ) {
				throw new WC_CLI_Exception( 'banda_cannot_update_fee', __( 'Cannot update fee, try again', 'banda' ) );
			}
		}
	}
	...
	protected function set_coupon( $order, $coupon, $action ) {
		...
		if ( isset( $coupon['amount'] ) && floatval( $coupon['amount'] ) < 0 ) {
			throw new WC_CLI_Exception( 'banda_invalid_coupon_total', __( 'Coupon discount total must be a positive amount', 'banda' ) );
		}
		...
		if ( 'create' === $action ) {
			...
			if ( empty( $coupon['code'] ) ) {
				throw new WC_CLI_Exception( 'banda_invalid_coupon_coupon', __( 'Coupon code is required', 'banda' ) );
			}

			$coupon_id = $order->add_coupon( $coupon['code'], isset( $coupon['amount'] ) ? floatval( $coupon['amount'] ) : 0 );

			if ( ! $coupon_id ) {
				throw new WC_CLI_Exception( 'banda_cannot_create_order_coupon', __( 'Cannot create coupon, try again', 'banda' ) );
			}
			...
			if ( ! $coupon_id ) {
				throw new WC_CLI_Exception( 'banda_cannot_update_order_coupon', __( 'Cannot update coupon, try again', 'banda' ) );
			}
		}
	}
}
