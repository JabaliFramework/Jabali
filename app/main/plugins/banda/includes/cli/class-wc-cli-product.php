...
class WC_CLI_Product extends WC_CLI_Command {
	...
	public function create( $args, $assoc_args ) {
		...
		try {
			...
			$data = apply_filters( 'banda_cli_create_product_data', $this->unflatten_array( $assoc_args ) );

			// Check if product title is specified
			if ( ! isset( $data['title'] ) ) {
				throw new WC_CLI_Exception( 'banda_missing_product_title', sprintf( __( 'Missing parameter %s', 'banda' ), 'title' ) );
			}
			...
			if ( ! in_array( wc_clean( $data['type'] ), array_keys( wc_get_product_types() ) ) ) {
				throw new WC_CLI_Exception( 'banda_cli_invalid_product_type', sprintf( __( 'Invalid product type - the product type must be any of these: %s', 'banda' ), implode( ', ', array_keys( wc_get_product_types() ) ) ) );
			}
			...
			if ( is_wp_error( $id ) ) {
				throw new WC_CLI_Exception( 'banda_cli_cannot_create_product', $id->get_error_message() );
			}
			...
			do_action( 'banda_cli_create_product', $id, $data );
	...
	public function delete( $args, $assoc_args ) {
		$exit_code = 0;
		foreach ( $args as $id ) {
			do_action( 'banda_cli_delete_product', $id );
	...
	public function get( $args, $assoc_args ) {
		try {
			...
			if ( ! $product ) {
				throw new WC_CLI_Exception( 'banda_cli_invalid_product', sprintf( __( 'Invalid product "%s"', 'banda' ), $args[0] ) );
			}
	...
	public function reviews( $args, $assoc_args ) {
		try {
			...
			if ( ! $product ) {
				throw new WC_CLI_Exception( 'banda_cli_invalid_product', sprintf( __( 'Invalid product "%s"', 'banda' ), $id ) );
			}
	...
	public function update( $args, $assoc_args ) {
		try {
			$id   = $args[0];
			$data = apply_filters( 'banda_cli_update_product_data', $this->unflatten_array( $assoc_args ) );
			...
			do_action( 'banda_cli_update_product', $id, $data );
	...
	private function save_product_meta( $product_id, $data ) {
		...
		if ( isset( $data['sku'] ) ) {
			...
			if ( '' == $new_sku ) {
				...
				if ( ! empty( $new_sku ) ) {
					...
					if ( ! $unique_sku ) {
						throw new WC_CLI_Exception( 'banda_cli_product_sku_already_exists', __( 'The SKU already exists on another product', 'banda' ) );
					} else {
		...
		do_action( 'banda_cli_process_product_meta_' . $product_type, $product_id, $data );
	...
	private function save_variations( $id, $data ) {
		...
		foreach ( $variations as $menu_order => $variation ) {
			...
			if ( is_wp_error( $variation_id ) ) {
				throw new WC_CLI_Exception( 'banda_cli_cannot_save_product_variation', $variation_id->get_error_message() );
			}
				...
				if ( $image && is_array( $image ) ) {
					if ( isset( $image['position'] ) && isset( $image['src'] ) && $image['position'] == 0 ) {
						...
						if ( is_wp_error( $upload ) ) {
							throw new WC_CLI_Exception( 'banda_cli_cannot_upload_product_image', $upload->get_error_message() );
						}
			...
			do_action( 'banda_cli_save_product_variation', $variation_id, $menu_order, $variation );
		}
	...
	private function allowed_image_mime_types() {
		return apply_filters( 'banda_cli_allowed_image_mime_types', array(
	...
	private function upload_product_image( $image_url ) {
		...
		if ( ! $parsed_url || ! is_array( $parsed_url ) ) {
			throw new WC_CLI_Exception( 'banda_cli_invalid_product_image', sprintf( __( 'Invalid URL %s', 'banda' ), $image_url ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			throw new WC_CLI_Exception( 'banda_cli_invalid_remote_product_image', sprintf( __( 'Error getting remote image %s.', 'banda' ), $image_url ) . ' ' . sprintf( __( 'Error: %s.', 'banda' ), $response->get_error_message() ) );
		} elseif ( 200 !== wp_remote_retrieve_response_code( $response ) ) {
			throw new WC_CLI_Exception( 'banda_cli_invalid_remote_product_image', sprintf( __( 'Error getting remote image %s.', 'banda' ), $image_url ) );
		}
		...
		if ( ! $wp_filetype['type'] ) {
			...
			if ( ! $wp_filetype['type'] ) {
				throw new WC_CLI_Exception( 'banda_cli_invalid_image_type', __( 'Invalid image type.', 'banda' ) );
			}
		}
		...
		if ( $upload['error'] ) {
			throw new WC_CLI_Exception( 'banda_cli_product_image_upload_error', $upload['error'] );
		}
		...
		if ( 0 == $filesize ) {
			@unlink( $upload['file'] );
			unset( $upload );
			throw new WC_CLI_Exception( 'banda_cli_product_image_upload_file_error', __( 'Zero size file downloaded', 'banda' ) );
		}
	...
	private function is_variable( $data ) {
		if ( isset( $data['type'] ) && isset( $data['variations'] ) && is_array( $data['variations'] ) ) {
			$types = apply_filters( 'banda_cli_get_product_variable_types', array( 'variable' ) );
