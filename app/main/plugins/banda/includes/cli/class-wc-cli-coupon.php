...
class WC_CLI_Coupon extends WC_CLI_Command {
	...
	public function create( $__, $assoc_args ) {
		...
		try {
			...
			$assoc_args = apply_filters( 'banda_cli_create_coupon_data', $assoc_args );

			// Check if coupon code is specified.
			if ( ! isset( $assoc_args['code'] ) ) {
				throw new WC_CLI_Exception( 'banda_cli_missing_coupon_code', sprintf( __( 'Missing parameter %s', 'banda' ), 'code' ) );
			}

			$coupon_code = apply_filters( 'banda_coupon_code', $assoc_args['code'] );
			...
			if ( $coupon_found ) {
				throw new WC_CLI_Exception( 'banda_cli_coupon_code_already_exists', __( 'The coupon code already exists', 'banda' ) );
			}
			...
			if ( ! in_array( wc_clean( $coupon_data['type'] ), array_keys( wc_get_coupon_types() ) ) ) {
				throw new WC_CLI_Exception( 'banda_cli_invalid_coupon_type', sprintf( __( 'Invalid coupon type - the coupon type must be any of these: %s', 'banda' ), implode( ', ', array_keys( wc_get_coupon_types() ) ) ) );
			}
			...
			if ( is_wp_error( $id ) ) {
				throw new WC_CLI_Exception( 'banda_cli_cannot_create_coupon', $id->get_error_message() );
			}
			...
			do_action( 'banda_cli_create_coupon', $id, $coupon_data );
	...
	public function delete( $args, $assoc_args ) {
		...
		foreach ( $this->get_many_coupons_from_ids_or_codes( $args, true ) as $coupon ) {
			do_action( 'banda_cli_delete_coupon', $coupon->id );
	...
	public function get( $args, $assoc_args ) {
		...
		try {
			...
			if ( ! $coupon ) {
				throw new WC_CLI_Exception( 'banda_cli_invalid_coupon', sprintf( __( 'Invalid coupon ID or code: %s', 'banda' ), $args[0] ) );
			}
			...
			$coupon_data = apply_filters( 'banda_cli_get_coupon', $coupon_data );
	...
	public function update( $args, $assoc_args ) {
		try {
			...
			$data        = apply_filters( 'banda_cli_update_coupon_data', $assoc_args, $id );
			if ( isset( $data['code'] ) ) {
				global $wpdb;
			...
			if ( 0 === $id ) {
				throw new WC_CLI_Exception( 'banda_cli_cannot_update_coupon', __( 'Failed to update coupon', 'banda' ) );
			}
			...
			do_action( 'banda_cli_update_coupon', $id, $data );
