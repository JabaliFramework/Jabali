 ...
 *
 * The Banda product variation class handles product variation data.
 *
...
class WC_Product_Variation extends WC_Product {
	...
	public function add_to_cart_url() {
		...
		return apply_filters( 'banda_product_add_to_cart_url', $url, $this );
	}
	...
	public function add_to_cart_text() {
		$text = $this->is_purchasable() && $this->is_in_stock() ? __( 'Add to cart', 'banda' ) : __( 'Read more', 'banda' );
	...
	public function variation_is_visible() {
		...
		return apply_filters( 'banda_variation_is_visible', $visible, $this->variation_id, $this->id, $this );
	}
	...
	public function variation_is_active() {
		return apply_filters( 'banda_variation_is_active', true, $this );
	}
	...
	public function is_purchasable() {
		...
		return apply_filters( 'banda_variation_is_purchasable', $purchasable, $this );
	}
	...
	public function get_price_html( $price = '' ) {
		...
		if ( $this->get_price() !== '' ) {
			if ( $this->is_on_sale() ) {
				$price = apply_filters( 'banda_variation_sale_price_html', '<del>' . wc_price( $display_regular_price ) . '</del> <ins>' . wc_price( $display_sale_price ) . '</ins>' . $this->get_price_suffix(), $this );
			} elseif ( $this->get_price() > 0 ) {
				$price = apply_filters( 'banda_variation_price_html', wc_price( $display_price ) . $this->get_price_suffix(), $this );
			} else {
				$price = apply_filters( 'banda_variation_free_price_html', __( 'Free!', 'banda' ), $this );
			}
		} else {
			$price = apply_filters( 'banda_variation_empty_price_html', '', $this );
		}

		return apply_filters( 'banda_get_variation_price_html', $price, $this );
	}
	...
	public function managing_stock() {
		if ( 'yes' === get_option( 'banda_manage_stock', 'yes' ) ) {
	...
	public function get_stock_quantity() {
		return apply_filters( 'banda_variation_get_stock_quantity', true === $this->managing_stock() ? wc_stock_amount( $this->stock ) : $this->parent->get_stock_quantity(), $this );
	}
	...
	public function is_in_stock() {
		 ...
		 */
		if ( true === $this->managing_stock() && ! $this->backorders_allowed() && $this->get_stock_quantity() <= get_option( 'banda_notify_no_stock_amount' ) ) {
			$status = false;
		}

		return apply_filters( 'banda_variation_is_in_stock', $status );
	}
	...
	public function set_stock( $amount = null, $mode = 'set' ) {
		...
		if ( ! is_null( $amount ) && true === $this->managing_stock() ) {
			...
			// Trigger action
			do_action( 'banda_variation_set_stock', $this );
	...
	public function set_stock_status( $status ) {
		...
		if ( true === $this->managing_stock() ) {
			...
			if ( ! $this->parent->backorders_allowed() && $this->parent->get_stock_quantity() <= get_option( 'banda_notify_no_stock_amount' ) ) {
				$status = 'outofstock';
			}
		}

		if ( update_post_meta( $this->variation_id, '_stock_status', $status ) ) {
			do_action( 'banda_variation_set_stock_status', $this->variation_id, $status );
	...
	public function get_formatted_variation_attributes( $flat = false ) {
		...
		if ( is_array( $variation_data ) ) {
			...
			foreach ( $attributes as $attribute ) {
				...
				$description_value        = __( 'Any', 'banda' );
				...
				if ( $attribute[ 'is_taxonomy' ] ) {
					...
					foreach ( $post_terms as $term ) {
						if ( $variation_selected_value === $term->slug ) {
							$description_value = esc_html( apply_filters( 'banda_variation_option_name', $term->name ) );
						}
					}
					...
					foreach ( $options as $option ) {
						...
						$description_value = esc_html( apply_filters( 'banda_variation_option_name', $option ) );
					}
				}
	...
	public function get_formatted_name() {
		...
		return sprintf( __( '%s &ndash; %s%s', 'banda' ), $identifier, $this->get_title(), $extra_data );
	}
