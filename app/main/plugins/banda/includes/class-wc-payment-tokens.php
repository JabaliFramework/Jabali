...
class WC_Payment_Tokens {
	...
	public static function get_tokens( $args ) {
		...
		$sql   = "SELECT * FROM {$wpdb->prefix}banda_payment_tokens";
		$where = array( '1=1' );
	...
	public static function get_customer_tokens( $customer_id, $gateway_id = '' ) {
		...
		return apply_filters( 'banda_get_customer_payment_tokens', $tokens, $customer_id, $gateway_id );
	}
	...
	public static function get_customer_default_token( $customer_id ) {
		...
		$token = $wpdb->get_row( $wpdb->prepare(
			"SELECT * FROM {$wpdb->prefix}banda_payment_tokens WHERE user_id = %d AND is_default = 1",
			$customer_id
		) );
	...
	public static function get_order_tokens( $order_id ) {
		...
		return apply_filters( 'banda_get_order_payment_tokens', $tokens, $order_id );
	}
	...
	public static function get( $token_id, $token_result = null ) {
		...
		if ( is_null( $token_result ) ) {
			$token_result = $wpdb->get_row( $wpdb->prepare(
				"SELECT * FROM {$wpdb->prefix}banda_payment_tokens WHERE token_id = %d",
				$token_id
			) );
	...
	public static function set_users_default( $user_id, $token_id ) {
		...
		foreach ( $users_tokens as $token ) {
			if ( $token_id === $token->get_id() ) {
				...
				do_action( 'banda_payment_token_set_default', $token_id, $token );
			} else {
	...
	public static function get_token_type_by_id( $token_id ) {
		...
		$type = $wpdb->get_var( $wpdb->prepare(
			"SELECT type FROM {$wpdb->prefix}banda_payment_tokens WHERE token_id = %d",
			$token_id
		) );
		return $type;
	}
