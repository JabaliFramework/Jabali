...
function wc_get_dimension( $dimension, $to_unit, $from_unit = '' ) {
	...
	if ( empty( $from_unit ) ) {
		$from_unit = strtolower( get_option( 'banda_dimension_unit' ) );
	}
...
function wc_get_weight( $weight, $to_unit, $from_unit = '' ) {
	...
	if ( empty( $from_unit ) ) {
		$from_unit = strtolower( get_option( 'banda_weight_unit' ) );
	}
 ...
 * @param  mixed $dp number of decimal points to use, blank to use banda_price_num_decimals, or false to avoid all rounding.
...
function wc_stock_amount( $amount ) {
	return apply_filters( 'banda_stock_amount', $amount );
}
 ...
 * @return string
 */
function get_banda_price_format() {
	$currency_pos = get_option( 'banda_currency_pos' );
	$format = '%1$s%2$s';
	...
	return apply_filters( 'banda_price_format', $format, $currency_pos );
}
...
function wc_get_price_thousand_separator() {
	$separator = stripslashes( get_option( 'banda_price_thousand_sep' ) );
	return $separator;
}
...
function wc_get_price_decimal_separator() {
	$separator = stripslashes( get_option( 'banda_price_decimal_sep' ) );
...
function wc_get_price_decimals() {
	return absint( get_option( 'banda_price_num_decimals', 2 ) );
}
...
function wc_price( $price, $args = array() ) {
	extract( apply_filters( 'wc_price_args', wp_parse_args( $args, array(
		...
		'price_format'       => get_banda_price_format()
	) ) ) );

	$negative        = $price < 0;
	$price           = apply_filters( 'raw_banda_price', floatval( $negative ? $price * -1 : $price ) );
	$price           = apply_filters( 'formatted_banda_price', number_format( $price, $decimals, $decimal_separator, $thousand_separator ), $price, $decimals, $decimal_separator, $thousand_separator );

	if ( apply_filters( 'banda_price_trim_zeros', false ) && $decimals > 0 ) {
		$price = wc_trim_zeros( $price );
	}

	$formatted_price = ( $negative ? '-' : '' ) . sprintf( $price_format, '<span class="banda-Price-currencySymbol">' . get_banda_currency_symbol( $currency ) . '</span>', $price );
	$return          = '<span class="banda-Price-amount amount">' . $formatted_price . '</span>';

	if ( $ex_tax_label && wc_tax_enabled() ) {
		$return .= ' <small class="banda-Price-taxLabel tax_label">' . WC()->countries->ex_tax_or_vat() . '</small>';
	}
...
/**
 * Banda Date Format - Allows to change date format for everything Banda.
 *
 * @return string
 */
function wc_date_format() {
	return apply_filters( 'banda_date_format', get_option( 'date_format' ) );
}

/**
 * Banda Time Format - Allows to change time format for everything Banda.
 *
 * @return string
 */
function wc_time_format() {
	return apply_filters( 'banda_time_format', get_option( 'time_format' ) );
}

/**
 * Banda Timezone - helper to retrieve the timezone string for a site until.
 * a WP core method exists (see https://core.trac.jabali.github.io/ticket/24730).
 *
...
function wc_format_postcode( $postcode, $country ) {
	...
	return apply_filters( 'banda_format_postcode', $postcode, $country );
}
...
function wc_format_content( $raw_string ) {
	return apply_filters( 'banda_format_content', do_shortcode( shortcode_unautop( wpautop( $raw_string ) ) ), $raw_string );
}
...
add_filter( 'banda_short_description', 'wc_format_product_short_description', 9999999 );
...
add_filter( 'banda_admin_settings_sanitize_option_banda_price_decimal_sep', 'wc_format_option_price_separators', 10, 3 );
add_filter( 'banda_admin_settings_sanitize_option_banda_price_thousand_sep', 'wc_format_option_price_separators', 10, 3 );
...
add_filter( 'banda_admin_settings_sanitize_option_banda_price_num_decimals', 'wc_format_option_price_num_decimals', 10, 3 );
...
function wc_format_option_hold_stock_minutes( $value, $option, $raw_value ) {
	...
	if ( '' !== $value ) {
		wp_schedule_single_event( time() + ( absint( $value ) * 60 ), 'banda_cancel_unpaid_orders' );
	}

	return $value;
}
add_filter( 'banda_admin_settings_sanitize_option_banda_hold_stock_minutes', 'wc_format_option_hold_stock_minutes', 10, 3 );
