 ...
 * PDF Export class
 */
if ( ! class_exists( 'Banda_PDF_Invoices_Export' ) ) {

	class Banda_PDF_Invoices_Export {
		...
		public function __construct() {					
			...
			$this->template_base_path = (defined('WC_TEMPLATE_PATH')?WC_TEMPLATE_PATH:$banda->template_url) . $this->template_directory_name . '/';
			$this->template_default_base_path = Banda_PDF_Invoices::$plugin_path . 'templates/' . $this->template_directory_name . '/';
			$this->template_default_base_uri = Banda_PDF_Invoices::$plugin_url . 'templates/' . $this->template_directory_name . '/';
			...
			add_filter( 'banda_email_attachments', array( $this, 'attach_pdf_to_email' ), 99, 3);
			add_filter( 'banda_api_order_response', array( $this, 'banda_api_invoice_numer' ), 10, 2 );
			...
			// Banda Subscriptions compatibility
			if ( class_exists('WC_Subscriptions') ) {
				if ( version_compare( WC_Subscriptions::$version, '2.0', '<' ) ) {
					add_action( 'banda_subscriptions_renewal_order_created', array( $this, 'banda_subscriptions_renewal_order_created' ), 10, 4 );
				} else {
					add_action( 'wcs_renewal_order_created', array( $this, 'wcs_renewal_order_created' ), 10, 2 );
				}
			}

			// Banda Product Bundles compatibility (add row classes)
			if ( class_exists('WC_Bundles') ) {
				add_filter( 'wpo_wcpdf_item_row_class', array( $this, 'add_product_bundles_classes' ), 10, 4 );
			}

			// Banda Chained Products compatibility (add row classes)
		...
		/**
		 * Copy DOMPDF fonts to jabali tmp folder
		 */
		public function copy_fonts ( $path ) {
			$dompdf_font_dir = Banda_PDF_Invoices::$plugin_path . "lib/dompdf/lib/fonts/";
		...
		public function tmp_path ( $type = '' ) {
			...
			switch ( $type ) {
				...
				case 'DOMPDF_FONT_CACHE':
					...
					break;
				case 'attachments':
					// original value : Banda_PDF_Invoices::$plugin_path . 'tmp/'
					// 1.5+           : $tmp_base . 'attachments/'
					$tmp_path = $old_tmp ? Banda_PDF_Invoices::$plugin_path . 'tmp/' : $tmp_base . 'attachments/';
					break;
				default:
		...
		public function generate_pdf( $template_type, $order_ids )	{
			...
			if ( !class_exists('DOMPDF') ) {
				...
				// than not checking...
				require_once( Banda_PDF_Invoices::$plugin_path . "lib/dompdf/dompdf_config.inc.php" );
			}
		...
		public function generate_pdf_ajax() {
			...
			if( apply_filters( 'wpo_wcpdf_check_privs', !current_user_can( 'manage_banda_orders' ) && !current_user_can( 'edit_shop_orders' ) && !isset( $_GET['my-account'] ), $order_ids ) ) {
				wp_die( __( 'You do not have sufficient permissions to access this page.', 'wpo_wcpdf' ) );
			}
		...
		public function set_invoice_number( $order_id ) {
			...
			$third_party = apply_filters('banda_invoice_number_by_plugin', false);
			if ($third_party) {
				$order = new WC_Order($order_id);
				$invoice_number = apply_filters('banda_generate_invoice_number', null, $order);
			}
			...
			// (http://jabali.github.io/support/topic/customer-invoice-selection-from-order-detail-page-doesnt-record-invoice-id?replies=1)
		...
		public function get_invoice_number( $order_id ) {
			// Call the banda_invoice_number filter and let third-party plugins set a number.
			// Default is null, so we can detect whether a plugin has set the invoice number
			$third_party_invoice_number = apply_filters( 'banda_invoice_number', null, $order_id );
		 ...
		 */
		public function banda_api_invoice_numer ( $data, $order ) {
		...
		/**
		 * Reset invoice data for Banda subscription renewal orders
		 * https://jabali.github.io/support/topic/subscription-renewal-duplicate-invoice-number?replies=6#post-6138110
		 */
		public function banda_subscriptions_renewal_order_created ( $renewal_order, $original_order, $product_id, $new_order_role ) {
			...
			if( sizeof( $items ) > 0 ) {
				foreach ( $items as $item_id => $item ) {
					...
					if(!empty($product)) {
						...
						// $data['single_price'] = banda_price ( $product->get_price() );
		...
		public function wc_price( $price, $args = array() ) {
			if ( version_compare( WOOCOMMERCE_VERSION, '2.1' ) >= 0 ) {
				...
				$formatted_price = banda_price( $price );
			}
		 ...
		 * @param  int    $rate_id  banda tax rate id
		 * @return float  $rate     percentage rate
		 */
		public function get_tax_rate_by_id( $rate_id ) {
			global $wpdb;
			$rate = $wpdb->get_var( $wpdb->prepare( "SELECT tax_rate FROM {$wpdb->prefix}banda_tax_rates WHERE tax_rate_id = %d;", $rate_id ) );
			return (float) $rate;
		}

		/**
		 * Returns a an array with rate_id => tax rate data (array) of all tax rates in banda
		 * @return array  $tax_rate_ids  keyed by id
		 */
		public function get_tax_rate_ids() {
			global $wpdb;
			$rates = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}banda_tax_rates" );
		 ...
		 * 
		 * uses the internal Banda/WP functions and extracts the image url or path
		...
		public function get_thumbnail ( $product ) {
			// Get default Banda img tag (url/http)
		...
		public function use_currency_font ( $template_type ) {
			add_filter( 'banda_currency_symbol', array( $this, 'wrap_currency_symbol' ), 10, 2);
