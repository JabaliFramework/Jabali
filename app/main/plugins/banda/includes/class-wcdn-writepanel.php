 ...
 * Writepanel class
 */
if ( !class_exists( 'Banda_Delivery_Notes_Writepanel' ) ) {

	class Banda_Delivery_Notes_Writepanel {
		...
		public function load_admin_hooks() {		
			// Hooks
			add_action( 'banda_admin_order_actions_end', array( $this, 'add_listing_actions' ) );
		...
		public function add_scripts() {
			if( $this->is_order_edit_page() || $this->is_order_post_page() ) {
				...
				wp_enqueue_script( 'banda-delivery-notes-admin', Banda_Delivery_Notes::$plugin_url . 'js/admin.js', array( 'jquery', 'banda-delivery-notes-print-link', 'thickbox' ) );
			}
		}	
		...
		public function add_listing_actions( $order ) {
			...
			<?php foreach( Banda_Delivery_Notes_Print::$templates as $template ) : ?>
				<?php $option = get_option( Banda_Delivery_Notes::$plugin_prefix . 'template_type_' . $template['type'] ); 
				if( $option ) : ?>
				
					<a href="<?php echo wcdn_get_print_link( $order->id, $template['type'] ); ?>" class="button tips print-preview-button <?php echo $template['type']; ?>" target="_blank" alt="<?php esc_attr_e( __( $template['labels']['print'], 'banda-delivery-notes' ) ); ?>" data-tip="<?php esc_attr_e( __( $template['labels']['print'], 'banda-delivery-notes' ) ); ?>">
						<?php _e( $template['labels']['print'], 'banda-delivery-notes' ); ?>
					</a>
					...
					jQuery(document).ready(function($) {		
						<?php foreach( Banda_Delivery_Notes_Print::$templates as $template ) : ?>
							...
							if( $option ) : ?>
								...
								$('<option>').val('wcdn_print_<?php echo $template['type']; ?>').attr('title', '<?php echo $template['type']; ?>').text('<?php echo esc_js( __( $template['labels']['print'], 'banda-delivery-notes' ) ); ?>').appendTo('select[name="action"]');
								$('<option>').val('wcdn_print_<?php echo $template['type']; ?>').attr('title', '<?php echo $template['type']; ?>').text('<?php echo esc_js( __( $template['labels']['print'], 'banda-delivery-notes' ) ); ?>').appendTo('select[name="action2"]');
			...
			if( $this->is_order_edit_page() ) {
				foreach( Banda_Delivery_Notes_Print::$templates as $template ) {
					if( isset( $_REQUEST['printed_' . $template['type']] ) ) {
						...
						<div id="banda-delivery-notes-bulk-print-message" class="updated">
							<p><?php _e( $message, 'banda-delivery-notes' ); ?> <a href="<?php echo urldecode( $_REQUEST['print_url'] ); ?>" target="_blank" class="print-preview-button" id="banda-delivery-notes-bulk-print-button"><?php _e( 'Print now', 'banda-delivery-notes' ) ?></a> <span class="print-preview-loading spinner"></span></p>
						</div>
		...
		public function add_box() {
			add_meta_box( 'banda-delivery-notes-box', __( 'Order Printing', 'banda-delivery-notes' ), array( $this, 'create_box_content' ), 'shop_order', 'side', 'low' );
		}
		...
		public function create_box_content() {
			...
			<div class="print-actions">
				<?php foreach( Banda_Delivery_Notes_Print::$templates as $template ) : ?>
					...
					if( $option ) : ?>
						...
						<a href="<?php echo wcdn_get_print_link( $post_id, $template['type'] ); ?>" class="button print-preview-button <?php echo $template['type']; ?>" target="_blank" alt="<?php esc_attr_e( __( $template['labels']['print'], 'banda-delivery-notes' ) ); ?>"><?php _e( $template['labels']['print'], 'banda-delivery-notes' ); ?></a>
			...
			<?php 
			$create_invoice_number = get_option( Banda_Delivery_Notes::$plugin_prefix . 'create_invoice_number' );
			$has_invoice_number = get_post_meta( $post_id, '_' . Banda_Delivery_Notes::$plugin_prefix . 'invoice_number', true );
			if( !empty( $create_invoice_number ) && $has_invoice_number ) : 
				...
				<ul class="print-info">
					<li><strong><?php _e( 'Invoice number: ', 'banda-delivery-notes' ); ?></strong> <?php echo $invoice_number; ?></li>
					<li><strong><?php _e( 'Invoice date: ', 'banda-delivery-notes' ); ?></strong> <?php echo $invoice_date; ?></li>
				</ul>
