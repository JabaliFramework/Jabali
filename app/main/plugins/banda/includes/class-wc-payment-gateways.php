...
class WC_Payment_Gateways {
	...
	public function init() {
		 ...
		 */
		if ( ! class_exists( 'WC_Gateway_Simplify_Commerce_Loader' ) && in_array( WC()->countries->get_base_country(), apply_filters( 'banda_gateway_simplify_commerce_supported_countries', array( 'US', 'IE' ) ) ) ) {
		...
		// Filter
		$load_gateways = apply_filters( 'banda_payment_gateways', $load_gateways );

		// Get sort order option
		$ordering  = (array) get_option( 'banda_gateway_order' );
		$order_end = 999;
	...
	public function get_available_payment_gateways() {
		...
		return apply_filters( 'banda_available_payment_gateways', $_available_gateways );
	}
	...
	public function process_admin_options() {
		...
		update_option( 'banda_gateway_order', $order );
	}
}
