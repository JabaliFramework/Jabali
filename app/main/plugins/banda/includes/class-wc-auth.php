...
class WC_Auth {
	...
	protected function get_permissions_in_scope( $scope ) {
		...
		switch ( $scope )  {
			case 'read' :
				$permissions[] = __( 'View coupons', 'banda' );
				$permissions[] = __( 'View customers', 'banda' );
				$permissions[] = __( 'View orders and sales reports', 'banda' );
				$permissions[] = __( 'View products', 'banda' );
			break;
			case 'write' :
				$permissions[] = __( 'Create webhooks', 'banda' );
				$permissions[] = __( 'Create coupons', 'banda' );
				$permissions[] = __( 'Create customers', 'banda' );
				$permissions[] = __( 'Create orders', 'banda' );
				$permissions[] = __( 'Create products', 'banda' );
			break;
			case 'read_write' :
				$permissions[] = __( 'Create webhooks', 'banda' );
				$permissions[] = __( 'View and manage coupons', 'banda' );
				$permissions[] = __( 'View and manage customers', 'banda' );
				$permissions[] = __( 'View and manage orders and sales reports', 'banda' );
				$permissions[] = __( 'View and manage products', 'banda' );
			break;
		}
		return apply_filters( 'banda_api_permissions_in_scope', $permissions, $scope );
	}
	...
	protected function make_validation() {
		...
		foreach ( $params as $param ) {
			if ( empty( $_REQUEST[ $param ] ) ) {
				throw new Exception( sprintf( __( 'Missing parameter %s', 'banda' ), $param ) );
			}
		}

		if ( ! in_array( $_REQUEST['scope'], array( 'read', 'write', 'read_write' ) ) ) {
			throw new Exception( sprintf( __( 'Invalid scope %s', 'banda' ), wc_clean( $_REQUEST['scope'] ) ) );
		}
		...
		foreach ( array( 'return_url', 'callback_url' ) as $param ) {
			...
			if ( false === filter_var( $param, FILTER_VALIDATE_URL ) ) {
				throw new Exception( sprintf( __( 'The %s is not a valid URL', 'banda' ), $param ) );
			}
		}
		...
		if ( 0 !== stripos( $callback_url, 'https://' ) ) {
			throw new Exception( __( 'The callback_url need to be over SSL', 'banda' ) );
		}
	}
	...
	protected function create_keys( $app_name, $app_user_id, $scope ) {
		...
		$description = sprintf( __( '%s - API %s (created on %s at %s).', 'banda' ), wc_clean( $app_name ), $this->get_i18n_scope( $scope ), date_i18n( wc_date_format() ), date_i18n( wc_time_format() ) );
	...
	protected function post_consumer_data( $consumer_data, $url ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			throw new Exception( __( 'An error occurred in the request and at the time were unable to send the consumer data', 'banda' ) );
		}
	...
	protected function auth_endpoint( $route ) {
		...
		try {
			if ( 'yes' !== get_option( 'banda_api_enabled' ) ) {
				throw new Exception( __( 'API disabled!', 'banda' ) );
			}
			...
			// Authorize endpoint
			} else if ( 'authorize' == $route && current_user_can( 'manage_banda' ) ) {
			...
			// Granted access endpoint
			} else if ( 'access_granted' == $route && current_user_can( 'manage_banda' ) ) {
				if ( ! isset( $_GET['wc_auth_nonce'] ) || ! wp_verify_nonce( $_GET['wc_auth_nonce'], 'wc_auth_grant_access' ) ) {
					throw new Exception( __( 'Invalid nonce verification', 'banda' ) );
				}
				...
				throw new Exception( __( 'You do not have permissions to access this page!', 'banda' ) );
			}
		} catch ( Exception $e ) {
			$this->maybe_delete_key( $consumer_data );

			wp_die( sprintf( __( 'Error: %s', 'banda' ), $e->getMessage() ), __( 'Access Denied', 'banda' ), array( 'response' => 401 ) );
		}
	}
	...
	private function maybe_delete_key( $key ) {
		...
		if ( isset( $key['key_id'] ) ) {
			$wpdb->delete( $wpdb->prefix . 'banda_api_keys', array( 'key_id' => $key['key_id'] ), array( '%d' ) );
		}
	}
}
