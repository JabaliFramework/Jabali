...
class WC_Shipping_Zone extends WC_Data {
	...
	public function __construct( $zone = null ) {
		if ( is_numeric( $zone ) && ! empty( $zone ) ) {
			...
			$this->set_id( 0 );
			$this->set_zone_name( __( 'Rest of the World', 'banda' ) );
			$this->read_zone_locations( 0 );
		} else {
			$this->set_zone_name( __( 'Zone', 'banda' ) );
		}
	}
	...
	public function create() {
		global $wpdb;
		$wpdb->insert( $wpdb->prefix . 'banda_shipping_zones', array(
	...
	public function read( $id ) {
		...
		if ( $zone_data = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}banda_shipping_zones WHERE zone_id = %d LIMIT 1;", $id ) ) ) {
	...
	public function update() {
		...
		if ( $this->get_id() ) {
			$wpdb->update( $wpdb->prefix . 'banda_shipping_zones', array(
				'zone_name'  => $this->get_zone_name(),
				'zone_order' => $this->get_zone_order(),
	...
	public function delete() {
		if ( $this->get_id() ) {
			global $wpdb;
			$wpdb->delete( $wpdb->prefix . 'banda_shipping_zone_methods', array( 'zone_id' => $this->get_id() ) );
			$wpdb->delete( $wpdb->prefix . 'banda_shipping_zone_locations', array( 'zone_id' => $this->get_id() ) );
			$wpdb->delete( $wpdb->prefix . 'banda_shipping_zones', array( 'zone_id' => $this->get_id() ) );
	...
	public function get_formatted_location( $max = 10 ) {
		...
		if ( sizeof( $location_parts ) > $max ) {
			...
			return sprintf( _n( '%s and %d other region', '%s and %d other regions', $remaining, 'banda' ), implode( ', ', array_splice( $location_parts, 0, $max ) ), $remaining );
		} elseif ( ! empty( $location_parts ) ) {
			return implode( ', ', $location_parts );
		} else {
			return __( 'Everywhere', 'banda' );
		}
	}
	...
	public function get_shipping_methods( $enabled_only = false ) {
		...
		$raw_methods_sql = $enabled_only ? "SELECT method_id, method_order, instance_id, is_enabled FROM {$wpdb->prefix}banda_shipping_zone_methods WHERE zone_id = %d AND is_enabled = 1 order by method_order ASC;" : "SELECT method_id, method_order, instance_id, is_enabled FROM {$wpdb->prefix}banda_shipping_zone_methods WHERE zone_id = %d order by method_order ASC;";
		...
		return apply_filters( 'banda_shipping_zone_shipping_methods', $methods, $raw_methods, $allowed_classes, $this );
	}
	...
	private function read_zone_locations( $zone_id ) {
		...
		if ( $locations = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}banda_shipping_zone_locations WHERE zone_id = %d;", $zone_id ) ) ) {
			foreach ( $locations as $location ) {
	...
	private function save_locations() {
		...
		foreach ( $this->get_zone_locations() as $location ) {
			$wpdb->insert( $wpdb->prefix . 'banda_shipping_zone_locations', array(
				'zone_id'       => $this->get_id(),
	...
	public function add_shipping_method( $type ) {
		...
		$wc_shipping     = WC_Shipping::instance();
		$allowed_classes = $wc_shipping->get_shipping_method_class_names();
		$count           = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->prefix}banda_shipping_zone_methods WHERE zone_id = %d", $this->get_id() ) );

		if ( in_array( $type, array_keys( $allowed_classes ) ) ) {
			$wpdb->insert(
				$wpdb->prefix . 'banda_shipping_zone_methods',
				array(
					'method_id'    => $type,
					'zone_id'      => $this->get_id(),
		...
		if ( $instance_id ) {
			do_action( 'banda_shipping_zone_method_added', $instance_id, $type, $this->get_id() );
		}
