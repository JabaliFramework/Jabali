 ...
 * Print class
 */
if ( ! class_exists( 'Banda_Delivery_Notes_Print' ) ) {

	class Banda_Delivery_Notes_Print {
		...
		public function __construct() {	
			...
			self::$templates = apply_filters( 'wcdn_template_registration', array(
				apply_filters( 'wcdn_template_registration_invoice', array(
					'type' => 'invoice',
					'labels' => array(
						'name' => __( 'Invoice', 'banda-delivery-notes' ),
						'name_plural' => __( 'Invoices', 'banda-delivery-notes' ),
						'print' => __( 'Print Invoice', 'banda-delivery-notes' ),
						'print_plural' => __( 'Print Invoices', 'banda-delivery-notes' ),
						'message' => __( 'Invoice created.', 'banda-delivery-notes' ),
						'message_plural' => __( 'Invoices created.', 'banda-delivery-notes' ),
						'setting' => __( 'Enable Invoices', 'banda-delivery-notes' )
					)
				) ),
				apply_filters( 'wcdn_template_registration_delivery_note', array(
					'type' => 'delivery-note',
					'labels' => array(
						'name' => __( 'Delivery Note', 'banda-delivery-notes' ),
						'name_plural' => __( 'Delivery Notes', 'banda-delivery-notes' ),
						'print' => __( 'Print Delivery Note', 'banda-delivery-notes' ),
						'print_plural' => __( 'Print Delivery Notes', 'banda-delivery-notes' ),
						'message' => __( 'Delivery Note created.', 'banda-delivery-notes' ),
						'message_plural' => __( 'Delivery Notes created.', 'banda-delivery-notes' ),
						'setting' => __( 'Enable Delivery Notes', 'banda-delivery-notes' )
					)
				) ),
				apply_filters( 'wcdn_template_registration_receipt', array(
					'type' => 'receipt',
					'labels' => array(
						'name' => __( 'Receipt', 'banda-delivery-notes' ),
						'name_plural' => __( 'Receipts', 'banda-delivery-notes' ),
						'print' => __( 'Print Receipt', 'banda-delivery-notes' ),
						'print_plural' => __( 'Print Receipts', 'banda-delivery-notes' ),
						'message' => __( 'Receipt created.', 'banda-delivery-notes' ),
						'message_plural' => __( 'Receipts created.', 'banda-delivery-notes' ),
						'setting' => __( 'Enable Receipts', 'banda-delivery-notes' )
					)
				) )
			) );
			...
			$this->template = array(
				'type' => 'order',
				'labels' => array(
					'name' => __( 'Order', 'banda-delivery-notes' ),
					'name_plural' => __( 'Orders', 'banda-delivery-notes' ),
					'print' => __( 'Print Order', 'banda-delivery-notes' ),
					'print_plural' => __( 'Print Orders', 'banda-delivery-notes' ),
					'message' => null,
					'message_plural' => null,
					'setting' => null
				)
			...
			$this->api_endpoints = array( 
				'print-order' => get_option( Banda_Delivery_Notes::$plugin_prefix . 'print_order_page_endpoint', 'print-order' )
			);
		...
		public function load_hooks() {	
			...
			$this->template_path_plugin = Banda_Delivery_Notes::$plugin_path . 'templates/' . $this->template_directory_name . '/';
			$this->template_url_plugin = Banda_Delivery_Notes::$plugin_url . 'templates/' . $this->template_directory_name . '/';
		...
		public function add_endpoints() {
			...
			if( get_transient( Banda_Delivery_Notes::$plugin_prefix . 'flush_rewrite_rules' ) == true ) {
				delete_transient( Banda_Delivery_Notes::$plugin_prefix . 'flush_rewrite_rules' );
				flush_rewrite_rules();
			}
		}
		...
		public function get_order_invoice_number( $order_id ) {						
			...
			$invoice_prefix = get_option( Banda_Delivery_Notes::$plugin_prefix . 'invoice_number_prefix' );
			$invoice_suffix = get_option( Banda_Delivery_Notes::$plugin_prefix . 'invoice_number_suffix' );
	
			// Add the invoice number to the order when it doesn't yet exist
			$meta_key = '_' . Banda_Delivery_Notes::$plugin_prefix . 'invoice_number';
			$meta_added = add_post_meta( $order_id, $meta_key, $invoice_prefix . ( $invoice_start + $invoice_counter ) . $invoice_suffix, true );
						
			// Update the total count
			if( $meta_added ) {
				update_option( Banda_Delivery_Notes::$plugin_prefix . 'invoice_number_counter', $invoice_counter + 1  );
			}
		...
		public function get_order_invoice_date( $order_id ) {	
			...
			$meta_key = '_' . Banda_Delivery_Notes::$plugin_prefix . 'invoice_date';
