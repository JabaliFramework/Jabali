...
class WC_AJAX {
	...
	public static function get_endpoint( $request = '' ) {
		return esc_url_raw( apply_filters( 'banda_ajax_get_endpoint', add_query_arg( 'wc-ajax', $request, remove_query_arg( array( 'remove_item', 'add-to-cart', 'added-to-cart' ) ) ), $request ) );
	}
	...
	/**
	 * Hook in methods - uses Jabali ajax handlers (admin-ajax).
	 */
	public static function add_ajax_events() {
		...
		foreach ( $ajax_events as $ajax_event => $nopriv ) {
			add_action( 'wp_ajax_banda_' . $ajax_event, array( __CLASS__, $ajax_event ) );

			if ( $nopriv ) {
				add_action( 'wp_ajax_nopriv_banda_' . $ajax_event, array( __CLASS__, $ajax_event ) );
	...
	public static function get_refreshed_fragments() {
		...
		$data = array(
			'fragments' => apply_filters( 'banda_add_to_cart_fragments', array(
					'div.widget_shopping_cart_content' => '<div class="widget_shopping_cart_content">' . $mini_cart . '</div>'
				)
			),
			'cart_hash' => apply_filters( 'banda_add_to_cart_hash', WC()->cart->get_cart_for_session() ? md5( json_encode( WC()->cart->get_cart_for_session() ) ) : '', WC()->cart->get_cart_for_session() )
		);
	...
	public static function remove_coupon() {
		...
		if ( ! isset( $coupon ) || empty( $coupon ) ) {
			wc_add_notice( __( 'Sorry there was a problem removing this coupon.', 'banda' ), 'error' );

		} else {

			WC()->cart->remove_coupon( $coupon );

			wc_add_notice( __( 'Coupon has been removed.', 'banda' ) );
		}
	...
	public static function update_order_review() {
		...
		if ( WC()->cart->is_empty() ) {
			$data = array(
				'fragments' => apply_filters( 'banda_update_order_review_fragments', array(
					'form.banda-checkout' => '<div class="banda-error">' . __( 'Sorry, your session has expired.', 'banda' ) . ' <a href="' . esc_url( wc_get_page_permalink( 'shop' ) ) . '" class="wc-backward">' . __( 'Return to shop', 'banda' ) . '</a></div>'
				) )
			);
		...
		do_action( 'banda_checkout_update_order_review', $_POST['post_data'] );
		...
		ob_start();
		banda_order_review();
		$banda_order_review = ob_get_clean();

		// Get checkout payment fragment
		ob_start();
		banda_checkout_payment();
		$banda_checkout_payment = ob_get_clean();
		...
		$data = array(
			...
			'fragments' => apply_filters( 'banda_update_order_review_fragments', array(
				'.banda-checkout-review-order-table' => $banda_order_review,
				'.banda-checkout-payment'            => $banda_checkout_payment
			) )
		);
	...
	public static function add_to_cart() {
		...
		$product_id        = apply_filters( 'banda_add_to_cart_product_id', absint( $_POST['product_id'] ) );
		...
		if ( $passed_validation && false !== WC()->cart->add_to_cart( $product_id, $quantity ) && 'publish' === $product_status ) {
			...
			do_action( 'banda_ajax_added_to_cart', $product_id );

			if ( get_option( 'banda_cart_redirect_after_add' ) == 'yes' ) {
			...
			$data = array(
				'error'       => true,
				'product_url' => apply_filters( 'banda_cart_redirect_after_error', get_permalink( $product_id ), $product_id )
			);
	...
	public static function feature_product() {
		if ( current_user_can( 'edit_products' ) && check_admin_referer( 'banda-feature-product' ) ) {
	...
	public static function mark_order_status() {
		if ( current_user_can( 'edit_shop_orders' ) && check_admin_referer( 'banda-mark-order-status' ) ) {
			...
			if ( wc_is_order_status( 'wc-' . $status ) && $order_id ) {
				...
				do_action( 'banda_order_edit_status', $order_id, $status );
			}
		}
	...
	public static function add_attribute() {
		...
		$attribute     = array(
			'name'         => $taxonomy,
			'value'        => '',
			'is_visible'   => apply_filters( 'banda_attribute_default_visibility', 1 ),
			'is_variation' => apply_filters( 'banda_attribute_default_is_variation', 0 ),
	...
	public static function add_variation() {
		...
		if ( $variation_id ) {
			...
			$tax_class_options[''] = __( 'Standard', 'banda' );
	...
	public static function revoke_access_to_download() {
		...
		$wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}banda_downloadable_product_permissions WHERE permission_id = %d;", $permission_id ) );

		do_action( 'banda_ajax_revoke_access_to_product_download', $download_id, $product_id, $order_id, $permission_id );
			...
			if ( ! empty( $files ) ) {
				foreach ( $files as $download_id => $file ) {
					if ( $inserted_id = wc_downloadable_file_permission( $download_id, $product_id, $order ) ) {
						...
						$download = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}banda_downloadable_product_permissions WHERE permission_id = %d", $inserted_id ) );
						...
						if ( isset( $file['name'] ) ) {
							...
							$file_count = sprintf( __( 'File %d', 'banda' ), $file_counter );
						}
	...
	public static function get_customer_details() {
		...
		$customer_data = apply_filters( 'banda_found_customer_details', $customer_data, $user_id, $type_to_load );
	...
	public static function add_order_item() {
		...
		if ( $item_id ) {
			...
			do_action( 'banda_ajax_add_order_item_meta', $item_id, $item );
		}
		...
		$item                    = apply_filters( 'banda_ajax_order_item', $item, $item_id );
		...
		if ( $order && ! empty( $order_items ) && sizeof( $order_item_ids ) > 0 ) {
			foreach ( $order_items as $item_id => $order_item ) {
				...
				if ( $_product->exists() && $_product->managing_stock() && isset( $order_item_qty[ $item_id ] ) && $order_item_qty[ $item_id ] > 0 ) {
					$stock_change = apply_filters( 'banda_reduce_order_stock_quantity', $order_item_qty[ $item_id ], $item_id );
					$new_stock    = $_product->reduce_stock( $stock_change );
					$item_name    = $_product->get_sku() ? $_product->get_sku() : $order_item['product_id'];
					$note         = sprintf( __( 'Item %s stock reduced from %s to %s.', 'banda' ), $item_name, $new_stock + $stock_change, $new_stock );
					$return[]     = $note;
					$order->add_order_note( $note );
					$order->send_stock_notifications( $_product, $new_stock, $order_item_qty[ $item_id ] );
				}
			}
			do_action( 'banda_reduce_order_stock', $order );
			if ( empty( $return ) ) {
				$return[] = __( 'No products had their stock reduced - they may not have stock management enabled.', 'banda' );
			}
			echo implode( ', ', $return );
		}
		die();
		...
		if ( $order && ! empty( $order_items ) && sizeof( $order_item_ids ) > 0 ) {
			foreach ( $order_items as $item_id => $order_item ) {
				...
				if ( $_product->exists() && $_product->managing_stock() && isset( $order_item_qty[ $item_id ] ) && $order_item_qty[ $item_id ] > 0 ) {
					...
					$stock_change = apply_filters( 'banda_restore_order_stock_quantity', $order_item_qty[ $item_id ], $item_id );
					$new_quantity = $_product->increase_stock( $stock_change );
					$item_name    = $_product->get_sku() ? $_product->get_sku(): $order_item['product_id'];
					$note         = sprintf( __( 'Item %s stock increased from %s to %s.', 'banda' ), $item_name, $old_stock, $new_quantity );
					$return[]     = $note;
					$order->add_order_note( $note );
				}
			}
			do_action( 'banda_restore_order_stock', $order );
			if ( empty( $return ) ) {
				$return[] = __( 'No products had their stock increased - they may not have stock management enabled.', 'banda' );
			}
			echo implode( ', ', $return );
		}
		die();
	...
	public static function add_order_item_meta() {
		...
		$meta_id = wc_add_order_item_meta( absint( $_POST['order_item_id'] ), __( 'Name', 'banda' ), __( 'Value', 'banda' ) );
	...
	public static function remove_order_item_meta() {
		...
		$wpdb->delete( "{$wpdb->prefix}banda_order_itemmeta", array(
	...
	public static function calc_line_taxes() {
		...
		// Action
		$items = apply_filters( 'banda_ajax_calc_line_taxes', $items, $order_id, $country, $_POST );
		...
		$items = apply_filters( 'banda_ajax_after_calc_line_taxes', $items, $order_id, $country, $_POST );
	...
	public static function add_order_note() {
		...
		if ( $post_id > 0 ) {
			...
			echo '</div><p class="meta"><a href="#" class="delete_note">'.__( 'Delete note', 'banda' ).'</a></p>';
			echo '</li>';
		}
	...
	public static function json_search_products( $term = '', $post_types = array( 'product' ) ) {
		...
		$found_products = apply_filters( 'banda_json_search_found_products', $found_products );
	...
	public static function json_search_grouped_products() {
		...
		$found_products = apply_filters( 'banda_json_search_found_grouped_products', $found_products );
	...
	public static function json_search_customers() {
		...
		$customers_query = new WP_User_Query( apply_filters( 'banda_json_search_customers_query', array(
			'fields'         => 'all',
		...
		$found_customers = apply_filters( 'banda_json_search_found_customers', $found_customers );
	...
	public static function product_ordering() {
		...
		do_action( 'banda_after_product_ordering' );
	...
	public static function refund_line_items() {
		...
		try {
			...
			if ( ! $refund_amount || $max_refund < $refund_amount || 0 > $refund_amount ) {
				throw new exception( __( 'Invalid refund amount', 'banda' ) );
			}
			...
			if ( $api_refund ) {
				...
				if ( isset( $payment_gateways[ $order->payment_method ] ) && $payment_gateways[ $order->payment_method ]->supports( 'refunds' ) ) {
					...
					do_action( 'banda_refund_processed', $refund, $result );

					if ( is_wp_error( $result ) ) {
						throw new Exception( $result->get_error_message() );
					} elseif ( ! $result ) {
						throw new Exception( __( 'Refund failed', 'banda' ) );
					}
				}
			}
			...
			foreach ( $line_item_qtys as $item_id => $qty ) {
				if ( $restock_refunded_items && $qty && isset( $order_items[ $item_id ] ) ) {
					...
					if ( $_product && $_product->exists() && $_product->managing_stock() ) {
						...
						$order->add_order_note( sprintf( __( 'Item #%s stock increased from %s to %s.', 'banda' ), $order_item['product_id'], $old_stock, $new_quantity ) );

						do_action( 'banda_restock_refunded_item', $_product->id, $old_stock, $new_quantity, $order, $_product );
					}
				}
			}
			...
			if ( $order->get_remaining_refund_amount() > 0 || ( $order->has_free_item() && $order->get_remaining_refund_items() > 0 ) ) {
				/**
				 * banda_order_partially_refunded.
				 *
				 * @since 2.4.0
				 * Note: 3rd arg was added in err. Kept for bw compat. 2.4.3.
				 */
				do_action( 'banda_order_partially_refunded', $order_id, $refund->id, $refund->id );
			} else {
				do_action( 'banda_order_fully_refunded', $order_id, $refund->id );

				$order->update_status( apply_filters( 'banda_order_fully_refunded_status', 'refunded', $order_id, $refund->id ) );
				$response_data['status'] = 'fully_refunded';
			}

			do_action( 'banda_order_refunded', $order_id, $refund->id );
	...
	public static function delete_refund() {
		...
		foreach ( $refund_ids as $refund_id ) {
			if ( $refund_id && 'shop_order_refund' === get_post_type( $refund_id ) ) {
				...
				do_action( 'banda_refund_deleted', $refund_id, $order_id );
			}
		}
		die();
	}
	...
	public static function rated() {
		...
		update_option( 'banda_admin_footer_text_rated', 1 );
		die();
	}
	...
	public static function update_api_key() {
		...
		try {
			if ( empty( $_POST['description'] ) ) {
				throw new Exception( __( 'Description is missing.', 'banda' ) );
			}
			if ( empty( $_POST['user'] ) ) {
				throw new Exception( __( 'User is missing.', 'banda' ) );
			}
			if ( empty( $_POST['permissions'] ) ) {
				throw new Exception( __( 'Permissions is missing.', 'banda' ) );
			}
			...
			if ( 0 < $key_id ) {
				...
				$data['message']         = __( 'API Key updated successfully.', 'banda' );
			} else {
				...
				$data['message']         = __( 'API Key generated successfully. Make sure to copy your new API keys now. You won\'t be able to see it again!', 'banda' );
				$data['revoke_url']      = '<a style="color: #a00; text-decoration: none;" href="' . esc_url( wp_nonce_url( add_query_arg( array( 'revoke-key' => $key_id ), admin_url( 'admin.php?page=wc-settings&tab=api&section=keys' ) ), 'revoke' ) ). '">' . __( 'Revoke Key', 'banda' ) . '</a>';
			}
	...
	public static function load_variations() {
		...
		// Get variations
		$args = apply_filters( 'banda_ajax_admin_get_variations_args', array(
			'post_type'      => 'product_variation',
	...
	public static function save_variations() {
		...
		do_action( 'banda_ajax_save_product_variations', $product_id );
		...
		if ( $errors = WC_Admin_Meta_Boxes::$meta_box_errors ) {
			...
			echo '<button type="button" class="notice-dismiss"><span class="screen-reader-text">' . __( 'Dismiss this notice.', 'banda' ) . '</span></button>';
			echo '</div>';

			delete_option( 'banda_meta_box_errors' );
		}
	...
	public static function bulk_edit_variations() {
		...
		if ( apply_filters( 'banda_bulk_edit_variations_need_children', true ) ) {
		...
		if ( method_exists( __CLASS__, "variation_bulk_action_$bulk_action" ) ) {
			...
			do_action( 'banda_bulk_edit_variations_default', $bulk_action, $data, $product_id, $variations );
		}

		do_action( 'banda_bulk_edit_variations', $bulk_action, $data, $product_id, $variations );
	...
	public static function shipping_zone_methods_save_changes() {
		...
		foreach ( $changes as $instance_id => $data ) {
			$method_id = $wpdb->get_var( $wpdb->prepare( "SELECT method_id FROM {$wpdb->prefix}banda_shipping_zone_methods WHERE instance_id = %d", $instance_id ) );

			if ( isset( $data['deleted'] ) ) {
				if ( $wpdb->delete( "{$wpdb->prefix}banda_shipping_zone_methods", array( 'instance_id' => $instance_id ) ) ) {
					do_action( 'banda_shipping_zone_method_deleted', $instance_id, $method_id, $zone_id );
				}
				continue;
			}
			...
			if ( isset( $method_data['method_order'] ) ) {
				$wpdb->update( "{$wpdb->prefix}banda_shipping_zone_methods", array( 'method_order' => absint( $method_data['method_order'] ) ), array( 'instance_id' => absint( $instance_id ) ) );
			}

			if ( isset( $method_data['enabled'] ) ) {
				$is_enabled = absint( 'yes' === $method_data['enabled'] );
				if ( $wpdb->update( "{$wpdb->prefix}banda_shipping_zone_methods", array( 'is_enabled' => $is_enabled ), array( 'instance_id' => absint( $instance_id ) ) ) ) {
					do_action( 'banda_shipping_zone_method_status_toggled', $instance_id, $method_id, $zone_id, $is_enabled );
				}
			}
		}
	...
	public static function shipping_classes_save_changes() {
		...
		foreach ( $changes as $term_id => $data ) {
			...
			do_action( 'banda_shipping_classes_save_class', $term_id, $data );
		}
