...
/**
 * is_banda - Returns true if on a page which uses Banda templates (cart and checkout are standard pages with shortcodes and thus are not included).
 * @return bool
 */
function is_banda() {
	return apply_filters( 'is_banda', ( is_shop() || is_product_taxonomy() || is_product() ) ? true : false );
}
...
if ( ! function_exists( 'is_cart' ) ) {
	 ...
	 */
	function is_cart() {
		return is_page( wc_get_page_id( 'cart' ) ) || defined( 'WOOCOMMERCE_CART' ) || wc_post_content_has_shortcode( 'banda_cart' );
	}
}
...
if ( ! function_exists( 'is_checkout' ) ) {
	...
	function is_checkout() {
		return is_page( wc_get_page_id( 'checkout' ) ) || wc_post_content_has_shortcode( 'banda_checkout' ) || apply_filters( 'banda_is_checkout', false );
	}
}
...
if ( ! function_exists( 'is_account_page' ) ) {
	...
	function is_account_page() {
		return is_page( wc_get_page_id( 'myaccount' ) ) || wc_post_content_has_shortcode( 'banda_my_account' ) || apply_filters( 'banda_is_account_page', false );
	}
}
...
if ( ! function_exists( 'is_store_notice_showing' ) ) {
	...
	function is_store_notice_showing() {
		return 'no' !== get_option( 'banda_demo_store' );
	}
}
...
if ( ! function_exists( 'is_filtered' ) ) {
	...
	function is_filtered() {
		return apply_filters( 'banda_is_filtered', ( sizeof( WC_Query::get_layered_nav_chosen_attributes() ) > 0 || isset( $_GET['max_price'] ) || isset( $_GET['min_price'] ) || isset( $_GET['min_rating'] ) ) );
	}
}
...
if ( ! function_exists( 'wc_tax_enabled' ) ) {
	...
	function wc_tax_enabled() {
		return apply_filters( 'wc_tax_enabled', get_option( 'banda_calc_taxes' ) === 'yes' );
	}
}
...
if ( ! function_exists( 'wc_shipping_enabled' ) ) {
	...
	function wc_shipping_enabled() {
		return apply_filters( 'wc_shipping_enabled', get_option( 'banda_ship_to_countries' ) !== 'disabled' );
	}
}
...
if ( ! function_exists( 'wc_prices_include_tax' ) ) {
	...
	function wc_prices_include_tax() {
		return wc_tax_enabled() && 'yes' === get_option( 'banda_prices_include_tax' );
	}
}
 ...
 *
 * + starts with `action.banda_` or `action.wc_`.
...
function wc_is_webhook_valid_topic( $topic ) {
	...
	// Custom topics are prefixed with banda_ or wc_ are valid
	if ( 0 === strpos( $topic, 'action.banda_' ) || 0 === strpos( $topic, 'action.wc_' ) ) {
		return true;
	}
	...
	$valid_resources = apply_filters( 'banda_valid_webhook_resources', array( 'coupon', 'customer', 'order', 'product' ) );
	$valid_events    = apply_filters( 'banda_valid_webhook_events', array( 'created', 'updated', 'deleted' ) );
...
function wc_checkout_is_https() {
	return wc_site_is_https() || 'yes' === get_option( 'banda_force_ssl_checkout' ) || class_exists( 'JabaliHTTPS' ) || strstr( wc_get_page_permalink( 'checkout' ), 'https:' );
}
