...
class WC_Shortcodes {
	...
	public static function init() {
		$shortcodes = array(
			...
			'banda_order_tracking' => __CLASS__ . '::order_tracking',
			'banda_cart'           => __CLASS__ . '::cart',
			'banda_checkout'       => __CLASS__ . '::checkout',
			'banda_my_account'     => __CLASS__ . '::my_account',
		);
		...
		add_shortcode( 'banda_messages', __CLASS__ . '::shop_messages' );
	}
	...
	private static function product_loop( $query_args, $atts, $loop_name ) {
		...
		$products                    = new WP_Query( apply_filters( 'banda_shortcode_products_query', $query_args, $atts, $loop_name ) );
		$columns                     = absint( $atts['columns'] );
		$banda_loop['columns'] = $columns;
		$banda_loop['name']    = $loop_name;
		...
		if ( $products->have_posts() ) {
			...
			<?php do_action( "banda_shortcode_before_{$loop_name}_loop" ); ?>
			...
			<?php do_action( "banda_shortcode_after_{$loop_name}_loop" ); ?>

			<?php
		} else {
			do_action( "banda_shortcode_{$loop_name}_loop_no_results" );
		}

		banda_reset_loop();
		wp_reset_postdata();

		return '<div class="banda columns-' . $columns . '">' . ob_get_clean() . '</div>';
	}
	...
	public static function product( $atts ) {
		...
		$products = new WP_Query( apply_filters( 'banda_shortcode_products_query', $args, $atts ) );
	...
	public static function product_add_to_cart( $atts ) {
		...
		ob_start();
		?>
		<p class="product banda add_to_cart_inline <?php echo esc_attr( $atts['class'] ); ?>"<?php echo $styles; ?>>

			<?php if ( 'true' == $atts['show_price'] ) : ?>
				<?php echo $product->get_price_html(); ?>
			<?php endif; ?>

			<?php banda_template_loop_add_to_cart( array( 'quantity' => $atts['quantity'] ) ); ?>
	...
	public static function product_page( $atts ) {
		...
		return '<div class="banda">' . ob_get_clean() . '</div>';
	}
	...
	/**
	 * banda_order_by_rating_post_clauses function.
	 *
	 * @param array $args
	 * @return array
	 */
