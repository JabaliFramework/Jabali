...
class WC_Product_Variable extends WC_Product {
	...
	public function add_to_cart_text() {
		$text = $this->is_purchasable() && $this->is_in_stock() ? __( 'Select options', 'banda' ) : __( 'Read more', 'banda' );

		return apply_filters( 'banda_product_add_to_cart_text', $text, $this );
	}
	...
	public function check_stock_status() {
		...
		} elseif ( $this->backorders_allowed() || $this->get_stock_quantity() > get_option( 'banda_notify_no_stock_amount' ) ) {
	...
	public function get_children( $visible_only = false ) {
		...
		if ( empty( $this->children ) || ! is_array( $this->children ) || ! isset( $this->children[ $key ] ) ) {
			...
			if ( $visible_only ) {
				if ( 'yes' === get_option( 'banda_hide_out_of_stock_items' ) ) {
			...
			$args                   = apply_filters( 'banda_variable_children_args', $args, $this, $visible_only );
			$this->children[ $key ] = get_posts( $args );

			set_transient( $transient_name, $this->children, DAY_IN_SECONDS * 30 );
		}

		return apply_filters( 'banda_get_children', $this->children[ $key ], $this, $visible_only );
	}
	...
	public function get_variation_regular_price( $min_or_max = 'min', $display = false ) {
		...
		return apply_filters( 'banda_get_variation_regular_price', $price, $this, $min_or_max, $display );
	}
	...
	public function get_variation_sale_price( $min_or_max = 'min', $display = false ) {
		...
		return apply_filters( 'banda_get_variation_sale_price', $price, $this, $min_or_max, $display );
	}
	...
	public function get_variation_price( $min_or_max = 'min', $display = false ) {
		...
		return apply_filters( 'banda_get_variation_price', $price, $this, $min_or_max, $display );
	}

	/**
	 * Get an array of all sale and regular prices from all variations. This is used for example when displaying the price range at variable product level or seeing if the variable product is on sale.
	 *
	 * Can be filtered by plugins which modify costs, but otherwise will include the raw meta costs unlike get_price() which runs costs through the banda_get_price filter.
	...
	public function get_variation_prices( $display = false ) {
		 ...
		 */
		if ( $display ) {
			$price_hash = array( get_option( 'banda_tax_display_shop', 'excl' ), WC_Tax::get_rates() );
		} else {
			$price_hash = array( false );
		}

		$filter_names = array( 'banda_variation_prices_price', 'banda_variation_prices_regular_price', 'banda_variation_prices_sale_price' );
		...
		$price_hash = md5( json_encode( apply_filters( 'banda_get_variation_prices_hash', $price_hash, $this, $display ) ) );
		...
		if ( empty( $this->prices_array[ $price_hash ] ) ) {
			...
			if ( empty( $prices_array[ $price_hash ] ) ) {
				...
				foreach ( $variation_ids as $variation_id ) {
					if ( $variation = $this->get_child( $variation_id ) ) {
						$price         = apply_filters( 'banda_variation_prices_price', $variation->price, $variation, $this );
						$regular_price = apply_filters( 'banda_variation_prices_regular_price', $variation->regular_price, $variation, $this );
						$sale_price    = apply_filters( 'banda_variation_prices_sale_price', $variation->sale_price, $variation, $this );
						...
						if ( $display ) {
							if ( 'incl' === get_option( 'banda_tax_display_shop' ) ) {
			 ...
			 */
			$this->prices_array[ $price_hash ] = apply_filters( 'banda_variation_prices', $prices_array[ $price_hash ], $this, $display );
		}
	...
	public function get_price_html( $price = '' ) {
		...
		if ( $this->get_price() === '' || empty( $prices['price'] ) ) {
			$price = apply_filters( 'banda_variable_empty_price_html', '', $this );
		} else {
			...
			if ( $this->is_on_sale() ) {
				...
				$regular_price     = $min_regular_price !== $max_regular_price ? sprintf( _x( '%1$s&ndash;%2$s', 'Price range: from-to', 'banda' ), wc_price( $min_regular_price ), wc_price( $max_regular_price ) ) : wc_price( $min_regular_price );
				$price             = apply_filters( 'banda_variable_sale_price_html', $this->get_price_html_from_to( $regular_price, $price ) . $this->get_price_suffix(), $this );
			} elseif ( $is_free ) {
				$price = apply_filters( 'banda_variable_free_price_html', __( 'Free!', 'banda' ), $this );
			} else {
				$price = apply_filters( 'banda_variable_price_html', $price . $this->get_price_suffix(), $this );
			}
		}
	...
	public function get_variation_default_attributes() {
		...
		return apply_filters( 'banda_product_default_attributes', array_filter( (array) maybe_unserialize( $default ) ), $this );
	}
	...
	public function get_available_variations() {
		...
		foreach ( $this->get_children() as $child_id ) {
			...
			if ( empty( $variation->variation_id ) || ( 'yes' === get_option( 'banda_hide_out_of_stock_items' ) && ! $variation->is_in_stock() ) ) {
				continue;
			}

			// Filter 'banda_hide_invisible_variations' to optionally hide invisible variations (disabled variations and variations with empty price)
			if ( apply_filters( 'banda_hide_invisible_variations', false, $this->id, $variation ) && ! $variation->variation_is_visible() ) {
				continue;
			}
	...
	public function get_available_variation( $variation ) {
		...
		$availability_html = apply_filters( 'banda_stock_html', $availability_html, $availability['availability'], $variation );

		return apply_filters( 'banda_available_variation', array(
			...
			'price_html'             => apply_filters( 'banda_show_variation_price', $variation->get_price() === "" || $this->get_variation_price( 'min' ) !== $this->get_variation_price( 'max' ), $this, $variation ) ? '<span class="price">' . $variation->get_price_html() . '</span>' : '',
			'availability_html'      => $availability_html,
			'sku'                    => $variation->get_sku(),
			'weight'                 => $variation->get_weight() . ' ' . esc_attr( get_option('banda_weight_unit' ) ),
	...
	public static function sync( $product_id ) {
		...
		if ( ! $children ) {
			...
			if ( is_admin() && 'publish' === get_post_status( $product_id ) ) {
				WC_Admin_Meta_Boxes::add_error( __( 'This variable product has no active variations. Add or enable variations to allow this product to be purchased.', 'banda' ) );
			}
		...
		// Loop the variations
			...
			foreach ( array( 'price', 'regular_price', 'sale_price' ) as $price_type ) {
				foreach ( $children as $child_id ) {
					...
					if ( 'yes' === get_option( 'banda_hide_out_of_stock_items' ) ) {
						...
						if ( $stock !== "" && $stock <= get_option( 'banda_notify_no_stock_amount' ) ) {
							continue;
						}
					}
			...
			do_action( 'banda_variable_product_sync', $product_id, $children );
		}
	}
}
