...
class WC_Download_Handler {
	...
	public static function init() {
		...
		add_action( 'banda_download_file_redirect', array( __CLASS__, 'download_file_redirect' ), 10, 2 );
		add_action( 'banda_download_file_xsendfile', array( __CLASS__, 'download_file_xsendfile' ), 10, 2 );
		add_action( 'banda_download_file_force', array( __CLASS__, 'download_file_force' ), 10, 2 );
	}
	...
	public static function download_product() {
		...
		if ( $_product && $download_data ) {
			...
			do_action( 'banda_download_product', $download_data->user_email, $download_data->order_key, $download_data->product_id, $download_data->user_id, $download_data->download_id, $download_data->order_id );
			...
			self::download_error( __( 'Invalid download link.', 'banda' ) );
		}
	}
	...
	private static function get_download_data( $args = array() ) {
		...
		$query = "SELECT * FROM " . $wpdb->prefix . "banda_downloadable_product_permissions ";
	...
	private static function check_order_is_valid( $download_data ) {
		if ( $download_data->order_id && ( $order = wc_get_order( $download_data->order_id ) ) && ! $order->is_download_permitted() ) {
			self::download_error( __( 'Invalid order.', 'banda' ), '', 403 );
		}
	}
	...
	private static function check_downloads_remaining( $download_data ) {
		if ( '0' == $download_data->downloads_remaining  ) {
			self::download_error( __( 'Sorry, you have reached your download limit for this file', 'banda' ), '', 403 );
		}
	}
	...
	private static function check_download_expiry( $download_data ) {
		if ( $download_data->access_expires > 0 && strtotime( $download_data->access_expires ) < strtotime( 'midnight', current_time( 'timestamp' ) ) ) {
			self::download_error( __( 'Sorry, this download has expired', 'banda' ), '', 403 );
		}
	}
	...
	private static function check_download_login_required( $download_data ) {
		if ( $download_data->user_id && 'yes' === get_option( 'banda_downloads_require_login' ) ) {
			if ( ! is_user_logged_in() ) {
				if ( wc_get_page_id( 'myaccount' ) ) {
					wp_safe_redirect( add_query_arg( 'wc_error', urlencode( __( 'You must be logged in to download files.', 'banda' ) ), wc_get_page_permalink( 'myaccount' ) ) );
					exit;
				} else {
					self::download_error( __( 'You must be logged in to download files.', 'banda' ) . ' <a href="' . esc_url( wp_login_url( wc_get_page_permalink( 'myaccount' ) ) ) . '" class="wc-forward">' . __( 'Login', 'banda' ) . '</a>', __( 'Log in to Download Files', 'banda' ), 403 );
				}
			} elseif ( ! current_user_can( 'download_file', $download_data ) ) {
				self::download_error( __( 'This is not your download link.', 'banda' ), '', 403 );
			}
		}
	}
	...
	public static function count_download( $download_data ) {
		...
		$wpdb->update(
			$wpdb->prefix . 'banda_downloadable_product_permissions',
			array(
	...
	public static function download( $file_path, $product_id ) {
		if ( ! $file_path ) {
			self::download_error( __( 'No file defined', 'banda' ) );
		}
		...
		$filename             = apply_filters( 'banda_file_download_filename', $filename, $product_id );
		$file_download_method = apply_filters( 'banda_file_download_method', get_option( 'banda_file_download_method', 'force' ), $product_id );

		// Add action to prevent issues in IE
		add_action( 'nocache_headers', array( __CLASS__, 'ie_nocache_headers_fix' ) );

		// Trigger download via one of the methods
		do_action( 'banda_download_file_' . $file_download_method, $file_path, $filename );
	}
	...
	public static function download_file_force( $file_path, $filename ) {
		...
		if ( ! self::readfile_chunked( $parsed_file_path['file_path'] ) ) {
			if ( $parsed_file_path['remote_file'] ) {
				...
				self::download_error( __( 'File not found', 'banda' ) );
			}
		}
	...
	private static function download_error( $message, $title = '', $status = 404 ) {
		if ( ! strstr( $message, '<a ' ) ) {
			$message .= ' <a href="' . esc_url( wc_get_page_permalink( 'shop' ) ) . '" class="wc-forward">' . __( 'Go to shop', 'banda' ) . '</a>';
		}
