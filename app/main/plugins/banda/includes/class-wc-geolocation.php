...
class WC_Geolocation {
	...
	public static function init() {
		...
		if ( 'geolocation' === get_option( 'banda_default_customer_address' ) || apply_filters( 'banda_geolocation_update_database_periodically', false ) ) {
			add_action( 'banda_geoip_updater', array( __CLASS__, 'update_database' ) );
		}
		add_filter( 'pre_update_option_banda_default_customer_address', array( __CLASS__, 'maybe_update_database' ), 10, 2 );
	}
	...
	public static function get_external_ip_address() {
		...
		if ( false === $external_ip_address ) {
			...
			$ip_lookup_services      = apply_filters( 'banda_geolocation_ip_lookup_apis', self::$ip_lookup_apis );
			...
			foreach ( $ip_lookup_services_keys as $service_name ) {
				...
				if ( ! is_wp_error( $response ) && $response['body'] ) {
					$external_ip_address = apply_filters( 'banda_geolocation_ip_lookup_api_response', wc_clean( $response['body'] ), $service_name );
					break;
				}
			}
	...
	public static function geolocate_ip( $ip_address = '', $fallback = true, $api_fallback = true ) {
		...
		$country_code = apply_filters( 'banda_geolocate_ip', false, $ip_address, $fallback, $api_fallback );
	...
	public static function get_local_database_path( $version = 'v4' ) {
		...
		return apply_filters( 'banda_geolocation_local_database_path', $upload_dir['basedir'] . '/GeoIP' . $version . '.dat', $version );
	}

	/**
	 * Update geoip database. Adapted from https://jabali.github.io/plugins/geoip-detect/.
	 */
	...
	private static function geolocate_via_api( $ip_address ) {
		...
		if ( false === $country_code ) {
			$geoip_services      = apply_filters( 'banda_geolocation_geoip_apis', self::$geoip_apis );
			...
			foreach ( $geoip_services_keys as $service_name ) {
				...
				if ( ! is_wp_error( $response ) && $response['body'] ) {
					switch ( $service_name ) {
						...
						break;
						default :
							$country_code = apply_filters( 'banda_geolocation_geoip_response_' . $service_name, '', $response['body'] );
						break;
					}
