...
class WC_Email_Cancelled_Order extends WC_Email {
	...
	public function __construct() {
		...
		$this->title            = __( 'Cancelled order', 'banda' );
		$this->description      = __( 'Cancelled order emails are sent to chosen recipient(s) when orders have been marked cancelled (if they were previously processing or on-hold).', 'banda' );
		$this->heading          = __( 'Cancelled order', 'banda' );
		$this->subject          = __( '[{site_title}] Cancelled order ({order_number})', 'banda' );
		$this->template_html    = 'emails/admin-cancelled-order.php';
		$this->template_plain   = 'emails/plain/admin-cancelled-order.php';

		// Triggers for this email
		add_action( 'banda_order_status_pending_to_cancelled_notification', array( $this, 'trigger' ) );
		add_action( 'banda_order_status_on-hold_to_cancelled_notification', array( $this, 'trigger' ) );
