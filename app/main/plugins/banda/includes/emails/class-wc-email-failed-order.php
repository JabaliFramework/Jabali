...
class WC_Email_Failed_Order extends WC_Email {
	...
	public function __construct() {
		...
		$this->title            = __( 'Failed order', 'banda' );
		$this->description      = __( 'Failed order emails are sent to chosen recipient(s) when orders have been marked failed (if they were previously processing or on-hold).', 'banda' );
		$this->heading          = __( 'Failed order', 'banda' );
		$this->subject          = __( '[{site_title}] Failed order ({order_number})', 'banda' );
		$this->template_html    = 'emails/admin-failed-order.php';
		$this->template_plain   = 'emails/plain/admin-failed-order.php';

		// Triggers for this email
		add_action( 'banda_order_status_pending_to_failed_notification', array( $this, 'trigger' ) );
		add_action( 'banda_order_status_on-hold_to_failed_notification', array( $this, 'trigger' ) );
