...
class WC_Email_Customer_Reset_Password extends WC_Email {
	...
	public function __construct() {
		...
		$this->title            = __( 'Reset password', 'banda' );
		$this->description      = __( 'Customer "reset password" emails are sent when customers reset their passwords.', 'banda' );
		$this->customer_email   = true;

		$this->template_html    = 'emails/customer-reset-password.php';
		$this->template_plain   = 'emails/plain/customer-reset-password.php';

		$this->subject          = __( 'Password Reset for {site_title}', 'banda');
		$this->heading          = __( 'Password Reset Instructions', 'banda');

		// Trigger
		add_action( 'banda_reset_password_notification', array( $this, 'trigger' ), 10, 2 );
