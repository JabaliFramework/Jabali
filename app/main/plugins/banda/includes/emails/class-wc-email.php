 ...
 * Email Class
 *
 * Banda Email Class which is extended by specific email template classes to add emails to Banda
 *
 * @class       WC_Email
 * @version     2.5.0
...
class WC_Email extends WC_Settings_API {
	...
	public function __construct() {
		...
		// Save settings hook
		add_action( 'banda_update_options_email_' . $this->id, array( $this, 'process_admin_options' ) );
	...
	public function format_string( $string ) {
		return str_replace( apply_filters( 'banda_email_format_string_find', $this->find, $this ), apply_filters( 'banda_email_format_string_replace', $this->replace, $this ), __( $string ) );
	}
	...
	public function get_subject() {
		return apply_filters( 'banda_email_subject_' . $this->id, $this->format_string( $this->subject ), $this->object );
	}
	...
	public function get_heading() {
		return apply_filters( 'banda_email_heading_' . $this->id, $this->format_string( $this->heading ), $this->object );
	}
	...
	public function get_recipient() {
		$recipient  = apply_filters( 'banda_email_recipient_' . $this->id, $this->recipient, $this->object );
	...
	public function get_headers() {
		return apply_filters( 'banda_email_headers', "Content-Type: " . $this->get_content_type() . "\r\n", $this->id, $this->object );
	}
	...
	public function get_attachments() {
		return apply_filters( 'banda_email_attachments', array(), $this->id, $this->object );
	}
	...
	public function get_title() {
		return apply_filters( 'banda_email_title', $this->title, $this );
	}
	...
	public function get_description() {
		return apply_filters( 'banda_email_description', $this->description, $this );
	}
	...
	public function get_option( $key, $empty_value = null ) {
		...
		return apply_filters( 'banda_email_get_option', $value, $this, $value, $key, $empty_value );
	}
	...
	public function is_enabled() {
		return apply_filters( 'banda_email_enabled_' . $this->id, 'yes' === $this->enabled, $this->object );
	}
	...
	public function style_inline( $content ) {
		...
		if ( in_array( $this->get_content_type(), array( 'text/html', 'multipart/alternative' ) ) && class_exists( 'DOMDocument' ) ) {
			...
			$css = apply_filters( 'banda_email_styles', ob_get_clean() );
	...
	public function get_from_name() {
		$from_name = apply_filters( 'banda_email_from_name', get_option( 'banda_email_from_name' ), $this );
	...
	public function get_from_address() {
		$from_address = apply_filters( 'banda_email_from_address', get_option( 'banda_email_from_address' ), $this );
	...
	public function send( $to, $subject, $message, $headers, $attachments ) {
		...
		$message = apply_filters( 'banda_mail_content', $this->style_inline( $message ) );
	...
	public function get_email_type_options() {
		$types = array( 'plain' => __( 'Plain text', 'banda' ) );

		if ( class_exists( 'DOMDocument' ) ) {
			$types['html']      = __( 'HTML', 'banda' );
			$types['multipart'] = __( 'Multipart', 'banda' );
		}
	...
	protected function save_template( $template_code, $template_path ) {
		if ( current_user_can( 'edit_themes' ) && ! empty( $template_code ) && ! empty( $template_path ) ) {
			$saved  = false;
			$file   = get_stylesheet_directory() . '/banda/' . $template_path;
			...
			if ( ! $saved ) {
				$redirect = add_query_arg( 'wc_error', urlencode( __( 'Could not write to template file.', 'banda' ) ) );
	...
	public function get_theme_template_file( $template ) {
		return get_stylesheet_directory() . '/' . apply_filters( 'banda_template_directory', 'banda', $template ) . '/' . $template;
	}
		...
		if ( $template = $this->get_template( $template_type ) ) {
			if ( ! empty( $template ) ) {
				...
				if ( wp_mkdir_p( dirname( $theme_file ) ) && ! file_exists( $theme_file ) ) {
					...
					$template_file = apply_filters( 'banda_locate_core_template', $core_file, $template, $this->template_base );

					// Copy template file
					copy( $template_file, $theme_file );

					/**
					 * banda_copy_email_template action hook.
					 *
					 * @param string $template_type The copied template type
					 * @param string $email The email object
					 */
					do_action( 'banda_copy_email_template', $template_type, $this );

					echo '<div class="updated"><p>' . __( 'Template file copied to theme.', 'banda' ) . '</p></div>';
				}
			}
		}
	}
		...
		if ( $template = $this->get_template( $template_type ) ) {
			...
			if ( ! empty( $template ) ) {
				...
				if ( file_exists( $theme_file ) ) {
					...
					/**
					 * banda_delete_email_template action hook.
					 *
					 * @param string $template The deleted template type
					 * @param string $email The email object
					 */
					do_action( 'banda_delete_email_template', $template_type, $this );

					echo '<div class="updated"><p>' . __( 'Template file deleted from theme.', 'banda' ) . '</p></div>';
				}
			}
		}
	}
	...
	protected function admin_actions() {
		...
		if (
			...
			if ( empty( $_GET['_wc_email_nonce'] ) || ! wp_verify_nonce( $_GET['_wc_email_nonce'], 'banda_email_template_nonce' ) ) {
				wp_die( __( 'Action failed. Please refresh the page and retry.', 'banda' ) );
			}

			if ( ! current_user_can( 'edit_themes' ) ) {
				wp_die( __( 'Cheatin&#8217; huh?', 'banda' ) );
			}
	...
	public function admin_options() {
		...
		$this->admin_actions();
		?>
		<h2><?php echo esc_html( $this->get_title() ); ?> <?php wc_back_link( __( 'Return to Emails', 'banda' ), admin_url( 'admin.php?page=wc-settings&tab=email' ) ); ?></h2>

		<?php echo wpautop( wp_kses_post( $this->get_description() ) ); ?>

		<?php
			/**
			 * banda_email_settings_before action hook.
			 * @param string $email The email object
			 */
			do_action( 'banda_email_settings_before', $this );
		?>
		...
		<?php
			/**
			 * banda_email_settings_after action hook.
			 * @param string $email The email object
			 */
			do_action( 'banda_email_settings_after', $this );
		?>
		...
		<?php if ( current_user_can( 'edit_themes' ) && ( ! empty( $this->template_html ) || ! empty( $this->template_plain ) ) ) { ?>
			...
			<?php
				$templates = array(
					'template_html'  => __( 'HTML template', 'banda' ),
					'template_plain' => __( 'Plain text template', 'banda' )
				);
				...
				foreach ( $templates as $template_type => $title ) :
					...
					$core_file     = $this->template_base . $template;
					$template_file = apply_filters( 'banda_locate_core_template', $core_file, $template, $this->template_base );
					$template_dir  = apply_filters( 'banda_template_directory', 'banda', $template );
					?>
					<div class="template <?php echo $template_type; ?>">
						...
						<?php if ( file_exists( $local_file ) ) { ?>
							...
							<p>
								...
								<?php if ( is_writable( $local_file ) ) : ?>
									<a href="<?php echo esc_url( wp_nonce_url( remove_query_arg( array( 'move_template', 'saved' ), add_query_arg( 'delete_template', $template_type ) ), 'banda_email_template_nonce', '_wc_email_nonce' ) ); ?>" class="delete_template button"><?php _e( 'Delete template file', 'banda' ); ?></a>
								<?php endif; ?>

								<?php printf( __( 'This template has been overridden by your theme and can be found in: <code>%s</code>.', 'banda' ), trailingslashit( basename( get_stylesheet_directory() ) ) . $template_dir . '/' . $template ); ?>
							</p>
						...
						<?php } elseif ( file_exists( $template_file ) ) { ?>
							...
							<p>
								...
								<?php if ( ( is_dir( get_stylesheet_directory() . '/' . $template_dir . '/emails/' ) && is_writable( get_stylesheet_directory() . '/' . $template_dir . '/emails/' ) ) || is_writable( get_stylesheet_directory() ) ) { ?>
									<a href="<?php echo esc_url( wp_nonce_url( remove_query_arg( array( 'delete_template', 'saved' ), add_query_arg( 'move_template', $template_type ) ), 'banda_email_template_nonce', '_wc_email_nonce' ) ); ?>" class="button"><?php _e( 'Copy file to theme', 'banda' ); ?></a>
								<?php } ?>

								<?php printf( __( 'To override and edit this email template copy <code>%s</code> to your theme folder: <code>%s</code>.', 'banda' ), plugin_basename( $template_file ) , trailingslashit( basename( get_stylesheet_directory() ) ) . $template_dir . '/' . $template ); ?>
							</p>
						...
						<?php } else { ?>
							...
							<p><?php _e( 'File was not found.', 'banda' ); ?></p>
			...
			wc_enqueue_js( "
				...
				var view = '" . esc_js( __( 'View template', 'banda' ) ) . "';
				var hide = '" . esc_js( __( 'Hide template', 'banda' ) ) . "';
				...
				jQuery( 'a.delete_template' ).click( function() {
					if ( window.confirm('" . esc_js( __( 'Are you sure you want to delete this template file?', 'banda' ) ) . "') ) {
						return true;
					}
