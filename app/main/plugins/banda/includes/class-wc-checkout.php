 ...
 * Checkout
 *
 * The Banda checkout class handles the checkout process, collecting user data and processing the payment.
 *
 * @class 		WC_Checkout
 * @version		2.1.0
...
class WC_Checkout {
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, __( 'Cheatin&#8217; huh?', 'banda' ), '2.1' );
	}
	...
	public function __construct () {
		add_action( 'banda_checkout_billing', array( $this,'checkout_form_billing' ) );
		add_action( 'banda_checkout_shipping', array( $this,'checkout_form_shipping' ) );

		$this->enable_signup         = get_option( 'banda_enable_signup_and_login_from_checkout' ) == 'yes' ? true : false;
		$this->enable_guest_checkout = get_option( 'banda_enable_guest_checkout' ) == 'yes' ? true : false;
		...
		if ( get_option( 'banda_registration_generate_username' ) == 'no' ) {
			$this->checkout_fields['account']['account_username'] = array(
				'type' 			=> 'text',
				'label' 		=> __( 'Account username', 'banda' ),
				'required'      => true,
				'placeholder' 	=> _x( 'Username', 'placeholder', 'banda' )
			);
		}

		if ( get_option( 'banda_registration_generate_password' ) == 'no' ) {
			$this->checkout_fields['account']['account_password'] = array(
				'type' 				=> 'password',
				'label' 			=> __( 'Account password', 'banda' ),
				'required'          => true,
				'placeholder' 		=> _x( 'Password', 'placeholder', 'banda' )
			);
		}
		...
		$this->checkout_fields['order']	= array(
			'order_comments' => array(
				'type' => 'textarea',
				'class' => array('notes'),
				'label' => __( 'Order Notes', 'banda' ),
				'placeholder' => _x('Notes about your order, e.g. special notes for delivery.', 'placeholder', 'banda')
			)
		);

		$this->checkout_fields = apply_filters( 'banda_checkout_fields', $this->checkout_fields );

		do_action( 'banda_checkout_init', $this );
	}
	...
	public function create_order() {
		...
		if ( $order_id = apply_filters( 'banda_create_order', null, $this ) ) {
			return $order_id;
		}
		...
		try {
			...
			$order_data = array(
				'status'        => apply_filters( 'banda_default_order_status', 'pending' ),
			...
			if ( $order_id && $order_data['cart_hash'] === get_post_meta( $order_id, '_cart_hash', true ) && ( $order = wc_get_order( $order_id ) ) && $order->has_status( array( 'pending', 'failed' ) ) ) {
				...
				if ( is_wp_error( $order ) ) {
					throw new Exception( sprintf( __( 'Error %d: Unable to create order. Please try again.', 'banda' ), 522 ) );
				} else {
					$order->remove_order_items();
					do_action( 'banda_resume_order', $order_id );
				}
				...
				if ( is_wp_error( $order ) ) {
					throw new Exception( sprintf( __( 'Error %d: Unable to create order. Please try again.', 'banda' ), 520 ) );
				} elseif ( false === $order ) {
					throw new Exception( sprintf( __( 'Error %d: Unable to create order. Please try again.', 'banda' ), 521 ) );
				} else {
					$order_id = $order->id;
					do_action( 'banda_new_order', $order_id );
				}
			}
			...
			foreach ( WC()->cart->get_cart() as $cart_item_key => $values ) {
				...
				if ( ! $item_id ) {
					throw new Exception( sprintf( __( 'Error %d: Unable to create order. Please try again.', 'banda' ), 525 ) );
				}

				// Allow plugins to add order item meta
				do_action( 'banda_add_order_item_meta', $item_id, $values, $cart_item_key );
			}
			...
			foreach ( WC()->cart->get_fees() as $fee_key => $fee ) {
				...
				if ( ! $item_id ) {
					throw new Exception( sprintf( __( 'Error %d: Unable to create order. Please try again.', 'banda' ), 526 ) );
				}

				// Allow plugins to add order item meta to fees
				do_action( 'banda_add_order_fee_meta', $order_id, $item_id, $fee, $fee_key );
			}
			...
			foreach ( WC()->shipping->get_packages() as $package_key => $package ) {
				if ( isset( $package['rates'][ $this->shipping_methods[ $package_key ] ] ) ) {
					...
					if ( ! $item_id ) {
						throw new Exception( sprintf( __( 'Error %d: Unable to create order. Please try again.', 'banda' ), 527 ) );
					}

					// Allows plugins to add order item meta to shipping
					do_action( 'banda_add_shipping_order_item', $order_id, $item_id, $package_key );
				}
			}

			// Store tax rows
			foreach ( array_keys( WC()->cart->taxes + WC()->cart->shipping_taxes ) as $tax_rate_id ) {
				if ( $tax_rate_id && ! $order->add_tax( $tax_rate_id, WC()->cart->get_tax_amount( $tax_rate_id ), WC()->cart->get_shipping_tax_amount( $tax_rate_id ) ) && apply_filters( 'banda_cart_remove_taxes_zero_rate_id', 'zero-rated' ) !== $tax_rate_id ) {
					throw new Exception( sprintf( __( 'Error %d: Unable to create order. Please try again.', 'banda' ), 528 ) );
				}
			}
			...
			foreach ( WC()->cart->get_coupons() as $code => $coupon ) {
				if ( ! $order->add_coupon( $code, WC()->cart->get_coupon_discount_amount( $code ), WC()->cart->get_coupon_discount_tax_amount( $code ) ) ) {
					throw new Exception( sprintf( __( 'Error %d: Unable to create order. Please try again.', 'banda' ), 529 ) );
				}
			}
			...
			// Update user meta
			if ( $this->customer_id ) {
				if ( apply_filters( 'banda_checkout_update_customer_data', true, $this ) ) {
				...
				do_action( 'banda_checkout_update_user_meta', $this->customer_id, $this->posted );
			}

			// Let plugins add meta
			do_action( 'banda_checkout_update_order_meta', $order_id, $this->posted );
	...
	public function process_checkout() {
		try {
			if ( empty( $_POST['_wpnonce'] ) || ! wp_verify_nonce( $_POST['_wpnonce'], 'banda-process_checkout' ) ) {
			...
			do_action( 'banda_before_checkout_process' );

			if ( WC()->cart->is_empty() ) {
				throw new Exception( sprintf( __( 'Sorry, your session has expired. <a href="%s" class="wc-backward">Return to shop</a>', 'banda' ), esc_url( wc_get_page_permalink( 'shop' ) ) ) );
			}

			do_action( 'banda_checkout_process' );
			...
			foreach ( $this->checkout_fields as $fieldset_key => $fieldset ) {
				...
				foreach ( $fieldset as $key => $field ) {
					...
					$this->posted[ $key ] = apply_filters( 'banda_process_checkout_' . sanitize_title( $field['type'] ) . '_field', $this->posted[ $key ] );
					$this->posted[ $key ] = apply_filters( 'banda_process_checkout_field_' . $key, $this->posted[ $key ] );

					// Validation: Required fields
					if ( isset( $field['required'] ) && $field['required'] && ( ! isset( $this->posted[ $key ] ) || "" === $this->posted[ $key ] ) ) {
						switch ( $fieldset_key ) {
							case 'shipping' :
								$field_label = sprintf( _x( 'Shipping %s', 'Shipping FIELDNAME', 'banda' ), $field['label'] );
							break;
							case 'billing' :
								$field_label = sprintf( _x( 'Billing %s', 'Billing FIELDNAME', 'banda' ), $field['label'] );
							break;
							default :
								$field_label = $field['label'];
							break;
						}
						wc_add_notice( apply_filters( 'banda_checkout_required_field_notice', sprintf( _x( '%s is a required field.', 'FIELDNAME is a required field.', 'banda' ), '<strong>' . $field_label . '</strong>' ), $field_label ), 'error' );
					}
									...
									case 'state' :
										...
										if ( ! empty( $valid_states ) && is_array( $valid_states ) && sizeof( $valid_states ) > 0 ) {
											if ( ! in_array( $this->posted[ $key ], array_keys( $valid_states ) ) ) {
												wc_add_notice( '<strong>' . $field['label'] . '</strong> ' . __( 'is not valid. Please enter one of the following:', 'banda' ) . ' ' . implode( ', ', $valid_states ), 'error' );
											}
										}
									break;
								}
			...
			// Terms
			if ( ! isset( $_POST['banda_checkout_update_totals'] ) && empty( $this->posted['terms'] ) && wc_get_page_id( 'terms' ) > 0 && apply_filters( 'banda_checkout_show_terms', true ) ) {
			...
			if ( WC()->cart->needs_shipping() ) {
				...
				if ( empty( $shipping_country ) ) {
					wc_add_notice( __( 'Please enter an address to continue.', 'banda' ), 'error' );
				...
				foreach ( $packages as $i => $package ) {
					if ( ! isset( $package['rates'][ $this->shipping_methods[ $i ] ] ) ) {
						wc_add_notice( __( 'No shipping method has been selected. Please double check your address, or contact us if you need any help.', 'banda' ), 'error' );
			...
			do_action( 'banda_after_checkout_validation', $this->posted );

			if ( ! isset( $_POST['banda_checkout_update_totals'] ) && wc_notice_count( 'error' ) == 0 ) {

				// Customer accounts
				$this->customer_id = apply_filters( 'banda_checkout_customer_id', get_current_user_id() );
				...
				if ( ! is_user_logged_in() && ( $this->must_create_account || ! empty( $this->posted['createaccount'] ) ) ) {
					...
					if ( $this->posted['billing_first_name'] && apply_filters( 'banda_checkout_update_customer_data', true, $this ) ) {
						$userdata = array(
							'ID'           => $this->customer_id,
						...
						wp_update_user( apply_filters( 'banda_checkout_customer_userdata', $userdata, $this ) );
					}
				}
				...
				do_action( 'banda_checkout_order_processed', $order_id, $this->posted );
				...
				if ( WC()->cart->needs_payment() ) {
					...
					if ( isset( $result['result'] ) && 'success' === $result['result'] ) {
						...
						$result = apply_filters( 'banda_payment_successful_result', $result, $order_id );
					...
					if ( is_ajax() ) {
						wp_send_json( array(
							'result' 	=> 'success',
							'redirect'  => apply_filters( 'banda_checkout_no_payment_needed_redirect', $return_url, $order )
						) );
					} else {
						wp_safe_redirect(
							apply_filters( 'banda_checkout_no_payment_needed_redirect', $return_url, $order )
						);
						exit;
					}
	...
	public function get_value( $input ) {
		if ( ! empty( $_POST[ $input ] ) ) {
			...
			$value = apply_filters( 'banda_checkout_get_value', null, $input );
