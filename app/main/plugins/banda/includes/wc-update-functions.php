	...
	if ( $existing_file_paths ) {
		...
		foreach( $existing_file_paths as $existing_file_path ) {
			...
			if ( ! empty( $old_file_path ) ) {
				...
				$wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->prefix}banda_downloadable_product_permissions SET download_id = %s WHERE product_id = %d", md5( $old_file_path ), $existing_file_path->post_id ) );
...
function wc_update_200_permalinks() {
	...
	if ( empty( $permalinks ) && $shop_page_id > 0 ) {
		...
		$category_base 	= get_option('banda_prepend_shop_page_to_urls') == "yes" ? trailingslashit( $base_slug ) : '';
		$category_slug 	= get_option('banda_product_category_slug') ? get_option('banda_product_category_slug') : _x( 'product-category', 'slug', 'banda' );
		$tag_slug 		= get_option('banda_product_tag_slug') ? get_option('banda_product_tag_slug') : _x( 'product-tag', 'slug', 'banda' );

		if ( 'yes' == get_option('banda_prepend_shop_page_to_products') ) {
			$product_base = trailingslashit( $base_slug );
		} else {
			if ( ( $product_slug = get_option('banda_product_slug') ) !== false && ! empty( $product_slug ) ) {
				$product_base = trailingslashit( $product_slug );
			} else {
				$product_base = trailingslashit( _x('product', 'slug', 'banda') );
			}
		}

		if ( get_option('banda_prepend_category_to_products') == 'yes' )
		...
		update_option( 'banda_permalinks', $permalinks );
	}
}

function wc_update_200_subcat_display() {
	// Update subcat display settings
	if ( get_option( 'banda_shop_show_subcategories' ) == 'yes' ) {
		if ( get_option( 'banda_hide_products_when_showing_subcategories' ) == 'yes' ) {
			update_option( 'banda_shop_page_display', 'subcategories' );
		} else {
			update_option( 'banda_shop_page_display', 'both' );
		}
	}

	if ( get_option( 'banda_show_subcategories' ) == 'yes' ) {
		if ( get_option( 'banda_hide_products_when_showing_subcategories' ) == 'yes' ) {
			update_option( 'banda_category_archive_display', 'subcategories' );
		} else {
			update_option( 'banda_category_archive_display', 'both' );
		}
	}
}
...
function wc_update_200_taxrates() {
	...
	// Update tax rates
	$loop = 0;
	$tax_rates = get_option( 'banda_tax_rates' );
	...
	$local_tax_rates = get_option( 'banda_local_tax_rates' );
		...
		foreach ( $local_tax_rates as $tax_rate ) {
			...
			if ( $tax_rate['locations'] ) {
				foreach ( $tax_rate['locations'] as $location ) {
					...
					$wpdb->insert(
						$wpdb->prefix . "banda_tax_rate_locations",
						array(
	...
	update_option( 'banda_tax_rates_backup', $tax_rates );
	update_option( 'banda_local_tax_rates_backup', $local_tax_rates );
	delete_option( 'banda_tax_rates' );
	delete_option( 'banda_local_tax_rates' );
}
		 	...
		 	if ( $item_id ) {
				...
				if ( sizeof( $meta_rows ) > 0 ) {
					$wpdb->query( $wpdb->prepare( "
						INSERT INTO {$wpdb->prefix}banda_order_itemmeta ( order_item_id, meta_key, meta_value )
...
function wc_update_200_images() {
	...
	foreach ( array( 'catalog', 'single', 'thumbnail' ) as $value ) {
		...
		$old_settings = array_filter( array(
			'width' => get_option( 'banda_' . $value . '_image_width' ),
			'height' => get_option( 'banda_' . $value . '_image_height' ),
			'crop' => get_option( 'banda_' . $value . '_image_crop' )
		) );

		if ( ! empty(  $old_settings  ) && update_option( 'shop_' . $value . '_image_size', $old_settings ) ){

			delete_option( 'banda_' . $value . '_image_width' );
			delete_option( 'banda_' . $value . '_image_height' );
			delete_option( 'banda_' . $value . '_image_crop' );
...
function wc_update_210_remove_pages() {
	...
	wp_trash_post( get_option('banda_pay_page_id') );
	wp_trash_post( get_option('banda_thanks_page_id') );
	...
	wp_trash_post( get_option('banda_lost_password_page_id') );
}
...
function wc_update_220_shipping() {
	$banda_ship_to_destination = 'shipping';

	if ( get_option( 'banda_ship_to_billing_address_only' ) === 'yes' ) {
		$banda_ship_to_destination = 'billing_only';
	} elseif ( get_option( 'banda_ship_to_billing' ) === 'yes' ) {
		$banda_ship_to_destination = 'billing';
	}

	add_option( 'banda_ship_to_destination', $banda_ship_to_destination, '', 'no' );
}
...
function wc_update_220_attributes() {
	...
	$attribute_taxonomies = $wpdb->get_results( "SELECT * FROM " . $wpdb->prefix . "banda_attribute_taxonomies" );
	...
	foreach ( $attribute_taxonomies as $attribute_taxonomy ) {
		...
		if ( $sanitized_attribute_name !== $attribute_taxonomy->attribute_name ) {
			if ( ! $wpdb->get_var( $wpdb->prepare( "SELECT 1=1 FROM {$wpdb->prefix}banda_attribute_taxonomies WHERE attribute_name = %s;", $sanitized_attribute_name ) ) ) {
				// Update attribute
				$wpdb->update(
...
function wc_update_230_options() {
	...
	// To prevent taxes being hidden when using a default 'no address' in a store with tax inc prices, set the banda_default_customer_address to use the store base address by default
	if ( '' === get_option( 'banda_default_customer_address', false ) && wc_prices_include_tax() ) {
		update_option( 'banda_default_customer_address', 'base' );
	}
}
...
function wc_update_240_options() {
	 ...
	 */
	update_option( 'banda_calc_discounts_sequentially', 'yes' );
}
...
function wc_update_240_shipping_methods() {
	...
	$shipping_methods = array(
		'banda_flat_rates'                        => new WC_Shipping_Legacy_Flat_Rate(),
		'banda_international_delivery_flat_rates' => new WC_Shipping_Legacy_International_Delivery()
	);
...
function wc_update_240_api_keys() {
	 ...
	 */
	$api_users = $wpdb->get_results( "SELECT user_id FROM $wpdb->usermeta WHERE meta_key = 'banda_api_consumer_key'" );
	$apps_keys = array();
	...
	foreach ( $api_users as $_user ) {
		...
		$apps_keys[] = array(
			'user_id'         => $user->ID,
			'permissions'     => $user->banda_api_key_permissions,
			'consumer_key'    => wc_api_hash( $user->banda_api_consumer_key ),
			'consumer_secret' => $user->banda_api_consumer_secret,
			'truncated_key'   => substr( $user->banda_api_consumer_secret, -7 )
		);
	}
	...
	if ( ! empty( $apps_keys ) ) {
		...
		foreach ( $api_users as $_user ) {
			...
			delete_user_meta( $user_id, 'banda_api_consumer_key' );
			delete_user_meta( $user_id, 'banda_api_consumer_secret' );
			delete_user_meta( $user_id, 'banda_api_key_permissions' );
		}
	}
}
...
function wc_update_240_webhooks() {
	...
	/**
	 * Webhooks.
	 * Make sure order.update webhooks get the banda_order_edit_status hook.
	 */
...
function wc_update_240_refunds() {
	...
	remove_all_actions( 'banda_order_status_refunded_notification' );
	remove_all_actions( 'banda_order_partially_refunded_notification' );
	remove_action( 'banda_order_status_refunded', array( 'WC_Emails', 'send_transactional_email' ) );
	remove_action( 'banda_order_partially_refunded', array( 'WC_Emails', 'send_transactional_email' ) );
	...
	foreach ( $refunded_orders as $refunded_order ) {
		...
		if ( $order_total > $refunded_total ) {
			wc_create_refund( array(
				...
				'reason'     => __( 'Order Fully Refunded', 'banda' ),
...
function wc_update_241_variations() {
	...
	// Select variations that don't have any _stock_status implemented on Banda 2.2
	$update_variations = $wpdb->get_results( "
...
function wc_update_250_currency() {
	...
	$current_currency = get_option( 'banda_currency' );

	if ( 'KIP' === $current_currency ) {
		update_option( 'banda_currency', 'LAK' );
	}
...
function wc_update_260_options() {
	/**
	 * banda_calc_shipping option has been removed in 2.6
	 */
	if ( 'no' === get_option( 'banda_calc_shipping' ) ) {
...
function wc_update_260_termmeta() {
	global $wpdb;
	/**
	 * Migrate term meta to Jabali tables
	 */
	if ( get_option( 'db_version' ) >= 34370 && $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->prefix}banda_termmeta';" ) ) {
		if ( $wpdb->query( "INSERT INTO {$wpdb->termmeta} ( term_id, meta_key, meta_value ) SELECT banda_term_id, meta_key, meta_value FROM {$wpdb->prefix}banda_termmeta;" ) ) {
...
function wc_update_260_zones() {
	 ...
	 */
	if ( $wpdb->get_var( "SHOW COLUMNS FROM `{$wpdb->prefix}banda_shipping_zones` LIKE 'zone_enabled';" ) ) {
		$wpdb->query( "ALTER TABLE {$wpdb->prefix}banda_shipping_zones CHANGE `zone_type` `zone_type` VARCHAR(40) NOT NULL DEFAULT '';" );
		$wpdb->query( "ALTER TABLE {$wpdb->prefix}banda_shipping_zones CHANGE `zone_enabled` `zone_enabled` INT(1) NOT NULL DEFAULT 1;" );
	}
}
...
function wc_update_260_zone_methods() {
	...
	/**
	 * Shipping zones in WC 2.6.0 use a table named banda_shipping_zone_methods.
	 * Migrate the old data out of banda_shipping_zone_shipping_methods into the new table and port over any known options (used by table rates and flat rate boxes).
	 */
	if ( $wpdb->get_var( "SHOW TABLES LIKE '{$wpdb->prefix}banda_shipping_zone_shipping_methods';" ) ) {
		$old_methods = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}banda_shipping_zone_shipping_methods;" );

		if ( $old_methods ) {
			$max_new_id = $wpdb->get_var( "SELECT MAX(instance_id) FROM {$wpdb->prefix}banda_shipping_zone_methods" );
			$max_old_id = $wpdb->get_var( "SELECT MAX(shipping_method_id) FROM {$wpdb->prefix}banda_shipping_zone_shipping_methods" );

			// Avoid ID conflicts
			$wpdb->query( $wpdb->prepare( "ALTER TABLE {$wpdb->prefix}banda_shipping_zone_methods AUTO_INCREMENT = %d;", max( $max_new_id, $max_old_id ) + 1 ) );
			...
			foreach ( $old_methods as $old_method ) {
				$wpdb->insert( $wpdb->prefix . 'banda_shipping_zone_methods', array(
				...
				// Move main settings
				$older_settings_key = 'banda_' . $old_method->shipping_method_type . '-' . $old_method->shipping_method_id . '_settings';
				$old_settings_key   = 'banda_' . $old_method->shipping_method_type . '_' . $old_method->shipping_method_id . '_settings';
				add_option( 'banda_' . $old_method->shipping_method_type . '_' . $new_instance_id . '_settings', get_option( $old_settings_key, get_option( $older_settings_key ) ) );

				// Handling for table rate and flat rate box shipping.
				if ( 'table_rate' === $old_method->shipping_method_type ) {
					// Move priority settings
					add_option( 'banda_table_rate_default_priority_' . $new_instance_id, get_option( 'banda_table_rate_default_priority_' . $old_method->shipping_method_id ) );
					add_option( 'banda_table_rate_priorities_' . $new_instance_id, get_option( 'banda_table_rate_priorities_' . $old_method->shipping_method_id ) );

					// Move rates
					$wpdb->update(
						$wpdb->prefix . 'banda_shipping_table_rates',
						array(
					...
					$wpdb->update(
						$wpdb->prefix . 'banda_shipping_flat_rate_boxes',
						array(
							'shipping_method_id' => $new_instance_id
						),
						array(
			...
			update_option( 'banda_updated_instance_ids', $changes );
			do_action( 'banda_updated_instance_ids', $changes );
		}
	}

	// Change ranges used to ...
	$wpdb->query( "UPDATE {$wpdb->prefix}banda_shipping_zone_locations SET location_code = REPLACE( location_code, '-', '...' );" );
}
...
function wc_update_260_refunds() {
	 ...
	 */
	$wpdb->query( "
	UPDATE {$wpdb->prefix}banda_order_itemmeta as item_meta
	LEFT JOIN {$wpdb->prefix}banda_order_items as items ON item_meta.order_item_id = items.order_item_id
