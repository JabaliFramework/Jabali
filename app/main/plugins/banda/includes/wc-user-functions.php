 ...
 *
 * Note: get_option( 'banda_lock_down_admin', true ) is a deprecated option here for backwards compat. Defaults to true.
 *
 * @access public
...
function wc_disable_admin_bar( $show_admin_bar ) {
	if ( apply_filters( 'banda_disable_admin_bar', get_option( 'banda_lock_down_admin', 'yes' ) === 'yes' ) && ! ( current_user_can( 'edit_posts' ) || current_user_can( 'manage_banda' ) ) ) {
...
if ( ! function_exists( 'wc_create_new_customer' ) ) {
	...
	function wc_create_new_customer( $email, $username = '', $password = '' ) {
		...
		if ( empty( $email ) || ! is_email( $email ) ) {
			return new WP_Error( 'registration-error-invalid-email', __( 'Please provide a valid email address.', 'banda' ) );
		}

		if ( email_exists( $email ) ) {
			return new WP_Error( 'registration-error-email-exists', __( 'An account is already registered with your email address. Please login.', 'banda' ) );
		}

		// Handle username creation.
		if ( 'no' === get_option( 'banda_registration_generate_username' ) || ! empty( $username ) ) {
			$username = sanitize_user( $username );

			if ( empty( $username ) || ! validate_username( $username ) ) {
				return new WP_Error( 'registration-error-invalid-username', __( 'Please enter a valid account username.', 'banda' ) );
			}

			if ( username_exists( $username ) ) {
				return new WP_Error( 'registration-error-username-exists', __( 'An account is already registered with that username. Please choose another.', 'banda' ) );
			}
		} else {
		...
		if ( 'yes' === get_option( 'banda_registration_generate_password' ) && empty( $password ) ) {
			$password           = wp_generate_password();
			$password_generated = true;
		} elseif ( empty( $password ) ) {
			return new WP_Error( 'registration-error-missing-password', __( 'Please enter an account password.', 'banda' ) );
		} else {
		...
		do_action( 'banda_register_post', $username, $email, $errors );

		$errors = apply_filters( 'banda_registration_errors', $errors, $username, $email );

		if ( $errors->get_error_code() ) {
			return $errors;
		}

		$new_customer_data = apply_filters( 'banda_new_customer_data', array(
			'user_login' => $username,
			'user_pass'  => $password,
			'user_email' => $email,
			'role'       => 'customer',
		...
		if ( is_wp_error( $customer_id ) ) {
			return new WP_Error( 'registration-error', '<strong>' . __( 'ERROR', 'banda' ) . '</strong>: ' . __( 'Couldn&#8217;t register you&hellip; please contact us if you continue to have problems.', 'banda' ) );
		}

		do_action( 'banda_created_customer', $customer_id, $new_customer_data, $password_generated );
...
function wc_update_new_customer_past_orders( $customer_id ) {
	...
	if ( ! empty( $customer_orders ) ) {
		foreach ( $customer_orders as $order_id ) {
			...
			do_action( 'banda_update_new_customer_past_order', $order_id, $customer );
...
add_action( 'banda_order_status_completed', 'wc_paying_customer' );
...
function wc_customer_bought_product( $customer_email, $user_id, $product_id ) {
	...
	if ( false === ( $result = get_transient( $transient_name ) ) ) {
		...
		$result = $wpdb->get_col( "
			...
			INNER JOIN {$wpdb->prefix}banda_order_items AS i ON p.ID = i.order_id
			INNER JOIN {$wpdb->prefix}banda_order_itemmeta AS im ON i.order_item_id = im.order_item_id
...
function wc_get_customer_download_permissions( $customer_id ) {
	...
	return apply_filters( 'banda_permission_list', $wpdb->get_results(
		$wpdb->prepare( "
			SELECT * FROM {$wpdb->prefix}banda_downloadable_product_permissions as permissions
			WHERE user_id = %d
	...
	if ( $results ) {
		foreach ( $results as $result ) {
			...
			$download_name = apply_filters(
				'banda_downloadable_product_name',
...
function wc_meta_update_last_update_time( $meta_id, $user_id, $meta_key, $_meta_value ) {
	$keys_to_track = apply_filters( 'banda_user_last_update_fields', array( 'first_name', 'last_name' ) );
	$update_time   = false;
...
function wc_get_customer_saved_methods_list( $customer_id ) {
	return apply_filters( 'banda_saved_payment_methods_list', array(), $customer_id );
}
