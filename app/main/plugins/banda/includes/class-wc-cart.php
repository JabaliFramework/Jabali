 ...
 * Banda cart
 *
 * The Banda cart class stores cart data and active coupons as well as handling customer sessions and some cart related urls.
...
class WC_Cart {
	...
	public function __construct() {
		...
		add_action( 'banda_add_to_cart', array( $this, 'calculate_totals' ), 20, 0 );
		add_action( 'banda_applied_coupon', array( $this, 'calculate_totals' ), 20, 0 );
	}
	...
	public function __get( $key ) {
		switch ( $key ) {
			...
			case 'round_at_subtotal' :
				return 'yes' === get_option( 'banda_tax_round_at_subtotal' );
			break;
			case 'tax_display_cart' :
				return get_option( 'banda_tax_display_cart' );
			break;
			case 'dp' :
			...
			return $this->tax;
			case 'discount_total':
				_deprecated_argument( 'WC_Cart->discount_total', '2.3', 'After tax coupons are no longer supported. For more information see: https://banda.jabali.com/2014/12/upcoming-coupon-changes-in-banda-2-3/' );
			return 0;
		}
	}

	/**
	 * Loads the cart data from the PHP session during Jabali init and hooks in other methods.
	 */
	public function init() {
		$this->get_cart_from_session();

		add_action( 'banda_check_cart_items', array( $this, 'check_cart_items' ), 1 );
		add_action( 'banda_check_cart_items', array( $this, 'check_cart_coupons' ), 1 );
		add_action( 'banda_after_checkout_validation', array( $this, 'check_customer_coupons' ), 1 );
	}
	...
	public function maybe_set_cart_cookies() {
		if ( ! headers_sent() && did_action( 'wp_loaded' ) ) {
			...
			} elseif ( isset( $_COOKIE['banda_items_in_cart'] ) ) {
	...
	private function set_cart_cookies( $set = true ) {
		if ( $set ) {
			wc_setcookie( 'banda_items_in_cart', 1 );
			wc_setcookie( 'banda_cart_hash', md5( json_encode( $this->get_cart_for_session() ) ) );
		} elseif ( isset( $_COOKIE['banda_items_in_cart'] ) ) {
			wc_setcookie( 'banda_items_in_cart', 0, time() - HOUR_IN_SECONDS );
			wc_setcookie( 'banda_cart_hash', '', time() - HOUR_IN_SECONDS );
		}
		do_action( 'banda_set_cart_cookies', $set );
	}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function get_cart_from_session() {
			...
			if ( is_null( $cart ) && ( $saved_cart = get_user_meta( get_current_user_id(), '_banda_persistent_cart', true ) ) ) {
				...
				foreach ( $cart as $key => $values ) {
					...
					if ( ! empty( $_product ) && $_product->exists() && $values['quantity'] > 0 ) {
						...
						if ( ! $_product->is_purchasable() ) {
							...
							$update_cart_session = true;
							wc_add_notice( sprintf( __( '%s has been removed from your cart because it can no longer be purchased. Please contact us if you need assistance.', 'banda' ), $_product->get_title() ), 'error' );
							do_action( 'banda_remove_cart_item_from_session', $key, $values );

						} else {

							// Put session data into array. Run through filter so other plugins can load their own session data
							$session_data = array_merge( $values, array( 'data' => $_product ) );
							$this->cart_contents[ $key ] = apply_filters( 'banda_get_cart_item_from_session', $session_data, $values, $key );
			...
			// Trigger action
			do_action( 'banda_cart_loaded_from_session', $this );
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function persistent_cart_update() {
			update_user_meta( get_current_user_id(), '_banda_persistent_cart', array(
		...
		public function persistent_cart_destroy() {
			delete_user_meta( get_current_user_id(), '_banda_persistent_cart' );
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function get_cart_contents_count() {
			return apply_filters( 'banda_cart_contents_count', array_sum( wp_list_pluck( $this->get_cart(), 'quantity' ) ) );
		}
		...
		public function get_cart_contents_weight() {
			...
			return apply_filters( 'banda_cart_contents_weight', $weight );
		}
		...
		*/
		...
		public function check_cart_item_validity() {
			...
			foreach ( $this->get_cart() as $cart_item_key => $values ) {
				...
				if ( ! $_product || ! $_product->exists() || 'trash' === $_product->post->post_status ) {
					...
					$return = new WP_Error( 'invalid', __( 'An item which is no longer available was removed from your cart.', 'banda' ) );
				}
			}
		...
		public function check_cart_item_stock() {
			...
			foreach ( $this->get_cart() as $cart_item_key => $values ) {
				...
				if ( ! $_product->is_in_stock() ) {
					$error->add( 'out-of-stock', sprintf(__( 'Sorry, "%s" is not in stock. Please edit your cart and try again. We apologise for any inconvenience caused.', 'banda' ), $_product->get_title() ) );
					return $error;
				}
				...
				if ( ! $_product->has_enough_stock( $check_qty ) ) {
					$error->add( 'out-of-stock', sprintf(__( 'Sorry, we do not have enough "%s" in stock to fulfill your order (%s in stock). Please edit your cart and try again. We apologise for any inconvenience caused.', 'banda' ), $_product->get_title(), $_product->get_stock_quantity() ) );
					return $error;
				}
				 ...
				 */
				if ( get_option( 'banda_hold_stock_minutes' ) > 0 && ! $_product->backorders_allowed() ) {
					...
					$held_stock = $wpdb->get_var(
						$wpdb->prepare( "
							...
							LEFT JOIN {$wpdb->prefix}banda_order_items as order_items ON posts.ID = order_items.order_id
							LEFT JOIN {$wpdb->prefix}banda_order_itemmeta as order_item_meta ON order_items.order_item_id = order_item_meta.order_item_id
							LEFT JOIN {$wpdb->prefix}banda_order_itemmeta as order_item_meta2 ON order_items.order_item_id = order_item_meta2.order_item_id
					...
					if ( $not_enough_stock ) {
						$error->add( 'out-of-stock', sprintf(__( 'Sorry, we do not have enough "%s" in stock to fulfill your order right now. Please try again in %d minutes or edit your cart and try again. We apologise for any inconvenience caused.', 'banda' ), $_product->get_title(), get_option( 'banda_hold_stock_minutes' ) ) );
						return $error;
					}
				}
			}
		...
		public function get_item_data( $cart_item, $flat = false ) {
			...
			if ( ! empty( $cart_item['data']->variation_id ) && is_array( $cart_item['variation'] ) ) {
				...
				foreach ( $cart_item['variation'] as $name => $value ) {
					...
					// If this is a custom option slug, get the options name
						...
						$value              = apply_filters( 'banda_variation_option_name', $value );
			...
			$item_data = apply_filters( 'banda_get_item_data', $item_data, $cart_item );
		...
		public function get_remove_url( $cart_item_key ) {
			...
			return apply_filters( 'banda_get_remove_url', $cart_page_url ? wp_nonce_url( add_query_arg( 'remove_item', $cart_item_key, $cart_page_url ), 'banda-cart' ) : '' );
		}
		...
		public function get_undo_url( $cart_item_key ) {
			...
			return apply_filters( 'banda_get_undo_url', $cart_page_url ? wp_nonce_url( add_query_arg( $query_args, $cart_page_url ), 'banda-cart' ) : '', $cart_item_key );
		}
		...
		public function get_cart() {
			if ( ! did_action( 'wp_loaded' ) ) {
				_doing_it_wrong( __FUNCTION__, __( 'Get cart should not be called before the wp_loaded action.', 'banda' ), '2.3' );
			}
			if ( ! did_action( 'banda_cart_loaded_from_session' ) ) {
				$this->get_cart_from_session();
			}
		...
		public function get_taxes() {
			...
			return apply_filters( 'banda_cart_get_taxes', $taxes, $this );
		}
		...
		public function get_tax_totals() {
			...
			foreach ( $taxes as $key => $tax ) {
				...
				if ( $code || $key === apply_filters( 'banda_cart_remove_taxes_zero_rate_id', 'zero-rated' ) ) {
			...
			if ( apply_filters( 'banda_cart_hide_zero_taxes', true ) ) {
				$amounts    = array_filter( wp_list_pluck( $tax_totals, 'amount' ) );
				$tax_totals = array_intersect_key( $tax_totals, $amounts );
			}

			return apply_filters( 'banda_cart_tax_totals', $tax_totals, $this );
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function generate_cart_id( $product_id, $variation_id = 0, $variation = array(), $cart_item_data = array() ) {
			...
			return apply_filters( 'banda_cart_id', md5( implode( '_', $id_parts ) ), $product_id, $variation_id, $variation, $cart_item_data );
		}
		...
		public function add_to_cart( $product_id = 0, $quantity = 1, $variation_id = 0, $variation = array(), $cart_item_data = array() ) {
			...
			try {
				...
				$cart_item_data = (array) apply_filters( 'banda_add_cart_item_data', $cart_item_data, $product_id, $variation_id );
				...
				if ( $product_data->is_sold_individually() ) {
					$quantity         = apply_filters( 'banda_add_to_cart_sold_individually_quantity', 1, $quantity, $product_id, $variation_id, $cart_item_data );
					$in_cart_quantity = $cart_item_key ? $this->cart_contents[ $cart_item_key ]['quantity'] : 0;

					if ( $in_cart_quantity > 0 ) {
						throw new Exception( sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', wc_get_cart_url(), __( 'View Cart', 'banda' ), sprintf( __( 'You cannot add another &quot;%s&quot; to your cart.', 'banda' ), $product_data->get_title() ) ) );
					}
				}

				// Check product is_purchasable
				if ( ! $product_data->is_purchasable() ) {
					throw new Exception( __( 'Sorry, this product cannot be purchased.', 'banda' ) );
				}

				// Stock check - only check if we're managing stock and backorders are not allowed
				if ( ! $product_data->is_in_stock() ) {
					throw new Exception( sprintf( __( 'You cannot add &quot;%s&quot; to the cart because the product is out of stock.', 'banda' ), $product_data->get_title() ) );
				}

				if ( ! $product_data->has_enough_stock( $quantity ) ) {
					throw new Exception( sprintf(__( 'You cannot add that amount of &quot;%s&quot; to the cart because there is not enough stock (%s remaining).', 'banda' ), $product_data->get_title(), $product_data->get_stock_quantity() ) );
				}
				...
				if ( $managing_stock = $product_data->managing_stock() ) {
					...
					if ( ! $product_data->has_enough_stock( $check_qty + $quantity ) ) {
						throw new Exception( sprintf(
							...
							wc_get_cart_url(),
							__( 'View Cart', 'banda' ),
							sprintf( __( 'You cannot add that amount to the cart &mdash; we have %s in stock and you already have %s in your cart.', 'banda' ), $product_data->get_stock_quantity(), $check_qty )
						) );
					}
				}
				...
				if ( $cart_item_key ) {
					...
					$this->cart_contents[ $cart_item_key ] = apply_filters( 'banda_add_cart_item', array_merge( $cart_item_data, array(
				...
				do_action( 'banda_add_to_cart', $cart_item_key, $product_id, $quantity, $variation_id, $variation, $cart_item_data );
		...
		public function remove_cart_item( $cart_item_key ) {
			if ( isset( $this->cart_contents[ $cart_item_key ] ) ) {
				...
				do_action( 'banda_remove_cart_item', $cart_item_key, $this );

				unset( $this->cart_contents[ $cart_item_key ] );

				do_action( 'banda_cart_item_removed', $cart_item_key, $this );
		...
		public function restore_cart_item( $cart_item_key ) {
			if ( isset( $this->removed_cart_contents[ $cart_item_key ] ) ) {
				...
				do_action( 'banda_restore_cart_item', $cart_item_key, $this );

				unset( $this->removed_cart_contents[ $cart_item_key ] );

				do_action( 'banda_cart_item_restored', $cart_item_key, $this );
		...
		public function set_quantity( $cart_item_key, $quantity = 1, $refresh_totals = true ) {
			if ( $quantity == 0 || $quantity < 0 ) {
				do_action( 'banda_before_cart_item_quantity_zero', $cart_item_key );
				unset( $this->cart_contents[ $cart_item_key ] );
			} else {
				$old_quantity = $this->cart_contents[ $cart_item_key ]['quantity'];
				$this->cart_contents[ $cart_item_key ]['quantity'] = $quantity;
				do_action( 'banda_after_cart_item_quantity_update', $cart_item_key, $quantity, $old_quantity );
			}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		private function reset( $unset_session = false ) {
			...
			do_action( 'banda_cart_reset', $this, $unset_session );
		}
		...
		public function calculate_totals() {
			...
			do_action( 'banda_before_calculate_totals', $this );
			...
			foreach ( $cart as $cart_item_key => $values ) {
				 ...
				 */
					 ...
					 *
 					 * The banda_adjust_non_base_location_prices filter can stop base taxes being taken off when dealing with out of base locations.
 					 * e.g. If a product costs 10 including tax, all users will pay 10 regardless of location and taxes.
 					 * This feature is experimental @since 2.4.7 and may change in the future. Use at your risk.
 					 */
					if ( $item_tax_rates !== $base_tax_rates && apply_filters( 'banda_adjust_non_base_location_prices', true ) ) {
			...
			if ( is_checkout() || is_cart() || defined('WOOCOMMERCE_CHECKOUT') || defined('WOOCOMMERCE_CART') ) {
				...
				do_action( 'banda_calculate_totals', $this );

				// Grand Total - Discounted product prices, discounted tax, shipping cost + tax
				$this->total = max( 0, apply_filters( 'banda_calculated_total', round( $this->cart_contents_total + $this->tax_total + $this->shipping_tax_total + $this->shipping_total + $this->fee_total, $this->dp ), $this ) );
			...
			do_action( 'banda_after_calculate_totals', $this );
		...
		public function remove_taxes() {
			...
			if ( apply_filters( 'banda_cart_remove_taxes_apply_zero_rate', true ) ) {
				$this->taxes = $this->shipping_taxes = array( apply_filters( 'banda_cart_remove_taxes_zero_rate_id', 'zero-rated' ) => 0 );
			} else {
		...
		public function needs_payment() {
			return apply_filters( 'banda_cart_needs_payment', $this->total > 0, $this );
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function get_shipping_packages() {
			...
			return apply_filters( 'banda_cart_shipping_packages', $packages );
		}
		...
		public function needs_shipping() {
			...
			return apply_filters( 'banda_cart_needs_shipping', $needs_shipping );
		}
		...
		public function needs_shipping_address() {
			...
			return apply_filters( 'banda_cart_needs_shipping_address', $needs_shipping_address );
		}
		...
		public function show_shipping() {
			...
			if ( 'yes' === get_option( 'banda_shipping_cost_requires_address' ) ) {
			...
			return apply_filters( 'banda_cart_ready_to_calc_shipping', true );
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function has_discount( $coupon_code = '' ) {
			return $coupon_code ? in_array( apply_filters( 'banda_coupon_code', $coupon_code ), $this->applied_coupons ) : sizeof( $this->applied_coupons ) > 0;
		}
		...
		public function add_discount( $coupon_code ) {
			...
			$coupon_code = apply_filters( 'banda_coupon_code', $coupon_code );
			...
			if ( $the_coupon->individual_use == 'yes' ) {
				$this->applied_coupons = apply_filters( 'banda_apply_individual_use_coupon', array(), $the_coupon, $this->applied_coupons );
			}
			...
			if ( $this->applied_coupons ) {
				foreach ( $this->applied_coupons as $code ) {
					...
					if ( $coupon->individual_use == 'yes' && false === apply_filters( 'banda_apply_with_individual_use_coupon', false, $the_coupon, $coupon, $this->applied_coupons ) ) {
			...
			do_action( 'banda_applied_coupon', $coupon_code );
		...
		public function remove_coupon( $coupon_code ) {
			...
			do_action( 'banda_removed_coupon', $coupon_code );
		...
		public function get_discounted_price( $values, $price, $add_totals = false ) {
			...
			if ( ! empty( $this->coupons ) ) {
				...
				foreach ( $this->coupons as $code => $coupon ) {
					if ( $coupon->is_valid() && ( $coupon->is_valid_for_product( $product, $values ) || $coupon->is_valid_for_cart() ) ) {
						$discount_amount = $coupon->get_discount_amount( 'yes' === get_option( 'banda_calc_discounts_sequentially', 'no' ) ? $price : $undiscounted_price, $values, true );
			...
			return apply_filters( 'banda_get_discounted_price', $price, $values, $this );
		}
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function calculate_fees() {
			...
			do_action( 'banda_cart_calculate_fees', $this );
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function get_total() {
			return apply_filters( 'banda_cart_total', wc_price( $this->total ) );
		}
		...
		public function get_total_ex_tax() {
			...
			return apply_filters( 'banda_cart_total_ex_tax', wc_price( $total ) );
		}
		...
		public function get_cart_total() {
			...
			return apply_filters( 'banda_cart_contents_total', $cart_contents_total );
		}
		...
		public function get_cart_subtotal( $compound = false ) {
			...
			return apply_filters( 'banda_cart_subtotal', $cart_subtotal, $compound, $this );
		}
		...
		public function get_product_price( $_product ) {
			...
			return apply_filters( 'banda_cart_product_price', wc_price( $product_price ), $_product );
		}
		...
		public function get_product_subtotal( $_product, $quantity ) {
			...
			return apply_filters( 'banda_cart_product_subtotal', $product_subtotal, $_product, $quantity, $this );
		}
		...
		public function get_cart_tax() {
			...
			return apply_filters( 'banda_get_cart_tax', $cart_total_tax ? wc_price( $cart_total_tax ) : '' );
		}
		...
		public function get_taxes_total( $compound = true, $display = true ) {
			...
			return apply_filters( 'banda_cart_taxes_total', $total, $compound, $display, $this );
		}
		...
		public function get_total_discount() {
			...
			return apply_filters( 'banda_cart_total_discount', $total_discount, $this );
		}
		...
		public function get_discounts_before_tax() {
			...
			return apply_filters( 'banda_cart_discounts_before_tax', $discounts_before_tax, $this );
		}
