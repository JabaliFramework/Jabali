...
class WC_Shipping {
	...
	/** @var array|null Stores methods loaded into banda. */
	...
	public function get_shipping_method_class_names() {
		...
		return apply_filters( 'banda_shipping_methods', $shipping_methods );
	}
	...
	public function load_shipping_methods( $package = array() ) {
		...
		do_action( 'banda_load_shipping_methods', $package );
	...
	public function get_shipping_classes() {
		...
		return apply_filters( 'banda_get_shipping_classes', $this->shipping_classes );
	}
	...
	public function calculate_shipping( $packages = array() ) {
		 ...
		 */
		$this->packages = apply_filters( 'banda_shipping_packages', $this->packages );
		...
		foreach ( $this->packages as $i => $package ) {
			...
			if ( sizeof( $package['rates'] ) > 0 ) {
				...
				if ( empty( $chosen_method ) || ! isset( $package['rates'][ $chosen_method ] ) || $method_count !== sizeof( $package['rates'] ) ) {
					$chosen_method        = apply_filters( 'banda_shipping_chosen_method', $this->get_default_method( $package['rates'], false ), $package['rates'], $chosen_method );
					$chosen_methods[ $i ] = $chosen_method;
					$method_counts[ $i ]  = sizeof( $package['rates'] );
					do_action( 'banda_shipping_method_chosen', $chosen_method );
				}
	...
	public function calculate_shipping_for_package( $package = array(), $package_key = 0 ) {
		...
		if ( ! is_array( $stored_rates ) || $package_hash !== $stored_rates['package_hash'] || ! empty( $status_options['shipping_debug_mode'] ) ) {
			...
			$package['rates'] = apply_filters( 'banda_package_rates', $package['rates'], $package );
