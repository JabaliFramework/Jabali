 ...
 * @author 		Jabali
 * @category 	Shortcodes
 * @package 	Banda/Shortcodes/My_Account
 * @version     2.0.0
 */
class WC_Shortcode_My_Account {
	...
	public static function output( $atts ) {
		...
		if ( ! is_user_logged_in() ) {
			$message = apply_filters( 'banda_my_account_message', '' );
			 ...
			 *
			 * If banda_account_content did not run, this is an old template
			 * so we need to render the endpoint content again.
			 */
			if ( ! did_action( 'banda_account_content' ) ) {
	...
	public static function view_order( $order_id ) {
		...
		if ( ! current_user_can( 'view_order', $order_id ) ) {
			echo '<div class="banda-error">' . __( 'Invalid order.', 'banda' ) . ' <a href="' . wc_get_page_permalink( 'myaccount' ).'" class="wc-forward">'. __( 'My Account', 'banda' ) .'</a>' . '</div>';
			return;
		}
	...
	public static function edit_address( $load_address = 'billing' ) {
		...
		foreach ( $address as $key => $field ) {
			...
			$address[ $key ]['value'] = apply_filters( 'banda_my_account_edit_address_field_value', $value, $key, $load_address );
		}

		wc_get_template( 'myaccount/form-edit-address.php', array(
			'load_address' 	=> $load_address,
			'address'		=> apply_filters( 'banda_address_to_edit', $address )
		) );
	}
	...
	public static function lost_password() {
		 ...
		 */
			...
			wc_add_notice( __( 'Your password has been reset.', 'banda' ) . ' <a class="button" href="' . esc_url( wc_get_page_permalink( 'myaccount' ) ) . '">' . __( 'Log in', 'banda' ) . '</a>' );
	 ...
	 *
	 * @uses $wpdb Jabali Database object
	...
	public static function retrieve_password() {
		...
		if ( empty( $login ) ) {
			...
			wc_add_notice( __( 'Enter a username or e-mail address.', 'banda' ), 'error' );
			return false;
		...
		if ( ! $user_data && is_email( $login ) && apply_filters( 'banda_get_username_from_email', true ) ) {
		...
		if ( ! $user_data ) {
			wc_add_notice( __( 'Invalid username or e-mail.', 'banda' ), 'error' );
			return false;
		}
		...
		if ( ! $allow ) {
			...
			wc_add_notice( __( 'Password reset is not allowed for this user', 'banda' ), 'error' );
			return false;
		...
		// Get password reset key (function introduced in Jabali 4.4).
		$key = get_password_reset_key( $user_data );

		// Send email notification
		WC()->mailer(); // load email classes
		do_action( 'banda_reset_password_notification', $user_login, $key );
	...
	public static function add_payment_method() {
		...
		if ( ! is_user_logged_in() ) {
			...
			do_action( 'before_banda_add_payment_method' );

			wc_print_notices();

			wc_get_template( 'myaccount/form-add-payment-method.php' );

			do_action( 'after_banda_add_payment_method' );
