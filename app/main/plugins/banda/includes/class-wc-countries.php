 ...
 * Banda countries
 *
 * The Banda countries class stores country/state data.
 *
 * @class       WC_Countries
 * @version     2.3.0
...
class WC_Countries {
	...
	public function get_countries() {
		if ( empty( $this->countries ) ) {
			$this->countries = apply_filters( 'banda_countries', include( WC()->plugin_path() . '/i18n/countries.php' ) );
			if ( apply_filters( 'banda_sort_countries', true ) ) {
	...
	public function get_continents() {
		if ( empty( $this->continents ) ) {
			$this->continents = apply_filters( 'banda_continents', include( WC()->plugin_path() . '/i18n/continents.php' ) );
		}
	...
	public function load_country_states() {
		...
		$this->states = apply_filters( 'banda_states', $states );
	}
	...
	public function get_base_country() {
		...
		return apply_filters( 'banda_countries_base_country', $default['country'] );
	}
	...
	public function get_base_state() {
		$default = wc_get_base_location();
		return apply_filters( 'banda_countries_base_state', $default['state'] );
	}
	...
	public function get_base_city() {
		return apply_filters( 'banda_countries_base_city', '' );
	}
	...
	public function get_base_postcode() {
		return apply_filters( 'banda_countries_base_postcode', '' );
	}
	...
	public function get_allowed_countries() {
		if ( 'all' === get_option( 'banda_allowed_countries' ) ) {
			return $this->countries;
		}

		if( 'all_except' === get_option( 'banda_allowed_countries' ) ) {
			$except_countries = get_option( 'banda_all_except_countries', array() );
			...
			if ( ! $except_countries ) {
				...
				return apply_filters( 'banda_countries_allowed_countries', $all_except_countries );
			}
		}

		$countries = array();

		$raw_countries = get_option( 'banda_specific_allowed_countries', array() );
		...
		return apply_filters( 'banda_countries_allowed_countries', $countries );
	}
	...
	public function get_shipping_countries() {
		if ( '' === get_option( 'banda_ship_to_countries' ) ) {
			return $this->get_allowed_countries();
		}

		if ( 'all' === get_option( 'banda_ship_to_countries' ) ) {
			return $this->countries;
		}

		$countries = array();

		$raw_countries = get_option( 'banda_specific_ship_to_countries' );
		...
		return apply_filters( 'banda_countries_shipping_countries', $countries );
	}
	...
	public function get_allowed_country_states() {
		if ( get_option( 'banda_allowed_countries' ) !== 'specific' ) {
			return $this->states;
		}

		$states = array();

		$raw_countries = get_option( 'banda_specific_allowed_countries' );
		...
		return apply_filters( 'banda_countries_allowed_country_states', $states );
	}
	...
	public function get_shipping_country_states() {
		...
		if ( get_option( 'banda_ship_to_countries' ) !== 'specific' ) {
			return $this->states;
		}
		...
		return apply_filters( 'banda_countries_shipping_country_states', $states );
	}
	...
	public function shipping_to_prefix( $country_code = '' ) {
		...
		$return       = in_array( $country_code, $countries ) ? __( 'to the', 'banda' ) : __( 'to', 'banda' );

		return apply_filters( 'banda_countries_shipping_to_prefix', $return, $country_code );
	}
	...
	public function estimated_for_prefix( $country_code = '' ) {
		...
		$countries    = array( 'GB', 'US', 'AE', 'CZ', 'DO', 'NL', 'PH', 'USAF' );
		$return       = in_array( $country_code, $countries ) ? __( 'the', 'banda' ) . ' ' : '';

		return apply_filters( 'banda_countries_estimated_for_prefix', $return, $country_code );
	}
	...
	public function tax_or_vat() {
		$return = in_array( $this->get_base_country(), $this->get_european_union_countries( 'eu_vat' ) ) ? __( 'VAT', 'banda' ) : __( 'Tax', 'banda' );

		return apply_filters( 'banda_countries_tax_or_vat', $return );
	}
	...
	public function inc_tax_or_vat() {
		$return = in_array( $this->get_base_country(), $this->get_european_union_countries( 'eu_vat' ) ) ? __( '(incl. VAT)', 'banda' ) : __( '(incl. tax)', 'banda' );

		return apply_filters( 'banda_countries_inc_tax_or_vat', $return );
	}
	...
	public function ex_tax_or_vat() {
		$return = in_array( $this->get_base_country(), $this->get_european_union_countries( 'eu_vat' ) ) ? __( '(ex. VAT)', 'banda' ) : __( '(ex. tax)', 'banda' );

		return apply_filters( 'banda_countries_ex_tax_or_vat', $return );
	}
	...
	public function get_address_formats() {
		if ( empty( $this->address_formats ) ) {
			...
			$this->address_formats = apply_filters( 'banda_localisation_address_formats', array(
				'default' => "{name}\n{company}\n{address_1}\n{address_2}\n{city}\n{state}\n{postcode}\n{country}",
				'AU' => "{name}\n{company}\n{address_1}\n{address_2}\n{city} {state} {postcode}\n{country}",
	...
	public function get_formatted_address( $args = array() ) {
		...
		if ( $country == $this->get_base_country() && ! apply_filters( 'banda_formatted_address_force_country_display', false ) ) {
		...
		$replace = array_map( 'esc_html', apply_filters( 'banda_formatted_address_replacements', array(
	...
	public function get_default_address_fields() {
		$fields = array(
			...
			'address_1' => array(
				'label'        => __( 'Address', 'banda' ),
				'placeholder'  => _x( 'Street address', 'placeholder', 'banda' ),
				'required'     => true,
		...
		return apply_filters( 'banda_default_address_fields', $fields );
	}
	...
	public function get_country_locale_field_selectors() {
		...
		return apply_filters( 'banda_country_locale_field_selectors', $locale_fields );
	}

	/**
	 * Get country locale settings.
	 * @return array
	 * @todo  [2.4] Check select2 4.0.0 compatibility with `placeholder` attribute and uncomment relevant lines. https://github.com/mtaandao/banda/issues/7729
	 */
	public function get_country_locale() {
		if ( empty( $this->locale ) ) {

			// Locale information used by the checkout
			$this->locale = apply_filters( 'banda_get_country_locale', array(
				'AE' => array(
					'postcode' => array(
						'required' => false,
						'hidden'   => true
				...
				'AU' => array(
					'city'      => array(
						'label'       => __( 'Suburb', 'banda' ),
					),
					'postcode'  => array(
						'label'       => __( 'Postcode', 'banda' ),
					),
					'state'     => array(
						'label'       => __( 'State', 'banda' ),
					)
				),
				'AX' => array(
					'postcode_before_city' => true,
			...
			$this->locale['default'] = apply_filters('banda_get_country_locale_default', $this->get_default_address_fields() );
			...
			$this->locale['default']                   = apply_filters( 'banda_get_country_locale_base', $this->locale['default'] );
			$this->locale[ $this->get_base_country() ] = apply_filters( 'banda_get_country_locale_base', $this->locale[ $this->get_base_country() ] );
		}
	...
	public function get_address_fields( $country = '', $type = 'billing_' ) {
		...
		$address_fields = apply_filters( 'banda_' . $type . 'fields', $address_fields, $country );
