...
class WC_Gateway_COD extends WC_Payment_Gateway {
	...
	public function __construct() {
		$this->id                 = 'cod';
		$this->icon               = apply_filters( 'banda_cod_icon', '' );
		$this->method_title       = __( 'Cash on Delivery', 'banda' );
		$this->method_description = __( 'Have your customers pay with cash (or by other means) upon delivery.', 'banda' );
		...
		add_action( 'banda_thankyou_cod', array( $this, 'thankyou_page' ) );
	...
	public function init_form_fields() {
		...
		$this->form_fields = array(
			'enabled' => array(
				'title'       => __( 'Enable COD', 'banda' ),
				'label'       => __( 'Enable Cash on Delivery', 'banda' ),
				'type'        => 'checkbox',
				'description' => '',
				'default'     => 'no'
			),
			'title' => array(
				...
				'default'     => __( 'Cash on Delivery', 'banda' ),
				'desc_tip'    => true,
			),
			'description' => array(
				...
				'default'     => __( 'Pay with cash upon delivery.', 'banda' ),
				'desc_tip'    => true,
			),
			'instructions' => array(
			...
			'enable_for_methods' => array(
				...
				'css'               => 'width: 450px;',
				'default'           => '',
				'description'       => __( 'If COD is only available for certain methods, set it up here. Leave blank to enable for all methods.', 'banda' ),
			...
			'enable_for_virtual' => array(
				...
				'label'             => __( 'Accept COD if the order is virtual', 'banda' ),
				'type'              => 'checkbox',
				'default'           => 'yes'
			)
	   );
	...
	public function process_payment( $order_id ) {
		...
		$order->update_status( apply_filters( 'banda_cod_process_payment_order_status', $order->has_downloadable_item() ? 'on-hold' : 'processing', $order ), __( 'Payment to be made upon delivery.', 'banda' ) );
