...
class WC_Gateway_Paypal_IPN_Handler extends WC_Gateway_Paypal_Response {
	...
	public function __construct( $sandbox = false, $receiver_email = '' ) {
		add_action( 'banda_api_wc_gateway_paypal', array( $this, 'check_response' ) );
	...
	protected function validate_currency( $order, $currency ) {
		if ( $order->get_order_currency() != $currency ) {
			...
			$order->update_status( 'on-hold', sprintf( __( 'Validation error: PayPal currencies do not match (code %s).', 'banda' ), $currency ) );
			exit;
		}
	}
	...
	protected function validate_amount( $order, $amount ) {
		if ( number_format( $order->get_total(), 2, '.', '' ) != number_format( $amount, 2, '.', '' ) ) {
			...
			// Put this order on-hold for manual checking.
			$order->update_status( 'on-hold', sprintf( __( 'Validation error: PayPal amounts do not match (gross %s).', 'banda' ), $amount ) );
			exit;
		}
	}
	 ...
	 * Banda -> Settings -> Checkout -> PayPal, it will log an error about it.
	 * @param WC_Order $order
	 * @param string $receiver_email
	 */
	protected function validate_receiver_email( $order, $receiver_email ) {
		if ( strcasecmp( trim( $receiver_email ), trim( $this->receiver_email ) ) != 0 ) {
			...
			// Put this order on-hold for manual checking.
			$order->update_status( 'on-hold', sprintf( __( 'Validation error: PayPal IPN response from a different email address (%s).', 'banda' ), $receiver_email ) );
			exit;
		}
	}
	...
	protected function payment_status_completed( $order, $posted ) {
		...
		if ( 'completed' === $posted['payment_status'] ) {
			$this->payment_complete( $order, ( ! empty( $posted['txn_id'] ) ? wc_clean( $posted['txn_id'] ) : '' ), __( 'IPN payment completed', 'banda' ) );
			...
			$this->payment_on_hold( $order, sprintf( __( 'Payment pending: %s', 'banda' ), $posted['pending_reason'] ) );
		}
	}
	...
	protected function payment_status_failed( $order, $posted ) {
		$order->update_status( 'failed', sprintf( __( 'Payment %s via IPN.', 'banda' ), wc_clean( $posted['payment_status'] ) ) );
	}
	...
	protected function payment_status_refunded( $order, $posted ) {
		...
		if ( $order->get_total() == ( $posted['mc_gross'] * -1 ) ) {
			...
			$order->update_status( 'refunded', sprintf( __( 'Payment %s via IPN.', 'banda' ), strtolower( $posted['payment_status'] ) ) );

			$this->send_ipn_email_notification(
				sprintf( __( 'Payment for order %s refunded', 'banda' ), '<a class="link" href="' . esc_url( admin_url( 'post.php?post=' . $order->id . '&action=edit' ) ) . '">' . $order->get_order_number() . '</a>' ),
				sprintf( __( 'Order #%s has been marked as refunded - PayPal reason code: %s', 'banda' ), $order->get_order_number(), $posted['reason_code'] )
			);
		}
	}
	...
	protected function payment_status_reversed( $order, $posted ) {
		$order->update_status( 'on-hold', sprintf( __( 'Payment %s via IPN.', 'banda' ), wc_clean( $posted['payment_status'] ) ) );

		$this->send_ipn_email_notification(
			sprintf( __( 'Payment for order %s reversed', 'banda' ), '<a class="link" href="' . esc_url( admin_url( 'post.php?post=' . $order->id . '&action=edit' ) ) . '">' . $order->get_order_number() . '</a>' ),
			sprintf( __( 'Order #%s has been marked on-hold due to a reversal - PayPal reason code: %s', 'banda' ), $order->get_order_number(), wc_clean( $posted['reason_code'] ) )
		);
	}
	...
	protected function payment_status_canceled_reversal( $order, $posted ) {
		$this->send_ipn_email_notification(
			sprintf( __( 'Reversal cancelled for order #%s', 'banda' ), $order->get_order_number() ),
			sprintf( __( 'Order #%s has had a reversal cancelled. Please check the status of payment and update the order status accordingly here: %s', 'banda' ), $order->get_order_number(), esc_url( admin_url( 'post.php?post=' . $order->id . '&action=edit' ) ) )
		);
	}
	...
	protected function send_ipn_email_notification( $subject, $message ) {
		$new_order_settings = get_option( 'banda_new_order_settings', array() );
		$mailer             = WC()->mailer();
