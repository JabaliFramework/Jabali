...
class WC_Gateway_Paypal extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->order_button_text  = __( 'Proceed to PayPal', 'banda' );
		$this->method_title       = __( 'PayPal', 'banda' );
		$this->method_description = sprintf( __( 'PayPal standard sends customers to PayPal to enter their payment information. PayPal IPN requires fsockopen/cURL support to update order statuses after payment. Check the %ssystem status%s page for more details.', 'banda' ), '<a href="' . admin_url( 'admin.php?page=wc-status' ) . '">', '</a>' );
		$this->supports           = array(
			'products',
			'refunds'
		);
	...
	public function get_icon() {
		...
		foreach ( $icon as $i ) {
			$icon_html .= '<img src="' . esc_attr( $i ) . '" alt="' . esc_attr__( 'PayPal Acceptance Mark', 'banda' ) . '" />';
		}

		$icon_html .= sprintf( '<a href="%1$s" class="about_paypal" onclick="javascript:window.open(\'%1$s\',\'WIPaypal\',\'toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=1060, height=700\'); return false;" title="' . esc_attr__( 'What is PayPal?', 'banda' ) . '">' . esc_attr__( 'What is PayPal?', 'banda' ) . '</a>', esc_url( $this->get_icon_url( WC()->countries->get_base_country() ) ) );

		return apply_filters( 'banda_gateway_icon', $icon_html, $this->id );
	}
	...
	protected function get_icon_image( $country ) {
		...
		return apply_filters( 'banda_paypal_icon', $icon );
	}
	...
	public function is_valid_for_use() {
		return in_array( get_banda_currency(), apply_filters( 'banda_paypal_supported_currencies', array( 'AUD', 'BRL', 'CAD', 'MXN', 'NZD', 'HKD', 'SGD', 'USD', 'EUR', 'JPY', 'TRY', 'NOK', 'CZK', 'DKK', 'HUF', 'ILS', 'MYR', 'PHP', 'PLN', 'SEK', 'CHF', 'TWD', 'THB', 'GBP', 'RMB', 'RUB' ) ) );
	}
	...
	public function admin_options() {
		if ( $this->is_valid_for_use() ) {
			...
			?>
			<div class="inline error"><p><strong><?php _e( 'Gateway Disabled', 'banda' ); ?></strong>: <?php _e( 'PayPal does not support your store currency.', 'banda' ); ?></p></div>
			<?php
		}
	}
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		...
		if ( ! $this->can_refund_order( $order ) ) {
			...
			return new WP_Error( 'error', __( 'Refund Failed: No transaction ID', 'banda' ) );
		}
		...
		switch ( strtolower( $result['ACK'] ) ) {
			...
			case 'successwithwarning':
				$order->add_order_note( sprintf( __( 'Refunded %s - Refund ID: %s', 'banda' ), $result['GROSSREFUNDAMT'], $result['REFUNDTRANSACTIONID'] ) );
				return true;
			break;
		}
