...
class WC_Gateway_Paypal_PDT_Handler extends WC_Gateway_Paypal_Response {
	...
	public function __construct( $sandbox = false, $identity_token = '' ) {
		add_action( 'banda_thankyou_paypal', array( $this, 'check_response' ) );
	...
	public function check_response() {
		...
		if ( $transaction_result && 'completed' === $status ) {
			if ( $order->get_total() != $amount ) {
				...
				$this->payment_on_hold( $order, sprintf( __( 'Validation error: PayPal amounts do not match (amt %s).', 'banda' ), $amount ) );
			} else {
				$this->payment_complete( $order, $transaction,  __( 'PDT payment completed', 'banda' ) );
