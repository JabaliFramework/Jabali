	...
	public function form() {
		...
		$default_fields = array(
			'routing-number' => '<p class="form-row form-row-first">
				<label for="' . esc_attr( $this->id ) . '-routing-number">' . __( 'Routing Number', 'banda' ) . ' <span class="required">*</span></label>
			...
			'account-number' => '<p class="form-row form-row-wide">
				<label for="' . esc_attr( $this->id ) . '-account-number">' . __( 'Account Number', 'banda' ) . ' <span class="required">*</span></label>
		...
		$fields = wp_parse_args( $fields, apply_filters( 'banda_echeck_form_fields', $default_fields, $this->id ) );
		?>

		<fieldset id="<?php echo esc_attr( $this->id ); ?>-cc-form" class='wc-echeck-form wc-payment-form'>
			<?php do_action( 'banda_echeck_form_start', $this->id ); ?>
			<?php
				foreach ( $fields as $field ) {
					echo $field;
				}
			?>
			<?php do_action( 'banda_echeck_form_end', $this->id ); ?>
			<div class="clear"></div>
		</fieldset><?php
	}
