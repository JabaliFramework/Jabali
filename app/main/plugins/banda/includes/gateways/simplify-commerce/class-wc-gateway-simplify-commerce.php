...
class WC_Gateway_Simplify_Commerce extends WC_Payment_Gateway_CC {
	...
	public function __construct() {
		...
		$this->method_title       = __( 'Simplify Commerce', 'banda' );
		$this->method_description = __( 'Take payments via Simplify Commerce - uses simplify.js to create card tokens and the Simplify Commerce SDK. Requires SSL when sandbox is disabled.', 'banda' );
		$this->new_method_label   = __( 'Use a new card', 'banda' );
		...
		$this->modal_color     = $this->get_option( 'modal_color', '#21b68e' );
		...
		add_action( 'banda_receipt_' . $this->id, array( $this, 'receipt_page' ) );
		add_action( 'banda_api_wc_gateway_simplify_commerce', array( $this, 'return_handler' ) );
	}
	...
	protected function init_simplify_sdk() {
		...
		Simplify::$userAgent  = 'Banda/' . WC()->version;
	}
	...
	public function admin_options() {
		?>
		<h3><?php _e( 'Simplify Commerce by MasterCard', 'banda' ); ?></h3>
		...
		<?php if ( empty( $this->public_key ) ) : ?>
			<div class="simplify-commerce-banner updated">
				...
				<p class="main"><strong><?php _e( 'Getting started', 'banda' ); ?></strong></p>
				<p><?php _e( 'Simplify Commerce is your merchant account and payment gateway all rolled into one. Choose Simplify Commerce as your Banda payment gateway to get access to your money quickly with a powerful, secure payment engine backed by MasterCard.', 'banda' ); ?></p>

				<p><a href="https://www.simplify.com/commerce/partners/banda#/signup" target="_blank" class="button button-primary"><?php _e( 'Sign up for Simplify Commerce', 'banda' ); ?></a> <a href="https://www.simplify.com/commerce/partners/banda#/" target="_blank" class="button"><?php _e( 'Learn more', 'banda' ); ?></a></p>
		...
		<table class="form-table">
			...
			<script type="text/javascript">
				jQuery( '#banda_simplify_commerce_sandbox' ).on( 'change', function() {
					var sandbox    = jQuery( '#banda_simplify_commerce_sandbox_public_key, #banda_simplify_commerce_sandbox_private_key' ).closest( 'tr' ),
						production = jQuery( '#banda_simplify_commerce_public_key, #banda_simplify_commerce_private_key' ).closest( 'tr' );
				...
				jQuery( '#banda_simplify_commerce_mode' ).on( 'change', function() {
					var color = jQuery( '#banda_simplify_commerce_modal_color' ).closest( 'tr' );
	...
	public function checks() {
		...
		if ( version_compare( phpversion(), '5.3', '<' ) ) {
			echo '<div class="error"><p>' . sprintf( __( 'Simplify Commerce Error: Simplify commerce requires PHP 5.3 and above. You are using version %s.', 'banda' ), phpversion() ) . '</p></div>';
		}

		// Check required fields
		elseif ( ! $this->public_key || ! $this->private_key ) {
			echo '<div class="error"><p>' . __( 'Simplify Commerce Error: Please enter your public and private keys', 'banda' ) . '</p></div>';
		}

		// Show message when using standard mode and no SSL on the checkout page
		elseif ( 'standard' == $this->mode && ! wc_checkout_is_https() ) {
			echo '<div class="error"><p>' . sprintf( __( 'Simplify Commerce is enabled, but the <a href="%s">force SSL option</a> is disabled; your checkout may not be secure! Please enable SSL and ensure your server has a valid SSL certificate - Simplify Commerce will only work in sandbox mode.', 'banda'), admin_url( 'admin.php?page=wc-settings&tab=checkout' ) ) . '</p></div>';
		}
	}
	...
	public function init_form_fields() {
		$this->form_fields = array(
			'enabled' => array(
				...
				'label'       => __( 'Enable Simplify Commerce', 'banda' ),
				'type'        => 'checkbox',
				'description' => '',
				'default'     => 'no'
			),
			...
			'mode' => array(
				'title'       => __( 'Payment Mode', 'banda' ),
				'label'       => __( 'Enable Hosted Payments', 'banda' ),
				'type'        => 'select',
				'description' => sprintf( __( 'Standard will display the credit card fields on your store (SSL required). %1$s Hosted Payments will display a Simplify Commerce modal dialog on your store (if SSL) or will redirect the customer to Simplify Commerce hosted page (if not SSL). %1$s Note: Hosted Payments need a new API Key pair with the hosted payments flag selected. %2$sFor more details check the Simplify Commerce docs%3$s.', 'banda' ), '<br />', '<a href="https://simplify.desk.com/customer/portal/articles/1792405-how-do-i-enable-hosted-payments" target="_blank">', '</a>' ),
				'default'     => 'standard',
				'options'     => array(
					'standard' => __( 'Standard', 'banda' ),
					'hosted'   => __( 'Hosted Payments', 'banda' )
				)
			),
			'modal_color' => array(
				'title'       => __( 'Modal Color', 'banda' ),
				'type'        => 'color',
				'description' => __( 'Set the color of the buttons and titles on the modal dialog.', 'banda' ),
				'default'     => '#21b68e',
				'desc_tip'    => true
			),
			'sandbox' => array(
				'title'       => __( 'Sandbox', 'banda' ),
				'label'       => __( 'Enable Sandbox Mode', 'banda' ),
				'type'        => 'checkbox',
				'description' => __( 'Place the payment gateway in sandbox mode using sandbox API keys (real payments will not be taken).', 'banda' ),
				'default'     => 'yes'
			),
			'sandbox_public_key' => array(
				'title'       => __( 'Sandbox Public Key', 'banda' ),
				'type'        => 'text',
				'description' => __( 'Get your API keys from your Simplify account: Settings > API Keys.', 'banda' ),
				'default'     => '',
				'desc_tip'    => true
			),
			'sandbox_private_key' => array(
				'title'       => __( 'Sandbox Private Key', 'banda' ),
				'type'        => 'text',
				'description' => __( 'Get your API keys from your Simplify account: Settings > API Keys.', 'banda' ),
				'default'     => '',
				'desc_tip'    => true
	...
	public function payment_fields() {
		...
		if ( 'yes' == $this->sandbox ) {
			$description .= ' ' . sprintf( __( 'TEST MODE ENABLED. Use a test card: %s', 'banda' ), '<a href="https://www.simplify.com/commerce/docs/tutorial/index#testing">https://www.simplify.com/commerce/docs/tutorial/index#testing</a>' );
		}
	...
	public function payment_scripts() {
		...
		wp_localize_script( 'wc-simplify-commerce', 'Simplify_commerce_params', array(
			'key'           => $this->public_key,
			'card.number'   => __( 'Card Number', 'banda' ),
			'card.expMonth' => __( 'Expiry Month', 'banda' ),
			'card.expYear'  => __( 'Expiry Year', 'banda' ),
			'is_invalid'    => __( 'is invalid', 'banda' ),
			'mode'          => $this->mode,
			'is_ssl'        => is_ssl()
		) );
	}
	...
	public function add_payment_method() {
		if ( empty ( $_POST['simplify_token'] ) ) {
			wc_add_notice( __( 'There was a problem adding this card.', 'banda' ), 'error' );
			return;
		}
	...
	protected function process_standard_payments( $order, $cart_token = '', $customer_token = '' ) {
		try {
			...
			if ( empty( $cart_token ) && empty( $customer_token ) ) {
				$error_msg = __( 'Please make sure your card details have been entered correctly and that your browser supports JavaScript.', 'banda' );

				if ( 'yes' == $this->sandbox ) {
					$error_msg .= ' ' . __( 'Developers: Please make sure that you\'re including jQuery and there are no JavaScript errors on the page.', 'banda' );
				}
	...
	public function do_payment( $order, $amount = 0, $token = array() ) {
		if ( $amount * 100 < 50 ) {
			return new WP_Error( 'simplify_error', __( 'Sorry, the minimum allowed order total is 0.50 to use this payment method.', 'banda' ) );
		}
		...
		try {
			...
			$data = array(
				...
				'description'         => sprintf( __( '%s - Order #%s', 'banda' ), esc_html( get_bloginfo( 'name', 'display' ) ), $order->get_order_number() ),
				'currency'            => strtoupper( get_banda_currency() ),
				'reference'           => $order->id
			);
			...
			$order->add_order_note( sprintf( __( 'Simplify payment error: %s', 'banda' ), $error_message ) );
		...
		if ( 'APPROVED' == $payment->paymentStatus ) {
			...
			// Add order note
			$order->add_order_note( sprintf( __( 'Simplify payment approved (ID: %s, Auth Code: %s)', 'banda' ), $payment->id, $payment->authCode ) );

			return true;
		} else {
			$order->add_order_note( __( 'Simplify payment declined', 'banda' ) );

			return new WP_Error( 'simplify_payment_declined', __( 'Payment was declined - please try another card.', 'banda' ) );
		}
	}
	...
	public function process_payment( $order_id ) {
		...
		if ( isset( $_POST['wc-simplify_commerce-payment-token'] ) && 'new' !== $_POST['wc-simplify_commerce-payment-token'] ) {
			...
			if ( $token->get_user_id() !== get_current_user_id() ) {
				wc_add_notice( __( 'Please make sure your card details have been entered correctly and that your browser supports JavaScript.', 'banda' ), 'error' );
				return;
			}
	...
	protected function get_hosted_payments_args( $order ) {
		$args = apply_filters( 'banda_simplify_commerce_hosted_args', array(
			'sc-key'          => $this->public_key,
			'amount'          => $order->order_total * 100,
			'reference'       => $order->id,
			'name'            => esc_html( get_bloginfo( 'name', 'display' ) ),
			'description'     => sprintf( __( 'Order #%s', 'banda' ), $order->get_order_number() ),
			'receipt'         => 'false',
	...
	public function receipt_page( $order_id ) {
		...
		echo '<p>' . __( 'Thank you for your order, please click the button below to pay with credit card using Simplify Commerce by MasterCard.', 'banda' ) . '</p>';
		...
		echo '<script type="text/javascript" src="https://www.simplify.com/commerce/simplify.pay.js"></script>
			<button class="button alt" id="simplify-payment-button" ' . implode( ' ', $button_args ) . '>' . __( 'Pay Now', 'banda' ) . '</button> <a class="button cancel" href="' . esc_url( $order->get_cancel_order_url() ) . '">' . __( 'Cancel order &amp; restore cart', 'banda' ) . '</a>
			';
	}
	...
	public function return_handler() {
		...
		if ( isset( $_REQUEST['reference'] ) && isset( $_REQUEST['paymentId'] ) && isset( $_REQUEST['signature'] ) ) {
			...
			if ( $signature === $_REQUEST['signature'] ) {
				...
				if ( ! $order_complete ) {
					$order->update_status( 'failed', __( 'Payment was declined by Simplify Commerce.', 'banda' ) );
				}
	...
	public function process_order_status( $order, $payment_id, $status, $auth_code ) {
		if ( 'APPROVED' == $status ) {
			...
			// Add order note
			$order->add_order_note( sprintf( __( 'Simplify payment approved (ID: %s, Auth Code: %s)', 'banda' ), $payment_id, $auth_code ) );
	...
	public function process_refund( $order_id, $amount = null, $reason = '' ) {
		try {
			...
			if ( 'APPROVED' == $refund->paymentStatus ) {
				...
				throw new Simplify_ApiException( __( 'Refund was declined.', 'banda' ) );
			}
	...
	public function get_icon() {
		...
		return apply_filters( 'banda_gateway_icon', $icon, $this->id );
	}
}
