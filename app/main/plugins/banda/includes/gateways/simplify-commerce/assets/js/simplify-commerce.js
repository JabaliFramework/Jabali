...
(function ( $ ) {
	...
	function simplifyFormHandler() {
		...
		if ( ( $( '#payment_method_simplify_commerce' ).is( ':checked' ) && 'new' === $( 'input[name="wc-simplify_commerce-payment-token"]:checked' ).val() ) || ( '1' === $( '#banda_add_payment_method' ).val() ) ) {
	...
	function simplifyResponseHandler( data ) {
		...
		if ( data.error ) {
			...
			$( '.banda-error, .simplify-token', ccForm ).remove();
			$form.unblock();
			...
			if ( 'validation' === data.error.code ) {
				...
				ccForm.prepend( '<ul class="banda-error">' + errorList + '</ul>' );
			}
