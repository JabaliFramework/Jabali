...
class WC_Addons_Gateway_Simplify_Commerce extends WC_Gateway_Simplify_Commerce {
	...
	public function __construct() {
		...
		if ( class_exists( 'WC_Subscriptions_Order' ) ) {
			add_action( 'banda_scheduled_subscription_payment_' . $this->id, array( $this, 'scheduled_subscription_payment' ), 10, 2 );
			add_action( 'banda_subscription_failing_payment_method_updated_' . $this->id, array( $this, 'update_failing_payment_method' ), 10, 2 );

			add_action( 'wcs_resubscribe_order_created', array( $this, 'delete_resubscribe_meta' ), 10 );

			// Allow store managers to manually set Simplify as the payment method on a subscription
			add_filter( 'banda_subscription_payment_meta', array( $this, 'add_subscription_payment_meta' ), 10, 2 );
			add_filter( 'banda_subscription_validate_payment_meta', array( $this, 'validate_subscription_payment_meta' ), 10, 2 );
		}
		...
		add_filter( 'banda_simplify_commerce_hosted_args', array( $this, 'hosted_payment_args' ), 10, 2 );
		add_action( 'banda_api_wc_addons_gateway_simplify_commerce', array( $this, 'return_handler' ) );
	}
	...
	protected function process_subscription( $order, $cart_token = '' ) {
		try {
			...
			if ( is_object( $customer ) && '' != $customer->id ) {
				...
				$error_msg = __( 'Error creating user in Simplify Commerce.', 'banda' );
		...
		if ( WC_Pre_Orders_Order::order_requires_payment_tokenization( $order->id ) ) {
			...
			try {
				if ( $order->order_total * 100 < 50 ) {
					$error_msg = __( 'Sorry, the minimum allowed order total is 0.50 to use this payment method.', 'banda' );
	...
	public function process_subscription_payment( $order, $amount = 0 ) {
		...
		if ( ! $customer_id ) {
			return new WP_Error( 'simplify_error', __( 'Customer not found', 'banda' ) );
		}
	...
	public function scheduled_subscription_payment( $amount_to_charge, $renewal_order ) {
		...
		if ( is_wp_error( $result ) ) {
			$renewal_order->update_status( 'failed', sprintf( __( 'Simplify Transaction Failed (%s)', 'banda' ), $result->get_error_message() ) );
		}
	}
	...
	public function process_pre_order_release_payment( $order ) {
		...
		try {
			...
			$pre_order_name = sprintf( __( '%s - Pre-order for "%s"', 'banda' ), esc_html( get_bloginfo( 'name', 'display' ) ), $order_item['name'] ) . ' ' . sprintf( __( '(Order #%s)', 'banda' ), $order->get_order_number() );
			...
			if ( 'APPROVED' == $payment->paymentStatus ) {
				...
				return new WP_Error( 'simplify_payment_declined', __( 'Payment was declined - the customer need to try another card.', 'banda' ) );
			}
		} catch ( Exception $e ) {
			$order_note = sprintf( __( 'Simplify Transaction Failed (%s)', 'banda' ), $e->getMessage() );
