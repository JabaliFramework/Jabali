 ...
 * Writepanel class
 */
if ( !class_exists( 'Banda_PDF_Invoices_Writepanels' ) ) {

	class Banda_PDF_Invoices_Writepanels {
		public $bulk_actions;
		...
		public function __construct() {
			...
			add_filter( 'banda_my_account_my_orders_actions', array( $this, 'my_account_pdf_link' ), 10, 2 );
			...
			add_filter( 'banda_shop_order_search_fields', array( $this, 'search_fields' ) );
		...
		public function add_styles() {
			if( $this->is_order_edit_page() ) {
				...
				if ( version_compare( WOOCOMMERCE_VERSION, '2.1' ) >= 0 ) {
					...
					wp_enqueue_style(
						'wpo-wcpdf-buttons',
						Banda_PDF_Invoices::$plugin_url . 'css/style-buttons.css',
						array(),
						Banda_PDF_Invoices::$version
					);
				} else {
					...
					wp_enqueue_style(
						'wpo-wcpdf-buttons',
						Banda_PDF_Invoices::$plugin_url . 'css/style-buttons-wc20.css',
						array(),
						Banda_PDF_Invoices::$version
					);
				}
		...
		public function add_scripts() {
			if( $this->is_order_edit_page() ) {
				wp_enqueue_script(
					'wpo-wcpdf',
					Banda_PDF_Invoices::$plugin_url . 'js/script.js',
					array( 'jquery' ),
					Banda_PDF_Invoices::$version
				);
				wp_localize_script(  
					...
					'wpo_wcpdf_ajax',  
					array(
						// 'ajaxurl'		=> add_query_arg( 'action', 'generate_wpo_wcpdf', admin_url( 'admin-ajax.php' ) ), // URL to Jabali ajax handling page  
						'ajaxurl'		=> admin_url( 'admin-ajax.php' ), // URL to Jabali ajax handling page  
		...
		public function add_listing_actions( $order ) {
			...
			$listing_actions = array(
				'invoice'		=> array (
					...
					'img'		=> Banda_PDF_Invoices::$plugin_url . 'images/invoice.png',
				...
				'packing-slip'	=> array (
					...
					'img'		=> Banda_PDF_Invoices::$plugin_url . 'images/packing-slip.png',
		...
		public function data_input_box_content ( $post ) {
			...
			<p class="form-field form-field-wide">
				...
				<?php if (!empty($invoice_exists)) : ?>
				<input type="text" class="date-picker-field" name="wcpdf_invoice_date" id="wcpdf_invoice_date" maxlength="10" value="<?php echo date_i18n( 'Y-m-d', strtotime( $invoice_date ) ); ?>" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" />@<input type="number" class="hour" placeholder="<?php _e( 'h', 'banda' ) ?>" name="wcpdf_invoice_date_hour" id="wcpdf_invoice_date_hour" min="0" max="23" size="2" value="<?php echo date_i18n( 'H', strtotime( $invoice_date ) ); ?>" pattern="([01]?[0-9]{1}|2[0-3]{1})" />:<input type="number" class="minute" placeholder="<?php _e( 'm', 'banda' ) ?>" name="wcpdf_invoice_date_minute" id="wcpdf_invoice_date_minute" min="0" max="59" size="2" value="<?php echo date_i18n( 'i', strtotime( $invoice_date ) ); ?>" pattern="[0-5]{1}[0-9]{1}" />
				<?php else : ?>
				<input type="text" class="date-picker-field" name="wcpdf_invoice_date" id="wcpdf_invoice_date" maxlength="10" disabled="disabled" value="" pattern="[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])" />@<input type="number" class="hour" disabled="disabled" placeholder="<?php _e( 'h', 'banda' ) ?>" name="wcpdf_invoice_date_hour" id="wcpdf_invoice_date_hour" min="0" max="23" size="2" value="" pattern="([01]?[0-9]{1}|2[0-3]{1})" />:<input type="number" class="minute" placeholder="<?php _e( 'm', 'banda' ) ?>" name="wcpdf_invoice_date_minute" id="wcpdf_invoice_date_minute" min="0" max="59" size="2" value="" pattern="[0-5]{1}[0-9]{1}" disabled="disabled" />
				<?php endif; ?>
			</p>
			<?php
