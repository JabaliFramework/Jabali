...
function wc_get_product( $the_product = false, $args = array() ) {
	if ( ! did_action( 'banda_init' ) ) {
		_doing_it_wrong( __FUNCTION__, __( 'wc_get_product should not be called before the banda_init action.', 'banda' ), '2.5' );
		return false;
	}
...
function wc_delete_product_transients( $post_id = 0 ) {
	...
	do_action( 'banda_delete_product_transients', $post_id );
}
...
function wc_product_post_type_link( $permalink, $post ) {
	...
	if ( ! empty( $terms ) ) {
		...
		$product_cat = _x( 'uncategorized', 'slug', 'banda' );
	}
...
function wc_placeholder_img_src() {
	return apply_filters( 'banda_placeholder_img_src', WC()->plugin_url() . '/assets/images/placeholder.png' );
}
...
function wc_placeholder_img( $size = 'shop_thumbnail' ) {
	...
	return apply_filters('banda_placeholder_img', '<img src="' . wc_placeholder_img_src() . '" alt="' . esc_attr__( 'Placeholder', 'banda' ) . '" width="' . esc_attr( $dimensions['width'] ) . '" class="banda-placeholder wp-post-image" height="' . esc_attr( $dimensions['height'] ) . '" />', $size, $dimensions );
}
...
add_action( 'banda_scheduled_sales', 'wc_scheduled_sales' );
...
function wc_get_attachment_image_attributes( $attr ) {
	if ( strstr( $attr['src'], 'banda_uploads/' ) ) {
...
function wc_prepare_attachment_for_js( $response ) {
	...
	if ( isset( $response['url'] ) && strstr( $response['url'], 'banda_uploads/' ) ) {
...
function wc_track_product_view() {
	if ( ! is_singular( 'product' ) || ! is_active_widget( false, false, 'banda_recently_viewed_products', true ) ) {
		return;
	}

	global $post;

	if ( empty( $_COOKIE['banda_recently_viewed'] ) )
		$viewed_products = array();
	else
		$viewed_products = (array) explode( '|', $_COOKIE['banda_recently_viewed'] );
	...
	wc_setcookie( 'banda_recently_viewed', implode( '|', $viewed_products ) );
}
...
function wc_get_product_types() {
	return (array) apply_filters( 'product_type_selector', array(
		'simple'   => __( 'Simple product', 'banda' ),
		'grouped'  => __( 'Grouped product', 'banda' ),
		'external' => __( 'External/Affiliate product', 'banda' ),
		'variable' => __( 'Variable product', 'banda' )
	) );
}
 ...
 * @since 2.2
 * @param int $product_id
 * @param string $sku Will be slashed to work around https://core.trac.jabali.github.io/ticket/27421
 * @return bool
 */
