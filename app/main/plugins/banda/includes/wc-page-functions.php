...
function wc_get_page_id( $page ) {
	...
	$page = apply_filters( 'banda_get_' . $page . '_page_id', get_option('banda_' . $page . '_page_id' ) );
...
function wc_get_page_permalink( $page ) {
	...
	return apply_filters( 'banda_get_' . $page . '_page_permalink', $permalink );
}
...
function wc_get_endpoint_url( $endpoint, $value = '', $permalink = '' ) {
	...
	return apply_filters( 'banda_get_endpoint_url', $url, $endpoint, $value, $permalink );
}
...
function wc_nav_menu_items( $items ) {
	if ( ! is_user_logged_in() ) {
		$customer_logout = get_option( 'banda_logout_endpoint', 'customer-logout' );
 ...
 *
 * https://github.com/mtaandao/banda/issues/177.
 *
...
function wc_list_pages( $pages ) {
	if ( is_banda() ) {
		...
		// Find shop_page_id through banda options.
