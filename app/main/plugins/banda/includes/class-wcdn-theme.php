 ...
 */
if ( !class_exists( 'Banda_Delivery_Notes_Theme' ) ) {

	class Banda_Delivery_Notes_Theme {
		...
		public function load_hooks() {	
			// hooks
			add_filter( 'banda_my_account_my_orders_actions', array( $this, 'create_print_button_account_page' ), 10, 2 );
			add_action( 'banda_view_order', array( $this, 'create_print_button_order_page' ) );
			add_action( 'banda_thankyou', array( $this, 'create_print_button_order_page' ) );
			add_action( 'wp_enqueue_scripts', array( $this, 'add_scripts' ) );
			add_action( 'banda_email_after_order_table', array( $this, 'add_email_print_url' ), 100, 3 );
		}
		...
		public function add_scripts() {
			if ( is_account_page() || is_order_received_page() || $this->is_banda_tracking_page() ) {
				wp_enqueue_script( 'banda-delivery-notes-print-link', Banda_Delivery_Notes::$plugin_url . 'js/jquery.print-link.js', array( 'jquery' ) );
				wp_enqueue_script( 'banda-delivery-notes-theme', Banda_Delivery_Notes::$plugin_url . 'js/theme.js', array( 'jquery', 'banda-delivery-notes-print-link' ) );
			}
		}		
		...
		public function create_print_button_account_page( $actions, $order ) {			
			if( get_option( Banda_Delivery_Notes::$plugin_prefix . 'print_button_on_my_account_page' ) ) {				
				// Add the print button
				$actions['print'] = array(
					'url'  => wcdn_get_print_link( $order->id, $this->get_template_type( $order ) ),
					'name' => __( 'Print', 'banda-delivery-notes' )
				);
			}		
			return $actions;
		}
		...
		public function create_print_button_order_page( $order_id ) {
			...
			if( get_option( Banda_Delivery_Notes::$plugin_prefix . 'print_button_on_view_order_page' ) ) {				
				...
				if( $this->is_banda_tracking_page() ) {
				...
				?>
				<p class="order-print">
					<a href="<?php echo $print_url; ?>" class="button print"><?php _e( 'Print', 'banda-delivery-notes' ); ?></a>
				</p>
				<?php
			}
		}
		...
		public function add_email_print_url( $order, $sent_to_admin = true, $plain_text = false ) {
			if( get_option( Banda_Delivery_Notes::$plugin_prefix . 'email_print_link' ) ) {				
				if( $order->billing_email && !$sent_to_admin ) {
					...
					if( $plain_text ) :
echo __( 'Print your order', 'banda-delivery-notes' ) . "\n\n";

echo $url . "\n";
 
echo "\n****************************************************\n\n";
					else : ?>
						<p><strong><?php _e( 'Print:', 'banda-delivery-notes' ); ?></strong> <a href="<?php echo $url; ?>"><?php _e( 'Open print view in browser', 'banda-delivery-notes' ); ?></a></p>
					<?php endif; 
				}
			}
		}
		...
		/**
		 * Is Banda 'Order Tracking' page
		 */
		public function is_banda_tracking_page() {
