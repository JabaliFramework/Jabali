...
class WC_API extends WC_Legacy_API {
	...
	public function __construct() {
		...
		add_action( 'banda_api_request', array( 'WC_Payment_Gateways', 'instance' ), 0 );
	...
	public function handle_api_requests() {
		...
		if ( ! empty( $wp->query_vars['wc-api'] ) ) {
			...
			do_action( 'banda_api_request', $api_request );

			// Is there actually something hooked into this API request? If not trigger 400 - Bad request.
			status_header( has_action( 'banda_api_' . $api_request ) ? 200 : 400 );

			// Trigger an action which plugins can hook into to fulfill the request.
			do_action( 'banda_api_' . $api_request );
	...
	private function rest_api_init() {
		...
		// REST API was included starting Jabali 4.4.
