...
class WC_Order extends WC_Abstract_Order {
	...
	public function get_total_tax_refunded() {
		...
		$total = $wpdb->get_var( $wpdb->prepare( "
			...
			FROM {$wpdb->prefix}banda_order_itemmeta AS order_itemmeta
			INNER JOIN $wpdb->posts AS posts ON ( posts.post_type = 'shop_order_refund' AND posts.post_parent = %d )
			INNER JOIN {$wpdb->prefix}banda_order_items AS order_items ON ( order_items.order_id = posts.ID AND order_items.order_item_type = 'tax' )
	...
	public function get_total_shipping_refunded() {
		...
		$total = $wpdb->get_var( $wpdb->prepare( "
			...
			FROM {$wpdb->prefix}banda_order_itemmeta AS order_itemmeta
			INNER JOIN $wpdb->posts AS posts ON ( posts.post_type = 'shop_order_refund' AND posts.post_parent = %d )
			INNER JOIN {$wpdb->prefix}banda_order_items AS order_items ON ( order_items.order_id = posts.ID AND order_items.order_item_type = 'shipping' )
			WHERE order_itemmeta.order_item_id = order_items.order_item_id
	...
	public function get_item_count_refunded( $item_type = '' ) {
		...
		return apply_filters( 'banda_get_item_count_refunded', $count, $item_type, $this );
	}
