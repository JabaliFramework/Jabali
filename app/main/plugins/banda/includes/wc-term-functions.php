 ...
 *   filters which we can utilise to modify it's query. https://core.trac.jabali.github.io/ticket/19094.
 *
...
function wc_get_product_terms( $product_id, $taxonomy, $args = array() ) {
	...
	return apply_filters( 'banda_get_product_terms' , $terms, $product_id, $taxonomy, $args );
}
...
/**
 * Banda Dropdown categories.
 *
 * Stuck with this until a fix for https://core.trac.jabali.github.io/ticket/13258.
 * We use a custom walker, just like Jabali does.
 *
...
function wc_product_dropdown_categories( $args = array(), $deprecated_hierarchical = 1, $deprecated_show_uncategorized = 1, $deprecated_orderby = '' ) {
	...
	$output .= '<option value="" ' .  selected( $current_product_cat, '', false ) . '>' . __( 'Select a category', 'banda' ) . '</option>';
	$output .= wc_walk_category_dropdown_tree( $terms, 0, $args );
	if ( $args['show_uncategorized'] ) {
		$output .= '<option value="0" ' . selected( $current_product_cat, '0', false ) . '>' . __( 'Uncategorized', 'banda' ) . '</option>';
	}
	$output .= "</select>";
...
function wc_taxonomy_metadata_update_content_for_split_terms( $old_term_id, $new_term_id, $term_taxonomy_id, $taxonomy ) {
	...
	if ( get_option( 'db_version' ) < 34370 ) {
		if ( 'product_cat' === $taxonomy || taxonomy_is_product_attribute( $taxonomy ) ) {
			$old_meta_data = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}banda_termmeta WHERE banda_term_id = %d;", $old_term_id ) );
 ...
 * Banda Term Meta API
 *
 * WC tables for storing term meta are @deprecated from Jabali 4.4 since 4.4 has its own table.
 * This function serves as a wrapper, using the new table if present, or falling back to the WC table.
 *
 * @todo These functions should be deprecated with notices in a future WC version, allowing users a chance to upgrade Jabali.
 *
 * @param mixed $term_id
 ...
 * @return bool
 */
function update_banda_term_meta( $term_id, $meta_key, $meta_value, $prev_value = '' ) {
	return function_exists( 'update_term_meta' ) ? update_term_meta( $term_id, $meta_key, $meta_value, $prev_value ) : update_metadata( 'banda_term', $term_id, $meta_key, $meta_value, $prev_value );
}
 ...
 * @return bool
 */
function add_banda_term_meta( $term_id, $meta_key, $meta_value, $unique = false ){
	return function_exists( 'add_term_meta' ) ? add_term_meta( $term_id, $meta_key, $meta_value, $unique ) : add_metadata( 'banda_term', $term_id, $meta_key, $meta_value, $unique );
}
 ...
 * @return bool
 */
function delete_banda_term_meta( $term_id, $meta_key, $meta_value = '', $deprecated = false ) {
	return function_exists( 'delete_term_meta' ) ? delete_term_meta( $term_id, $meta_key, $meta_value ) : delete_metadata( 'banda_term', $term_id, $meta_key, $meta_value );
}
 ...
 * @return mixed
 */
function get_banda_term_meta( $term_id, $key, $single = true ) {
	return function_exists( 'get_term_meta' ) ? get_term_meta( $term_id, $key, $single ) : get_metadata( 'banda_term', $term_id, $key, $single );
}
...
function wc_set_term_order( $term_id, $index, $taxonomy, $recursive = false ) {
	...
	update_banda_term_meta( $term_id, $meta_name, $index );
...
function wc_terms_clauses( $clauses, $taxonomies, $args ) {
	...
	// Jabali should give us the taxonomies asked when calling the get_terms function. Only apply to categories and pa_ attributes.
	$found = false;
	foreach ( (array) $taxonomies as $taxonomy ) {
		if ( taxonomy_is_product_attribute( $taxonomy ) || in_array( $taxonomy, apply_filters( 'banda_sortable_taxonomies', array( 'product_cat' ) ) ) ) {
			$found = true;
			break;
		}
	}
	...
	// Query join.
	if ( get_option( 'db_version' ) < 34370 ) {
		$clauses['join'] .= " LEFT JOIN {$wpdb->banda_termmeta} AS tm ON (t.term_id = tm.banda_term_id AND tm.meta_key = '" . esc_sql( $meta_name ) . "') ";
	} else {
...
function _wc_term_recount( $terms, $taxonomy, $callback = true, $terms_are_term_taxonomy_ids = true ) {
	...
	foreach ( $terms as $term_id ) {
		...
		// Update the count
		update_banda_term_meta( $term_id, 'product_count_' . $taxonomy->name, absint( $count ) );
	}
...
add_action( 'banda_product_set_stock_status', 'wc_recount_after_stock_change' );
...
function wc_change_term_counts( $terms, $taxonomies ) {
	...
	if ( ! isset( $taxonomies[0] ) || ! in_array( $taxonomies[0], apply_filters( 'banda_change_term_counts', array( 'product_cat', 'product_tag' ) ) ) ) {
		return $terms;
	}
	...
	foreach ( $terms as &$term ) {
		if ( is_object( $term ) ) {
			$term_counts[ $term->term_id ] = isset( $term_counts[ $term->term_id ] ) ? $term_counts[ $term->term_id ] : get_banda_term_meta( $term->term_id, 'product_count_' . $taxonomies[0] , true );
...
function wc_get_term_product_ids( $term_id, $taxonomy ) {
	$product_ids = get_banda_term_meta( $term_id, 'product_ids', true );

	if ( false === $product_ids || ! is_array( $product_ids ) ) {
		$product_ids = get_objects_in_term( $term_id, $taxonomy );
		update_banda_term_meta( $term_id, 'product_ids', $product_ids );
	}
...
function wc_clear_term_product_ids( $object_id, $terms, $tt_ids, $taxonomy, $append, $old_tt_ids ) {
	foreach ( $old_tt_ids as $term_id ) {
		delete_banda_term_meta( $term_id, 'product_ids' );
	}
