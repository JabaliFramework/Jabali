...
final class Banda {
	 ...
	 * Main Banda Instance.
	 *
	 * Ensures only one instance of Banda is loaded or can be loaded.
	 *
	 * @since 2.1
	 * @static
	 * @see WC()
	 * @return Banda - Main instance.
	 */
	...
	public function includes() {
		...
		if ( $this->is_request( 'cron' ) && 'yes' === get_option( 'banda_allow_tracking', 'no' ) ) {
	...
	/**
	 * Function used to Init Banda Template Functions - This makes them pluggable by plugins and themes.
	 */
	...
	/**
	 * Init Banda when Jabali Initialises.
	 */
	public function init() {
		// Before init action.
		do_action( 'before_banda_init' );
		...
		if ( $this->is_request( 'frontend' ) || $this->is_request( 'cron' ) ) {
			$session_class  = apply_filters( 'banda_session_handler', 'WC_Session_Handler' );
	 ...
	 *
	 * Locales found in:
	 *      - WP_LANG_DIR/banda/banda-LOCALE.mo
	 *      - WP_LANG_DIR/plugins/banda-LOCALE.mo
	 */
	public function load_plugin_textdomain() {
		$locale = apply_filters( 'plugin_locale', get_locale(), 'banda' );

		load_textdomain( 'banda', WP_LANG_DIR . '/banda/banda-' . $locale . '.mo' );
		load_plugin_textdomain( 'banda', false, plugin_basename( dirname( __FILE__ ) ) . '/i18n/languages' );
	}
	...
	public function template_path() {
		return apply_filters( 'banda_template_path', 'banda/' );
	}
	...
	public function api_request_url( $request, $ssl = null ) {
		...
		return esc_url_raw( apply_filters( 'banda_api_request_url', $api_request_url, $request, $ssl ) );
	}
	...
	private function load_webhooks() {
		if ( false === ( $webhooks = get_transient( 'banda_webhook_ids' ) ) ) {
			...
			set_transient( 'banda_webhook_ids', $webhooks );
		}
	...
	/**
	 * Banda Payment Token Meta API and Term/Order item Meta - set table names.
	 */
	public function wpdb_table_fix() {
		global $wpdb;
		$wpdb->payment_tokenmeta    = $wpdb->prefix . 'banda_payment_tokenmeta';
		$wpdb->order_itemmeta       = $wpdb->prefix . 'banda_order_itemmeta';
		$wpdb->tables[]             = 'banda_payment_tokenmeta';
		$wpdb->tables[]             = 'banda_order_itemmeta';

		if ( get_option( 'db_version' ) < 34370 ) {
			$wpdb->banda_termmeta = $wpdb->prefix . 'banda_termmeta';
			$wpdb->tables[]             = 'banda_termmeta';
		}
	}
