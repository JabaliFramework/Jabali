/* global wp, banda_admin_meta_boxes_variations, banda_admin, accounting */
jQuery( function( $ ) {
	...
	var wc_meta_boxes_product_variations_actions = {
		...
		init: function() {
			...
			$( '#banda-product-data' ).on( 'banda_variations_loaded', this.variations_loaded );
			$( document.body ).on( 'banda_variations_added', this.variation_added );
		},
		...
		variable_is_downloadable: function() {
			$( this ).closest( '.banda_variation' ).find( '.show_if_variation_downloadable' ).hide();

			if ( $( this ).is( ':checked' ) ) {
				$( this ).closest( '.banda_variation' ).find( '.show_if_variation_downloadable' ).show();
			}
		},
		...
		variable_is_virtual: function() {
			$( this ).closest( '.banda_variation' ).find( '.hide_if_variation_virtual' ).show();

			if ( $( this ).is( ':checked' ) ) {
				$( this ).closest( '.banda_variation' ).find( '.hide_if_variation_virtual' ).hide();
			}
		},
		...
		variable_manage_stock: function() {
			$( this ).closest( '.banda_variation' ).find( '.show_if_variation_manage_stock' ).hide();

			if ( $( this ).is( ':checked' ) ) {
				$( this ).closest( '.banda_variation' ).find( '.show_if_variation_manage_stock' ).show();
			}
		},
		...
		variations_loaded: function( event, needsUpdate ) {
			...
			var wrapper = $( '#banda-product-data' );
			...
			if ( ! needsUpdate ) {
				...
				$( '.banda_variation', wrapper ).each( function( index, el ) {
					var $el       = $( el ),
				...
				$( '.banda_variations .variation-needs-update', wrapper ).removeClass( 'variation-needs-update' );
			...
			$( '.banda_variations .tips, .banda_variations .help_tip, .banda_variations .banda-help-tip', wrapper ).tipTip({
				'attribute': 'data-tip',
				'fadeIn':    50,
				'fadeOut':   50,
				'delay':     200
			...
			// Allow sorting
			$( '.banda_variations', wrapper ).sortable({
				items:                '.banda_variation',
				cursor:               'move',
				axis:                 'y',
				handle:               '.sort',
		...
		set_menu_order: function( event ) {
			...
			var $menu_order  = $( this ).closest( '.banda_variation' ).find('.variation_menu_order');
			var value        = window.prompt( banda_admin_meta_boxes_variations.i18n_enter_menu_order, $menu_order.val() );
		...
		variation_row_indexes: function() {
			var wrapper      = $( '#variable_product_options' ).find( '.banda_variations' ),
				current_page = parseInt( wrapper.attr( 'data-page' ), 10 ),
				offset       = parseInt( ( current_page - 1 ) * banda_admin_meta_boxes_variations.variations_per_page, 10 );

			$( '.banda_variations .banda_variation' ).each( function ( index, el ) {
				$( '.variation_menu_order', el ).val( parseInt( $( el ).index( '.banda_variations .banda_variation' ), 10 ) + 1 + offset ).change();
			});
		}
	};
	...
	var wc_meta_boxes_product_variations_media = {
		...
		add_image: function( event ) {
			...
			if ( $button.is( '.remove' ) ) {
				...
				wc_meta_boxes_product_variations_media.setting_variation_image.find( 'img' ).eq( 0 ).attr( 'src', banda_admin_meta_boxes_variations.banda_placeholder_img_src );
				...
				wc_meta_boxes_product_variations_media.variable_image_frame = wp.media.frames.variable_image = wp.media({
					...
					title: banda_admin_meta_boxes_variations.i18n_choose_image,
					button: {
						text: banda_admin_meta_boxes_variations.i18n_set_image
					},
					states: [
	...
	var wc_meta_boxes_product_variations_ajax = {
		...
		init: function() {
			...
			$( document.body )
				.on( 'change', '#variable_product_options .banda_variations :input', this.input_changed )
		...
		check_for_changes: function() {
			var need_update = $( '#variable_product_options' ).find( '.banda_variations .variation-needs-update' );

			if ( 0 < need_update.length ) {
				if ( window.confirm( banda_admin_meta_boxes_variations.i18n_edited_variations ) ) {
		...
		initial_load: function() {
			if ( 0 === $( '#variable_product_options' ).find( '.banda_variations .banda_variation' ).length ) {
		...
		load_variations: function( page, per_page ) {
			page     = page || 1;
			per_page = per_page || banda_admin_meta_boxes_variations.variations_per_page;
			...
			$.ajax({
				url: banda_admin_meta_boxes_variations.ajax_url,
				data: {
					action:     'banda_load_variations',
					security:   banda_admin_meta_boxes_variations.load_variations_nonce,
					product_id: banda_admin_meta_boxes_variations.post_id,
				...
				success: function( response ) {
					...
					$( '#banda-product-data' ).trigger( 'banda_variations_loaded' );
		...
		save_changes: function( callback ) {
			...
			if ( 0 < need_update.length ) {
				...
				data.action          = 'banda_save_variations';
				data.security        = banda_admin_meta_boxes_variations.save_variations_nonce;
				data.product_id      = banda_admin_meta_boxes_variations.post_id;
				...
				$.ajax({
					...
					success: function( response ) {
						...
						$( '#banda-product-data' ).trigger( 'banda_variations_saved' );
		...
		save_variations: function() {
			$( '#variable_product_options' ).trigger( 'banda_variations_save_variations_button' );

			wc_meta_boxes_product_variations_ajax.save_changes( function( error ) {
				var wrapper = $( '#variable_product_options' ).find( '.banda_variations' ),
					current = wrapper.attr( 'data-page' );

				$( '#variable_product_options' ).find( '#banda_errors' ).remove();
		...
		save_on_submit: function( e ) {
			...
			if ( 0 < need_update.length ) {
				e.preventDefault();
				$( '#variable_product_options' ).trigger( 'banda_variations_save_variations_on_submit' );
		...
		cancel_variations: function() {
			var current = parseInt( $( '#variable_product_options' ).find( '.banda_variations' ).attr( 'data-page' ), 10 );

			$( '#variable_product_options' ).find( '.banda_variations .variation-needs-update' ).removeClass( 'variation-needs-update' );
			$( '.variations-defaults select' ).each( function() {
		...
		add_variation: function() {
			...
			var data = {
				action: 'banda_add_variation',
				post_id: banda_admin_meta_boxes_variations.post_id,
				loop: $( '.banda_variation' ).length,
				security: banda_admin_meta_boxes_variations.add_variation_nonce
			};

			$.post( banda_admin_meta_boxes_variations.ajax_url, data, function( response ) {
				var variation = $( response );
				variation.addClass( 'variation-needs-update' );

				$( '#variable_product_options' ).find( '.banda_variations' ).prepend( variation );
				$( 'button.cancel-variation-changes, button.save-variation-changes' ).removeAttr( 'disabled' );
				$( '#variable_product_options' ).trigger( 'banda_variations_added', 1 );
				wc_meta_boxes_product_variations_ajax.unblock();
			});
		...
		remove_variation: function() {
			...
			if ( window.confirm( banda_admin_meta_boxes_variations.i18n_remove_variation ) ) {
				var variation     = $( this ).attr( 'rel' ),
					variation_ids = [],
					data          = {
						action: 'banda_remove_variations'
					};
				...
				if ( 0 < variation ) {
					...
					data.security      = banda_admin_meta_boxes_variations.delete_variations_nonce;

					$.post( banda_admin_meta_boxes_variations.ajax_url, data, function() {
						var wrapper      = $( '#variable_product_options' ).find( '.banda_variations' ),
							current_page = parseInt( wrapper.attr( 'data-page' ), 10 ),
							total_pages  = Math.ceil( ( parseInt( wrapper.attr( 'data-total' ), 10 ) - 1 ) / banda_admin_meta_boxes_variations.variations_per_page ),
							page         = 1;

						$( '#banda-product-data' ).trigger( 'banda_variations_removed' );
		...
		link_all_variations: function() {
			...
			if ( window.confirm( banda_admin_meta_boxes_variations.i18n_link_all_variations ) ) {
				wc_meta_boxes_product_variations_ajax.block();

				var data = {
					action: 'banda_link_all_variations',
					post_id: banda_admin_meta_boxes_variations.post_id,
					security: banda_admin_meta_boxes_variations.link_variation_nonce
				};
				...
				$.post( banda_admin_meta_boxes_variations.ajax_url, data, function( response ) {
					...
					if ( 1 === count ) {
						window.alert( count + ' ' + banda_admin_meta_boxes_variations.i18n_variation_added );
					} else if ( 0 === count || count > 1 ) {
						window.alert( count + ' ' + banda_admin_meta_boxes_variations.i18n_variations_added );
					} else {
						window.alert( banda_admin_meta_boxes_variations.i18n_no_variations_added );
					}

					if ( count > 0 ) {
						wc_meta_boxes_product_variations_pagenav.go_to_page( 1, count );
						$( '#variable_product_options' ).trigger( 'banda_variations_added', count );
					} else {
						wc_meta_boxes_product_variations_ajax.unblock();
					}
				});
		...
		input_changed: function() {
			$( this )
				.closest( '.banda_variation' )
				.addClass( 'variation-needs-update' );

			$( 'button.cancel-variation-changes, button.save-variation-changes' ).removeAttr( 'disabled' );

			$( '#variable_product_options' ).trigger( 'banda_variations_input_changed' );
		},
		...
		defaults_changed: function() {
			$( this )
				...
				.find( '.banda_variation:first' )
				.addClass( 'variation-needs-update' );

			$( 'button.cancel-variation-changes, button.save-variation-changes' ).removeAttr( 'disabled' );

			$( '#variable_product_options' ).trigger( 'banda_variations_defaults_changed' );
		},
		...
		do_variation_action: function() {
			...
			switch ( do_variation_action ) {
				...
				case 'link_all_variations' :
					...
					return;
				case 'delete_all' :
					if ( window.confirm( banda_admin_meta_boxes_variations.i18n_delete_all_variations ) ) {
						if ( window.confirm( banda_admin_meta_boxes_variations.i18n_last_warning ) ) {
							data.allowed = true;
							changes      = parseInt( $( '#variable_product_options' ).find( '.banda_variations' ).attr( 'data-total' ), 10 ) * -1;
						}
					}
					break;
				...
				case 'variable_sale_price_decrease' :
					value = window.prompt( banda_admin_meta_boxes_variations.i18n_enter_a_value_fixed_or_percent );

					if ( value != null ) {
						if ( value.indexOf( '%' ) >= 0 ) {
							data.value = accounting.unformat( value.replace( /\%/, '' ), banda_admin.mon_decimal_point ) + '%';
						} else {
							data.value = accounting.unformat( value, banda_admin.mon_decimal_point );
						}
					}
					break;
				...
				case 'variable_download_expiry' :
					value = window.prompt( banda_admin_meta_boxes_variations.i18n_enter_a_value );
				...
				case 'variable_sale_schedule' :
					data.date_from = window.prompt( banda_admin_meta_boxes_variations.i18n_scheduled_sale_start );
					data.date_to   = window.prompt( banda_admin_meta_boxes_variations.i18n_scheduled_sale_end );
			...
			$.ajax({
				url: banda_admin_meta_boxes_variations.ajax_url,
				data: {
					action:       'banda_bulk_edit_variations',
					security:     banda_admin_meta_boxes_variations.bulk_edit_variations_nonce,
					product_id:   banda_admin_meta_boxes_variations.post_id,
	...
	var wc_meta_boxes_product_variations_pagenav = {
		...
		init: function() {
			$( document.body )
				.on( 'banda_variations_added', this.update_single_quantity )
		...
		update_variations_count: function( qty ) {
			...
			if ( 1 === total ) {
				displaying_num.text( banda_admin_meta_boxes_variations.i18n_variation_count_single.replace( '%qty%', total ) );
			} else {
				displaying_num.text( banda_admin_meta_boxes_variations.i18n_variation_count_plural.replace( '%qty%', total ) );
			}
		...
		set_paginav: function( qty ) {
			var wrapper          = $( '#variable_product_options' ).find( '.banda_variations' ),
				...
				total_pages      = Math.ceil( new_qty / banda_admin_meta_boxes_variations.variations_per_page ),
				options          = '';
		...
		page_selector: function() {
			var selected = parseInt( $( this ).val(), 10 ),
				wrapper  = $( '#variable_product_options' ).find( '.banda_variations' );
		...
		last_page: function() {
			if ( wc_meta_boxes_product_variations_pagenav.check_is_enabled( this ) ) {
				var last_page = $( '#variable_product_options' ).find( '.banda_variations' ).attr( 'data-total_pages' );
