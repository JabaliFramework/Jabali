			...
			ShippingMethod       = Backbone.Model.extend({
				...
				save: function() {
					if ( _.size( this.changes ) ) {
						$.post( ajaxurl + ( ajaxurl.indexOf( '?' ) > 0 ? '&' : '?' ) + 'action=banda_shipping_zone_methods_save_changes', {
			...
			ShippingMethodView = Backbone.View.extend({
				...
				onConfigureShippingMethodSubmitted: function( event, target, posted_data ) {
					if ( 'wc-modal-shipping-method-settings' === target ) {
						...
						$.post( ajaxurl + ( ajaxurl.indexOf( '?' ) > 0 ? '&' : '?' ) + 'action=banda_shipping_zone_methods_save_settings', {
							wc_shipping_zones_nonce : data.wc_shipping_zones_nonce,
							instance_id             : posted_data.instance_id,
							data                    : posted_data
						}, function( response, textStatus ) {
							if ( 'success' === textStatus && response.success ) {
								$( 'table.wc-shipping-zone-methods' ).parent().find( '#banda_errors' ).remove();
				...
				showErrors: function( errors ) {
					var error_html = '<div id="banda_errors" class="error notice is-dismissible">';
				...
				onAddShippingMethodSubmitted: function( event, target, posted_data ) {
					if ( 'wc-modal-add-shipping-method' === target ) {
						...
						$.post( ajaxurl + ( ajaxurl.indexOf( '?' ) > 0 ? '&' : '?' ) + 'action=banda_shipping_zone_add_method', {
							wc_shipping_zones_nonce : data.wc_shipping_zones_nonce,
