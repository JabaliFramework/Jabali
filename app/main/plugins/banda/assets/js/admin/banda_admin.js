...
jQuery( function ( $ ) {
	...
	$( document.body )
		...
		.on( 'wc_add_error_tip', function( e, element, error_type ) {
			...
			if ( element.parent().find( '.wc_error_tip' ).length === 0 ) {
				element.after( '<div class="wc_error_tip ' + error_type + '">' + banda_admin[error_type] + '</div>' );
		...
		.on( 'change', '.wc_input_price[type=text], .wc_input_decimal[type=text], .wc-order-totals #refund_amount[type=text]', function() {
			...
			if ( $( this ).is( '.wc_input_price' ) || $( this ).is( '#refund_amount' ) ) {
				regex = new RegExp( '[^\-0-9\%\\' + banda_admin.mon_decimal_point + ']+', 'gi' );
			} else {
				regex = new RegExp( '[^\-0-9\%\\' + banda_admin.decimal_point + ']+', 'gi' );
			}
		...
		.on( 'change', '#_sale_price.wc_input_price[type=text], .wc_input_price[name^=variable_sale_price]', function() {
			...
			var sale_price    = parseFloat( window.accounting.unformat( sale_price_field.val(), banda_admin.mon_decimal_point ) );
			var regular_price = parseFloat( window.accounting.unformat( regular_price_field.val(), banda_admin.mon_decimal_point ) );
		...
		.on( 'init_tooltips', function() {
			...
			$( '.tips, .help_tip, .banda-help-tip' ).tipTip( tiptip_args );
	...
	$( '.banda_page_wc-settings .shippingrows tbody tr:even, table.attributes-table tbody tr:nth-child(odd)' ).addClass( 'alternate' );
	...
	// Demo store notice
	$( 'input#banda_demo_store' ).change(function() {
		if ( $( this ).is( ':checked' ) ) {
			$( '#banda_demo_store_notice' ).closest( 'tr' ).show();
		} else {
			$( '#banda_demo_store_notice' ).closest( 'tr' ).hide();
		}
	}).change();
	...
	$( '#contextual-help-link' ).on( 'click', function() {
		var frame = $( '#tab-panel-banda_101_tab iframe' );
