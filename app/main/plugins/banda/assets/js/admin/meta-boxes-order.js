/*global banda_admin_meta_boxes, banda_admin, accounting, banda_admin_meta_boxes_order */
jQuery( function ( $ ) {
	...
	var wc_meta_boxes_order = {
		states: null,
		init: function() {
			if ( ! ( typeof banda_admin_meta_boxes_order === 'undefined' || typeof banda_admin_meta_boxes_order.countries === 'undefined' ) ) {
				/* State/Country select boxes */
				this.states = $.parseJSON( banda_admin_meta_boxes_order.countries.replace( /&quot;/g, '"' ) );
			}
			...
			$( '#banda-order-actions input, #banda-order-actions a' ).click(function() {
		...
		change_country: function( e, stickValue ) {
			...
			var $this = $( this ),
				...
				value = $this.data( 'banda.stickState-' + country ) ? $this.data( 'banda.stickState-' + country ) : $state.val(),
				placeholder = $state.attr( 'placeholder' );

			if ( stickValue ){
				$this.data( 'banda.stickState-' + country, value );
			}
			...
			if ( ! $.isEmptyObject( wc_meta_boxes_order.states[ country ] ) ) {
				...
				$states_select.append( $( '<option value="">' + banda_admin_meta_boxes_order.i18n_select_state_text + '</option>' ) );
			...
			$( document.body ).trigger( 'contry-change.banda', [country, $( this ).closest( 'div' )] );
			$( document.body ).trigger( 'country-change.banda', [country, $( this ).closest( 'div' )] );
		},
		...
		change_state: function() {
			...
			$country.data( 'banda.stickState-' + country, state );
		},
		...
		load_billing: function( force ) {
			if ( true === force || window.confirm( banda_admin_meta_boxes.load_billing ) ) {

				// Get user ID to load data for
				var user_id = $( '#customer_user' ).val();

				if ( ! user_id ) {
					window.alert( banda_admin_meta_boxes.no_customer_selected );
					return false;
				}
				...
				var data = {
					...
					action:       'banda_get_customer_details',
					security:     banda_admin_meta_boxes.get_customer_details_nonce
				};
				...
				$.ajax({
					url: banda_admin_meta_boxes.ajax_url,
					data: data,
					type: 'POST',
		...
		load_shipping: function( force ) {
			if ( true === force || window.confirm( banda_admin_meta_boxes.load_shipping ) ) {
		...
		copy_billing_to_shipping: function() {
			if ( window.confirm( banda_admin_meta_boxes.copy_billing ) ) {
	...
	var wc_meta_boxes_order_items = {
		...
		unblock: function() {
			$( '#banda-order-items' ).unblock();
		},

		reload_items: function() {
			var data = {
				order_id: banda_admin_meta_boxes.post_id,
				action:   'banda_load_order_items',
				security: banda_admin_meta_boxes.order_item_nonce
			};
			...
			$.ajax({
				...
				type: 'POST',
				success: function( response ) {
					$( '#banda-order-items' ).find( '.inside' ).empty();
					$( '#banda-order-items' ).find( '.inside' ).append( response );
		...
		quantity_changed: function() {
			...
			// Totals
			var unit_total = accounting.unformat( line_total.attr( 'data-total' ), banda_admin.mon_decimal_point ) / o_qty;
			line_total.val(
				parseFloat( accounting.formatNumber( unit_total * qty, banda_admin_meta_boxes.rounding_precision, '' ) )
					.toString()
					.replace( '.', banda_admin.mon_decimal_point )
			);

			var unit_subtotal = accounting.unformat( line_subtotal.attr( 'data-subtotal' ), banda_admin.mon_decimal_point ) / o_qty;
			line_subtotal.val(
				parseFloat( accounting.formatNumber( unit_subtotal * qty, banda_admin_meta_boxes.rounding_precision, '' ) )
					.toString()
					.replace( '.', banda_admin.mon_decimal_point )
			);
			...
			$( 'input.line_tax', $row ).each( function() {
				...
				var unit_total_tax     = accounting.unformat( $line_total_tax.attr( 'data-total_tax' ), banda_admin.mon_decimal_point ) / o_qty;
				var $line_subtotal_tax = $( 'input.line_subtotal_tax[data-tax_id="' + tax_id + '"]', $row );
				var unit_subtotal_tax  = accounting.unformat( $line_subtotal_tax.attr( 'data-subtotal_tax' ), banda_admin.mon_decimal_point ) / o_qty;

				if ( 0 < unit_total_tax ) {
					$line_total_tax.val(
						parseFloat( accounting.formatNumber( unit_total_tax * qty, banda_admin_meta_boxes.rounding_precision, '' ) )
							.toString()
							.replace( '.', banda_admin.mon_decimal_point )
					);
				}
				...
				if ( 0 < unit_subtotal_tax ) {
					$line_subtotal_tax.val(
						parseFloat( accounting.formatNumber( unit_subtotal_tax * qty, banda_admin_meta_boxes.rounding_precision, '' ) )
							.toString()
							.replace( '.', banda_admin.mon_decimal_point )
					);
				}
		...
		refund_items: function() {
			...
			$( '#banda-order-items' ).find( 'div.refund' ).show();
		...
		cancel: function() {
			...
			$( '#banda-order-items' ).find( 'div.refund' ).hide();
		...
		add_fee: function() {
			...
			var data = {
				action:   'banda_add_order_fee',
				order_id: banda_admin_meta_boxes.post_id,
				security: banda_admin_meta_boxes.order_item_nonce
			};

			$.post( banda_admin_meta_boxes.ajax_url, data, function( response ) {
				$( 'table.banda_order_items tbody#order_fee_line_items' ).append( response );
				wc_meta_boxes_order_items.unblock();
			});
		...
		add_shipping: function() {
			...
			var data = {
				action:   'banda_add_order_shipping',
				order_id: banda_admin_meta_boxes.post_id,
				security: banda_admin_meta_boxes.order_item_nonce
			};

			$.post( banda_admin_meta_boxes.ajax_url, data, function( response ) {
				$( 'table.banda_order_items tbody#order_shipping_line_items' ).append( response );
				wc_meta_boxes_order_items.unblock();
			});
		...
		delete_item: function() {
			var answer = window.confirm( banda_admin_meta_boxes.remove_item_notice );
			...
			if ( answer ) {
				...
				var data = {
					...
					action:         'banda_remove_order_item',
					security:       banda_admin_meta_boxes.order_item_nonce
				};
		...
		delete_tax: function() {
			if ( window.confirm( banda_admin_meta_boxes.i18n_delete_tax ) ) {
				wc_meta_boxes_order_items.block();

				var data = {
					action:   'banda_remove_order_tax',
		...
		calculate_tax: function() {
			if ( window.confirm( banda_admin_meta_boxes.calc_line_taxes ) ) {
				wc_meta_boxes_order_items.block();
				...
				if ( 'shipping' === banda_admin_meta_boxes.tax_based_on ) {
				...
				if ( 'billing' === banda_admin_meta_boxes.tax_based_on || ! country ) {
				...
				var data = {
					action:   'banda_calc_line_taxes',
					order_id: banda_admin_meta_boxes.post_id,
					items:    $( 'table.banda_order_items :input[name], .wc-order-totals-items :input[name]' ).serialize(),
					country:  country,
					state:    state,
					postcode: postcode,
					city:     city,
					security: banda_admin_meta_boxes.calc_totals_nonce
				};
		...
		calculate_totals: function() {
			if ( window.confirm( banda_admin_meta_boxes.calc_totals ) ) {
				...
				$( '.banda_order_items tr.shipping input.line_total' ).each(function() {
					var cost  = $( this ).val() || '0';
					cost      = accounting.unformat( cost, banda_admin.mon_decimal_point );
					shipping  = shipping + parseFloat( cost );
				});

				$( '.banda_order_items input.line_tax' ).each(function() {
					var cost = $( this ).val() || '0';
					cost     = accounting.unformat( cost, banda_admin.mon_decimal_point );
					tax      = tax + parseFloat( cost );
				});

				$( '.banda_order_items tr.item, .banda_order_items tr.fee' ).each(function() {
					var line_total = $( this ).find( 'input.line_total' ).val() || '0';
					line_totals    = line_totals + accounting.unformat( line_total.replace( ',', '.' ) );
				});

				// Tax
				if ( 'yes' === banda_admin_meta_boxes.round_at_subtotal ) {
					tax = parseFloat( accounting.toFixed( tax, banda_admin_meta_boxes.rounding_precision ) );
				}

				// Set Total
				$( '#_order_total' )
					.val( accounting.formatNumber( line_totals + tax + shipping, banda_admin_meta_boxes.currency_format_num_decimals, '', banda_admin.mon_decimal_point ) )
					.change();
		...
		refunds: {
			...
			do_refund: function() {
				...
				if ( window.confirm( banda_admin_meta_boxes.i18n_do_refund ) ) {
					...
					$( '.refund input.refund_line_total' ).each(function( index, item ) {
						if ( $( item ).closest( 'tr' ).data( 'order_item_id' ) ) {
							line_item_totals[ $( item ).closest( 'tr' ).data( 'order_item_id' ) ] = accounting.unformat( item.value, banda_admin.mon_decimal_point );
						}
					});
					...
					$( '.refund input.refund_line_tax' ).each(function( index, item ) {
						if ( $( item ).closest( 'tr' ).data( 'order_item_id' ) ) {
							...
							line_item_tax_totals[ $( item ).closest( 'tr' ).data( 'order_item_id' ) ][ tax_id ] = accounting.unformat( item.value, banda_admin.mon_decimal_point );
						}
					});

					var data = {
						action:                 'banda_refund_line_items',
						order_id:               banda_admin_meta_boxes.post_id,
			...
			delete_refund: function() {
				if ( window.confirm( banda_admin_meta_boxes.i18n_delete_refund ) ) {
			...
			input_changed: function() {
				var refund_amount = 0;
				var $items        = $( '.banda_order_items' ).find( 'tr.item, tr.fee, tr.shipping' );
				...
				$items.each(function() {
					...
					refund_cost_fields.each(function( index, el ) {
						refund_amount += parseFloat( accounting.unformat( $( el ).val() || 0, banda_admin.mon_decimal_point ) );
					});
				});
				...
				$( '#refund_amount' )
					.val( accounting.formatNumber(
						refund_amount,
						banda_admin_meta_boxes.currency_format_num_decimals,
						'',
						banda_admin.mon_decimal_point
					) )
					.change();
			},

			amount_changed: function() {
				var total = accounting.unformat( $( this ).val(), banda_admin.mon_decimal_point );

				$( 'button .wc-order-refund-amount .amount' ).text( accounting.formatMoney( total, {
					symbol:    banda_admin_meta_boxes.currency_format_symbol,
					decimal:   banda_admin_meta_boxes.currency_format_decimal_sep,
					thousand:  banda_admin_meta_boxes.currency_format_thousand_sep,
					precision: banda_admin_meta_boxes.currency_format_num_decimals,
					format:    banda_admin_meta_boxes.currency_format
				} ) );
			},
			...
			refund_quantity_changed: function() {
				...
				// Totals
				var unit_total = accounting.unformat( line_total.attr( 'data-total' ), banda_admin.mon_decimal_point ) / qty;

				refund_line_total.val(
					parseFloat( accounting.formatNumber( unit_total * refund_qty, banda_admin_meta_boxes.rounding_precision, '' ) )
						.toString()
						.replace( '.', banda_admin.mon_decimal_point )
				).change();
				...
				$( '.refund_line_tax', $row ).each( function() {
					...
					var unit_total_tax         = accounting.unformat( line_total_tax.data( 'total_tax' ), banda_admin.mon_decimal_point ) / qty;

					if ( 0 < unit_total_tax ) {
						$refund_line_total_tax.val(
							parseFloat( accounting.formatNumber( unit_total_tax * refund_qty, banda_admin_meta_boxes.rounding_precision, '' ) )
								.toString()
								.replace( '.', banda_admin.mon_decimal_point )
						).change();
					} else {
				...
				if ( refund_qty > 0 ) {
					...
					$( '.banda_order_items input.refund_order_item_qty' ).each( function() {
						if ( $( this ).val() > 0 ) {
							$( '#restock_refunded_items' ).closest( 'tr' ).show();
						}
					});
		...
		item_meta: {
			...
			add: function() {
				...
				var data = {
					...
					action:        'banda_add_order_item_meta',
					security:      banda_admin_meta_boxes.order_item_nonce
				};
			...
			remove: function() {
				if ( window.confirm( banda_admin_meta_boxes.remove_item_meta ) ) {
					var $row = $( this ).closest( 'tr' );

					var data = {
						meta_id:  $row.attr( 'data-meta_id' ),
						action:   'banda_remove_order_item_meta',
						security: banda_admin_meta_boxes.order_item_nonce
					};
		...
		bulk_actions: {
			...
			do_delete: function( e ) {
				e.preventDefault();
				var $table = $( 'table.banda_order_items' );
				var $rows = $table.find( 'tr.selected' );

				if ( $rows.length && window.confirm( banda_admin_meta_boxes.remove_item_notice ) ) {
			...
			do_increase_stock: function( e ) {
				...
				var data = {
					...
					action:         'banda_increase_order_item_stock',
					security:       banda_admin_meta_boxes.order_item_nonce
				};
			...
			do_reduce_stock: function( e ) {
				...
				var data = {
					...
					order_item_ids: item_ids,
					order_item_qty: quantities,
					action:         'banda_reduce_order_item_stock',
					security:       banda_admin_meta_boxes.order_item_nonce
				};
		...
		backbone: {
			...
			add_item: function( add_item_ids ) {
				...
				if ( add_item_ids ) {
					...
					$.each( add_item_ids, function( index, value ) {
						...
						$.post( banda_admin_meta_boxes.ajax_url, data, function( response ) {
							$( 'table.banda_order_items tbody#order_line_items' ).append( response );
			...
			add_tax: function( rate_id, manual_rate_id ) {
				...
				if ( -1 === $.inArray( rate_id, rates ) ) {
					...
					window.alert( banda_admin_meta_boxes.i18n_tax_rate_already_exists );
				}
			}
		},
		...
		stupidtable: {
			init: function() {
				$( '.banda_order_items' ).stupidtable();
				$( '.banda_order_items' ).on( 'aftertablesort', this.add_arrows );
			},
	...
	var wc_meta_boxes_order_notes = {
		...
		add_order_note: function() {
			...
			var data = {
				action:    'banda_add_order_note',
				post_id:   banda_admin_meta_boxes.post_id,
				note:      $( 'textarea#add_order_note' ).val(),
				note_type: $( 'select#order_note_type' ).val(),
				security:  banda_admin_meta_boxes.add_order_note_nonce
			};

			$.post( banda_admin_meta_boxes.ajax_url, data, function( response ) {
				$( 'ul.order_notes' ).prepend( response );
				$( '#banda-order-notes' ).unblock();
				$( '#add_order_note' ).val( '' );
			});
		...
		delete_order_note: function() {
			if ( window.confirm( banda_admin_meta_boxes.i18n_delete_note ) ) {
				...
				var data = {
					action:   'banda_delete_order_note',
					note_id:  $( note ).attr( 'rel' ),
					security: banda_admin_meta_boxes.delete_order_note_nonce
				};

				$.post( banda_admin_meta_boxes.ajax_url, data, function() {
					$( note ).remove();
				});
			}
	...
	var wc_meta_boxes_order_downloads = {
		...
		grant_access: function() {
			...
			var data = {
				action:      'banda_grant_access_to_download',
				product_ids: products,
				loop:        $('.order_download_permissions .wc-metabox').length,
				order_id:    banda_admin_meta_boxes.post_id,
				security:    banda_admin_meta_boxes.grant_access_nonce
			};
			...
			$.post( banda_admin_meta_boxes.ajax_url, data, function( response ) {
				...
				if ( response ) {
					...
					window.alert( banda_admin_meta_boxes.i18n_download_permission_fail );
				}
		...
		revoke_access: function () {
			if ( window.confirm( banda_admin_meta_boxes.i18n_permission_revoke ) ) {
				...
				if ( product > 0 ) {
					...
					var data = {
						action:        'banda_revoke_access_to_download',
						product_id:    product,
						download_id:   file,
						permission_id: permission_id,
						order_id:      banda_admin_meta_boxes.post_id,
						security:      banda_admin_meta_boxes.revoke_access_nonce
					};
