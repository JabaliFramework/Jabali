...
jQuery( function( $ ) {
	...
	// Type box.
	$( '.type_box' ).appendTo( '#banda-product-data .hndle span' );

	$( function() {
		// Prevent inputs in meta box headings opening/closing contents.
		$( '#banda-product-data' ).find( '.hndle' ).unbind( 'click.postboxes' );

		$( '#banda-product-data' ).on( 'click', '.hndle', function( event ) {
			...
			$( '#banda-product-data' ).toggleClass( 'closed' );
		});
	});
	...
	$( '#catalog-visibility' ).find( '.save-post-visibility' ).click( function() {
		...
		if ( $( 'input[name=_featured]' ).is( ':checked' ) ) {
			label = label + ', ' + banda_admin_meta_boxes.featured_label;
	...
	$( 'select#product-type' ).change( function() {
		...
		$( document.body ).trigger( 'banda-product-type-change', select_val, $( this ) );

	}).change();

	$( document.body ).on( 'banda-product-type-change', function( e, select_val ) {
		if ( 'variable' !== select_val && 0 < $( '#variable_product_options' ).find( 'input[name^=variable_sku]' ).length && $( document.body ).triggerHandler( 'banda-display-product-type-alert', select_val ) !== false ) {
			window.alert( banda_admin_meta_boxes.i18n_product_type_alert );
		}
	});
	...
	function show_and_hide_panels() {
		...
		$.each( banda_admin_meta_boxes.product_types, function( index, value ) {
		...
		$( '.banda_options_panel' ).each( function() {
	...
	$( '#banda-product-data' ).on( 'click', '.sale_schedule', function() {
	...
	$( '#banda-product-data' ).on( 'click', '.cancel_sale_schedule', function() {
		var $wrap = $( this ).closest( 'div, table' );
	...
	// File inputs.
	$( '#banda-product-data' ).on( 'click','.downloadable_files a.insert', function() {
		$( this ).closest( '.downloadable_files' ).find( 'tbody' ).append( $( this ).data( 'row' ) );
		return false;
	});
	$( '#banda-product-data' ).on( 'click','.downloadable_files a.delete',function() {
	...
	// Initial order.
	var banda_attribute_items = $( '.product_attributes' ).find( '.banda_attribute' ).get();

	banda_attribute_items.sort( function( a, b ) {
	   var compA = parseInt( $( a ).attr( 'rel' ), 10 );
	   var compB = parseInt( $( b ).attr( 'rel' ), 10 );
	   return ( compA < compB ) ? -1 : ( compA > compB ) ? 1 : 0;
	});
	$( banda_attribute_items ).each( function( index, el ) {
		$( '.product_attributes' ).append( el );
	});

	function attribute_row_indexes() {
		$( '.product_attributes .banda_attribute' ).each( function( index, el ) {
			$( '.attribute_position', el ).val( parseInt( $( el ).index( '.product_attributes .banda_attribute' ), 10 ) );
		});
	}
	...
	$( 'button.add_attribute' ).on( 'click', function() {
		var size         = $( '.product_attributes .banda_attribute' ).length;
		var attribute    = $( 'select.attribute_taxonomy' ).val();
		var $wrapper     = $( this ).closest( '#product_attributes' );
		var $attributes  = $wrapper.find( '.product_attributes' );
		var product_type = $( 'select#product-type' ).val();
		var data         = {
			action:   'banda_add_attribute',
			taxonomy: attribute,
			i:        size,
			security: banda_admin_meta_boxes.add_attribute_nonce
		};
		...
		$.post( banda_admin_meta_boxes.ajax_url, data, function( response ) {
			...
			$( document.body ).trigger( 'banda_added_attribute' );
		});
	...
	$( '.product_attributes' ).on( 'blur', 'input.attribute_name', function() {
		$( this ).closest( '.banda_attribute' ).find( 'strong.attribute_name' ).text( $( this ).val() );
	});
	...
	$( '.product_attributes' ).on( 'click', '.remove_row', function() {
		if ( window.confirm( banda_admin_meta_boxes.remove_attribute ) ) {
	...
	$( '.product_attributes' ).on( 'click', 'button.add_new_attribute', function() {
		...
		var $wrapper           = $( this ).closest( '.banda_attribute' );
		var attribute          = $wrapper.data( 'taxonomy' );
		var new_attribute_name = window.prompt( banda_admin_meta_boxes.new_attribute_prompt );

		if ( new_attribute_name ) {

			var data = {
				action:   'banda_add_new_attribute',
				taxonomy: attribute,
	...
	$( '.save_attributes' ).on( 'click', function() {
		...
		var data = {
			...
			action:   'banda_save_attributes',
			security: banda_admin_meta_boxes.save_attributes_nonce
		};

		$.post( banda_admin_meta_boxes.ajax_url, data, function() {
			// Reload variations panel.
			var this_page = window.location.toString();
			this_page = this_page.replace( 'post-new.php?', 'post.php?post=' + banda_admin_meta_boxes.post_id + '&action=edit&' );
