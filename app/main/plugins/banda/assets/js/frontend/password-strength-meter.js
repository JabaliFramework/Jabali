...
jQuery( function( $ ) {
	...
	var wc_password_strength_meter = {
		...
		includeMeter: function( wrapper, field ) {
			var meter = wrapper.find( '.banda-password-strength' );

			if ( '' === field.val() ) {
				meter.remove();
				$( document.body ).trigger( 'wc-password-strength-removed' );
			} else if ( 0 === meter.length ) {
				field.after( '<div class="banda-password-strength" aria-live="polite"></div>' );
		...
		checkPasswordStrength: function( wrapper, field ) {
			var meter     = wrapper.find( '.banda-password-strength' );
			var hint      = wrapper.find( '.banda-password-hint' );
			var hint_html = '<small class="banda-password-hint">' + wc_password_strength_meter_params.i18n_password_hint + '</small>';
