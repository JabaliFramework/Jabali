( function( $ ) {
	$( function() {
		var wcTokenizationForm = (function() {
			function wcTokenizationForm( target ) {
				...
				this.onDisplay = function() {
					...
					if ( 0 === $( ':input.banda-SavedPaymentMethods-tokenInput:checked', $target ).length ) {
						$( ':input.banda-SavedPaymentMethods-tokenInput:last', $target ).prop( 'checked', true );
					}

					// Don't show the "use new" radio button if we only have one method..
					if ( 0 === $target.data( 'count' ) ) {
						$( '.banda-SavedPaymentMethods-new', $target ).hide();
					}

					// Trigger change event
					$( ':input.banda-SavedPaymentMethods-tokenInput:checked', $target ).trigger( 'change' );
				...
				this.showSaveNewCheckbox = function() {
					$( '.banda-SavedPaymentMethods-saveNew', $formWrap ).show();
				};

				this.hideSaveNewCheckbox = function() {
					$( '.banda-SavedPaymentMethods-saveNew', $formWrap ).hide();
				};

				// When a radio button is changed, make sure to show/hide our new CC info area
				$( ':input.banda-SavedPaymentMethods-tokenInput', $target ).change( this.onTokenChange );
		...
		$( document.body ).on( 'updated_checkout wc-credit-card-form-init', function() {
			...
			var $saved_payment_methods = $( 'ul.banda-SavedPaymentMethods' );
