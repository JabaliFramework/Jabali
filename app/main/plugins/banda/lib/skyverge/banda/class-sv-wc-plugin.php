 ...
 * functionality and features suitable for the implementation of a Banda
...
abstract class SV_WC_Plugin {
	...
	public function __construct( $id, $version, $args = array() ) {
		...
		// include library files after banda is loaded
	...
	public function __clone() {
		...
		_doing_it_wrong( __FUNCTION__, sprintf( esc_html__( 'You cannot clone instances of %s.', 'banda-plugin-framework' ), $this->get_plugin_name() ), '3.1.0' );
	}
	...
	public function __wakeup() {
		/* translators: Placeholders: %s - plugin name */
		_doing_it_wrong( __FUNCTION__, sprintf( esc_html__( 'You cannot unserialize instances of %s.', 'banda-plugin-framework' ), $this->get_plugin_name() ), '3.1.0' );
	}
	...
	public function load_translations() {
		...
		$framework_domain = 'banda-plugin-framework';
	...
	public function plugin_action_links( $actions ) {
		...
		if ( $this->get_documentation_url() ) {
			...
			$custom_actions['docs'] = sprintf( '<a href="%s">%s</a>', $this->get_documentation_url(), esc_html__( 'Docs', 'banda-plugin-framework' ) );
		}

		// support url if any
		if ( $this->get_support_url() ) {
			$custom_actions['support'] = sprintf( '<a href="%s">%s</a>', $this->get_support_url(), esc_html_x( 'Support', 'noun', 'banda-plugin-framework' ) );
		}
	...
	/**
	 * Saves errors or messages to Banda Log (banda/logs/plugin-id-xxx.txt)
	 *
	 * @since 2.0.0
	...
	/**
	 * Returns the plugin full name including "Banda", ie
	 * "Banda X".  This method is defined abstract for localization purposes
	 *
	 * @since 2.0.0
	...
	public function get_settings_link( $plugin_id = null ) {
		...
		if ( $settings_url ) {
			return sprintf( '<a href="%s">%s</a>', $settings_url, esc_html__( 'Configure', 'banda-plugin-framework' ) );
		}
	 ...
	 * /path/to/main/plugins/plugin-directory
	 *
	 * @since 2.0.0
	 ...
	 * http://skyverge.com/main/plugins/plugin-directory
	 *
	 * @since 2.0.0
	...
	/**
	 * Returns the banda uploads path, without trailing slash.  Oddly Banda
	 * core does not provide a way to get this
	 *
	 * @since 2.0.0
	 * @return string upload path for banda
	 */
	public static function get_banda_uploads_path() {
		$upload_dir = wp_upload_dir();
		return $upload_dir['basedir'] . '/banda_uploads';
	}
