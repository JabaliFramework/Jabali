 ...
 *
 * @package   SkyVerge/Banda/Plugin/Classes
 * @author    SkyVerge
...
/**
 * # SkyVerge Banda Plugin Framework Bootstrap
 *
...
class SV_WC_Framework_Bootstrap {
	...
	public function load_framework_plugins() {
		...
		foreach ( $this->registered_plugins as $plugin ) {
			...
			if ( ! class_exists( 'SV_WC_Plugin' ) ) {
				require_once( $this->get_plugin_path( $plugin['path'] ) . '/lib/skyverge/banda/class-sv-wc-plugin.php' );
			...
			if ( isset( $plugin['args']['is_payment_gateway'] ) && ! class_exists( 'SV_WC_Payment_Gateway' ) ) {
				require_once( $this->get_plugin_path( $plugin['path'] ) . '/lib/skyverge/banda/payment-gateway/class-sv-wc-payment-gateway-plugin.php' );
			}
		 ...
		 * hook into this action rather than `plugins_loaded`/`banda_loaded`
		 * as needed.
		 *
		 * @since 2.0.0
		 */
	 ...
	 * versions, or incompatiblities with the current Banda or Jabali
	 * versions
	 *
	 * @since 2.0.0
	 */
	public function render_update_notices() {
		...
		if ( ! empty( $this->incompatible_framework_plugins ) ) {
			...
			echo '<div class="error">';
				...
				echo '<p>';
					echo esc_html( _n( 'The following plugin is inactive because it is out of date and requires a newer version to function properly:', 'The following plugins are inactive because they are out of date and require a newer version to function properly:', $plugin_count, 'banda-plugin-framework' ) );
				echo '</p>';
				...
				echo '<p>';
					printf(
						...
						esc_html( _n( 'To reactivate, please %1$supdate the above plugin (recommended)%2$s %3$sor%4$s %5$sdeactivate the following%6$s:', 'To reactivate, please %1$supdate the above plugins (recommended)%2$s %3$sor%4$s %5$sdeactivate the following%6$s:', $plugin_count, 'banda-plugin-framework' ) ),
		...
		if ( ! empty( $this->incompatible_wc_version_plugins ) ) {
			...
			printf( '<div class="error"><p>%s</p><ul>', count( $this->incompatible_wc_version_plugins ) > 1 ? esc_html__( 'The following plugins are inactive because they require a newer version of Banda:', 'banda-plugin-framework' ) : esc_html__( 'The following plugin is inactive because it requires a newer version of Banda:', 'banda-plugin-framework' ) );

			foreach ( $this->incompatible_wc_version_plugins as $plugin ) {

				/* translators: Placeholders: %1$s - plugin name, %2$s - Banda version number */
				echo '<li>' . sprintf( esc_html__( '%1$s requires Banda %2$s or newer', 'banda-plugin-framework' ), $plugin['plugin_name'], $plugin['args']['minimum_wc_version'] ) . '</li>';
			}

			/* translators: Placeholders: %1$s - <a> tag, %2$s - </a> tag */
			echo '</ul><p>' . sprintf( esc_html__( 'Please %1$supdate Banda%2$s', 'banda-plugin-framework' ), '<a href="' . admin_url( 'update-core.php' ) . '">', '&nbsp;&raquo;</a>' ) . '</p></div>';
		}

		// must update WP notice
		if ( ! empty( $this->incompatible_wp_version_plugins ) ) {

			printf( '<div class="error"><p>%s</p><ul>', count( $this->incompatible_wp_version_plugins ) > 1 ? 'The following plugins are inactive because they require a newer version of Jabali:' : 'The following plugin is inactive because it requires a newer version of Jabali:' );

			foreach ( $this->incompatible_wp_version_plugins as $plugin ) {
				printf( '<li>%s requires Jabali %s or newer</li>', $plugin['plugin_name'], $plugin['args']['minimum_wp_version'] );
			}

			echo '</ul><p>Please <a href="' . admin_url( 'update-core.php' ) . '">update Jabali&nbsp;&raquo;</a></p></div>';
		}
	}
	...
	/**
	 * Is the Banda plugin installed and active? This method is handy for
	 * frameworked plugins that are listed on jabali.github.io and thus don't have
	 * access to the Woo Helper functions bundled with Jabali-listed plugins.
	 *
	 ...
	 * need to check for Banda being active, define your own method.
	 *
	 * @since 4.0.0
	 * @return boolean true if the Banda plugin is installed and active
	 */
	public static function is_banda_active() {
		...
		return in_array( 'banda/banda.php', $active_plugins ) || array_key_exists( 'banda/banda.php', $active_plugins );
	}
	...
	/**
	 * Returns the Banda version number, backwards compatible to
	 * WC 1.5
	 *
	 * @since 3.0.0
	 * @return null|string
