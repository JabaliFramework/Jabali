...
if ( ! class_exists( 'YIT_Plugin_Panel_Banda' ) ) {
    /**
     * YIT Plugin Panel for Banda
     *
    ...
    class YIT_Plugin_Panel_Banda extends YIT_Plugin_Panel {
        ...
        public function __construct( $args = array() ) {
            ...
            if ( ! empty( $args ) ) {
                ...
                add_action( 'admin_init', array( $this, 'banda_update_options' ) );
	            add_filter( 'banda_screen_ids', array( $this, 'add_allowed_screen_id' ) );
                add_filter( 'banda_admin_settings_sanitize_option', array( $this, 'maybe_unserialize_panel_data' ), 10, 3 );

                /* Add VideoBox and InfoBox */
                add_action( 'banda_admin_field_boxinfo', array( $this, 'add_infobox' ), 10, 1 );
                add_action( 'banda_admin_field_videobox', array( $this, 'add_videobox' ), 10, 1 );
        ...
        public function yit_panel() {
            ...
            require_once( YIT_CORE_PLUGIN_TEMPLATE_PATH . '/panel/banda/banda-panel.php' );
        }
        ...
        public function yit_upload( $args = array() ) {
            if ( ! empty( $args ) ) {
                ...
                include( YIT_CORE_PLUGIN_TEMPLATE_PATH . '/panel/banda/banda-upload.php' );
            }
        }

	    /**
	     * Add the plugin banda page settings in the screen ids of banda
	     *
	     * @param $screen_ids
	     *
	     * @return mixed
        ...
        public function add_fields() {
            ...
            banda_admin_fields( $yit_options[$current_tab] );
        }
        ...
        public function print_panel_content() {
            ...
            if ( $custom_tab_action ) {
                ...
                require_once( YIT_CORE_PLUGIN_TEMPLATE_PATH . '/panel/banda/banda-form.php' );
            }
        }
        ...
        public function banda_update_options() {
            ...
            if ( isset( $_POST['yit_panel_wc_options_nonce'] ) && wp_verify_nonce( $_POST['yit_panel_wc_options_nonce'], 'yit_panel_wc_options_'.$this->settings['page'] ) ) {
                ...
                banda_update_options( $yit_options[ $current_tab ] );
        ...
        public function admin_enqueue_scripts() {
            global $banda, $pagenow;
            
            $banda_version = function_exists( 'WC' ) ? WC()->version : $banda->version;

            wp_enqueue_style( 'raleway-font', '//fonts.googleapis.com/css?family=Raleway:400,500,600,700,800,100,200,300,900' );

            wp_enqueue_media();
            wp_enqueue_style( 'banda_admin_styles', $banda->plugin_url() . '/assets/css/admin.css', array(), $banda_version );
            wp_register_style( 'yit-plugin-style', YIT_CORE_PLUGIN_URL . '/assets/css/yit-plugin-panel.css', $banda_version );
            wp_register_style( 'colorbox', YIT_CORE_PLUGIN_URL . '/assets/css/colorbox.css', array(), $banda_version );
            wp_register_style( 'yit-upgrade-to-pro', YIT_CORE_PLUGIN_URL . '/assets/css/yit-upgrade-to-pro.css', array( 'colorbox' ), $banda_version );
            ...
            $banda_settings_deps = array( 'jquery', 'jquery-ui-datepicker', 'jquery-ui-sortable', 'iris' );
            
            if( version_compare( '2.5', $banda_version, '<=' ) ){
                // Banda > 2.6
                $banda_settings_deps[] = 'select2';
            }
            
            else {
                // Banda < 2.6
                $banda_settings_deps[] = 'jquery-ui-dialog';
                $banda_settings_deps[] = 'chosen';
            }
            
            wp_enqueue_script( 'banda_settings', $banda->plugin_url() . '/assets/js/admin/settings.min.js', $banda_settings_deps, $banda_version, true );
        ...
        /**
         * Add the banda body class in plugin panel page
         *
         * @author Andrea Grillo <andrea.grillo@yithemes.com>
         * @since 2.0
        ...
        public function admin_body_class( $admin_body_classes ){
            global $pagenow;
            return 'admin.php' == $pagenow && substr_count( $admin_body_classes, 'banda' ) == 0 ? $admin_body_classes .= ' banda ' : $admin_body_classes;
        }
