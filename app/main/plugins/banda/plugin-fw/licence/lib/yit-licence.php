    ...
    abstract class YIT_Licence {
        ...
        public function get_response_code_message( $code, $args = array() ) {
            ...
            $messages = array(
                ...
                '108'           => __( 'Current product is not included with your membership key', 'banda-software-add-on' ),
                '200'           => sprintf( '<strong>%s</strong>! %s',__( 'Great', 'yith-plugin-fw' ), __( 'License successfully activated', 'yith-plugin-fw' ) ),
                'deactivated'   => sprintf( '%s <strong>%s</strong>', __( 'License key deactivated for website', 'banda-software-add-on' ), isset( $instance ) ? $instance : '' )
            );
        ...
        public function display_product_name( $product_name ){
            return str_replace( array( 'YITH', 'Banda', 'Premium', 'Theme' ), '', $product_name );
        }
