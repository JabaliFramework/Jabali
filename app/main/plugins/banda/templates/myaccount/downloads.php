 ...
 *
 * This template can be overridden by copying it to yourtheme/banda/myaccount/downloads.php.
 *
...
do_action( 'banda_before_account_downloads', $has_downloads ); ?>

<?php if ( $has_downloads ) : ?>

	<?php do_action( 'banda_before_available_downloads' ); ?>

	<table class="banda-MyAccount-downloads shop_table shop_table_responsive">
		<thead>
			<tr>
			...
			<tr>
				<?php foreach ( wc_get_account_downloads_columns() as $column_id => $column_name ) : ?>
					<td class="<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
						<?php if ( has_action( 'banda_account_downloads_column_' . $column_id ) ) : ?>
							<?php do_action( 'banda_account_downloads_column_' . $column_id, $download ); ?>
							...
							<?php
								$actions = array(
									'download'  => array(
										...
										'name' => __( 'Download', 'banda' )
									)
								);

								if ( $actions = apply_filters( 'banda_account_download_actions', $actions, $download ) ) {
									foreach ( $actions as $key => $action ) {
										echo '<a href="' . esc_url( $action['url'] ) . '" class="button banda-Button ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>';
									}
								}
							?>
...
<?php else : ?>
	<div class="banda-Message banda-Message--info banda-info">
		<a class="banda-Button button" href="<?php echo esc_url( apply_filters( 'banda_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
			<?php esc_html_e( 'Go Shop', 'banda' ) ?>
		</a>
		<?php esc_html_e( 'No downloads available yet.', 'banda' ); ?>
	</div>
<?php endif; ?>

<?php do_action( 'banda_after_account_downloads', $has_downloads ); ?>
