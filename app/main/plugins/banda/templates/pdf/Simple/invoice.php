		...
		<tr class="<?php echo apply_filters( 'wpo_wcpdf_item_row_class', $item_id, $wpo_wcpdf->export->template_type, $wpo_wcpdf->export->order, $item_id ); ?>">
			<td class="product">
				...
				<dl class="meta">
					...
					<?php if( !empty( $item['weight'] ) ) : ?><dt class="weight"><?php _e( 'Weight:', 'wpo_wcpdf' ); ?></dt><dd class="weight"><?php echo $item['weight']; ?><?php echo get_option('banda_weight_unit'); ?></dd><?php endif; ?>
				</dl>
	...
	<tfoot>
		<tr class="no-borders">
			...
			<td class="no-borders" colspan="2">
				<table class="totals">
					<tfoot>
						<?php foreach( $wpo_wcpdf->get_banda_totals() as $key => $total ) : ?>
