 ...
 *
 */
class DJengaShortCode_VC_Row extends DJengaShortCode {
	...
	protected function shortcodeScripts() {
		wp_register_script( 'vc_jquery_skrollr_js', vc_asset_url( 'lib/bower/skrollr/dist/skrollr.min.js' ), array( 'jquery' ), DJ_VC_VERSION, true );
		wp_register_script( 'vc_youtube_iframe_api_js', '//www.youtube.com/iframe_api', array(), DJ_VC_VERSION, true );
	}
	...
	public function contentAdmin( $atts, $content = null ) {
		...
		for ( $i = 0; $i < count( $width ); $i ++ ) {
			...
			$output .= '<div class="vc_row vc_row-fluid dj_row_container vc_container_for_children">';
	...
	public function cssAdminClass() {
		...
		return 'dj_' . $this->settings['base'] . $sortable . '' . ( ! empty( $this->settings['class'] ) ? ' ' . $this->settings['class'] : '' );
	}
	...
	public function customAdminBockParams() {
		// _deprecated_function( 'DJengaShortCode_VC_Row::customAdminBockParams', '4.5 (will be removed in 4.10)' );
	...
	public function buildStyle( $bg_image = '', $bg_color = '', $bg_image_repeat = '', $font_color = '', $padding = '', $margin_bottom = '' ) {
		// _deprecated_function( 'DJengaShortCode_VC_Row::buildStyle', '4.5 (will be removed in 4.10)' );
