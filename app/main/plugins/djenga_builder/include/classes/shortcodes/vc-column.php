 ...
 *
 */
class DJengaShortCode_VC_Column extends DJengaShortCode {
	/**
	 * @var array
	 */
	...
	public function contentAdmin( $atts, $content = null ) {
		...
		for ( $i = 0; $i < count( $width ); $i ++ ) {
			...
			$output .= str_replace( '%column_size%', dj_translateColumnWidthToFractional( $width[ $i ] ), $column_controls );
			...
			$output .= '</div>';
			$output .= str_replace( '%column_size%', dj_translateColumnWidthToFractional( $width[ $i ] ), $column_controls_bottom );
			$output .= '</div>';
		}
	...
	public function mainHtmlBlockParams( $width, $i ) {
		...
		return 'data-element_type="' . $this->settings['base'] . '" data-vc-column-width="' . dj_vc_get_column_width_indent( $width[ $i ] ) . '" class="dj_' . $this->settings['base'] . ' ' . $sortable . '' . ( ! empty( $this->settings['class'] ) ? ' ' . $this->settings['class'] : '' ) . ' ' . $this->templateWidth() . ' dj_content_holder"' . $this->customAdminBlockParams();
	}
	...
	public function containerHtmlBlockParams( $width, $i ) {
		return 'class="dj_column_container vc_container_for_children"';
	}
