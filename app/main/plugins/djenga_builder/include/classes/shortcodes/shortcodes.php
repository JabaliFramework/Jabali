...
/**
 * Djenga Builder Shortcodes main
 *
...
if ( ! class_exists( 'DJengaVisualComposerAbstract' ) ) {
	/**
	 * Class DJengaVisualComposerAbstract
	 */
	abstract class DJengaVisualComposerAbstract {
		/**
		 * @var
		 */
		...
		public function assetURL( $asset ) {
			// _deprecated_function( 'DJengaShortCode::assetURL', '4.5 (will be removed in 4.10)', 'vc_asset_url' );
...
if ( ! class_exists( 'DJengaShortCode' ) ) {
	/**
	 * Class DJengaShortCode
	 */
	abstract class DJengaShortCode extends DJengaVisualComposerAbstract {
		...
		public function printIconStyles() {
            ...
            <style>
                ...
                .dj_' . esc_attr( $this->settings['base'] ) . ' > .dj_element_wrapper > .dj_element_title > .vc_element-icon,
                .vc_el-container > #' . esc_attr( $this->settings['base'] ) . ' > .vc_element-icon,
                ...
                .dj_' . esc_attr( $this->settings['base'] ) . ' > .dj_element_wrapper > .dj_element_title > .vc_element-icon,
                .dj_' . esc_attr( $this->settings['base'] ) . ' > .dj_element_wrapper > .dj_element_title > .vc_element-icon[data-is-container="true"] {
		...
		protected function registerJs( $param ) {
			if ( is_array( $param ) && ! empty( $param ) ) {
				...
				wp_register_script( $name, $param, array( 'jquery' ), DJ_VC_VERSION, true );
			}
		}
		...
		protected function registerCss( $param ) {
			if ( is_array( $param ) && ! empty( $param ) ) {
				...
				wp_register_style( $name, $param, array( 'js_composer' ), DJ_VC_VERSION );
			}
		}
		...
		protected function loadTemplate( $atts, $content = null ) {
			...
			if ( $this->html_template ) {
				...
				trigger_error( sprintf( __( 'Template file is missing for `%s` shortcode. Make sure you have `%s` file in your theme folder.', 'js_composer' ), $this->shortcode, 'main/themes/your_theme/vc_templates/' . $this->shortcode . '.php' ) );
			}
		...
		public function contentAdmin( $atts, $content = null ) {
			...
			if ( isset( $this->settings['custom_markup'] ) && '' !== $this->settings['custom_markup'] ) {
				...
				$elem = str_ireplace( '%dj_element_content%', $this->customMarkup( $markup, $content ), $elem );
				$output .= $elem;
			} else {
				$inner = $this->outputTitle( $this->settings['name'] );
				$inner .= $this->paramsHtmlHolders( $atts );
				$elem = str_ireplace( '%dj_element_content%', $inner, $elem );
				$output .= $elem;
			}
		...
		public function enqueueDefaultScripts() {
			if ( false === self::$default_scripts_enqueued ) {
				wp_enqueue_script( 'dj_composer_front_js' );
		...
		/**
		 * Return shortcode attributes, see \DJengaShortCode::output
		 * @since 4.4
		 * @return array
		 */
		...
		public function getCSSAnimation( $css_animation ) {
			...
			if ( '' !== $css_animation ) {
				...
				$output = ' dj_animate_when_almost_visible dj_' . $css_animation . ' ' . $css_animation;
			}
		...
		/**
		 * Create HTML comment for blocks only if dj_debug=true
		 *
		...
		public function endBlockComment( $string ) {
			// _deprecated_function( 'DJengaShortCode::endBlockComment', '4.7 (will be removed in 4.10)', 'vc_asset_url' );

			return dj_debug() ? '<!-- END ' . $string . ' -->' : '';
		}

		/**
		 * if dj_debug=true return HTML comment
		 *
		 * @since 4.7
		 *
		...
		public function getElementHolder( $width ) {
			...
			$sortable = ( vc_user_access_check_shortcode_all( $this->shortcode ) ? 'dj_sortable' : $this->nonDraggableClass );
			$css_class = 'dj_' . $this->settings['base'] . ' dj_content_element ' . $sortable . '' . ( ! empty( $this->settings['class'] ) ? ' ' . $this->settings['class'] : '' );
			$output .= '<div data-element_type="' . $this->settings['base'] . '" class="' . $css_class . '">';
			$output .= str_replace( '%column_size%', dj_translateColumnWidthToFractional( $width ), $column_controls );
			$output .= $this->getCallbacks( $this->shortcode );
			$output .= '<div class="dj_element_wrapper ' . $this->settings( 'wrapper_class' ) . '">';
			$output .= '%dj_element_content%';
			$output .= '</div>';
			$output .= '</div>';
		...
		public function getControlsList() {
			...
			$allAccess = vc_user_access_check_shortcode_all( $this->shortcode );
			if ( $allAccess ) {
				return apply_filters( 'vc_djakery_shortcode_get_controls_list', $this->controls_list, $this->shortcode );
			} else {
				$controls = apply_filters( 'vc_djakery_shortcode_get_controls_list', $this->controls_list, $this->shortcode );
				if ( $editAccess ) {
		...
		public function getColumnControlsModular( $extended_css = '' ) {
			ob_start();
			vc_include_template( apply_filters( 'vc_djakery_shortcode_get_column_controls_modular_template', $this->controls_template_file ), array(
		...
		public function getCallbacks( $id ) {
			...
			if ( isset( $this->settings['js_callback'] ) ) {
				foreach ( $this->settings['js_callback'] as $text_val => $val ) {
					// TODO: name explain
					$output .= '<input type="hidden" class="dj_vc_callback dj_vc_' . $text_val . '_callback " name="' . $text_val . '" value="' . $val . '" />';
				}
			}
		 ...
		 * @param $value
		 *
		 * vc_filter: vc_djakeryshortcode_single_param_html_holder_value - hook to override param value (param type and etc is available in args)
		 *
		 * @return string
		 */
		public function singleParamHtmlHolder( $param, $value ) {
			$value = apply_filters( 'vc_djakeryshortcode_single_param_html_holder_value', $value, $param, $this->settings, $this->atts );
			$output = '';
			// Compatibility fixes
			$old_names = array(
				'yellow_message',
			...
			if ( ! empty( $param['holder'] ) ) {
				if ( 'input' === $param['holder'] ) {
					$output .= '<' . $param['holder'] . ' readonly="true" class="dj_vc_param_value ' . $param_name . ' ' . $type . ' ' . $class . '" name="' . $param_name . '" value="' . $value . '">';
				} elseif ( in_array( $param['holder'], array(
					'img',
					'iframe',
				) ) ) {
					$output .= '<' . $param['holder'] . ' class="dj_vc_param_value ' . $param_name . ' ' . $type . ' ' . $class . '" name="' . $param_name . '" src="' . $value . '">';
				} elseif ( 'hidden' !== $param['holder'] ) {
					$output .= '<' . $param['holder'] . ' class="dj_vc_param_value ' . $param_name . ' ' . $type . ' ' . $class . '" name="' . $param_name . '">' . $value . '</' . $param['holder'] . '>';
				}
			}
		...
		protected function outputTitle( $title ) {
			...
			return '<h4 class="dj_element_title"> ' . $this->getIcon( $params ) . esc_attr( $title ) . '</h4>';
		}
...
if ( ! class_exists( 'DJengaShortCodesContainer' ) ) {
	/**
	 * Class DJengaShortCodesContainer
	 */
	abstract class DJengaShortCodesContainer extends DJengaShortCode {
		/**
		 * @var array
		 */
		...
		public function mainHtmlBlockParams( $width, $i ) {
			...
			return 'data-element_type="' . $this->settings['base'] . '" class="dj_' . $this->settings['base'] . ' ' . $sortable . '' . ( ! empty( $this->settings['class'] ) ? ' ' . $this->settings['class'] : '' ) . ' dj_content_holder vc_shortcodes_container"' . $this->customAdminBlockParams();
		}
		...
		public function containerContentClass() {
			return 'dj_column_container vc_container_for_children vc_clearfix';
		}
		...
		public function contentAdmin( $atts, $content = null ) {
			...
			for ( $i = 0; $i < count( $width ); $i ++ ) {
				...
				$output .= '<div class="dj_element_wrapper">';
		...
		protected function outputTitle( $title ) {
			...
			return '<h4 class="dj_element_title"> ' . $this->getIcon( $params ) . '</h4>';
		}
...
if ( ! class_exists( 'DJengaShortCodeFishBones' ) ) {
	/**
	 * Class DJengaShortCodeFishBones
	 */
	class DJengaShortCodeFishBones extends DJengaShortCode {
		/**
		 * @var bool
		 */
		...
		/**
		 * @return DJengaShortCodeFishBones
		 */
		public function shortcodeClass() {
			// _deprecated_function( '\DJengaShortCodeFishBones::shortcodeClass', '4.9 (will be removed in 4.11)' );
			if ( false !== $this->shortcode_class ) {
				return $this->shortcode_class;
			}

			require_once vc_path_dir( 'SHORTCODES_DIR', 'jabali-widgets.php' );

			$class_name = $this->settings( 'php_class_name' ) ? $this->settings( 'php_class_name' ) : 'DJengaShortCode_' . $this->settings( 'base' );
			...
			if ( class_exists( $class_name ) && is_subclass_of( $class_name, 'DJengaShortCode' ) ) {
				$this->shortcode_class = new $class_name( $this->settings );
			} else {
				$this->shortcode_class = new DJengaShortCodeFishBones( $this->settings );
			}
		...
		public static function getElementClass( $tag ) {
			$settings = DJMap::getShortCode( $tag );
			require_once vc_path_dir( 'SHORTCODES_DIR', 'jabali-widgets.php' );

			$class_name = ! empty( $settings['php_class_name'] ) ? $settings['php_class_name'] : 'DJengaShortCode_' . $tag;
			...
			if ( class_exists( $class_name ) && is_subclass_of( $class_name, 'DJengaShortCode' ) ) {
				...
				$shortcode_class = new DJengaShortCodeFishBones( $settings );
			}
...
final class Vc_Shortcodes_Manager {
	 ...
	 */
	public function settings( $name ) {
		$settings = DJMap::getShortCode( $this->tag );
	...
	public function buildShortcodesAssets() {
		$elements = DJMap::getAllShortCodes();
	...
	public function buildShortcodesAssetsForEditable() {
		$elements = DJMap::getAllShortCodes(); // @todo create pull to use only where it is set inside function. BC problem
		foreach ( $elements as $tag => $settings ) {
			$element_class = $this->getElementClass( $tag );
			$element_class->printIconStyles();
		}
