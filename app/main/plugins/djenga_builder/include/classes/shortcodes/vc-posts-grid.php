...
class DJengaShortCode_VC_Posts_Grid extends DJengaShortCode {
	...
	protected function getMainCssClass( $filter ) {
		return 'dj_' . ( 'yes' === $filter ? 'filtered_' : '' ) . 'grid';
	}
	...
	protected function getPostThumbnail( $post_id, $grid_thumb_size ) {
		return dj_getImageBySize( array( 'post_id' => $post_id, 'thumb_size' => $grid_thumb_size ) );
	}
