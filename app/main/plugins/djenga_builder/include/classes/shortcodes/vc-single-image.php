...
class DJengaShortCode_VC_Single_image extends DJengaShortCode {
	...
	public function jsScripts() {
		wp_register_script( 'zoom', vc_asset_url( 'lib/bower/zoom/jquery.zoom.min.js' ), array(), DJ_VC_VERSION );
	...
	public function singleParamHtmlHolder( $param, $value ) {
		...
		if ( 'attach_image' === $param['type'] && 'image' === $param_name ) {
			$output .= '<input type="hidden" class="dj_vc_param_value ' . $param_name . ' ' . $type . ' ' . $class . '" name="' . $param_name . '" value="' . $value . '" />';
			$element_icon = $this->settings( 'icon' );
			$img = dj_getImageBySize( array(
	...
	protected function outputTitleTrue( $title ) {
		return '<h4 class="dj_element_title">' . $title . ' ' . $this->settings( 'logo' ) . '</h4>';
	}
}
