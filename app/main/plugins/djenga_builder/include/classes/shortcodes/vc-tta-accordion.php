...
class DJengaShortCode_VC_Tta_Accordion extends DJengaShortCodesContainer {
	...
	/**
	 * @var DJengaShortCode_VC_Tta_Section
	 */
	...
	public function setGlobalTtaInfo() {
		...
		/** @var $sectionClass DJengaShortCode_VC_Tta_Section */
		if ( is_object( $sectionClass ) ) {
			VcShortcodeAutoloader::getInstance()->includeClass( 'DJengaShortCode_VC_Tta_Section' );
			DJengaShortCode_VC_Tta_Section::$tta_base_shortcode = $this;
			DJengaShortCode_VC_Tta_Section::$self_count = 0;
			DJengaShortCode_VC_Tta_Section::$section_info = array();
	...
	public function getParamContent( $atts, $content ) {
		$panelsContent = dj_js_remove_wpautop( $content );
	...
	function getActiveSection( $atts, $strict_bounds = false ) {
		...
		if ( $strict_bounds ) {
			...
			if ( $active_section < 1 ) {
				$active_section = 1;
			} elseif ( $active_section > DJengaShortCode_VC_Tta_Section::$self_count ) {
				$active_section = DJengaShortCode_VC_Tta_Section::$self_count;
			}
		}
	...
	public function getParamPaginationList( $atts, $content ) {
		...
		if ( ! $isPageEditabe ) {
			VcShortcodeAutoloader::getInstance()->includeClass( 'DJengaShortCode_VC_Tta_Section' );
			foreach ( DJengaShortCode_VC_Tta_Section::$section_info as $nth => $section ) {
	...
	public function enqueueTtaStyles() {
		wp_register_style( 'vc_tta_style', vc_asset_url( 'css/js_composer_tta.min.css' ), false, DJ_VC_VERSION );
	...
	public function enqueueTtaScript() {
		...
		wp_register_script( 'vc_tta_autoplay_script', vc_asset_url( 'lib/vc-tta-autoplay/vc-tta-autoplay.min.js' ), array( 'vc_accordion_script' ), DJ_VC_VERSION, true );
