...
/**
 * Class DJengaShortCode_VC_Btn
 * @since 4.5
 */
class DJengaShortCode_VC_Btn extends DJengaShortCode {

	public static function convertAttributesToButton3( $atts ) {
		//size btn1 to size btn2
		$btn1_sizes = array( 'dj_regularsize', 'btn-large', 'btn-small', 'btn-mini' );
		...
		/*
		__( 'Classic Grey', 'js_composer' ) => 'dj_button',
		...
		*/
		...
		if ( ( ! isset( $atts['add_icon'] ) || 'true' != $atts['add_icon'] ) && isset( $atts['icon'] ) && strlen( $atts['icon'] ) > 0 && 'none' != $atts['icon'] ) {
			...
			$atts['icon_pixelicons'] = 'vc_pixel_icon vc_pixel_icon-' . str_replace( 'dj_', '', $atts['icon'] );
		}
	...
	protected function outputTitle( $title ) {
		...
		return '<h4 class="dj_element_title"><span class="vc_general vc_element-icon vc_btn3-icon' . ( ! empty( $icon ) ? ' ' . $icon : '' ) . '"></span></h4>';
	}
}
