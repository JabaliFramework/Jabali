 ...
 *
 * @link http://jabali.github.io/Docs/Settings_API Jabali settings API
 * @since 3.4
 */
class Vc_Settings {
	public $tabs;
	 ...
	 * @var string
	 */
	protected $option_group = 'dj_js_composer_settings';
	/**
	 * @var string
	 */
	 ...
	 * @var string
	 */
	protected static $field_prefix = 'dj_js_';
	/**
	 * @var string
	 */
	protected static $notification_name = 'dj_js_notify_user_about_element_class_names';
	/**
	 * @var
	 */
	...
	public function initAdmin() {
		 ...
		 */
		add_action( 'update_option_dj_js_compiled_js_composer_less', array(
			$this,
			'buildCustomColorCss',
		) );
		 ...
		 * @since 4.5 used to call update file once option is changed
		 */
		add_action( 'update_option_dj_js_custom_css', array(
			$this,
			'buildCustomCss',
		) );
		 ...
		 * @since 4.5 used to call update file once option is changed
		 */
		add_action( 'add_option_dj_js_compiled_js_composer_less', array(
			$this,
			'buildCustomColorCss',
		) );
		 ...
		 * @since 4.5 used to call update file once option is changed
		 */
		add_action( 'add_option_dj_js_custom_css', array(
			$this,
			'buildCustomCss',
		) );
	...
	function adminLoad() {
		wp_register_script( 'dj_js_composer_settings', vc_asset_url( 'js/dist/settings.min.js' ), array(), DJ_VC_VERSION, true );
		wp_enqueue_style( 'js_composer_settings', vc_asset_url( 'css/js_composer_settings.min.css' ), false, DJ_VC_VERSION, false );
		wp_enqueue_script( 'backbone' );
		wp_enqueue_script( 'shortcode' );
		wp_enqueue_script( 'underscore' );
		wp_enqueue_script( 'jquery-ui-accordion' );
		wp_enqueue_script( 'jquery-ui-sortable' );
		wp_enqueue_script( 'dj_js_composer_settings' );
		$this->locale = array(
		...
		wp_localize_script( 'dj_js_composer_settings', 'vcData', apply_filters( 'vc_global_js_data', array(
			'version' => DJ_VC_VERSION,
			'debug' => dj_debug(),
		) ) );
		wp_localize_script( 'dj_js_composer_settings', 'i18nLocaleSettings', $this->locale );
	}
	...
	public function groups_access_rules_callback() {
		...
		$show_types = array(
			'all' => __( 'Show Djenga Builder & default editor', 'js_composer' ),
			'only' => __( 'Show only Djenga Builder', 'js_composer' ),
			'no' => __( "Don't allow to use Djenga Builder", 'js_composer' ),
		);
		$shortcodes = DJMap::getShortCodes();
		$size_line = ceil( count( array_keys( $shortcodes ) ) / 3 );
		?>
		<div class="dj_settings_accordion" id="dj_js_settings_access_groups" xmlns="http://www.w3.org/1999/html">
		<?php
		if ( is_array( $groups ) ) :
			foreach ( $groups as $key => $params ) :
				if ( ( isset( $params['capabilities']['edit_posts'] ) && true === $params['capabilities']['edit_posts'] ) || ( isset( $params['capabilities']['edit_pages'] ) && true === $params['capabilities']['edit_pages'] ) ) :
					...
					?>
					<h3 id="dj-settings-group-<?php echo $key ?>-header">
						<a href="#dj-settings-group-<?php echo $key ?>">
							<?php echo $params['name'] ?>
						</a>
					</h3>
					<div id="dj-settings-group-<?php echo $key ?>" class="accordion-body">
						<div class="visibility settings-block">
							<label
								for="dj_composer_access_<?php echo $key ?>"><b><?php _e( 'Djenga Builder access', 'js_composer' ) ?></b></label>
							<select id="dj_composer_access_<?php echo $key ?>"
						...
						<div class="select-all">
							<a href="#"
								class="dj-settings-select-all-shortcodes"><?php echo __( 'Select All', 'js_composer' ) ?></a>
							| <a href="#"
								class="dj-settings-select-none-shortcodes"><?php echo __( 'Select none', 'js_composer' ) ?></a>
						</div>
					</div>
					</div>
					<?php
	...
	public function content_types_field_callback() {
		...
		$pt_array = ( $pt_array = get_option( 'dj_js_content_types' ) ) ? ( $pt_array ) : vc_default_editor_post_types();
			...
			if ( ! in_array( $pt, $this->getExcluded() ) ) {
				...
				<label>
					<input type="checkbox"<?php echo $checked; ?> value="<?php echo $pt; ?>"
						id="dj_js_post_types_<?php echo $pt; ?>"
		...
		?>
		<p
			class="description indicator-hint"><?php _e( 'Select content types available to Djenga Builder.', 'js_composer' ); ?></p>
		<?php
	}
	...
	public function theme_content_types_field_callback() {
		...
		$pt_array = ( $pt_array = get_option( 'dj_js_theme_content_types' ) ) ? $pt_array : vc_manager()->editorPostTypes();
		foreach ( $this->getPostTypes() as $pt ) {
			if ( ! in_array( $pt, $this->getExcluded() ) ) {
				$checked = ( in_array( $pt, $pt_array ) ) ? ' checked' : '';
				?>
	...
	public function custom_css_field_callback() {
		...
		echo '<textarea name="' . self::$field_prefix . 'custom_css' . '" class="dj_csseditor custom_css" style="display:none">' . $value . '</textarea>';
		echo '<pre id="dj_csseditor" class="dj_content_element custom_css" >' . $value . '</pre>';
	...
	public function not_responsive_css_field_callback() {
		...
		<label>
			<input type="checkbox"<?php echo( $checked ? ' checked' : '' ) ?> value="1"
				id="dj_js_not_responsive_css" name="<?php echo self::$field_prefix . 'not_responsive_css' ?>">
	...
	public function google_fonts_subsets_callback() {
		...
		foreach ( $this->getGoogleFontsSubsets() as $pt ) {
			if ( ! in_array( $pt, $this->getGoogleFontsSubsetsExcluded() ) ) {
				...
				<label>
					<input type="checkbox"<?php echo $checked; ?> value="<?php echo $pt; ?>"
						id="dj_js_gf_subsets_<?php echo $pt; ?>"
	...
	public function use_custom_callback() {
		...
		<label>
			<input type="checkbox"<?php echo( $checked ? ' checked' : '' ) ?> value="1"
				id="dj_js_<?php echo $field; ?>" name="<?php echo self::$field_prefix . $field ?>">
	...
	public function setting_section_callback_function( $tab ) {
		if ( 'dj_js_composer_settings_color' === $tab['id'] ) : ?>
			<div class="tab_intro">
				<p>
					<?php _e( 'Here you can tweak default Djenga Builder content elements visual appearance. By default Djenga Builder is using neutral light-grey theme. Changing "Main accent color" will affect all content elements if no specific "content block" related color is set.', 'js_composer' ) ?>
				</p>
			</div>
		<?php endif;
	}
	...
	public static function buildCustomColorCss() {
		 ...
		 * Filesystem API init.
		 * */
		$url = wp_nonce_url( 'admin.php?page=vc-color&build_css=1', 'dj_js_settings_save_action' );
		...
		if ( strlen( trim( $css_string ) ) > 0 ) {
			update_option( self::$field_prefix . 'less_version', DJ_VC_VERSION );
			delete_option( self::$field_prefix . 'compiled_js_composer_less' );
	...
	protected static function getFileSystem( $url = '' ) {
		if ( empty( $url ) ) {
			$url = wp_nonce_url( 'admin.php?page=vc-general', 'dj_js_settings_save_action' );
		}
 ...
 */
class DJengaVisualComposerSettings extends Vc_Settings {
	/**
	 * @deprecated 5.0
	 * DJengaVisualComposerSettings constructor.
	 */
	public function __construct() {
		_deprecated_function( '\DJengaVisualComposerSettings::__construct', '4.8 (will be removed in 5.1)' );
	}
}
