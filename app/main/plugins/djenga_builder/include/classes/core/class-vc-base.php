...
class Vc_Base {
	 ...
	 * @since  4.2
	 * @access public
	 * @var array DJengaShortCodeFishBones
	 */
	...
	public function initAdmin() {
		...
		add_action( 'wp_ajax_dj_single_image_src', array(
			$this,
			'singleImageSrc',
		) ); // @todo move it
		add_action( 'wp_ajax_dj_gallery_html', array(
			$this,
			'galleryHTML',
		) ); // @todo move it
	...
	public function addShortCode( array $shortcode ) {
		...
		if ( ! isset( $this->shortcodes[ $shortcode['base'] ] ) ) {
			...
			$this->shortcodes[ $shortcode['base'] ] = new DJengaShortCodeFishBones( $shortcode );
		}
	 ...
	 *
	 * This function widely used by DJMap class methods to modify shortcodes mapping
	 *
	 * @since 4.3
	 *
	 * @param $tag
	 ...
	 * Function creates meta data for post with the key '_dj_shortcodes_custom_css'
	...
	public function buildShortcodesCustomCss( $post_id ) {
		...
		if ( empty( $css ) ) {
			delete_post_meta( $post_id, '_dj_shortcodes_custom_css' );
		} else {
			update_post_meta( $post_id, '_dj_shortcodes_custom_css', $css );
		}
	}
	 ...
	 * like 'css_editor'.
	 *
	 * @see    DJengaVisualComposerCssEditor
	 * @since  4.2
	 * @access public
	 *
	 * @param $content
	...
	public function parseShortcodesCustomCss( $content ) {
		...
		DJMap::addAllMappedShortcodes();
		preg_match_all( '/' . get_shortcode_regex() . '/', $content, $shortcodes );
		foreach ( $shortcodes[2] as $index => $tag ) {
			$shortcode = DJMap::getShortCode( $tag );
	 ...
	 *
	 * Method gets post meta value for page by key '_dj_post_custom_css' and if it is not empty
	...
	public function addPageCustomCss( $id = null ) {
		...
		if ( $id ) {
			$post_custom_css = get_post_meta( $id, '_dj_post_custom_css', true );
	 ...
	 *
	 * Method gets post meta value for page by key '_dj_shortcodes_custom_css' and if it is not empty
	 * outputs css string wrapped into style tag.
	 *
	 * @since  4.2
	 * @access public
	...
	public function addShortcodesCustomCss( $id = null ) {
		...
		if ( $id ) {
			$shortcodes_custom_css = get_post_meta( $id, '_dj_shortcodes_custom_css', true );
	...
	public function addNoScript() {
		...
		echo ' .dj_animate_when_almost_visible { opacity: 1; }';
		echo '</style>';
		echo '</noscript>';
	}
	...
	public function frontCss() {
		wp_register_style( 'flexslider', vc_asset_url( 'lib/bower/flexslider/flexslider.min.css' ), array(), DJ_VC_VERSION );
		wp_register_style( 'nivo-slider-css', vc_asset_url( 'lib/bower/nivoslider/nivo-slider.min.css' ), array(), DJ_VC_VERSION );
		wp_register_style( 'nivo-slider-theme', vc_asset_url( 'lib/bower/nivoslider/themes/default/default.min.css' ), array( 'nivo-slider-css' ), DJ_VC_VERSION );
		wp_register_style( 'prettyphoto', vc_asset_url( 'lib/prettyphoto/css/prettyPhoto.min.css' ), array(), DJ_VC_VERSION );
		wp_register_style( 'isotope-css', vc_asset_url( 'css/lib/isotope.min.css' ), array(), DJ_VC_VERSION );
		wp_register_style( 'font-awesome', vc_asset_url( 'lib/bower/font-awesome/css/font-awesome.min.css' ), array(), DJ_VC_VERSION );
		wp_register_style( 'animate-css', vc_asset_url( 'lib/bower/animate-css/animate.min.css' ), array(), DJ_VC_VERSION, false );
		...
		wp_register_style( 'js_composer_front', $front_css_file, array(), DJ_VC_VERSION );
		...
		if ( is_file( $upload_dir['basedir'] . '/' . $vc_upload_dir . '/custom.css' ) && filesize( $custom_css_path ) > 0 ) {
			...
			wp_register_style( 'js_composer_custom_css', $custom_css_url, array(), DJ_VC_VERSION );
		}
	...
	public function frontJsRegister() {
		wp_register_script( 'prettyphoto', vc_asset_url( 'lib/prettyphoto/js/jquery.prettyPhoto.min.js' ), array( 'jquery' ), DJ_VC_VERSION, true );
		...
		wp_register_script( 'isotope', vc_asset_url( 'lib/bower/isotope/dist/isotope.pkgd.min.js' ), array( 'jquery' ), DJ_VC_VERSION, true );

		wp_register_script( 'twbs-pagination', vc_asset_url( 'lib/bower/twbs-pagination/jquery.twbsPagination.min.js' ), array( 'jquery' ), DJ_VC_VERSION, true );
		wp_register_script( 'nivo-slider', vc_asset_url( 'lib/bower/nivoslider/jquery.nivo.slider.pack.js' ), array( 'jquery' ), DJ_VC_VERSION, true );
		wp_register_script( 'flexslider', vc_asset_url( 'lib/bower/flexslider/jquery.flexslider-min.js' ), array( 'jquery' ), DJ_VC_VERSION, true );
		wp_register_script( 'dj_composer_front_js', vc_asset_url( 'js/dist/js_composer_front.min.js' ), array( 'jquery' ), DJ_VC_VERSION, true );
	...
	public function pluginActionLinks( $links, $file ) {
		if ( plugin_basename( vc_path_dir( 'APP_DIR', '/js_composer.php' ) ) == $file ) {
			$title = __( 'Djenga Builder Settings', 'js_composer' );
	...
	public function addMetaData() {
		echo '<meta name="generator" content="Powered by Djenga Builder - drag and drop page builder for Jabali."/>' . "\n";
	}
	...
	public function getEditorsLocale() {
		return array(
			...
			'main_button_title' => __( 'Djenga Builder', 'js_composer' ),
 ...
 */
class DJengaVisualComposer extends Vc_Base {

	/**
	 * @deprecated since 4.3
	 */
	function __construct() {
		_deprecated_function( 'DJengaVisualComposer class', '4.3 (will be removed in 5.1)', 'Vc_Base class' );
	}
	...
	public static function getUserTemplate( $template ) {
		_deprecated_function( 'DJengaVisualComposer getUserTemplate', '4.3 (will be removed in 5.1)', 'Vc_Base getShortcodesTemplateDir' );
