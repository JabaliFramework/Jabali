	...
	protected function callActivities() {
		...
		while ( $activity = each( $this->init_activity ) ) {
			...
			if ( 'mapper' === $object ) {
				switch ( $method ) {
					case 'map':
						DJMap::map( $params['tag'], $params['attributes'] );
						break;
					case 'drop_param':
						DJMap::dropParam( $params['name'], $params['attribute_name'] );
						break;
					case 'add_param':
						DJMap::addParam( $params['name'], $params['attribute'] );
						break;
					case 'mutate_param':
						DJMap::mutateParam( $params['name'], $params['attribute'] );
						break;
					case 'drop_all_shortcodes':
						DJMap::dropAllShortcodes();
						break;
					case 'drop_shortcode':
						DJMap::dropShortcode( $params['name'] );
						break;
					case 'modify':
						DJMap::modify( $params['name'], $params['setting_name'], $params['value'] );
						break;
				}
			}
		}
