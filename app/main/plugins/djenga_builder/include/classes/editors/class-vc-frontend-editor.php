...
/**
 * Djenga Builder front end editor
 *
...
class Vc_Frontend_Editor implements Vc_Editor_Interface {
	 ...
	 * @var string
	 */
	protected static $brand_url = 'http://mtaandao.co.ke/plugins/djenga/?utm_campaign=VCplugin&utm_source=vc_user&utm_medium=frontend_editor';
	...
	public static function shortcodesRegexp() {
		$tagnames = array_keys( DJMap::getShortCodes() );
	...
	function renderEditor() {
		...
		$post_custom_css = strip_tags( get_post_meta( $this->post_id, '_dj_post_custom_css', true ) );
	...
	function setEditorTitle() {
		return sprintf( __( 'Edit %s with Djenga Builder', 'js_composer' ), $this->post_type->labels->singular_name );
	}
	...
	function renderEditButton( $link ) {
		if ( $this->showButton( get_the_ID() ) ) {
			return $link . ' <a href="' . self::getInlineUrl() . '" id="vc_load-inline-editor" class="vc_inline-link">' . __( 'Edit with Djenga Builder', 'js_composer' ) . '</a>';
		}
	...
	function renderRowAction( $actions ) {
		...
		if ( $this->showButton( $post->ID ) ) {
			$actions['edit_vc'] = '<a
		href="' . $this->getInlineUrl( '', $post->ID ) . '">' . __( 'Edit with Djenga Builder', 'js_composer' ) . '</a>';
		}
		...
		if ( is_singular() ) {
			if ( $this->showButton( get_the_ID() ) ) {
				$wp_admin_bar->add_menu( array(
					...
					'title' => __( 'Edit with Djenga Builder', 'js_composer' ),
	...
	function loadTinyMceSettings() {
		if ( ! class_exists( '_WP_Editors' ) ) {
			require( ABSPATH . RES . '/class-wp-editor.php' );
		}
	...
	function loadIFrameJsCss() {
		...
		wp_enqueue_style( 'vc_inline_css', vc_asset_url( 'css/js_composer_frontend_editor_iframe.min.css' ), array(), DJ_VC_VERSION );
		wp_enqueue_script( 'waypoints' );
		wp_enqueue_script( 'dj_scrollTo_js', vc_asset_url( 'lib/bower/scrollTo/jquery.scrollTo.min.js' ), array( 'jquery' ), DJ_VC_VERSION, true );
		wp_enqueue_style( 'js_composer_custom_css' );

		wp_enqueue_script( 'dj_php_js', vc_asset_url( 'lib/php.default/php.default.min.js' ), array( 'jquery' ), DJ_VC_VERSION, true );
	...
	function adminFile( $path ) {
		return ABSPATH . 'admin/' . $path;
	}

	public function registerJs() {

		wp_register_script( 'vc_bootstrap_js', vc_asset_url( 'lib/bower/bootstrap3/dist/js/bootstrap.min.js' ), array( 'jquery' ), DJ_VC_VERSION, true );
		wp_register_script( 'vc_accordion_script', vc_asset_url( 'lib/vc_accordion/vc-accordion.min.js' ), array( 'jquery' ), DJ_VC_VERSION, true );
		wp_register_script( 'dj_php_js', vc_asset_url( 'lib/php.default/php.default.min.js' ), array( 'jquery' ), DJ_VC_VERSION, true );
		// used as polyfill for JSON.stringify and etc
		wp_register_script( 'dj_json-js', vc_asset_url( 'lib/bower/json-js/json2.min.js' ), array(), DJ_VC_VERSION, true );
		// used in post settings editor
		wp_register_script( 'ace-editor', vc_asset_url( 'lib/bower/ace-builds/src-min-noconflict/ace.js' ), array( 'jquery' ), DJ_VC_VERSION, true );
		wp_register_script( 'webfont', '//ajax.googleapis.com/ajax/libs/webfont/1.4.7/webfont.js' ); // Google Web Font CDN
		wp_register_script( 'dj_scrollTo_js', vc_asset_url( 'lib/bower/scrollTo/jquery.scrollTo.min.js' ), array( 'jquery' ), DJ_VC_VERSION, true );
		wp_register_script( 'vc_accordion_script', vc_asset_url( 'lib/vc_accordion/vc-accordion.min.js' ), array( 'jquery' ), DJ_VC_VERSION, true );
		wp_register_script( 'vc-frontend-editor-min-js', vc_asset_url( 'js/dist/frontend-editor.min.js' ), array(), DJ_VC_VERSION, true );
	...
	public function registerCss() {
		wp_register_style( 'ui-custom-theme', vc_asset_url( 'css/ui-custom-theme/jquery-ui-less.custom.min.css' ), false, DJ_VC_VERSION, false );
		wp_register_style( 'animate-css', vc_asset_url( 'lib/bower/animate-css/animate.min.css' ), false, DJ_VC_VERSION, 'screen' );
		wp_register_style( 'font-awesome', vc_asset_url( 'lib/bower/font-awesome/css/font-awesome.min.css' ), false, DJ_VC_VERSION, 'screen' );

		wp_register_style( 'vc_inline_css', vc_asset_url( 'css/js_composer_frontend_editor.min.css' ), array(), DJ_VC_VERSION );
	...
	function enqueueMappedShortcode() {
		foreach ( DJMap::getUserShortCodes() as $shortcode ) {
	...
	public function enqueueMappedShortcodeJs( $value ) {
		wp_enqueue_script( 'front_enqueue_js_' . md5( $value ), $value, array( 'vc-frontend-editor-min-js' ), DJ_VC_VERSION, true );
	}

	public function enqueueMappedShortcodeCss( $value ) {
		wp_enqueue_style( 'front_enqueue_css_' . md5( $value ), $value, array( 'vc_inline_css' ), DJ_VC_VERSION );
	}
	...
	function parseShortcodesString( $content, $is_container = false, $parent_id = false ) {
		...
		foreach ( $found[2] as $index => $s ) {
			...
			if ( false !== DJMap::getParam( $s, 'content' ) ) {
