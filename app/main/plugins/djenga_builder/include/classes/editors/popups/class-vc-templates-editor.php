...
class Vc_Templates_Editor implements Vc_Render {
	 ...
	 * @var string
	 */
	protected $option_name = 'dj_js_templates';
	/**
	 * @var bool
	 */
	...
	public function init() {
		...
		add_action( 'wp_ajax_dj_save_template', array( &$this, 'save' ) );
		add_action( 'wp_ajax_vc_backend_template', array( &$this, 'load' ) );
		add_action( 'wp_ajax_dj_load_template_shortcodes', array(
			&$this,
		...
		add_action( 'wp_ajax_dj_delete_template', array( &$this, 'delete' ) );
	}
	...
	public function outputMenuButton( $id, $params ) {
		...
		$output = '<li data-vc-ui-element="template" class="dj_template_li"><a data-template_id="' . $id . '" href="#">' . htmlspecialchars( $params['name'] ) . '</a> <span class="dj_remove_template" title="' . __( 'Delete template', 'js_composer' ) . '" rel="' . $id . '"><i class="icon dj_template_delete_icon"> </i></span></li>';
	...
	public function renderMenu( $only_list = false ) {
		...
		if ( false === $only_list ) {
			$output .= '<li><ul>
                        <li id="dj_save_template"><a href="#" id="dj_save_template_button" class="button">' . __( 'Save current page as a Template', 'js_composer' ) . '</a></li>
                        <li class="divider"></li>
                        <li class="nav-header">' . __( 'Load Template', 'js_composer' ) . '</li>
                        </ul></li>
                        <li>
                        <ul class="dj_templates_list">';
		}
		if ( empty( $templates ) ) {
			$output .= '<li class="dj_no_templates"><span>' . __( 'No custom templates yet.', 'js_composer' ) . '</span></li></ul></li>';
			echo $output;
