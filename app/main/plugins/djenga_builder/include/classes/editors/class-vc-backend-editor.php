...
class Vc_Backend_Editor implements Vc_Editor_Interface {
	...
	/**
	 *    Calls add_meta_box to create Editor block. Block is rendered by DJengaVisualComposerLayout.
	 *
	 * @see DJengaVisualComposerLayout
	 * @since  4.2
	 * @access public
	 *
	 * @param $post_type
	...
	public function render( $post_type ) {
		if ( $this->isValidPostType( $post_type ) ) {
			...
			// meta box to render
			add_meta_box( 'dj_visual_composer', __( 'Djenga Builder', 'js_composer' ), array(
				$this,
				'renderEditor',
	...
	public function renderEditor( $post = null ) {
		...
		$this->post = $post;
		$post_custom_css = strip_tags( get_post_meta( $post->ID, '_dj_post_custom_css', true ) );
	...
	public function enqueueEditorScripts() {
		if ( $this->editorEnabled() ) {
			$this->enqueueJs();
			$this->enqueueCss();
			DJengaShortCodeFishBones::enqueueCss();
			DJengaShortCodeFishBones::enqueueJs();
		} else {
	...
	public function registerBackendJavascript() {
		...
		wp_register_script( 'vc-backend-min-js', vc_asset_url( 'js/dist/backend.min.js' ), array( 'vc-backend-actions-js' ), DJ_VC_VERSION, true );
	...
	public function registerBackendCss() {
		wp_register_style( 'js_composer', vc_asset_url( 'css/js_composer_backend_editor.min.css' ), array(), DJ_VC_VERSION, false );

		if ( $this->editorEnabled() ) {
			/**
			 * @deprecated, used for accordions/tabs/tours
			 */
			wp_register_style( 'ui-custom-theme', vc_asset_url( 'css/ui-custom-theme/jquery-ui-less.custom.min.css' ), array(), DJ_VC_VERSION, false );

			/**
			 * @todo check vc_add-element-deprecated-warning for fa icon usage ( set to our font )
			 * also used in vc_icon shortcode
			 */
			wp_register_style( 'font-awesome', vc_asset_url( 'lib/bower/font-awesome/css/font-awesome.min.css' ), array(), DJ_VC_VERSION, false );
