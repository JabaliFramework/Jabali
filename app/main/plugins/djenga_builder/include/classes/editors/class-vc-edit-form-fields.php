...
/**
 * Djenga Builder shortcode attributes fields
 *
...
class Vc_Edit_Form_Fields implements Vc_Render {
	...
	public function __construct( $tag, $atts ) {
		...
		$this->setSettings( DJMap::getShortCode( $this->tag ) );
	}
	...
	public function render() {
		...
		if ( ! empty( $deprecated ) ) {
			$output .= '<div class="vc_row vc_ui-flex-row vc_shortcode-edit-form-deprecated-message"><div class="vc_col-sm-12 dj_element_wrapper">' . vc_message_warning( sprintf( __( 'You are using outdated element, it is deprecated since version %s.', 'js_composer' ), $this->setting( 'deprecated' ) ) ) . '</div></div>';
		}
	...
	protected function renderField( $param, $value ) {
		...
		$output .= ( isset( $param['heading'] ) ) ? '<div class="dj_element_label">' . $param['heading'] . '</div>' : '';
