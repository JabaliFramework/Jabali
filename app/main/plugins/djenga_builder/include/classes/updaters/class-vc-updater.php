...
class Vc_Updater {
	 ...
	 * @var string
	 */
	protected $version_url = 'http://updates.mtaandao.co.ke/';
	 ...
	 * @var string
	 */
	protected $download_link_url = 'http://support.mtaandao.co.ke/updates/download-link';

	/**
	 * @var string
	 */
	public $title = 'Djenga Builder';
	...
	protected function getUrl() {
		...
		$url = $this->download_link_url . '?product=vc&url=' . $host . '&key=' . $key . '&version=' . DJ_VC_VERSION;
	...
	public function preUpgradeFilter( $reply, $package, $updater ) {
		...
		$res = $updater->fs_connect( array( MAIN_DIR ) );
		if ( ! $res ) {
		...
		if ( ! vc_license()->isActivated() ) {
			...
			return new WP_Error( 'no_credentials', __( 'To receive automatic updates license activation is required. Please visit <a href="' . $url . '' . '" target="_blank">Settings</a> to activate your Djenga Builder.', 'js_composer' ) . ' ' . sprintf( ' <a href="http://go.mtaandao.co.ke/faq-update-in-theme" target="_blank">%s</a>', __( 'Got Djenga Builder in theme?', 'js_composer' ) ) );
		}
	...
	public function upgradeFilterFromEnvato( $reply, $package, $updater ) {
		...
		if ( ( isset( $updater->skin->plugin ) && vc_plugin_name() === $updater->skin->plugin ) || ( isset( $updater->skin->plugin_info ) && $updater->skin->plugin_info['Name'] === $this->title ) ) {
			...
			if ( ! vc_license()->isActivated() || empty( $username ) || empty( $api_key ) || empty( $purchase_code ) ) {
				return new WP_Error( 'no_credentials', __( 'To receive automatic updates license activation is required. Please visit <a href="' . admin_url( 'admin.php?page=vc-updater' ) . '' . '" target="_blank">Settings</a> to activate your Djenga Builder.', 'js_composer' ) );
			}
