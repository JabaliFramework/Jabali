...
class Vc_Vendor_Woocommerce implements Vc_Vendor_Interface {
	...
	public function load() {
		if ( class_exists( 'Banda' ) ) {
			...
			add_filter( 'banda_product_tabs', array(
				$this,
	...
	public function enqueueJsBackend() {
		wp_enqueue_script( 'vc_vendor_banda_backend', vc_asset_url( 'js/vendors/banda.js' ), array( 'vc-backend-min-js' ), '1.0', true );
	}
	...
	public function enqueueJsFrontend() {
		wp_enqueue_script( 'vc_vendor_banda_frontend', vc_asset_url( 'js/vendors/banda.js' ), array( 'vc-frontend-editor-min-js' ), '1.0', true );
	}
		...
		switch ( $tag ) {
			case 'banda_cart':
				$settings = array(
					...
					'base' => 'banda_cart',
					'icon' => 'icon-dj-banda',
					'category' => __( 'Banda', 'js_composer' ),
					'description' => __( 'Displays the cart contents', 'js_composer' ),
					'show_settings_on_create' => false,
					'php_class_name' => 'Vc_Banda_NotEditable',
				);
				break;
			case 'banda_checkout':
				/**
				 * @shortcode banda_checkout
				...
				break;
			case 'banda_order_tracking':
				/**
				 * @shortcode banda_order_tracking
				...
				break;
			case 'banda_my_account':
				/**
				 * @shortcode banda_my_account
			...
			case 'recent_products':
				 ...
				 * To learn more about the default ‘orderby’ parameters please reference the Jabali Codex: http://jabali.github.io/Docs/Class_Reference/WP_Query
				 *
				...
				$settings = array(
					...
					'params' => array(
						...
						array(
							...
							'save_always' => true,
							'description' => sprintf( __( 'Select how to sort retrieved products. More at %s.', 'js_composer' ), '<a href="http://jabali.github.io/Docs/Class_Reference/WP_Query#Order_.26_Orderby_Parameters" target="_blank">Jabali codex page</a>' ),
						),
						array(
							'type' => 'dropdown',
							...
							'save_always' => true,
							'description' => sprintf( __( 'Designates the ascending or descending order. More at %s.', 'js_composer' ), '<a href="http://jabali.github.io/Docs/Class_Reference/WP_Query#Order_.26_Orderby_Parameters" target="_blank">Jabali codex page</a>' ),
						),
					),
				);
				break;
				...
				$settings = array(
					...
					'params' => array(
						...
						array(
							...
							'std' => 'title',
							'save_always' => true,
							'description' => sprintf( __( 'Select how to sort retrieved products. More at %s. Default by Title', 'js_composer' ), '<a href="http://jabali.github.io/Docs/Class_Reference/WP_Query#Order_.26_Orderby_Parameters" target="_blank">Jabali codex page</a>' ),
						),
						array(
							'type' => 'dropdown',
							'heading' => __( 'Sort order', 'js_composer' ),
							...
							'value' => $order_way_values,
							'save_always' => true,
							'description' => sprintf( __( 'Designates the ascending or descending order. More at %s. Default by ASC', 'js_composer' ), '<a href="http://jabali.github.io/Docs/Class_Reference/WP_Query#Order_.26_Orderby_Parameters" target="_blank">Jabali codex page</a>' ),
						),
						array(
							'type' => 'autocomplete',
			...
			case 'product_category':
				 ...
				 * Go to: Banda > Products > Categories to find the slug column.
				 */
	...
	/**
	 * Add banda shortcodes and hooks/filters for it.
	 * @since 4.4
	 */
	public function mapShortcodes() {
		add_action( 'wp_ajax_vc_banda_get_attribute_terms', array(
			&$this,
	...
	public function mapGridItemShortcodes( array $shortcodes ) {
		require_once vc_path_dir( 'VENDORS_DIR', 'plugins/banda/class-vc-gitem-banda-shortcode.php' );
		require_once vc_path_dir( 'VENDORS_DIR', 'plugins/banda/grid-item-attributes.php' );
		$wc_shortcodes = include vc_path_dir( 'VENDORS_DIR', 'plugins/banda/grid-item-shortcodes.php' );
	...
	public function getAttributeTermsAjax() {
		...
		foreach ( $values as $label => $v ) {
			$param_line .= ' <label class="vc_checkbox-label"><input id="' . $param['param_name'] . '-' . $v . '" value="' . $v . '" class="dj_vc_param_value ' . $param['param_name'] . ' ' . $param['type'] . '" type="checkbox" name="' . $param['param_name'] . '"' . '> ' . $label . '</label>';
		}
 ...
 * Class Vc_Banda_NotEditable
 * @since 4.4
 */
class Vc_Banda_NotEditable extends DJengaShortCode {
	/**
	 * @since 4.4
	 * @var array
	 */
