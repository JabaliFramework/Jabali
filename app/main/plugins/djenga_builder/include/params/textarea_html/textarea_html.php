...
function vc_textarea_html_form_field( $settings, $value ) {
	...
	if ( false !== $vc_html_editor_already_is_use ) {
		$output .= '<textarea name="'
		           . $settings['param_name']
		           . '" class="dj_vc_param_value dj-textarea '
		...
		// WP 3.3+
		ob_start();
		wp_editor( '', 'dj_tinymce_' . $settings['param_name'], array(
			'editor_class' => 'dj-textarea visual_composer_tinymce ' . $settings['param_name'] . ' ' . $settings['type'],
			'media_buttons' => true,
			'wpautop' => false,
		) );
		...
		$output .= $output_value
		           ...
		           . '"  class="vc_textarea_html_content dj_vc_param_value '
		           . $settings['param_name']
