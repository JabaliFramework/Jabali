...
function vc_loop_form_field( $settings, $value ) {
	...
	return '<div class="vc_loop">'
	       . '<input name="' . $settings['param_name'] . '" class="dj_vc_param_value  ' . $settings['param_name'] . ' ' . $settings['type'] . '_field" type="hidden" value="' . $value . '"/>'
 ...
 * @since 4.2
 * @link http://jabali.github.io/Docs/Class_Reference/WP_Query
 */
...
add_action( 'wp_ajax_dj_get_loop_suggestion', 'vc_get_loop_suggestion' );
add_action( 'wp_ajax_dj_get_loop_settings', 'vc_get_loop_settings_json' );
