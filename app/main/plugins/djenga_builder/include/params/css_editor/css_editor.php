...
if ( ! class_exists( 'DJengaVisualComposerCssEditor' ) ) {
	/**
	 * Class DJengaVisualComposerCssEditor
	 */
	class DJengaVisualComposerCssEditor {
		/**
		 * @var array
		...
		function render() {
			...
			$output .= '<div class="vc_col-xs-5 vc_settings">'
			           ...
			           . '</div>';
			$output .= '<input name="' . $this->setting( 'param_name' ) . '" class="dj_vc_param_value  ' . $this->setting( 'param_name' ) . ' ' . $this->setting( 'type' ) . '_field" type="hidden" value="' . esc_attr( $this->value() ) . '"/>';
...
function vc_css_editor_form_field( $settings, $value ) {
	$css_editor = new DJengaVisualComposerCssEditor();
