 ...
 * @var $css_animation
 * Shortcode class
 * @var $this DJengaShortCode_VC_gallery
 */
$thumbnail = '';
...
if ( 'nivo' === $type ) {
	...
	$el_end = '</li>';
	$slides_wrap_start = '<ul class="dj_image_grid_ul">';
	$slides_wrap_end = '</ul>';
}
...
foreach ( $images as $i => $image ) {
	switch ( $source ) {
		case 'media_library':
			if ( $image > 0 ) {
				$img = dj_getImageBySize( array( 'attach_id' => $image, 'thumb_size' => $img_size ) );
...
$class_to_filter = 'dj_gallery dj_content_element vc_clearfix';
$class_to_filter .= vc_shortcode_custom_css_class( $css, ' ' ) . $this->getExtraClass( $el_class ) . $this->getCSSAnimation( $css_animation );
$css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, $class_to_filter, $this->settings['base'], $atts );

$output = '';
$output .= '<div class="' . $css_class . '">';
$output .= '<div class="dj_wrapper">';
$output .= dj_widget_title( array( 'title' => $title, 'extraclass' => 'dj_gallery_heading' ) );
$output .= '<div class="dj_gallery_slides' . $type . '" data-interval="' . $interval . '"' . $flex_fx . '>' . $slides_wrap_start . $gal_images . $slides_wrap_end . '</div>';
$output .= '</div>';
$output .= '</div>';
