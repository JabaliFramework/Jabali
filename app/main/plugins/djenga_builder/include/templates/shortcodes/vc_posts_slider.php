 ...
 * @var $css
 * Shortcode class
 * @var $this DJengaShortCode_VC_Posts_slider
 */
...
if ( 'nivo' === $type ) {
	$type = ' dj_slider_nivo theme-default';
...
if ( 'flexslider' === $type || 'flexslider_fade' === $type || 'fading' === $type ) {
	$type = ' dj_flexslider flexslider_fade flexslider';
	$flex_fx = ' data-flex_fx="fade"';
} elseif ( 'flexslider_slide' === $type ) {
	$type = ' dj_flexslider flexslider_slide flexslider';
...
if ( '' !== $categories ) {
	...
	$gc = implode( ',', $gc );
	// http://snipplr.com/view/17434/jabali-get-category-slug/
...
while ( $my_query->have_posts() ) {
	...
	$post_thumbnail = dj_getImageBySize( array( 'post_id' => $post_id, 'thumb_size' => $thumb_size ) );
	...
	$description = '';
	if ( '' !== $slides_content && '' !== $content && ( ' dj_flexslider flexslider_fade flexslider' === $type || ' dj_flexslider flexslider_slide flexslider' === $type ) ) {
...
$class_to_filter = 'dj_gallery dj_posts_slider dj_content_element';
...
$output = '
	<div class="' . esc_attr( $css_class ) . '">
		<div class="dj_wrapper">
			' . dj_widget_title( array( 'title' => $title, 'extraclass' => 'dj_posts_slider_heading' ) ) . '
			<div class="dj_gallery_slides' . $type . '" data-interval="' . $interval . '"' . $flex_fx . '>' . $teasers . '</div>
		</div>
	</div>
';
