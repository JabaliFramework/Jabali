 ...
 * @var $title
 * Shortcode class
 * @var $this DJengaShortCode_Vc_Carousel
 */
			...
			foreach ( $data as $block ) {
				...
				if ( 'title' === $block->name ) {
					...
					if ( 'featured' === $block->image ) {
						...
						$post->thumbnail_data = dj_getImageBySize( array(
...
$css_class = $this->settings['base'] . ' dj_content_element vc_carousel_slider_' . $slides_per_view . ' vc_carousel_' . $mode . ( empty( $el_class ) ? '' : ' ' . $el_class );
$carousel_id = esc_attr( 'vc_carousel-' . DJengaShortCode_Vc_Carousel::getCarouselIndex() );
?>
<div
	...
	<div class="dj_wrapper">
		<?php echo dj_widget_title( array( 'title' => $title, 'extraclass' => 'dj_gallery_heading' ) ) ?>
