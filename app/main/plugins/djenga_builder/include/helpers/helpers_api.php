...
function vc_lean_map( $tag, $settings_function = null, $settings_file = null ) {
	DJMap::leanMap( $tag, $settings_function, $settings_file );
}
...
function vc_map( $attributes ) {
	...
	DJMap::map( $attributes['base'], $attributes );
}
 ...
 */
function dj_map( $attributes ) {
	_deprecated_function( 'dj_map', '4.2 (will be removed in 5.1)', 'vc_map' );
 ...
 */
function dj_remove( $shortcode ) {
	_deprecated_function( 'dj_remove', '4.2 (will be removed in 5.1)', 'vc_remove_element' );
...
function vc_add_param( $shortcode, $attributes ) {
	DJMap::addParam( $shortcode, $attributes );
}
 ...
 * @deprecated
 */
function dj_add_param( $shortcode, $attributes ) {
	_deprecated_function( 'dj_add_param', '4.2 (will be removed in 5.1)', 'vc_add_param' );

	vc_add_param( $shortcode, $attributes );
}

/**
 * Shorthand function for DJMap::modify
 *
 * @param $name
 * @param $setting
 * @param string $value
...
function vc_map_update( $name = '', $setting = '', $value = '' ) {
	return DJMap::modify( $name, $setting, $value );
}

/**
 * Shorthand function for DJMap::mutateParam
 *
 * @param $name
...
function vc_update_shortcode_param( $name, $attribute = array() ) {
	return DJMap::mutateParam( $name, $attribute );
}

/**
 * Shorthand function for DJMap::dropParam
 *
 * @param $name
...
function vc_remove_param( $name = '', $attribute_name = '' ) {
	return DJMap::dropParam( $name, $attribute_name );
}
...
if ( ! function_exists( 'vc_set_shortcodes_templates_dir' ) ) {
	/**
	 * Sets directory where Djenga Builder should look for template files for content elements.
	 * @since 4.2
	 *
...
function vc_map_integrate_shortcode( $shortcode, $field_prefix = '', $group_prefix = '', $change_fields = null, $dependency = null ) {
	if ( is_string( $shortcode ) ) {
		$shortcode_data = DJMap::getShortCode( $shortcode );
	} else {
		$shortcode_data = $shortcode;
	}
	if ( is_array( $shortcode_data ) && ! empty( $shortcode_data ) ) {
		/**
		 * @var $shortcode DJengaShortCodeFishBones
		 */
...
function vc_map_integrate_get_params( $base_shortcode, $integrated_shortcode, $field_prefix = '' ) {
	$shortcode_data = DJMap::getShortCode( $base_shortcode );
	$params = array();
