...
/**
 * Djenga Builder helpers functions
 *
 ...
 *
 * @since 4.2
 * vc_filter: vc_dj_getimagesize - to override output of this function
 * @return array|bool
 */
function dj_getImageBySize( $params = array() ) {
	...
	if ( is_string( $thumb_size ) && ( ( ! empty( $_wp_additional_image_sizes[ $thumb_size ] ) && is_array( $_wp_additional_image_sizes[ $thumb_size ] ) ) || in_array( $thumb_size, array(
		...
		if ( is_array( $thumb_size ) ) {
			...
			$p_img = dj_resize( $attach_id, null, $thumb_size[0], $thumb_size[1], true );
	...
	return apply_filters( 'vc_dj_getimagesize', array(
		'thumbnail' => $thumbnail,
 ...
 * @return string
 */
function dj_getColumnControls( $width ) {
	_deprecated_function( 'dj_getColumnControls', '4.5 (will be removed in 5.1)' );
 ...
 * @return string
 */
function dj_translateColumnWidthToFractional( $width ) {
	switch ( $width ) {
		case 'vc_col-sm-2' :
			$w = '1/6';
			break;
 ...
 * @return bool|string
 */
function dj_translateColumnWidthToSpan( $width ) {
 ...
 * @return string
 */
function dj_js_remove_wpautop( $content, $autop = false ) {

	if ( $autop ) { // Possible to use !preg_match('('.DJMap::getTagsRegexp().')', $content)
...
if ( ! function_exists( 'shortcode_exists' ) ) {
	/**
	 * Check if a shortcode is registered in Jabali.
	 *
 ...
 * @return array
 */
function dj_removeNotExistingImgIDs( $param_value ) {
...
if ( ! function_exists( 'dj_resize' ) ) {
	 ...
	 * @return array
	 */
	function dj_resize( $attach_id = null, $img_url = null, $width, $height, $crop = false ) {
...
if ( ! function_exists( 'dj_debug' ) ) {
	/**
	 * Returns bool if dj_debug is provided in url - set visual composer debug mode.
	 * Used for example in shortcodes (end block comment for example)
	 * @since 4.2
	 * @return bool
	 */
	function dj_debug() {
		if ( ( isset( $_GET['dj_debug'] ) && 'true' === $_GET['dj_debug'] ) || ( isset( $_GET['vc_debug'] ) && 'true' === $_GET['vc_debug'] ) ) {
			return true;
		} else {
			return false;
		}
 ...
 * Used in main/plugins/js_composer/include/classes/core/class-vc-base.php\Vc_Base\bodyClass
 *
 * @param $classes
 *
 * @since 4.2
...
function js_composer_body_class( $classes ) {
	$classes[] = 'dj-js-composer js-comp-ver-' . DJ_VC_VERSION;
 ...
 * @return string
 */
function dj_vc_get_column_width_indent( $width ) {
	$identy = '11';
 ...
 * @return string
 */
function dj_stripslashes_if_gpc_magic_quotes( $string ) {
	_deprecated_function( 'dj_stripslashes_if_gpc_magic_quotes', '4.5 (will be removed in 5.1)', 'stripslashes' );
...
function vc_parse_options_string( $string, $tag, $param ) {
	...
	$settings = DJMap::getParam( $tag, $param );
 ...
 * @deprecated 4.2
 */
function dj_js_composer_check_version_schedule_deactivation() {
	_deprecated_function( 'dj_js_composer_check_version_schedule_deactivation', '4.2 (will be removed in 5.1)' );
	wp_clear_scheduled_hook( 'dj_check_for_update' );
	delete_option( 'dj_js_composer_show_new_version_message' );
}
...
function vc_message_warning( $message ) {
	return '<div class="dj_element_wrapper"><div class="vc_message_box vc_message_box-standard vc_message_box-rounded vc_color-warning">
