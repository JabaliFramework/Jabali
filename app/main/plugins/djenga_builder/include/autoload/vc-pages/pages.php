...
function vc_page_css_enqueue() {
	wp_enqueue_style( 'vc_page-css', vc_asset_url( 'css/js_composer_settings.min.css' ), array(), DJ_VC_VERSION );
}
...
function vc_pages_group_build( $slug, $title, $tab = '' ) {
	...
	$pages_group->setSlug( $slug )
	            ...
	            ->setActivePage( $page )
	            ->setTemplatePath( 'pages/vc-settings/index.php' );
...
function vc_menu_page_build() {
	...
	add_menu_page( __( 'Djenga Builder', 'js_composer' ), __( 'Djenga', 'js_composer' ), 'exist', VC_PAGE_MAIN_SLUG, null, 'dashicons-chart-bar', 76 );
