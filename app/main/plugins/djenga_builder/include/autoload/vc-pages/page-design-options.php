...
function vc_check_for_custom_css_build() {
	...
	if ( vc_user_access()
			...
			->part( 'settings' )
			->can( 'vc-color-tab' )
			->get() && vc_settings()->useCustomCss() && ( ! $version || version_compare( DJ_VC_VERSION, $version, '<>' ) )
	) {
...
function vc_custom_css_admin_notice() {
	...
	if ( is_object( $current_screen ) && isset( $current_screen->id ) && 'visual-composer_page_vc-color' === $current_screen->id ) {
		$message = __( 'You have an outdated version of Djenga Builder Design Options. It is required to review and save it.', 'js_composer' );
...
function vc_page_settings_tab_color_submit_attributes( $submitButtonAttributes ) {
	...
	$submitButtonAttributes['data-vc-less-variables'] = json_encode( apply_filters( 'vc_settings-less-variables', array(
		...
		// Hover color
		'vc_grey_hover' => array(
			'key' => 'dj_js_vc_color_hover',
			'default' => vc_settings()->getDefault( 'vc_color_hover' ),
		),
		'vc_image_slider_link_active' => 'dj_js_vc_color_hover',
		// Call to action background color
		'vc_call_to_action_bg' => 'dj_js_vc_color_call_to_action_bg',
		'vc_call_to_action_2_bg' => 'dj_js_vc_color_call_to_action_bg',
		'vc_call_to_action_border' => array(
			'key' => 'dj_js_vc_color_call_to_action_border',
			// darken 5%
			'default_key' => 'dj_js_vc_color',
			'modify_output' => array(
				array(
					'plain' => array(
						'darken({{ value }}, 5%)',
		...
		'vc_google_maps_bg' => 'dj_js_vc_color_google_maps_bg',
		// Post slider caption background color
		'vc_post_slider_caption_bg' => 'dj_js_vc_color_post_slider_caption_bg',
		// Progress bar background color
		'vc_progress_bar_bg' => 'dj_js_vc_color_progress_bar_bg',
		// Separator border color
		'vc_separator_border' => 'dj_js_vc_color_separator_border',
		// Tabs navigation background color
		'vc_tab_bg' => 'dj_js_vc_color_tab_bg',
		// Active tab background color
		'vc_tab_bg_active' => 'dj_js_vc_color_tab_bg_active',
...
function vc_page_settings_desing_options_load() {
	...
	wp_enqueue_script( 'vc_less_js', vc_asset_url( 'lib/bower/lessjs/dist/less.min.js' ), array(), DJ_VC_VERSION );
}
