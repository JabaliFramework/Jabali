...
/**
 * Return true value for filter 'dj_vc_js_status_filter'.
 * It allows to start backend editor on load.
 * @since 4.12
 *
 * @return string
 */
function vc_set_default_content_for_post_type_dj_vc_js_status_filter() {
	return 'true';
}
...
function vc_set_default_content_for_post_type( $post_content, $post ) {
	...
	if ( ( $new_post_content = $template_settings->getTemplateByPostType( $post->post_type ) ) !== null ) {
		add_filter( 'dj_vc_js_status_filter', 'vc_set_default_content_for_post_type_dj_vc_js_status_filter' );
