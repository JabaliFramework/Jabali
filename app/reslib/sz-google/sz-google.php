...
if (!class_exists('SZGoogleCheck'))
{
	class SZGoogleCheck
	{
		...
		function activate()
		{
			...
			{
				$HTML  = '<div>Activation plugin Google for Jabali in not possible:</div>';
				$HTML .= '<ul>';
				...
				wp_die($HTML,'Activation (Google for Jabali) is not possible',array('back_link' => true));
			};
		}
