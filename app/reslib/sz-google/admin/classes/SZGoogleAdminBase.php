		...
		private function moduleCheckRequestAPI() 
		{
			...
			if ($pagenow == 'admin.php' && preg_match('#^sz-google#',$adminpage) === 1) 
			{
				...
				if (isset($_GET['state']) and $_GET['state'] == 'token' and isset($_GET['code'])) 
				{
					...
					// and send an error message on the console jabali
				...
				// I add a notification message on standard section jabali
...
/* SE 22222222222222222222 */
		...
		function callback_base_authenticator() 
		{
			...
			$this->moduleCommonFormDescription(__('with this module you can enable two-factor authentication to be added to the standard jabali. Before starting this option carefully read the documentation. Each authorized user must synchronize the code with a mobile device.','sz-google'));
		}

		function callback_base_calendar()
		{
			$this->moduleCommonFormCheckboxYesNo('sz_google_options_base','calendar');
			$this->moduleCommonFormDescription(__('activating this module you can get a widget and shortcode to insert one of the components of google calendar on your jabali. You will find several options in the configuration screen to customize your best result.','sz-google'));
		}

		function callback_base_drive()
		{
			$this->moduleCommonFormCheckboxYesNo('sz_google_options_base','drive');
			$this->moduleCommonFormDescription(__('through this module you can insert into jabali some features of google drive, you will find widgets and shortcodes to help you with this task. Obviously many functions can only work if you login with a valid account on google.','sz-google'));
		}
		...
		function callback_base_fonts()
		{
			...
			$this->moduleCommonFormDescription(__('with this module you can load into your jabali theme fonts made ​​available on Google CDN. Simply select the desired font and HTML parts concerned. The plugin will automatically add all the necessary parts of the code.','sz-google'));
		}

		function callback_base_groups() 
		{
			$this->moduleCommonFormCheckboxYesNo('sz_google_options_base','groups');
			$this->moduleCommonFormDescription(__('enabling this module you get a widget and a shortcode to perform embed on google groups. Then you can insert into a jabali page or in a sidebar content navigable for a group. You can specify various customization options.','sz-google'));
		}
		...
		function callback_base_maps() 
		{
			...
			$this->moduleCommonFormDescription(__('activating this module you can use the google maps and put them in an article or in a widget on the sidebar of jabali. Read the documentation to learn about all the configuration options and the features available.','sz-google'));
		}
		...
		function callback_base_recaptcha() 
		{
			...
			$this->moduleCommonFormDescription(__('with this form you can activate the functions of reCAPTCHA on some components present in jabali. Just ask for the activation keys to google and configure the module in the configuration panel.','sz-google'));
		}
		...
		function callback_base_youtube() 
		{
			...
			$this->moduleCommonFormDescription(__('with this module can be inserted in jabali a video youtube, you can also use a widget to the inclusion of video in the sidebar on your theme. Through the options in the shortcode you can configure many parameters to customize the embed code.','sz-google'));
		}
		...
		function callback_base_tinymce() 
		{
			...
			$this->moduleCommonFormDescription(__('activating this option all shortcodes plugin that are active will be present in the editor of jabali when inserting a post or page. Using the drop-down menu that you will find in the editor you can specify all the options shortcode via a pop-up window.','sz-google'));
		}
...
$a .= admin_url('admin/admin.php?page=sz-google-admin.php');
