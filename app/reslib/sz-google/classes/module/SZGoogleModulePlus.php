...
if (!class_exists('SZGoogleModulePlus'))
{
	class SZGoogleModulePlus extends SZGoogleModule
	{
		 ...
		 * questa funzione viene aggiunta all'azione WP_HEAD() di jabali
		 *
		 * @return void
		 */
		...
		function getPlusPlusoneCode($atts=array(),$content=null) 
		{
			...
			// Creazione codice HTML per embed code da inserire nella pagina jabali
		...
		function addAdminNoticesRewrite() {
			...
			echo __('Google+ Rewrite option is enable but jabali permalink is disable. Please disable option for rewrite.','sz-google');
			echo '</p></div>';
		}
