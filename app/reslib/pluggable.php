 ...
 *
 * Some Jabali functionality is based on the current user and not based on
...
function wp_mail( $to, $subject, $message, $headers = '', $attachments = array() ) {
	...
	if ( ! ( $phpmailer instanceof PHPMailer ) ) {
		require_once ABSPATH . RES . '/class-phpmailer.php';
		require_once ABSPATH . RES . '/class-smtp.php';
	...
	/* If we don't have an email from the input headers default to jabali@$sitename
	 * Some hosts will block outgoing mail from this address if it doesn't exist but
	 * there's no easy alternative. Defaulting to admin_email might appear to be another
	 * option but some hosts may refuse to relay mail from an unknown domain. See
	 * https://core.trac.jabali.github.io/ticket/5007.
	 */
...
function wp_set_auth_cookie( $user_id, $remember = false, $secure = '', $token = '' ) {
	if ( $remember ) {
		...
		/** This filter is documented in reslib/pluggable.php */
...
function auth_redirect() {
	...
	if ( $secure && !is_ssl() && false !== strpos($_SERVER['REQUEST_URI'], 'admin') ) {
	...
	if ( $user_id = wp_validate_auth_cookie( '',  $scheme) ) {
		...
		if ( !$secure && get_user_option('use_ssl', $user_id) && false !== strpos($_SERVER['REQUEST_URI'], 'admin') ) {
			if ( 0 === strpos( $_SERVER['REQUEST_URI'], 'http' ) ) {
				wp_redirect( set_url_scheme( $_SERVER['REQUEST_URI'], 'https' ) );
				exit();
			} else {
...
function wp_notify_postauthor( $comment_id, $deprecated = null ) {
	...
	$wp_email = 'jabali@' . preg_replace('#^www\.#', '', strtolower($_SERVER['SERVER_NAME']));
 ...
 *
 * @global wpdb         $wpdb      Jabali database object for queries.
...
function wp_new_user_notification( $user_id, $deprecated = null, $notify = '' ) {
	...
	$message .= '<' . network_site_url("login.php?action=rp&key=$key&login=" . rawurlencode($user->user_login), 'login') . ">\r\n\r\n";
 ...
 * {@link https://api.jabali.github.io/secret-key/1.1/salt/ secret key created} just
 * for you.
 *
 ...
 * @since 2.5.0
 *
 * @link https://api.jabali.github.io/secret-key/1.1/salt/ Create secrets for db.php
 *
...
function wp_text_diff( $left_string, $right_string, $args = null ) {
	...
	if ( ! class_exists( 'WP_Text_Diff_Renderer_Table', false ) )
		require( ABSPATH . RES . '/wp-diff.php' );
