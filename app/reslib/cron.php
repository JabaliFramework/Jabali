 ...
 *
 * Schedules an event which will execute once by the Jabali actions core at
 * a time which you specify. The action will fire off when someone visits your
 * Jabali site, if the schedule time has passed.
 *
 ...
 *
 * Schedules a hook which will be executed by the Jabali actions core on a
 * specific interval, specified by you. The action will trigger when someone
 * visits your Jabali site, if the scheduled time has passed.
 *
...
function wp_schedule_event( $timestamp, $recurrence, $hook, $args = array()) {
	...
	/** This filter is documented in reslib/cron.php */
	$event = apply_filters( 'schedule_event', $event );
...
function spawn_cron( $gmt_time = 0 ) {
	...
	if ( defined( 'ALTERNATE_WP_CRON' ) && ALTERNATE_WP_CRON ) {
		...
		WP_DEBUG ? include_once( ABSPATH . 'cron.php' ) : @include_once( ABSPATH . 'cron.php' );
		return;
	}
	...
	$cron_request = apply_filters( 'cron_request', array(
		...
		'args' => array(
			'timeout'   => 0.01,
			'blocking'  => false,
			/** This filter is documented in reslib/class-wp-http-streams.php */
...
function wp_cron() {
	...
	if ( strpos($_SERVER['REQUEST_URI'], '/cron.php') !== false || ( defined('DISABLE_WP_CRON') && DISABLE_WP_CRON ) )
		return;
