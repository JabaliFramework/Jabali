<?php
/**
 * Facilitates adding of the Jabali editor as used on the Write and Edit screens.
 *
 * @package Jabali
 * @since 3.3.0
 *
 * Private, not included by default. See wp_editor() in reslib/general-template.php.
 */
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				if ( $set['teeny'] ) {
					 ...
					 * @param string $editor_id Unique editor identifier, e.g. 'content'.
					 */
					self::$plugins = $plugins = apply_filters( 'teeny_mce_plugins', array( 'colorpicker', 'lists', 'fullscreen', 'image', 'jabali', 'wpeditimage', 'wplink' ), $editor_id );
				} else {
					 ...
					 * 'myplugin' => 'http://mysite.com/main/plugins/myfolder/mce_plugin.js'.
					 *
					 ...
					 * in Jabali should be added to the TinyMCE instance.
					 *
					 * @since 3.3.0
					 *
				...
				$mce_css = array(
					$dashicons,
					self::$baseurl . '/skins/jabali/main.css?' . $version
				);
