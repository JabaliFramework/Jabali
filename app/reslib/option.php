...
function get_option( $option, $default = false ) {
	...
	if ( ! wp_installing() ) {
		...
		if ( isset( $notoptions[ $option ] ) ) {
			 ...
			 * @since 3.4.0
			 * @since 4.4.0 The `$option` parameter was added.
			 * @since 17.01.0 The `$passed_default` parameter was added to distinguish between a `false` value and the default parameter value.
			 *
		...
		if ( isset( $alloptions[$option] ) ) {
			...
			if ( false === $value ) {
				...
				if ( is_object( $row ) ) {
					...
					/** This filter is documented in reslib/option.php */
					return apply_filters( 'default_option_' . $option, $default, $option, $passed_default );
				}
			}
		}
...
/**
 * Protect Jabali special option from being modified.
 *
 ...
 * @param string|bool $autoload Optional. Whether to load the option when Jabali starts up. For existing options,
 ...
 * aren't adding a protected Jabali option. Care should be taken to not name
 ...
 * @param string|bool    $autoload    Optional. Whether to load the option when Jabali starts up.
...
/**
 * Removes option by name. Prevents removal of protected Jabali options.
 *
 * @since 1.2.0
 *
 * @global wpdb $wpdb Jabali database abstraction object.
...
function get_network_option( $network_id, $option, $default = false ) {
	 ...
	 * @since 3.0.0
	 * @since 4.4.0 The `$option` parameter was added.
	 * @since 17.01.0 The `$network_id` parameter was added.
	 *
...
/**
 * Register default settings available in Jabali.
 *
 * The settings registered here are primarily useful for the REST API, so this
 * does not encompass all settings available in Jabali.
 *
 * @since 17.01.0
 */
function register_initial_settings() {
	...
	register_setting( 'general', 'WPLANG', array(
		...
		'type'         => 'string',
		'description'  => __( 'Jabali locale code.' ),
		'default'      => 'en_US',
	) );
 ...
 *
 * @since 2.7.0
 * @since 17.01.0 `$args` can be passed to set flags on the setting, similar to `register_meta()`.
 *
 ...
 *
 * @since 2.7.0
 * @since 17.01.0 `$sanitize_callback` was deprecated. The callback from `register_setting()` is now used instead.
 *
 * @global array $new_whitelist_options
 *
