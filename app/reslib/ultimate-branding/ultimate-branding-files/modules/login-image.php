    ...
    function __construct() {
        ...
        if (!isset($login_image_old) || $login_image_old == '') {
            ...
            if (!$login_image) {//add default image if not exists
                $response = wp_remote_head(admin_url() . 'images/jabali-logo.svg');
                if (!is_wp_error($response) && !empty($response['response']['code']) && $response['response']['code'] == '200') {//support for 3.8+
                    ub_update_option('ub_login_image', admin_url() . 'images/jabali-logo.svg');
                } else {
                    ub_update_option('ub_login_image', admin_url() . 'images/jabali-logo.png'); // for older version
                }
            }
    ...
    function stylesheet() {
        ...
        if (isset($login_image_old) && trim($login_image_old) !== '') {
            ...
            if ($login_image_id) {
                ...
                $response = wp_remote_head(admin_url() . 'images/jabali-logo.svg');
                if (!is_wp_error($response) && !empty($response['response']['code']) && $response['response']['code'] == '200') {//support for 3.8+
                    $login_image = admin_url() . 'images/jabali-logo.svg';
                } else {
                    $login_image = admin_url() . 'images/jabali-logo.png';
                }
            }
        }
    ...
    function process() {
        ...
        if (isset($_GET['reset']) && isset($_GET['page']) && $_GET['page'] == 'branding') {
            ...
            if (!is_wp_error($response) && !empty($response['response']['code']) && $response['response']['code'] == '200') {//support for 3.8+
                ...
                ub_update_option('ub_login_image', admin_url() . 'images/jabali-logo.png');
            }
