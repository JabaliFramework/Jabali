	...
	class WPMUDEV_Dashboard_Notice3 {
		...
		function init() {
			...
			if ( is_admin() && current_user_can( 'install_plugins' ) ) {
				...
				if ( file_exists(RES . '/wpmudev-updates/update-notifications.php') ) {
					if ( !get_site_option('wdp_un_autoactivated') ) {
						//include plugin API if necessary
						if ( !function_exists('activate_plugin') ) require_once(ABSPATH . 'admin/includes/plugin.php');
...
#the-list .wpmu-update-row a:hover{color:#e8da42;-webkit-transition:0.2s}
		...
		function updates_check() {
			...
			if( empty($plugins_root) ) {
				$plugins_root = ABSPATH . 'main/plugins';
			}
			...
			if( empty($mu_plugins_root) ) {
				$mu_plugins_root = ABSPATH . 'main/mu-plugins';
			}
			...
			// main directory
			//----------------------------------------------------------------------------------//
			$content_plugins_root = MAIN_DIR;
			if( empty($content_plugins_root) ) {
				$content_plugins_root = ABSPATH . 'main';
			}
			...
			//themes directory
			//----------------------------------------------------------------------------------//
			$themes_root = MAIN_DIR . '/themes';
			if ( empty($themes_root) ) {
				$themes_root = ABSPATH . 'main/themes';
			}
			...
			$wp = is_multisite() ? "Jabali Multisite $wp_version" : "Jabali $wp_version";
		...
		function set_activate_flag($plugin) {
			$data = $this->get_id_plugin( RES . '/' . $plugin );
			if ( isset($data['id']) && !empty($data['id']) ) {
