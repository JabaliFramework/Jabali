...
function wpmu_validate_user_signup($user_name, $user_email) {
	...
	/** This filter is documented in reslib/user.php */
 ...
 * the way that Jabali validates new site signups.
 *
 * @since MU
 *
 * @global wpdb   $wpdb
...
function wpmu_signup_blog_notification( $domain, $path, $title, $user_login, $user_email, $key, $meta = array() ) {
	...
	$from_name = get_site_option( 'site_name' ) == '' ? 'Jabali' : esc_html( get_site_option( 'site_name' ) );
...
function install_blog( $blog_id, $blog_title = '' ) {
	...
	if ( $wpdb->get_results( "DESCRIBE {$wpdb->posts}" ) )
		die( '<h1>' . __( 'Already Installed' ) . '</h1><p>' . __( 'You appear to have already installed Jabali. To reinstall please clear your old database tables first.' ) . '</p></body></html>' );
...
--The Team @ SITE_NAME' );
	...
	if ( empty( $current_network->site_name ) )
		$current_network->site_name = 'Jabali';
 ...
 *
 * Jabali ships with a set of allowed upload filetypes,
 * which is defined in reslib/functions.php in
 ...
 *
 * Jabali MS stores a blog's post count as an option so as
...
function maybe_add_existing_user_to_blog() {
	...
	wp_die( sprintf( __( 'You have been added to this site. Please visit the <a href="%s">homepage</a> or <a href="%s">log in</a> using your username and password.' ), home_url(), admin_url() ), __( 'Jabali &rsaquo; Success' ), array( 'response' => 200 ) );
}
...
function wp_maybe_update_network_user_counts() {
	...
	/** This filter is documented in reslib/ms-functions.php */
...
function get_subdirectory_reserved_names() {
	$names = array(
		'page', 'comments', 'blog', 'files', 'feed', 'admin',
		'main', 'reslib', 'wp-json', 'embed'
	);
	 ...
	 *
	 * @since 3.0.0
	 * @since 4.4.0 'admin', 'main', 'reslib', 'wp-json', and 'embed' were added
