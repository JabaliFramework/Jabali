 ...
 * context of the inner workings of Jabali, pages, posts, and attachments
 ...
 * criteria of posts to retrieve. The 'post_type' can be anything, but Jabali
...
/**
 * Retrieve all of the Jabali supported post statuses.
 *
...
/**
 * Retrieve all of the Jabali support page statuses.
 *
 ...
 * @since 17.01.0 Introduced `show_in_rest`, 'rest_base' and 'rest_controller_class'
 ...
 * @since 4.6.0 Converted the `$post_type` parameter to accept a WP_Post_Type object.
 * @since 17.01.0 Added the `view_items` and `attributes` labels.
 *
 * @access private
 *
...
function get_post_type_labels( $post_type_object ) {
	$nohier_vs_hier_defaults = array(
		'name' => array( _x('Blog', 'post type general name'), _x('Pages', 'post type general name') ),
		'singular_name' => array( _x('Post', 'post type singular name'), _x('Page', 'post type singular name') ),
		'add_new' => array( _x('New Post', 'post'), _x('New Page', 'page') ),
...
function wp_count_posts( $type = 'post', $perm = '' ) {
	...
	if ( false !== $counts ) {
		/** This filter is documented in reslib/post.php */
 ...
 * @since 17.01.0 $uri can be an array of URIs.
 *
 * @global wpdb $wpdb Jabali database abstraction object.
 *
 ...
 *
 * @since 2.0.0
 * @since 17.01.0 Added the `$wp_error` parameter to allow a WP_Error to be returned on failure.
 *
 * @see wp_insert_post()
 *
...
function wp_get_attachment_url( $post_id = 0 ) {
	...
	if ( $file = get_post_meta( $post->ID, '_wp_attached_file', true ) ) {
		...
		if ( ( $uploads = wp_get_upload_dir() ) && false === $uploads['error'] ) {
			...
			} elseif ( false !== strpos($file, 'main/uploads') ) {
	...
	if ( is_ssl() && ! is_admin() && 'login.php' !== $GLOBALS['pagenow'] ) {
...
function wp_mime_type_icon( $mime = 0 ) {
	...
	if ( empty($icon) ) {
		...
		if ( !is_array($icon_files) ) {
			 ...
			 */
			$icon_dir = apply_filters( 'icon_dir', ABSPATH . RES . '/images/media' );
