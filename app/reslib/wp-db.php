...
/**
 * Jabali Database Access Abstraction Object
 *
 * It is possible to replace this class with your own
 * by setting the $wpdb global variable in wp-contentconfig/db.php
 * file to your class. The wpdb class will still be included,
 * so you can extend it or simply use your own.
 *
 * @link https://codex.jabali.github.io/Function_Reference/wpdb_Class
 *
 * @package Jabali
 * @subpackage Database
 * @since 0.71
...
class wpdb {
	 ...
	 * Jabali table prefix
	 *
	 * You can set this to have multiple Jabali installations
	...
	/**
	 * List of Jabali per-blog tables
	 *
	 * @since 2.5.0
	 * @access private
	 * @see wpdb::tables()
	...
	/**
	 * List of deprecated Jabali tables
	 *
	...
	/**
	 * Jabali Term Relationships table
	 *
	 * @since 2.3.0
	 * @access public
	 * @var string
	 ...
	 * against the required MySQL version for Jabali. Normally, a replacement
	 ...
	 * to the database.
	 *
	 * @link https://core.trac.jabali.github.io/ticket/3354
	 * @since 2.0.8
	 *
	...
	public function __construct( $dbuser, $dbpassword, $dbname, $dbhost ) {
		 ...
		 *  - We are a development version of Jabali, or
	...
	/**
	 * Change the current SQL mode, and ensure its Jabali compatibility.
	 *
	...
	/**
	 * Sets the table prefix for the Jabali tables.
	 *
	 * @since 2.5.0
	 *
	...
	/**
	 * Returns an array of Jabali tables.
	 *
	 * Also allows for the CUSTOM_USER_TABLE and CUSTOM_USER_META_TABLE to
	 * override the Jabali users and usermeta tables that would otherwise
		...
		if ( ! $success ) {
			...
			if ( ! did_action( 'template_redirect' ) ) {
				...
				$message .= '<p>' . sprintf(
					...
					__( 'If you don&#8217;t know how to set up a database you should <strong>contact your host</strong>. If all else fails you may find help at the <a href="%s">Jabali Support Forums</a>.' ),
	...
	public function print_error( $str = '' ) {
		...
		if ( $caller = $this->get_caller() ) {
			...
			$error_str = sprintf( __( 'Jabali database error %1$s for query %2$s made by %3$s' ), $str, $this->last_query, $caller );
		} else {
			/* translators: 1: Database error message, 2: SQL query */
			$error_str = sprintf( __( 'Jabali database error %1$s for query %2$s' ), $str, $this->last_query );
		}
	...
	public function db_connect( $allow_bail = true ) {
		...
		if ( ! $this->dbh && $allow_bail ) {
			...
			$message .= '<p>' . sprintf(
				/* translators: %s: support forums URL */
				__( 'If you&#8217;re unsure what these terms mean you should probably contact your host. If you still need help you can always visit the <a href="%s">Jabali Support Forums</a>.' ),
				__( 'https://jabali.github.io/support/' )
			) . "</p>\n";
	...
	public function check_database_version() {
		...
		if ( version_compare($this->db_version(), $required_mysql_version, '<') ) {
			/* translators: 1: Jabali version number, 2: Minimum required MySQL version number */
			return new WP_Error('database_version', sprintf( __( '<strong>ERROR</strong>: Jabali %1$s requires MySQL %2$s or higher' ), $wp_version, $required_mysql_version ));
		}
	}
	 ...
	 *
	 * Called when Jabali is generating the table scheme.
	 *
