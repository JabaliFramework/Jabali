 ...
 * Jabali workflow, then change the former. If you just have the locale in a
	...
	foreach ( $wp_theme_directories as $theme_root ) {
		...
		foreach ( $dirs as $dir ) {
			...
			if ( file_exists( $theme_root . '/' . $dir . '/style.css' ) ) {
				// main/themes/a-single-theme
				// main/themes is $theme_root, a-single-theme is $dir
				...
				$found_theme = false;
				// main/themes/a-folder-of-themes/*
				// main/themes is $theme_root, a-folder-of-themes is $dir, then themes are $sub_dirs
...
function get_theme_root_uri( $stylesheet_or_template = false, $theme_root = false ) {
	 ...
	 * @param string $siteurl                Jabali web address which is set in General Options.
...
function get_theme_mod( $name, $default = false ) {
	...
	/** This filter is documented in reslib/theme.php */
	...
	$core_content = array(
		...
		'nav_menus' => array(
			...
			'link_email' => array(
				...
				'url' => 'mailto:jabali@example.com',
			),
			'link_facebook' => array(
				'title' => _x( 'Facebook', 'Theme starter content' ),
				'url' => 'https://www.facebook.com/jabali',
			),
 ...
 * @since 17.01.0 The `starter-content` feature was added
 *
...
function _custom_header_background_just_in_time() {
	...
	if ( current_theme_supports( 'custom-header' ) ) {
		...
		if ( is_admin() ) {
			require_once( ABSPATH . 'admin/custom-header.php' );
	...
	if ( current_theme_supports( 'custom-background' ) ) {
		...
		if ( is_admin() ) {
			require_once( ABSPATH . 'admin/custom-background.php' );
...
function check_theme_switched() {
	if ( $stylesheet = get_option( 'theme_switched' ) ) {
		...
		if ( $old_theme->exists() ) {
			...
			/** This action is documented in reslib/theme.php */
 ...
 * Jabali is loading, especially in the Customizer preview
...
function _wp_customize_include() {
	...
	require_once ABSPATH . RES . '/class-wp-customize-manager.php';
 ...
 *
 * @since 3.4.0
 * @since 17.01.0 Support for IE8 and below is explicitly removed via conditional comments.
 */
