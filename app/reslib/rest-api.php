...
function rest_handle_deprecated_function( $function, $replacement, $version ) {
	if ( ! empty( $replacement ) ) {
		/* translators: 1: function name, 2: Jabali version number, 3: new function name */
		$string = sprintf( __( '%1$s (since %2$s; use %3$s instead)' ), $function, $version, $replacement );
	} else {
		/* translators: 1: function name, 2: Jabali version number */
...
function rest_handle_deprecated_argument( $function, $message, $version ) {
	if ( ! empty( $message ) ) {
		/* translators: 1: function name, 2: Jabali version number, 3: error message */
 ...
 *
 * Jabali' built-in cookie authentication is always active
	...
	if ( isset( $args['format'] ) ) {
		switch ( $args['format'] ) {
			...
			case 'email' :
				...
				//
				// https://core.trac.jabali.github.io/ticket/38506
