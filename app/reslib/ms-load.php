 ...
 * manually, define `WP_PLUGIN_DIR` and `WP_PLUGIN_URL` in `db.php`.
 *
 * @access private
 * @since 3.1.0
 *
 ...
 * use the main/blog-deleted.php, blog-inactive.php and
 ...
 *
 * @since 3.9.0
 * @since 17.01.0 Updated to always return a `WP_Site` object.
 *
...
function get_site_by_path( $domain, $path, $segments = null ) {
	 ...
	 * @since 3.9.0
	 *
	 * @param int|null $segments The number of path segments to consider. Jabali by default looks at
...
function ms_load_current_site_and_network( $domain, $path, $subdomain = false ) {
	...
	if ( empty( $current_site ) ) {
		/** This action is documented in reslib/ms-settings.php */
		do_action( 'ms_network_not_found', $domain, $path );
...
function ms_not_installed( $domain, $path ) {
	...
	if ( ! $wpdb->get_var( $query ) ) {
		$msg .= '<p>' . sprintf(
			...
			__( '<strong>Database tables are missing.</strong> This means that MySQL is not running, Jabali was not installed properly, or someone deleted %s. You really should look at your database now.' ),
			'<code>' . $wpdb->site . '</code>'
		) . '</p>';
	} else {
		$msg .= '<p>' . sprintf(
	...
	$msg .= sprintf( __( 'Read the <a href="%s" target="_blank">bug report</a> page. Some of the guidelines there may help you figure out what went wrong.' ),
		__( 'https://codex.jabali.github.io/Debugging_a_Jabali_Network' )
	);
