...
require( ABSPATH . RES . '/option.php' );
 ...
 * if $gmt is false, the output is adjusted with the GMT offset in the Jabali option.
 *
 * @since 1.0.0
 *
...
function date_i18n( $dateformatstring, $unixtimestamp = false, $gmt = false ) {
	 ...
	 * See https://core.trac.jabali.github.io/ticket/9396
	 */
...
function maybe_serialize( $data ) {
	...
	// See https://core.trac.jabali.github.io/ticket/12930
 ...
 * @since 1.5.0
 *
 * @global wpdb $wpdb Jabali database abstraction object.
 *
...
/**
 * Retrieve the number of database queries during the Jabali execution.
 *
 * @since 2.0.0
 *
 * @global wpdb $wpdb Jabali database abstraction object.
...
function do_feed_rdf() {
	load_template( ABSPATH . RES . '/feed-rdf.php' );
}
...
function do_feed_rss() {
	load_template( ABSPATH . RES . '/feed-rss.php' );
}
...
function do_feed_rss2( $for_comments ) {
	if ( $for_comments )
		load_template( ABSPATH . RES . '/feed-rss2-comments.php' );
	else
		load_template( ABSPATH . RES . '/feed-rss2.php' );
}
...
function do_feed_atom( $for_comments ) {
	if ($for_comments)
		load_template( ABSPATH . RES . '/feed-atom-comments.php');
	else
		load_template( ABSPATH . RES . '/feed-atom.php' );
}
...
/**
 * Test whether Jabali is already installed.
 *
 * The cache will be checked first. If you have a cache plugin, which saves
 * the cache values, then this will work. If you use the default Jabali
 * cache, and the database goes away, then you might have problems.
 *
 * Checks for the 'siteurl' option for whether Jabali is installed.
 *
 * @since 2.1.0
 *
 * @global wpdb $wpdb Jabali database abstraction object.
 ...
 * It may be overridden by the WP_TEMP_DIR constant in your db.php file.
 *
 * @since 2.5.0
 *
...
function _wp_upload_dir( $time = null ) {
	...
	if ( is_multisite() && ! ( is_main_network() && is_main_site() && defined( 'MULTISITE' ) ) ) {
		...
		if ( ! get_site_option( 'ms_files_rewriting' ) ) {
			 ...
			 * had main/uploads for the main site.)
			 */
			 ...
			 * 1) We are not on the main site in a post-MU network, as main/uploads is used
			 *    there, and
...
function wp_unique_filename( $dir, $filename, $unique_filename_callback = null ) {
	...
	/** This filter is documented in reslib/functions.php */
...
function wp_nonce_ays( $action ) {
	...
	wp_die( $html, __( 'Jabali Failure Notice' ), 403 );
}

/**
 * Kill Jabali execution and display HTML message with error message.
 *
 ...
 *     @type string $text_direction The text direction. This is only useful internally, when Jabali
...
function wp_die( $message = '', $title = '', $args = array() ) {
	...
	if ( wp_doing_ajax() ) {
		/**
		 * Filters the callback for killing Jabali execution for Ajax requests.
		 *
		 * @since 3.4.0
		 *
		...
		/**
		 * Filters the callback for killing Jabali execution for XML-RPC requests.
		 *
		 * @since 3.4.0
		 *
		 * @param callable $function Callback function name.
		...
		/**
		 * Filters the callback for killing Jabali execution for all non-Ajax, non-XML-RPC requests.
		 *
		 * @since 3.0.0
		 *
		 * @param callable $function Callback function name.
...
/**
 * Kills Jabali execution and display HTML message with error message.
 *
...
function _default_wp_die_handler( $message, $title = '', $args = array() ) {
	...
	if ( ! did_action( 'admin_head' ) ) :
		...
		if ( empty($title) )
			$title = $have_gettext ? __('Jabali &rsaquo; Error') : 'Jabali &rsaquo; Error';
...
/**
 * Kill Jabali execution and display XML message with error message.
 *
 ...
 *
 * @since 3.5.0
 * @since 17.01.0 The `$status_code` parameter was added.
 *
...
/**
 * Retrieve the Jabali home page URL.
 *
 ...
 * @see WP_SITEURL
 *
 * @param string $url URL to set the Jabali site location.
 * @return string The Jabali Site URL.
 */
 ...
 *
 * Will only set the direction to 'rtl', if the Jabali locale has
 ...
 *
 * This function is used throughout Jabali to allow for both string or array
 ...
 *
 * @since 3.0.0
 * @since 17.01.0 Uses WP_List_Util class.
 *
...
function wp_maybe_load_widgets() {
	...
	require_once( ABSPATH . RES . '/default-widgets.php' );
...
/**
 * Load custom DB error or display Jabali DB error.
 *
 * If a file exists in the main directory named db-error.php, then it will
 * be loaded instead of displaying the Jabali DB error. If it is not found,
 * then the Jabali DB error will be displayed instead.
 *
 * The Jabali DB error sets the HTTP status header to 500 to try to prevent
 * search engines from caching the message. Custom DB messages should do the
 * same.
 *
 * This function was backported to Jabali 2.3.2, but originally was added
 * in Jabali 2.5.0.
 *
 * @since 2.3.2
 *
 ...
 * @param string $version     The version of Jabali that deprecated the function.
 ...
 * @param string $version     The version of Jabali that deprecated the file.
 ...
 *
 * @param string $function The function that was called.
 * @param string $version  The version of Jabali that deprecated the argument used.
 ...
 * @param string $version     The version of Jabali that deprecated the hook.
...
function _deprecated_hook( $hook, $version, $replacement = null, $message = null ) {
	...
	if ( WP_DEBUG && apply_filters( 'deprecated_hook_trigger_error', true ) ) {
		$message = empty( $message ) ? '' : ' ' . $message;
		if ( ! is_null( $replacement ) ) {
			/* translators: 1: Jabali hook name, 2: version number, 3: alternative hook name */
			trigger_error( sprintf( __( '%1$s is <strong>deprecated</strong> since version %2$s! Use %3$s instead.' ), $hook, $version, $replacement ) . $message );
		} else {
			/* translators: 1: Jabali hook name, 2: version number */
 ...
 * @param string $version  The version of Jabali where the message was added.
 */
 ...
 *
 * Will remove admin links to retrieve only return URLs not in the wp-admin
 * directory.
 *
 * @since 2.6.0
 *
 ...
 *
 * @since 2.9.0
 * @since 17.01.0 Added the `$locale` parameter.
 *
 ...
 * @access private
 *
 * @see https://core.trac.jabali.github.io/ticket/8497
 *
 ...
 * @since 17.01.0 Added 'urn' to the protocols array.
 *
 * @see wp_kses()
 * @see esc_url()
 *
 ...
 * @since 3.4.0
 *
 * @see https://core.trac.jabali.github.io/ticket/19589
 *
 ...
 * @access private
 *
 * @see https://core.trac.jabali.github.io/ticket/23688
 *
...
function wp_raise_memory_limit( $context = 'admin' ) {
	...
	switch ( $context ) {
		case 'admin':
			 ...
			 *
			 * @param int|string $filtered_limit The maximum Jabali memory limit. Accepts an integer
