...
function list_core_update( $update ) {
	...
	if ( 'development' == $update->response ) {
		$message = __('You are using a development version of Jabali. You can update to the latest nightly build automatically:');
	} else {
		if ( $current ) {
	...
	elseif ( 'en_US' == $update->locale && get_locale() != 'en_US' && ( ! $update->packages->partial && $wp_version == $update->partial_version ) ) {
	    echo '<p class="hint">'.sprintf( __('You are about to install Jabali %s <strong>in English (US).</strong> There is a chance this update will break your translation. You may prefer to wait for the localized version to be released.'), $update->response != 'development' ? $update->current : '' ).'</p>';
	}
	echo '</form>';
...
/**
 * Display upgrade Jabali for downloading latest or upgrading automatically form.
 *
 * @since 2.7.0
 *
...
function core_upgrade_preamble() {
	...
	if ( !isset($updates[0]->response) || 'latest' == $updates[0]->response ) {
		echo '<h2>';
		_e('You have the latest version of Jabali.');
		...
		_e( 'An updated version of Jabali is available.' );
		echo '</h2>';
	}
	...
	if ( $updates && ( count( $updates ) > 1 || $updates[0]->response != 'latest' ) ) {
		...
		echo '<p>' . sprintf( __( '<a href="%s">Learn more about Jabali %s</a>.' ), esc_url( self_admin_url( 'about.php' ) ), $normalized_version ) . '</p>';
	}
	dismissed_updates();
}
...
<?php
	foreach ( (array) $plugins as $plugin_file => $plugin_data ) {
		// Get plugin compat for running version of Jabali.
		if ( isset($plugin_data->update->tested) && version_compare($plugin_data->update->tested, $cur_wp_version, '>=') ) {
			$compat = '<br />' . sprintf(__('Compatibility with Jabali %1$s: 100%% (according to its author)'), $cur_wp_version);
		} elseif ( isset($plugin_data->update->compatibility->{$cur_wp_version}) ) {
			$compat = $plugin_data->update->compatibility->{$cur_wp_version};
			$compat = '<br />' . sprintf(__('Compatibility with Jabali %1$s: %2$d%% (%3$d "works" votes out of %4$d total)'), $cur_wp_version, $compat->percent, $compat->votes, $compat->total_votes);
		} else {
			$compat = '<br />' . sprintf(__('Compatibility with Jabali %1$s: Unknown'), $cur_wp_version);
		}
		// Get plugin compat for updated version of Jabali.
		if ( $core_update_version ) {
			if ( isset( $plugin_data->update->tested ) && version_compare( $plugin_data->update->tested, $core_update_version, '>=' ) ) {
				$compat .= '<br />' . sprintf( __( 'Compatibility with Jabali %1$s: 100%% (according to its author)' ), $core_update_version );
				...
				$compat .= '<br />' . sprintf(__('Compatibility with Jabali %1$s: %2$d%% (%3$d "works" votes out of %4$d total)'), $core_update_version, $update_compat->percent, $update_compat->votes, $update_compat->total_votes);
			} else {
				$compat .= '<br />' . sprintf(__('Compatibility with Jabali %1$s: Unknown'), $core_update_version);
			}
		}
...
<?php
	...
	show_message( '<span class="hide-if-no-js">' . sprintf( __( 'Welcome to Jabali %1$s. You will be redirected to the About Jabali screen. If not, click <a href="%2$s">here</a>.' ), $result, esc_url( self_admin_url( 'about.php?updated' ) ) ) . '</span>' );
	show_message( '<span class="hide-if-js">' . sprintf( __( 'Welcome to Jabali %1$s. <a href="%2$s">Learn more</a>.' ), $result, esc_url( self_admin_url( 'about.php?updated' ) ) ) . '</span>' );
	?>
	</div>
	<script type="text/javascript">
...
$title = __('Jabali Updates');
$parent_file = 'index.php';

$updates_overview  = '<p>' . __( 'On this screen, you can update to the latest version of Jabali, as well as update your themes, plugins, and translations from the Jabali.org repositories.' ) . '</p>';
...
$updates_howto  = '<p>' . __( '<strong>Jabali</strong> &mdash; Updating your Jabali installation is a simple one-click procedure: just <strong>click on the &#8220;Update Now&#8221; button</strong> when you are notified that a new version is available.' ) . ' ' . __( 'In most cases, Jabali will automatically apply maintenance and security updates in the background for you.' ) . '</p>';
$updates_howto .= '<p>' . __( '<strong>Themes and Plugins</strong> &mdash; To update individual themes or plugins from this screen, use the checkboxes to make your selection, then <strong>click on the appropriate &#8220;Update&#8221; button</strong>. To update all of your themes or plugins at once, you can check the box at the top of the section to select all before clicking the update button.' ) . '</p>';

if ( 'en_US' != get_locale() ) {
	$updates_howto .= '<p>' . __( '<strong>Translations</strong> &mdash; The files translating Jabali into your language are updated for you whenever any other updates occur. But if these files are out of date, you can <strong>click the &#8220;Update Translations&#8221;</strong> button.' ) . '</p>';
}
...
if ( 'upgrade-core' == $action ) {
	...
	/**
	 * Fires for each custom update action on the Jabali Updates screen.
	 *
