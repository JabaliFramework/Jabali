...
( function( api, wp, $ ) {
	...
	api.Menus.AvailableMenuItemsPanelView = wp.Backbone.View.extend({
		 ...
		 *
		 * @since 4.3.0
		 * @since 17.01.0 Changed function signature to take list of item types instead of single type/object.
		 * @access private
		 *
	...
	function displayNavMenuName( name ) {
		name = name || '';
		name = $( '<div>' ).text( name ).html(); // Emulate esc_html() which is used in admin/nav-menus.php.
		name = $.trim( name );
