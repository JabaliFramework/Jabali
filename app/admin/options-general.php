...
/** Jabali Translation Install API */
...
if ( ! is_multisite() ) {
	$options_help .= '<p>' . __('The Jabali URL and the Site URL can be the same (example.com) or different; for example, having the Jabali core files (example.com/jabali) in a subdirectory instead of the root directory.') . '</p>' .
...
<?php if ( !is_multisite() ) { ?>
<tr>
<th scope="row"><label for="siteurl"><?php _e('Jabali Address (URL)') ?></label></th>
		...
		<td>
			...
			if ( defined( 'WPLANG' ) && ( '' !== WPLANG ) && $locale !== WPLANG ) {
				if ( is_super_admin() ) {
					?>
					<p class="description">
						<strong><?php _e( 'Note:' ); ?></strong> <?php printf( __( 'The %s constant in your %s file is no longer needed.' ), '<code>WPLANG</code>', '<code>db.php</code>' ); ?>
					</p>
					<?php
				}
				_deprecated_argument( 'define()', '4.0.0', sprintf( __( 'The %s constant in your %s file is no longer needed.' ), 'WPLANG', 'config/db.php' ) );
			}
			?>
		</td>
	</tr>
