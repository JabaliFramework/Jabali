<?php
/**
 * Jabali Plugin Install Administration API
 *
 * @package Jabali
...
/**
 * Retrieves plugin installer pages from the Jabali.org Plugins API.
 *
 ...
 *
 * The second filter, {@see 'plugins_api'}, allows a plugin to override the Jabali.org
 ...
 *         @type bool $requires          Whether to return the required Jabali version. Default true.
 ...
 *         @type bool $added             Whether to return the date when the plugin was added to the jabali.github.io
 *                                       repository. Default true.
 *         @type bool $tags              Whether to return the assigned tags. Default true.
 *         @type bool $compatibility     Whether to return the Jabali compatibility list. Default true.
 ...
 *         {@link https://developer.jabali.github.io/reference/functions/plugins_api/ function reference article}
...
function plugins_api( $action, $args = array() ) {
	...
	/**
	 * Filters the Jabali.org Plugin Install API arguments.
	 *
	...
	/**
	 * Filters the response for the current Jabali.org Plugin Install API request.
	 *
	 * Passing a non-false value will effectively short-circuit the Jabali.org API request.
	 *
	...
	if ( false === $res ) {
		$url = $http_url = 'http://api.jabali.github.io/plugins/info/1.0/';
		...
		if ( $ssl && is_wp_error( $request ) ) {
			trigger_error(
				sprintf(
					...
					__( 'An unexpected error occurred. Something may be wrong with Jabali.org or this server&#8217;s configuration. If you continue to have problems, please try the <a href="%s">support forums</a>.' ),
					__( 'https://jabali.github.io/support/' )
				) . ' ' . __( '(Jabali could not establish a secure connection to Jabali.org. Please contact your server administrator.)' ),
...
/**
 * Retrieve popular Jabali plugin tags.
 *
 * @since 2.7.0
 *
 * @param array $args
...
function install_dashboard() {
	?>
	<p><?php printf( __( 'Plugins extend and expand the functionality of Jabali. You may automatically install plugins from the <a href="%1$s">Jabali Plugin Directory</a> or upload a plugin in .zip format by clicking the button at the top of this page.' ), __( 'https://jabali.github.io/plugins/' ) ); ?></p>
...
function install_plugins_favorites_form() {
	...
	?>
	<p class="install-help"><?php _e( 'If you have marked plugins as favorites on Jabali.org, you can browse them here.' ); ?></p>
	<form method="get">
		<input type="hidden" name="tab" value="favorites" />
		<p>
			<label for="user"><?php _e( 'Your Jabali.org username:' ); ?></label>
...
function display_plugins_table() {
	...
	switch ( current_filter() ) {
		...
		case 'install_plugins_beta' :
			printf(
				'<p>' . __( 'You are using a development version of Jabali. These feature plugins are also under development. <a href="%s">Learn more</a>.' ) . '</p>',
				'https://make.jabali.github.io/core/handbook/about/release-cycle/features-as-plugins/'
			);
			break;
	}
...
<div id="<?php echo $_tab; ?>-content" class='<?php echo $_with_banner; ?>'>
	<div class="fyi">
		<ul>
			...
			<?php } if ( ! empty( $api->requires ) ) { ?>
				<li>
					<strong><?php _e( 'Requires Jabali Version:' ); ?></strong>
					<?php
			...
			<?php } if ( ! empty( $api->slug ) && empty( $api->external ) ) { ?>
				<li><a target="_blank" href="<?php echo __( 'https://jabali.github.io/plugins/' ) . $api->slug; ?>/"><?php _e( 'Jabali.org Plugin Page &#187;' ); ?></a></li>
		...
		if ( ! empty( $api->ratings ) && array_sum( (array) $api->ratings ) > 0 ) { ?>
			...
			<p class="fyi-description"><?php _e( 'Read all reviews on Jabali.org or write your own!' ); ?></p>
			<?php
			foreach ( $api->ratings as $key => $ratecount ) {
				...
				<div class="counter-container">
						<span class="counter-label"><a href="https://jabali.github.io/support/view/plugin-reviews/<?php echo $api->slug; ?>?filter=<?php echo $key; ?>"
			...
			<ul class="contributors">
				...
				foreach ( (array) $api->contributors as $contrib_username => $contrib_profile ) {
					...
					if ( empty( $contrib_profile ) ) {
						echo "<li><img src='https://jabali.github.io/grav-redirect.php?user={$contrib_username}&amp;s=36' width='18' height='18' alt='' />{$contrib_username}</li>";
					} else {
						echo "<li><a href='{$contrib_profile}' target='_blank'><img src='https://jabali.github.io/grav-redirect.php?user={$contrib_username}&amp;s=36' width='18' height='18' alt='' />{$contrib_username}</a></li>";
					}
				}
				?>
			</ul>
	...
	if ( ! empty( $api->tested ) && version_compare( substr( $wp_version, 0, strlen( $api->tested ) ), $api->tested, '>' ) ) {
		echo '<div class="notice notice-warning notice-alt"><p>' . __( '<strong>Warning:</strong> This plugin has <strong>not been tested</strong> with your current version of Jabali.' ) . '</p></div>';
	} elseif ( ! empty( $api->requires ) && version_compare( substr( $wp_version, 0, strlen( $api->requires ) ), $api->requires, '<' ) ) {
		echo '<div class="notice notice-warning notice-alt"><p>' . __( '<strong>Warning:</strong> This plugin has <strong>not been marked as compatible</strong> with your version of Jabali.' ) . '</p></div>';
	}

	foreach ( (array) $api->sections as $section_name => $content ) {
		$content = links_add_base_url( $content, 'https://jabali.github.io/plugins/' . $api->slug . '/' );
