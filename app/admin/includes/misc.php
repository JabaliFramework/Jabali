<?php
/**
 * Misc Jabali Administration API.
 *
 * @package Jabali
...
function save_mod_rewrite_rules() {
	...
	if ((!file_exists($htaccess_file) && is_writable($home_path) && $wp_rewrite->using_mod_rewrite_permalinks()) || is_writable($htaccess_file)) {
		if ( got_mod_rewrite() ) {
			...
			return insert_with_markers( $htaccess_file, 'Jabali', $rules );
		}
	}
...
/**
 * Check if rewrite rule for Jabali already exists in the IIS 7+ configuration file
 *
 * @since 2.8.0
 *
 * @return bool
...
function iis7_rewrite_rule_exists($filename) {
	...
	$rules = $xpath->query('/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'jabali\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'Jabali\')]');
	if ( $rules->length == 0 )
		return false;
	else
		return true;
...
/**
 * Delete Jabali rewrite rule from web.config file if it exists there
 *
 * @since 2.8.0
 *
...
/**
 * Add Jabali rewrite rule to the IIS 7+ configuration file.
 *
 * @since 2.8.0
 *
 * @param string $filename The file path to the configuration file
...
function iis7_add_rewrite_rule($filename, $rewrite_rule) {
	...
	$jabali_rules = $xpath->query('/configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'jabali\')] | /configuration/system.webServer/rewrite/rules/rule[starts-with(@name,\'Jabali\')]');
	if ( $jabali_rules->length > 0 )
		return true;
