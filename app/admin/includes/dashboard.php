<?php
/**
 * Jabali Dashboard Widget Administration Screen API
 *
 * @package Jabali
...
function wp_dashboard_setup() {
	...
	// Jabali News
	wp_add_dashboard_widget( 'dashboard_primary', __( 'Jabali News' ), 'wp_dashboard_primary' );
	...
	/** This action is documented in admin/edit-form-advanced.php */
...
function wp_dashboard_primary() {
	$feeds = array(
		'news' => array(
			...
			/**
			 * Filters the primary link URL for the 'Jabali News' dashboard widget.
			 *
			 * @since 2.5.0
			 *
			 * @param string $link The widget's primary link URL.
			 */
			'link' => apply_filters( 'dashboard_primary_link', __( 'https://jabali.github.io/news/' ) ),

			/**
			 * Filters the primary feed URL for the 'Jabali News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 * @param string $url The widget's primary feed URL.
			 */
			'url' => apply_filters( 'dashboard_primary_feed', __( 'http://jabali.github.io/news/feed/' ) ),

			/**
			 * Filters the primary link title for the 'Jabali News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 * @param string $title Title attribute for the widget's primary link.
			 */
			'title'        => apply_filters( 'dashboard_primary_title', __( 'Jabali Blog' ) ),
			'items'        => 1,
			'show_summary' => 1,
			'show_author'  => 0,
			'show_date'    => 1,
		...
		'planet' => array(
			...
			/**
			 * Filters the secondary link URL for the 'Jabali News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 * @param string $link The widget's secondary link URL.
			 */
			'link' => apply_filters( 'dashboard_secondary_link', __( 'https://planet.jabali.github.io/' ) ),

			/**
			 * Filters the secondary feed URL for the 'Jabali News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 * @param string $url The widget's secondary feed URL.
			 */
			'url' => apply_filters( 'dashboard_secondary_feed', __( 'https://planet.jabali.github.io/feed/' ) ),

			/**
			 * Filters the secondary link title for the 'Jabali News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 * @param string $title Title attribute for the widget's secondary link.
			 */
			'title'        => apply_filters( 'dashboard_secondary_title', __( 'Other Jabali News' ) ),

			/**
			 * Filters the number of secondary link items for the 'Jabali News' dashboard widget.
			 *
			 * @since 4.4.0
			 *
	...
	if ( ( ! defined( 'DISALLOW_FILE_MODS' ) || ! DISALLOW_FILE_MODS ) && ( ! is_multisite() && is_blog_admin() && current_user_can( 'install_plugins' ) ) || ( is_network_admin() && current_user_can( 'manage_network_plugins' ) && current_user_can( 'install_plugins' ) ) ) {
		$feeds['plugins'] = array(
			'link'         => '',
			'url'          => array(
				'popular' => 'http://jabali.github.io/plugins/rss/browse/popular/',
			),
			'title'        => '',
			'items'        => 1,
			'show_summary' => 0,
...
/**
 * Display plugins text for the Jabali news widget.
 *
 * @since 2.5.0
 *
...
function wp_dashboard_browser_nag() {
	...
	if ( $response ) {
		if ( $response['insecure'] ) {
			...
			$msg = sprintf( __( "It looks like you're using an insecure version of %s. Using an outdated browser makes your computer unsafe. For the best Jabali experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		} else {
			/* translators: %s: browser name and link */
			$msg = sprintf( __( "It looks like you're using an old version of %s. For the best Jabali experience, please update your browser." ),
				sprintf( '<a href="%s">%s</a>', esc_url( $response['update_url'] ), esc_html( $response['name'] ) )
			);
		}
...
function wp_check_browser_version() {
	...
	if ( false === ($response = get_site_transient('browser_' . $key) ) ) {
		$options = array(
			...
			'user-agent'	=> 'Jabali/' . get_bloginfo( 'version' ) . '; ' . home_url()
		);

		$response = wp_remote_post( 'http://api.jabali.github.io/core/browse-happy/1.1/', $options );
...
/**
 * Displays a welcome panel to introduce users to Jabali.
 *
 * @since 3.3.0
 */
function wp_welcome_panel() {
	...
	<h2><?php _e( 'Welcome to Jabali!' ); ?></h2>
	...
	<div class="welcome-panel-column welcome-panel-last">
		...
		<?php endif; ?>
			<li><?php printf( '<a href="%s" class="welcome-icon welcome-learn-more">' . __( 'Learn more about getting started' ) . '</a>', __( 'https://codex.jabali.github.io/First_Steps_With_Jabali' ) ); ?></li>
		</ul>
	</div>
	</div>
	</div>
