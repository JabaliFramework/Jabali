...
class WP_Press_This {
	...
	public function fetch_source_html( $url ) {
		...
		$remote_url = wp_safe_remote_get( $url, array(
			...
			'user-agent' => 'Press This (Jabali/' . get_bloginfo( 'version' ) . '); ' . get_bloginfo( 'url' )
		) );
	 ...
	 *
	 * Excluded URLs include share-this type buttons, loaders, spinners, spacers, Jabali interface images,
	 * tiny buttons or thumbs, mathtag.com or quantserve.com images, or the Jabali.com stats gif.
	 *
	 * @ignore
	 * @since 4.2.0
	 *
	...
	private function _limit_img( $src ) {
		...
		if ( preg_match( '!/ad[sx]?/!i', $src ) ) {
			...
			return '';
		} else if ( false !== stripos( $src, '/reslib/' ) ) {
			// Classic Jabali interface images
			return '';
...
?>
<body class="admin wp-core-ui <?php echo $admin_body_classes . ' ' . $admin_body_class; ?>">
	<div id="adminbar" class="adminbar">
		<h1 id="current-site" class="current-site">
			<a class="current-site-link" href="<?php echo esc_url( home_url( '/' ) ); ?>" target="_blank" rel="home">
				<span class="dashicons dashicons-jabali"></span>
		...
		<div class="editor-wrapper">
			...
			<div id="app-container" class="editor">
				...
				wp_editor( $post_content, 'pressthis', array(
					...
					'tinymce'          => array(
						'resize'                => false,
						'jabali_adv_hidden'  => false,
						'add_unload_trigger'    => false,
						'statusbar'             => false,
						'autoresize_min_height' => 600,
						'wp_autoresize_on'      => true,
						'plugins'               => 'lists,media,paste,tabfocus,fullscreen,jabali,wpautoresize,wpeditimage,wpgallery,wplink,wptextpattern,wpview',
