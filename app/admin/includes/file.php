...
/**
 * Get the description for standard Jabali theme files and other various standard
 * Jabali files
 *
 * @since 1.5.0
 *
...
/**
 * Get the absolute filesystem path to the root of the Jabali installation
 *
 * @since 1.5.0
 *
 * @return string Full filesystem path to the root of the Jabali installation
 */
...
/**
 * Handle PHP uploads in Jabali, sanitizing file names, checking extensions for mime type,
...
function _wp_handle_upload( &$file, $overrides, $time, $action ) {
	...
	/**
	 * Filters the data for a file before it is uploaded to Jabali.
	 *
...
/**
 * Downloads a URL to a local temporary file using the Jabali HTTP Class.
...
/**
 * Unzips a specified ZIP file to a location on the Filesystem via the Jabali Filesystem Abstraction.
...
/**
 * Initialises and connects the Jabali Filesystem Abstraction classes.
 * This function will include the chosen transport and attempt connecting.
 *
 * Plugins may add extra transports, And force Jabali to use them by returning
...
function get_filesystem_method( $args = array(), $context = '', $allow_relaxed_file_ownership = false ) {
	...
	if ( ! $method ) {
		...
		if ( $temp_handle ) {
			...
			// Attempt to determine the file owner of the Jabali files, and that of newly created files
			...
			if ( $wp_file_owner !== false && $wp_file_owner === $temp_file_owner ) {
				// Jabali is creating files as the same owner as the Jabali files,
 ...
 *
 * Hostnames may be in the form of hostname:portnumber (eg: jabali.github.io:2467)
...
<p id="request-filesystem-credentials-desc"><?php
	...
	_e('To perform the requested action, Jabali needs to access your web server.');
	echo ' ';
...
<label for="hostname">
	...
	<input name="hostname" type="text" id="hostname" aria-describedby="request-filesystem-credentials-desc" class="code" placeholder="<?php esc_attr_e( 'example: www.jabali.github.io' ) ?>" value="<?php echo esc_attr($hostname); if ( !empty($port) ) echo ":$port"; ?>"<?php disabled( defined('FTP_HOST') ); ?> />
</label>
