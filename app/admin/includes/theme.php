...
function delete_theme($stylesheet, $redirect = '') {
	...
	if ( false === $credentials ) {
		if ( ! empty( $data ) ){
			include_once( ABSPATH . 'admin/admin-header.php');
			echo $data;
	...
	if ( empty( $themes_dir ) ) {
		return new WP_Error( 'fs_no_themes_dir', __( 'Unable to locate Jabali theme directory.' ) );
	}
 ...
 *
 * @since 1.5.0
 * @since 17.01.0 Added the `$post_type` parameter.
 *
...
/**
 * Retrieve list of Jabali theme features (aka theme tags)
 *
 * @since 3.1.0
 *
 * @param bool $api Optional. Whether try to fetch tags from the Jabali.org API. Defaults to true.
...
/**
 * Retrieves theme installer pages from the Jabali.org Themes API.
 *
 ...
 *
 * The second filter, {@see 'themes_api'}, allows a plugin to override the Jabali.org
 ...
 *         {@link https://developer.jabali.github.io/reference/functions/themes_api/ function reference article}
...
function themes_api( $action, $args = array() ) {
	...
	/**
	 * Filters arguments used to query for installer pages from the Jabali.org Themes API.
	 *
	 * Important: An object MUST be returned to this filter.
	 *
	 * @since 2.8.0
	 *
	 * @param object $args   Arguments used to query for installer pages from the Jabali.org Themes API.
	...
	/**
	 * Filters whether to override the Jabali.org Themes API.
	 *
	 ...
	 * @since 2.8.0
	 *
	 * @param false|object|array $override Whether to override the Jabali.org Themes API. Default false.
	 * @param string             $action   Requested action. Likely values are 'theme_information',
	 *                                    'feature_list', or 'query_themes'.
	...
	if ( ! $res ) {
		$url = $http_url = 'http://api.jabali.github.io/themes/info/1.0/';
	...
	/**
	 * Filters the returned Jabali.org Themes API response.
	 *
	 * @since 2.8.0
	 *
	 * @param array|object|WP_Error $res    Jabali.org Themes API response.
	 * @param string                $action Requested action. Likely values are 'theme_information',
	 *                                      'feature_list', or 'query_themes'.
	 * @param object                $args   Arguments used to query for installer pages from the Jabali.org Themes API.
	 */
