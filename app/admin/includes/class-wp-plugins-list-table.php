...
class WP_Plugins_List_Table extends WP_List_Table {
	...
	public function no_items() {
		...
		if ( ! empty( $_REQUEST['s'] ) ) {
			...
			if ( ! is_multisite() && current_user_can( 'install_plugins' ) ) {
				echo ' <a href="' . esc_url( admin_url( 'plugin-install.php?tab=search&s=' . urlencode( $s ) ) ) . '">' . __( 'Search for plugins in the Jabali Plugin Directory.' ) . '</a>';
			}
	...
	protected function extra_tablenav( $which ) {
		...
		if ( 'recently_activated' == $status ) {
			...
			echo '<p>' . sprintf( __( 'Drop-ins are advanced plugins in the %s directory that replace Jabali functionality when present.' ),
