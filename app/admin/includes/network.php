<?php
/**
 * Jabali Network Administration API.
 *
 * @package Jabali
...
function network_step1( $errors = false ) {
	...
	<p><?php _e( 'Fill in the information below and you&#8217;ll be on your way to creating a network of Jabali sites. We will create configuration files in the next step.' ); ?></p>
	<?php
	...
	if ( allow_subdomain_install() && allow_subdirectory_install() ) : ?>
		...
		<p><?php _e( 'Please choose whether you would like sites in your Jabali network to use sub-domains or sub-directories.' ); ?>
...
<?php
			...
			<tr>
				...
				<td><?php
					printf(
						...
						__( 'Because you are using %1$s, the sites in your Jabali network must use sub-directories. Consider using %2$s if you wish to use sub-domains.' ),
						'<code>localhost</code>',
		...
		<?php elseif ( !allow_subdomain_install() ) : ?>
			<tr>
				<th scope="row"><?php esc_html_e( 'Sub-directory Install' ); ?></th>
				<td><?php
					_e( 'Because your install is in a directory, the sites in your Jabali network must use sub-directories.' );
					// Uh oh:
					if ( !allow_subdirectory_install() )
						echo ' <strong>' . __( 'Warning!' ) . ' ' . __( 'The main site in a sub-directory install will need to use a modified permalink structure, potentially breaking existing links.' ) . '</strong>';
				?></td>
		...
		<?php elseif ( !allow_subdirectory_install() ) : ?>
			<tr>
				...
				<td><?php _e( 'Because your install is not new, the sites in your Jabali network must use sub-domains.' );
...
function network_step2( $errors = false ) {
	...
	if ( ! file_exists( ABSPATH . 'config/db.php' ) && file_exists( dirname(dirname( ABSPATH )) . 'config/db.php' ) ) {
...
?>
	<div class="error"><p><strong><?php _e('Warning:'); ?></strong> <?php _e( 'An existing Jabali network was detected.' ); ?></p></div>
...
?>
		<ol>
			<li><p><?php printf(
				/* translators: 1: db.php 2: location of wp-config file, 3: translated version of "That's all, stop editing! Happy blogging." */
				 ...
				 * https://i18n.svn.jabali.github.io/<locale code>/branches/<wp version>/dist/config/sample.php
				 */
...
<?php
	...
	if ( ! empty( $keys_salts ) ) {
		$keys_salts_str = '';
		$from_api = wp_remote_get( 'https://api.jabali.github.io/secret-key/1.1/salt/' );
    ...
    <system.webServer>
        <rewrite>
            <rules>
                ...
                <rule name="Jabali Rule 4" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . '(wp-(content|admin|includes).*)" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . '{R:1}" />
                </rule>
                <rule name="Jabali Rule 5" stopProcessing="true">
                    <match url="^' . $iis_subdir_match . '([_0-9a-zA-Z-]+/)?(.*\.php)$" ignoreCase="false" />
                    <action type="Rewrite" url="' . $iis_rewrite_base . '{R:2}" />
                </rule>
                <rule name="Jabali Rule 6" stopProcessing="true">
                    <match url="." ignoreCase="false" />
...
';
		...
		printf(
			...
			__( 'Add the following to your %1$s file in %2$s, <strong>replacing</strong> other Jabali rules:' ),
			'<code>web.config</code>',
			'<code>' . $home_path . '</code>'
		);
		echo '</p>';
...
RewriteRule ^{$subdir_match}wp-admin$ {$subdir_replacement_01}admin/ [R=301,L]
