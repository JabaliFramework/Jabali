...
/** Load Jabali Administration Upgrade API */
require_once( ABSPATH . 'admin/includes/upgrade.php' );

/** Load Jabali Translation Install API */
...
elseif ( file_exists( dirname( ABSPATH ) . '/config/sample.php' ) )
	...
	wp_die( __( 'Sorry, I need a config/sample.php file to work from. Please re-upload this file to your Jabali installation.' ) );

// Check if db.php has been created
if ( file_exists( ABSPATH . 'config/db.php' ) )
	wp_die( '<p>' . sprintf(
			/* translators: %s: install.php */
			__( "The file 'config/db.php' already exists. If you need to reset any of the configuration items in this file, please delete it first. You may try <a href='%s'>installing now</a>." ),
			'install.php'
		) . '</p>'
	);

// Check if db.php exists above the root directory but is not part of another install
if ( @file_exists( ABSPATH . '../config/db.php' ) && ! @file_exists( ABSPATH . '../settings.php' ) ) {
	wp_die( '<p>' . sprintf(
			/* translators: %s: install.php */
			__( "The file 'config/db.php' already exists one level above your Jabali installation. If you need to reset any of the configuration items in this file, please delete it first. You may try <a href='%s'>installing now</a>." ),
			'install.php'
		) . '</p>'
	);
}
...
<head>
	...
	<title><?php _e( 'Jabali &rsaquo; Setup Configuration File' ); ?></title>
...
<p><?php _e( 'Welcome to Jabali. Before getting started, we need some information on the database. You will need to know the following items before proceeding.' ) ?></p>
<ol>
	<li><?php _e( 'Database name' ); ?></li>
	<li><?php _e( 'Database username' ); ?></li>
	<li><?php _e( 'Database password' ); ?></li>
	<li><?php _e( 'Database host' ); ?></li>
	<li><?php _e( 'Table prefix (if you want to run more than one Jabali in a single database)' ); ?></li>
</ol>
<p><?php
	...
	?>
	<strong><?php
		/* translators: 1: config/sample.php, 2: db.php */
...
<form method="post" action="setup-config.php?step=2">
	...
	<table class="form-table">
		<tr>
			...
			<td><input name="dbname" id="dbname" type="text" size="25" value="jabali" /></td>
			<td><?php _e( 'The name of the database you want to use with Jabali.' ); ?></td>
		</tr>
		<tr>
...
<?php
	...
	try {
		...
		if ( ! $no_api ) {
			$secret_keys = wp_remote_get( 'https://api.jabali.github.io/secret-key/1.1/salt/' );
		}
...
<?php
		 ...
		 */
		if ( file_exists( ABSPATH . 'config/sample.php' ) )
			$path_to_wp_config = ABSPATH . 'config/db.php';
		else
			$path_to_wp_config = dirname(dirname( ABSPATH )) . 'config/db.php';
...
<p><?php _e( 'All right, sparky! You&#8217;ve made it through this part of the installation. Jabali can now communicate with your database. If you are ready, time now to&hellip;' ); ?></p>
